...
type nativeCommand35PresentationJob struct {
	actor                     *battle.Unit
	plan                      *battle.NativeCompoundCommand35Plan
	...
	then                      func(battle.NativeCompoundCommand35Result)
}

func nativeCommand35ClonedState(st *battle.State, states []battle.NativeCommand35TargetState) (*battle.State, error) {
	if st == nil || len(st.Units) == 0 || len(states) == 0 {
		return nil, errors.New("native command35 cloned state unavailable")
	}
	clone := *st
	...
	for index, state := range states {
		...
		if copied == nil {
			return nil, fmt.Errorf("native command35 cloned target %d unavailable", index)
		}
...
func (g *Game) startNativeCommand35Presentation(actor, confirmed *battle.Unit, then func(battle.NativeCompoundCommand35Result)) error {
	if !g.nativeFullPresentationEnabled() || g == nil || g.st == nil || actor == nil || confirmed == nil {
		return errors.New("native command35 presentation context unavailable")
	}
	if g.nativeCmd35Presentation != nil || g.nativeCmd34Presentation != nil || g.nativeCmd33Presentation != nil || g.nativeCmd32Presentation != nil || g.nativeHealPresentation != nil ||
		...
		return errors.New("native command35 presentation already active")
	}
	if !g.st.HasNativeMapViewState || len(g.nativeMapWork) != indexedmap.NativeUnitPresentWorkSize ||
		...
		return errors.New("native command35 indexed map state unavailable")
	}
	plan, err := g.st.PlanNativeCompoundCommand35(actor, confirmed, g.nativeRNGState)
	if err != nil {
		return err
	}
	...
	commonEffect, err := figani.LoadSeparatedArchiveResource(separatedAssetPath("animations"), "FDOTHER.DAT", 68)
	if err != nil {
		return err
	}
	commonSchedule, err := figani.BuildNativeCompoundPresentationSchedule(35, commonEffect)
	if err != nil || !commonSchedule.TailEnabled || commonSchedule.MixerSample2Frame != 1 {
		return errors.New("native command35 common schedule unavailable")
	}
	tailSchedule, err := fdother.BuildNativeCommand35TailSchedule()
	if err != nil {
		return err
	}
	a := g.nativeMapAssets
	for _, stage := range tailSchedule {
		if len(a.FDOTHER6) < stage.EffectStart+stage.EffectFrames || len(a.CommandHealDigits) <= stage.DigitBias+9 {
			return errors.New("native command35 tail FDOTHER #5/#6 descriptors unavailable")
		}
	}
	paletteDAC, err := fdother.ReadResource(fdotherArchive, 0)
	if err != nil || len(paletteDAC) != 256*3 {
		return errors.New("native command35 battle DAC unavailable")
	}
	...
	if actor.NativeRecordByte6 != 0 {
		...
		if taiArchive == "" {
			return errors.New("native command35 TAI.DAT unavailable")
		}
	...
	if err != nil || len(mainImages) == 0 {
		return errors.New("native command35 main frames unavailable")
	}
	...
	if !ok {
		return errors.New("native command35 post-transaction HUD unavailable")
	}
	...
	for stageIndex, schedule := range tailSchedule {
		...
		clonedState, cloneErr := nativeCommand35ClonedState(g.st, plan.Result.StageStates[stageIndex])
		if cloneErr != nil {
			return cloneErr
		}
		...
		for targetIndex, target := range tailTargets {
			...
			application := plan.Result.Stages[stageIndex].Results[targetIndex]
			processed, value := application.Applied, application.Damage.Actual
			if processed {
	...
	frames[mainStart].sound = loadWav(assetPath("assets/sfx/battle_94_02.wav"))
	...
	commonSample1 := loadWav(assetPath("assets/sfx/battle_94_01.wav"))
	...
	required := [][]byte{frames[mainStart].sound, commonSample1}
	for stageIndex, schedule := range tailSchedule {
	...
	if !osMuteOrShot(g) {
		for _, sample := range required {
			if len(sample) == 0 {
				return errors.New("native command35 required raw sample unavailable")
			}
		}
	}
	g.nativeCmd35Presentation = &nativeCommand35PresentationJob{
...
func (g *Game) cancelNativeCommand35Presentation() {
	j := g.nativeCmd35Presentation
	if j == nil {
		return
	}
	_ = battle.AbortNativeCompoundCommand35(j.plan)
	...
	g.nativeCmd35Presentation = nil
}

func (g *Game) failNativeCommand35Presentation(err error) {
	g.cancelNativeCommand35Presentation()
	g.loadErr = "native command35 presentation: " + err.Error()
}

func (g *Game) stepNativeCommand35Presentation() {
	j := g.nativeCmd35Presentation
	if j == nil {
		return
	}
	if j.holding {
		...
		if err := battle.CompleteNativeCompoundCommand35(j.plan); err != nil {
			g.failNativeCommand35Presentation(err)
			return
		}
	...
	for stage := 0; stage < 3; stage++ {
		if next == j.publishAt[stage] && j.published == stage {
			if err := battle.PublishNativeCompoundCommand35Stage(j.plan, stage); err != nil {
				g.failNativeCommand35Presentation(err)
				return
			}
	...
	for stage := 0; stage < 3; stage++ {
		if next == j.stageEnd[stage] {
			if j.published != stage+1 {
				g.failNativeCommand35Presentation(errors.New("required modifier boundary was not presented"))
				return
			}
			j.frame = next - 1
	...
	if j.frame >= len(j.frames) {
		g.failNativeCommand35Presentation(errors.New("command35 frame boundary unavailable"))
	}
}

func (g *Game) drawNativeCommand35Presentation(screen *ebiten.Image) bool {
	j := g.nativeCmd35Presentation
