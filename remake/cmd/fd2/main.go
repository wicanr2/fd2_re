...
func (g *Game) materializeNativeMapRuntime(n *campaign.Node) bool {
	...
	// 節點常數走它自己的入口契約（進場即繪的靜止視圖），不是執行期狀態的
	// 那一套；兩者的差別寫在 campaign.NativeMapViewConfig.Validate。
	if err := view.Validate(g.st.W, g.st.H); err != nil {
...
func (g *Game) campInput() bool {
	...
	case "preparation", "church":
		...
		if g.churchMode == "transfer_full" {
			g.handleNativeChurchTransferInput(nativeChurchTransferInput{
...
func (g *Game) confirm() {
	...
	if g.nativeCommand0Targeting {
		...
		if id == 32 {
			actor := g.sel
			err := g.startNativeCommand32Presentation(actor, tgt, func(result battle.NativeCompoundCommand32Result) {
				g.nativeRNGState = result.RNGState
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.msg = "原始指令 32：完成 indexed command damage"
			...
			g.msg = "原始指令 32：indexed 演出進行中"
			return
		}
		if id == 33 {
			actor := g.sel
			err := g.startNativeCommand33Presentation(actor, tgt, func(result battle.NativeCompoundCommand33Result) {
				g.nativeRNGState = result.Restore.RNGState
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.msg = "原始指令 33：完成 indexed 清除與回復"
				g.finishSuccessfulUnitAction(actor, func() {
					g.resetNativeTargetField()
					g.st.MaterializeNativeMapRangeMode(1)
					g.nativeCommand0Targeting, g.nativeCommandTargetID, g.sel, g.reach, g.moved = false, 0, nil, nil, false
			...
			if err != nil {
				g.msg = fmt.Sprintf("原始指令 33：請選擇有效目標 (%v)", err)
				return
			}
			g.msg = "原始指令 33：indexed 演出進行中"
			return
		}
		if id == 34 {
			actor := g.sel
			err := g.startNativeCommand34Presentation(actor, tgt, func(result battle.NativeCompoundCommand34Result) {
				g.nativeRNGState = result.RNGState
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.msg = "原始指令 34：完成三段 indexed modifier"
				g.finishSuccessfulUnitAction(actor, func() {
					g.resetNativeTargetField()
					g.st.MaterializeNativeMapRangeMode(1)
					g.nativeCommand0Targeting, g.nativeCommandTargetID, g.sel, g.reach, g.moved = false, 0, nil, nil, false
			...
			if err != nil {
				g.msg = fmt.Sprintf("原始指令 34：請選擇有效目標 (%v)", err)
				return
			}
			g.msg = "原始指令 34：indexed 演出進行中"
			return
		}
		if id == 35 {
			actor := g.sel
		...
		if id >= 20 && id <= 22 {
			...
			err := g.startNativeCommand2022Presentation(actor, tgt, id, true, func(plan *battle.NativeAICommand2022Plan) {
				...
				g.msg = fmt.Sprintf("原始指令 %d：完成palette與indexed尾段 (%d targets)", id, len(plan.Results))
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.finishSuccessfulUnitAction(actor, func() {
					g.resetNativeTargetField()
					g.st.MaterializeNativeMapRangeMode(1)
		...
		if id == 24 {
			actor := g.sel
			err := g.startNativeCommand24Presentation(actor, tgt, func(results []battle.NativeCommand24Damage) {
				total := 0
				for _, result := range results {
					total += result.Damage
					if result.Damage > 0 {
				...
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.msg = fmt.Sprintf("原始指令 24：傷害 %d", total)
				g.finishSuccessfulUnitAction(actor, func() {
					g.resetNativeTargetField()
					g.st.MaterializeNativeMapRangeMode(1)
					g.nativeCommand0Targeting, g.nativeCommandTargetID, g.sel, g.reach, g.moved = false, 0, nil, nil, false
			...
			if err != nil {
				g.msg = "原始指令 24：indexed 演出不可用 (" + err.Error() + ")"
			}
			return
		}
		message := ""
		...
		case id == 2:
			actor := g.sel
			err := g.startNativeCommand2Presentation(actor, tgt, func(results []battle.NativeCommandDamageResult) {
				hit, total := 0, 0
				for _, result := range results {
					if result.Hit {
						hit++
			...
			if err != nil {
				g.msg = "原始指令 2：indexed 演出不可用 (" + err.Error() + ")"
			}
			return
		case id == 3:
			actor := g.sel
			err = g.startNativeCommand3Presentation(actor, tgt, func(results []battle.NativeCommandDamageResult) {
				hit, total := 0, 0
				for _, result := range results {
					if result.Hit {
						hit++
			...
			if err != nil {
				g.msg = "原始指令 3：indexed 演出不可用 (" + err.Error() + ")"
			}
			return
		case id == 5:
			actor := g.sel
			err = g.startNativeCommand5Presentation(actor, tgt, func(results []battle.NativeCommandDamageResult) {
				hit, total := 0, 0
				for _, result := range results {
					if result.Hit {
						hit++
			...
			if err != nil {
				g.msg = "原始指令 5：indexed 演出不可用 (" + err.Error() + ")"
			}
			return
		case id == 7:
			actor := g.sel
			err = g.startNativeCommand7Presentation(actor, tgt, func(results []battle.NativeCommandDamageResult) {
				hit, total := 0, 0
				for _, result := range results {
					if result.Hit {
						hit++
		...
		case id >= 10 && id <= 12:
			actor := g.sel
			err = g.startNativeCommand1012Presentation(actor, tgt, id, func(results []battle.NativeCommandDamageResult) {
				hit, total := 0, 0
				for _, result := range results {
					if result.Hit {
						hit++
				...
				actor.SetMapPose(dirToward(actor.X, actor.Y, g.curX, g.curY))
				g.msg = fmt.Sprintf("原始指令 %d：命中 %d，傷害 %d", id, hit, total)
				g.finishSuccessfulUnitAction(actor, func() {
					g.resetNativeTargetField()
					g.st.MaterializeNativeMapRangeMode(1)
					g.nativeCommand0Targeting, g.nativeCommandTargetID, g.sel, g.reach, g.moved = false, 0, nil, nil, false
		...
		case id == 6:
			...
			if err != nil {
				g.msg = "原始指令 6：indexed 演出不可用 (" + err.Error() + ")"
			}
			return
		case id == 29:
			actor := g.sel
...
func (g *Game) Update() error {
	...
	if g.shotPath != "" || g.shotSeries != "" {
		...
		if !g.shotSetup && g.frame >= g.shotFrame-1 {
			...
			if spec := os.Getenv("FD2_SHOT_SHOP_TRANSFER_SUCCESS_STATE"); spec != "" {
				mode, source, item, destination, postSource, selection, start, gold, ok :=
					parseNativeShopTransferSuccessShotState(spec)
				if !ok || !g.setNativeShopTransferSuccessShotState(
					mode, source, item, destination, postSource, selection, start, gold,
				) {
					return fmt.Errorf(
						"FD2_SHOT_SHOP_TRANSFER_SUCCESS_STATE expects success_intro|success_items,source,item,destination,post_source,selection,start,gold on an admitted native transfer party: %q",
						spec,
					)
				}
			}
	...
	g.stepNativeAICommandModifierPresentation()  // native 0x1541F enemy command17..19 handler tail
	...
	g.stepNativeCommand2Presentation()           // native 0x2A6BD→0x26528 command2 battle presentation
	g.stepNativeCommand3Presentation()           // native 0x2A6BD→0x26795 command3 battle presentation
	...
	g.stepNativeCommand1012Presentation()        // native 0x21527/0x2185F/0x21A9E player map presentation
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	if g.nativeAICommandModifier != nil {
		...
		if !g.drawNativeAICommandModifierPresentation(screen) {
			g.failNativeAICommandModifierPresentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native AI modifier presentation unavailable")
		}
		return
	}
	...
	if g.nativeCmd2Presentation != nil {
		screen.Fill(color.Black)
		if !g.drawNativeCommand2Presentation(screen) {
			g.failNativeCommand2Presentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native command2 presentation unavailable")
		}
		if g.shotPath != "" && !g.shotTaken && g.frame >= g.shotFrame {
			g.captureShot(screen)
		}
	...
	if g.nativeCmd3Presentation != nil {
		screen.Fill(color.Black)
		if !g.drawNativeCommand3Presentation(screen) {
			g.failNativeCommand3Presentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native command3 presentation unavailable")
		}
		if g.shotPath != "" && !g.shotTaken && g.frame >= g.shotFrame {
			g.captureShot(screen)
		}
	...
	if g.nativeCmd5Presentation != nil {
		screen.Fill(color.Black)
		if !g.drawNativeCommand5Presentation(screen) {
			g.failNativeCommand5Presentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native command5 presentation unavailable")
		}
		if g.shotPath != "" && !g.shotTaken && g.frame >= g.shotFrame {
			g.captureShot(screen)
		}
	...
	if g.nativeCmd1012 != nil {
		...
		if !g.drawNativeCommand1012Presentation(screen) {
			g.failNativeCommand1012Presentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native command10-12 presentation unavailable")
		}
		if g.shotPath != "" && !g.shotTaken && g.frame >= g.shotFrame {
			g.captureShot(screen)
		}
	...
	if g.nativeCmd32Presentation != nil {
		...
		if !g.drawNativeCommand32Presentation(screen) {
			g.failNativeCommand32Presentation(errors.New("draw unavailable"))
			ebitenutil.DebugPrint(screen, "native command32 presentation unavailable")
		}
		if g.shotPath != "" && !g.shotTaken && g.frame >= g.shotFrame {
			g.captureShot(screen)
		}
	...
	if g.nativeCmd33Presentation != nil {
		...
		if !g.drawNativeCommand33Presentation(screen) {
			g.failNativeCommand33Presentation(errors.New("draw input unavailable"))
		}
		return
	}
...
func loadGame() *Game {
	...
	learnSelectorPath := assetPath("assets/data/class_change_growth.json")
	if !fileExists(learnSelectorPath) {
		learnSelectorPath = "../docs/data/exe_tables/growth.json"
	}
	if selectors, e := battle.LoadCommandLearnSelectors(learnSelectorPath); e == nil {
