...
func TestNativeCommand4EnemyPresentationPublishesOnlyAfterDraw(t *testing.T) {
	book := make([]battle.NativeCommandRecord, battle.NativeCommandRecordCount)
	for id := range book {
		book[id].ID = id
	}
	book[4] = battle.NativeCommandRecord{ID: 4, Damage: 90, Hit: 100, SelectionMode: 1, EffectMode: 1, MPCost: 2}
	actor := &battle.Unit{Camp: battle.Own, X: 0, Y: 0, HP: 20, MP: 5, OnField: true}
	target := &battle.Unit{Camp: battle.Enemy, ClassID: 5, X: 1, Y: 0, HP: 103, OnField: true}
	state := &battle.State{W: 2, H: 1, Units: []*battle.Unit{actor, target}, NativeCompositionEventBytes: make([]byte, 2), NativeCommandBook: book}
	plan, err := state.PlanNativeCommandDamage(actor, target, 4, map[int]int{5: 10}, 3)
	if err != nil {
		t.Fatal(err)
	}
	callback := 0
	g := &Game{nativeRNGState: plan.RNGBefore}
	g.nativeCmd5Presentation = &nativeCommand5PresentationJob{commandID: 4, actor: actor, plan: plan,
		prelude: make([]*ebiten.Image, 1), actorBlack: make([]*ebiten.Image, 1), actorPulse: make([]*ebiten.Image, 1),
		actorSpecs: []battlepresent.NativeCommand0ActorFrame{{PublishMP: true}}, actorMPBefore: actor.MP,
		actorActedBefore: actor.Acted, targetHPBefore: []int{target.HP},
		then: func(results []battle.NativeCommandDamageResult) { callback = len(results) }}
	for stage := 1; stage <= plan.DamageStages; stage++ {
		g.nativeCmd5Presentation.handler = append(g.nativeCmd5Presentation.handler, nativeCommand5HandlerFrame{targetIndex: 0, hpStage: stage})
	}
	g.stepNativeCommand5Presentation()
	if actor.MP != plan.MPBefore || actor.Acted || target.HP != plan.Results[0].HPBefore {
		t.Fatal("未繪製的第4指令改變狀態")
	}
	for g.nativeCmd5Presentation != nil {
		g.nativeCmd5Presentation.drawn = true
		g.stepNativeCommand5Presentation()
	}
	if actor.MP != plan.MPAfter || !actor.Acted || target.HP != plan.Results[0].HPAfter || callback != 1 {
		t.Fatalf("第4指令交易未完成：mp=%d acted=%v hp=%d callback=%d", actor.MP, actor.Acted, target.HP, callback)
	}
}
