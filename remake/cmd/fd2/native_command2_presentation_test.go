...
func nativeCommand2PresentationTestPlan(t *testing.T) (*battle.NativeCommandDamagePlan, *battle.Unit, *battle.Unit) {
	t.Helper()
	book := make([]battle.NativeCommandRecord, battle.NativeCommandRecordCount)
	for id := range book {
		book[id].ID = id
	}
	book[2] = battle.NativeCommandRecord{ID: 2, Damage: 90, Hit: 100, SelectionMode: 1, EffectMode: 0, MPCost: 2, TargetCode: 0}
	...
	plan, err := state.PlanNativeCommandDamage(actor, target, 2, map[int]int{5: 10}, 3)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNativeCommand2PresentationPublishesOnlyAfterDraw(t *testing.T) {
	plan, actor, target := nativeCommand2PresentationTestPlan(t)
	callback := 0
	...
	for stage := 1; stage <= plan.DamageStages; stage++ {
		g.nativeCmd2Presentation.targets = append(g.nativeCmd2Presentation.targets, nativeCommand2TargetFrame{targetIndex: 0, hpStage: stage})
	}
	g.stepNativeCommand2Presentation()
	if actor.MP != plan.MPBefore || actor.Acted || target.HP != plan.Results[0].HPBefore {
		t.Fatal("未繪製的第 2 指令改變了戰鬥狀態")
	}
	for g.nativeCmd2Presentation != nil {
	...
	if actor.MP != plan.MPAfter || !actor.Acted || target.HP != plan.Results[0].HPAfter || g.nativeRNGState != plan.RNGAfter || callback != 1 {
		t.Fatalf("第 2 指令交易未完成：mp=%d acted=%v hp=%d rng=%#x callback=%d", actor.MP, actor.Acted, target.HP, g.nativeRNGState, callback)
	}
}

func TestNativeCommand2PresentationRollsBackRuntimeFailure(t *testing.T) {
	plan, actor, target := nativeCommand2PresentationTestPlan(t)
	g := &Game{nativeRNGState: plan.RNGBefore}
	g.nativeCmd2Presentation = &nativeCommand2PresentationJob{
		actor: actor, plan: plan, phase: nativeCommand2Actor,
		actorBlack: make([]*ebiten.Image, 1), actorPulse: make([]*ebiten.Image, 1),
		actorSpecs: []battlepresent.NativeCommand0ActorFrame{{PublishMP: true}},
		targets:    []nativeCommand2TargetFrame{{targetIndex: 0, hpStage: 2}}, tail: make([]*ebiten.Image, 1),
	...
	g.nativeCmd2Presentation.drawn = true
	g.stepNativeCommand2Presentation()
	if g.nativeCmd2Presentation != nil || actor.MP != plan.MPBefore || actor.Acted || target.HP != plan.Results[0].HPBefore || g.nativeRNGState != plan.RNGBefore || g.loadErr == "" {
