...
func BuildNativeCommand24PreludeFrames(in NativeCommand24PreludeInput) ([]NativeCommand24PreludeFrame, error) {
	...
	if len(in.Base) != frameBytes || len(in.BaselineDAC) != 256*3 ||
		...
		len(in.ActorIdle.Mask) != len(in.ActorIdle.Pixels) {
		return nil, errors.New("battlepresent: command24 prelude inputs unavailable")
	}
	if _, err := fdother.ParseVGAPalette(in.BaselineDAC); err != nil {
		return nil, err
	}
	if in.RawSide != 0 && in.Platform == nil {
		return nil, errors.New("battlepresent: command24 nonzero-side prelude requires TAI")
	}
	...
	for stage := 8; stage >= 0; stage-- {
		...
		for y := 0; y < NativeCommand24BackgroundHeight; y++ {
			copy(work[y*nativeCommand24WorkStride+viewport:], in.Base[y*NativeCommand24BackgroundWidth:(y+1)*NativeCommand24BackgroundWidth])
		}
