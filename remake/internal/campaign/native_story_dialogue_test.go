...
func TestCh15PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_016.bin")
	if err != nil {
		t.Skip("extracted FDTXT_016 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	beats, issues, err := CompileHandlerBinding("../../assets/cutscenes/bindings/ch15_post.json")
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch15_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 3)
	var collect func([]Beat)
	collect = func(items []Beat) {
		for _, beat := range items {
			if beat.Op == "dialog" && beat.NativeDialogue != nil {
	...
	collect(beats)
	total := 0
	for stringIndex := 2; stringIndex <= 4; stringIndex++ {
		words, err := stringsTable.Words(stringIndex)
		if err != nil {
			t.Fatal(err)
		}
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_016", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_016 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 23 {
		t.Fatalf("ch15_post native dialogue utterances=%d, want 23", total)
	}
}

func TestCh16PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_017.bin")
	if err != nil {
		t.Skip("extracted FDTXT_017 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	beats, issues, err := CompileHandlerBinding("../../assets/cutscenes/bindings/ch16_post.json")
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch16_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 4)
	var collect func([]Beat)
	collect = func(items []Beat) {
	...
	collect(beats)
	total := 0
	for _, stringIndex := range []int{5, 7, 6, 8} {
		words, err := stringsTable.Words(stringIndex)
		if err != nil {
			t.Fatal(err)
		}
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_017", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_017 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 26 {
		t.Fatalf("ch16_post native dialogue utterances=%d, want 26", total)
	}
}

func TestCh09PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_010.bin")
	if err != nil {
		t.Skip("extracted FDTXT_010 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	beats, issues, err := CompileHandlerBinding("../../assets/cutscenes/bindings/ch09_post.json")
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch09_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 2)
	for _, beat := range beats {
		if beat.Op == "dialog" && beat.NativeDialogue != nil {
	...
	for stringIndex := 4; stringIndex <= 5; stringIndex++ {
		...
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_010", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_010 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 35 {
		t.Fatalf("ch09_post native dialogue utterances=%d, want 35", total)
	}
}
...
func TestCh21PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_022.bin")
	if err != nil {
		t.Skip("extracted FDTXT_022 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	beats, issues, err := CompileHandlerBinding("../../assets/cutscenes/bindings/ch21_post.json")
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch21_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 3)
	for _, beat := range beats {
		if beat.Op == "dialog" && beat.NativeDialogue != nil {
	...
	total := 0
	for stringIndex := 4; stringIndex <= 6; stringIndex++ {
		words, err := stringsTable.Words(stringIndex)
		if err != nil {
			t.Fatal(err)
		}
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_022", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_022 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 11 {
		t.Fatalf("ch21_post native dialogue utterances=%d, want 11", total)
	}
}

func TestCh23PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_024.bin")
	if err != nil {
		t.Skip("extracted FDTXT_024 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch23_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 2)
	for _, beat := range beats {
		if beat.Op == "dialog" && beat.NativeDialogue != nil {
	...
	total := 0
	for stringIndex := 2; stringIndex <= 3; stringIndex++ {
		words, err := stringsTable.Words(stringIndex)
		if err != nil {
			t.Fatal(err)
		}
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_024", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_024 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 11 {
		t.Fatalf("ch23_post native dialogue utterances=%d, want 11", total)
	}
}
...
func TestCh25PostNativeDialogueLayoutsMatchOriginalControlWords(t *testing.T) {
	raw, err := os.ReadFile("../../../extracted/raw/FDTXT/FDTXT_026.bin")
	if err != nil {
		t.Skip("extracted FDTXT_026 oracle is absent")
	}
	stringsTable, err := fdtxt.Parse(raw)
	if err != nil {
		t.Fatal(err)
	...
	if err != nil || len(issues) != 0 {
		t.Fatalf("compile ch25_post binding: issues=%v err=%v", issues, err)
	}
	got := make(map[int][]*NativeDialogueLayout, 7)
	var collect func([]Beat)
	collect = func(items []Beat) {
		for i := range items {
			beat := items[i]
	...
	collect(beats)
	total := 0
	for stringIndex := 5; stringIndex <= 11; stringIndex++ {
		words, err := stringsTable.Words(stringIndex)
		if err != nil {
			t.Fatal(err)
		}
		want, err := decodeOriginalNativeDialogueLayouts("FDTXT_026", stringIndex, words, glyphs)
		if err != nil {
			t.Fatal(err)
		}
		if !reflect.DeepEqual(got[stringIndex], want) {
			t.Fatalf("FDTXT_026 index%d layouts=%d want=%d\ngot  %#v\nwant %#v",
				stringIndex, len(got[stringIndex]), len(want), got[stringIndex], want)
		}
		total += len(want)
	}
	if total != 41 {
		t.Fatalf("ch25_post native dialogue utterances=%d, want 41", total)
	}
}
