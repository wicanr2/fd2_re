...
| 回歸範圍 | 沿用 115 的定案：已 passed 的章不因為新修正而重跑；第十一章的收據就是這一輪的驗收。[#41](https://github.com/wicanr2/fd2_re/issues/41) 也不另外重跑（2026-09-18 定案） |
...
- **[#41](https://github.com/wicanr2/fd2_re/issues/41)**：攻擊演出收尾（`0x1D3FF`、`0x15510`／`0x1563B`／
  `0x1565C`）、`0x1DB65` 的 `0x1DEAE`、訊息關框 `0x19742` 三類重繪還沒經過 HUD anchor 入口。
  **不為了這條重跑任何已 passed 的章（使用者 2026-09-18 定案）**：第十一章收據若剛好出現對應差異就
  順手接掉並關 #41，沒出現就維持未接、在 `56` §#40 記一句仍未被覆蓋，不自行加評估點。
