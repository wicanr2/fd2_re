...
- 新旗標 `-boost-base-ap N`、`-boost-base-dp N`、`-boost-base-dx N`（包裝器 `--boost-base-ap`／
  ...
  不直接改 `+0x48`／`+0x4A`：下一次換裝或升級重算就會蓋回去，兩側會在不同時點分岔。word 上限
  `0x7FFF` 失敗即關閉。
- **閃避沒有獨立的基底欄位（已證實）。** `0x1145A`／`0x1B750` 的重算以 `+0x3E` DX 同時當 HIT 與 EV
  ...
  要拉高閃避就是拉高 DX，命中會一起變高；不要直接改 `+0x4E`，下一次重算就蓋回去。
...
- **閃避（DX）可以拉高。** AI 物理評分只看 `+0x48`／`+0x4A`，不看 EV，所以敵人照樣出手，只是比較常
  被閃掉；攻擊、未命中與反擊的節拍仍抽得到。命中判定（已證實，`sub_29F72`，
  `remake/internal/battle/native_physical_damage.go`）是 `rand()%100 < 攻方 +0x4C − 守方 +0x4E`：
  我方 EV 大於等於敵人 HIT 就是必定未命中。X 要讓 EV 留在敵人 HIT 之下（校準時看該章敵人 +0x4C
  的最小值），每回合至少要有一次敵方命中，否則「受傷、升級對話底圖、我方回復」這些節拍會消失。
...
| 政策值 | 定案 | 校準輪 | 依據 |
|---|---|---|---|
| 基底 AP 加值 A | **200** | c1、c2 | c1（`work/parity-slot-ch08-boost/sample-c1`）：我方攻擊多半一擊擊倒，騎士第 6 回合倒下；持續進攻時約第 10 回合敵人全滅，撐不到第 15 回合，所以 c2 起第 7 回合後停手 |
| 基底 DP 加值 D | **0** | c1、c2 | 維持原值，敵人照常出手；c2 第 7～15 回合停手期間每回合都有敵方命中 |
| DX 加值 X（HIT 與 EV 共用基底） | **60** | c1＝30、c2＝60 | c2（`work/parity-slot-ch08-boost-x60/sample-c2`）：索爾 EV 124 高於本章敵人 HIT 上限 116；停手期間凱麗（DX 10、EV 70）第 10 回合前陣亡，使用者 2026-09-17 同意接受並記進 limitations；洛娜是 FDFIELD 記錄、不吃建槽強化，開場第 2 回合陣亡 |
