...
- **[已解]**(第14輪)~~`0x1f73f`(esi==0x1c2 與 esi==0xa 觸發)~~ → 完整逐指令反組譯 + 資源解圖
  比對,見 doc23 §2.4⑥。結論:`(esi,edi,idxA,idxB)` 中 **idxA 是 idxB 的專屬 768B 調色盤**(非另一
  張圖);`0x17aa9` 查證後**不是 BGM/SFX,是 tick 計數忙等**(讀全域 `[0x46c]`),上面「BGM/SFX」的
  猜測已修正。兩個觸發點解圖分別對上 doc23 §2.4③ **item①守護者**(#100/#99)與 **item⑥浮空城**
  (#75/#76)——**不是原猜測的分鏡⑨惡魔臉**;⑨的真正來源仍未查到,不可套用本節結論。
...
### 10.8 屠龍幕「揮劍小閃光」即時播放驗證(第N輪,dosbox 即時錄影對照)[驗]

> 起因:使用者反映 remake 實機看不到屠龍幕揮劍後的小閃光,懷疑 `loadCutClip` 同步解碼
> 觸發 ebiten Update 追幀導致中間幀被跳過(掉幀),或 res5 tick 換算過快(時序錯)。
> 兩個假說都用**即時錄影**(非逐幀截圖,規則 64 的延伸——這裡截圖 oracle 換成錄影 oracle)
> 交叉驗證,結論兩者皆不成立,但**額外抓到一個真實的結構性時序落差**(見下)。

**方法**:remake 端(`ffmpeg x11grab` 對 Xvfb `:53` 即時錄 8s/30fps)與 DOS 原版端(`dq3-dosbox`
容器內 `dosbox 0.74-3` 連到同一 Xvfb `:53`,`FLAME2.CONF` 原始設定 `machine=vga cycles=auto`,
即時錄 20s/30fps)各錄一次開場,逐幀量測平均灰階亮度找閃光,再用離線 `afm.DecodeResource`
解出的 res5(35 幀)參考序列逐像素核對命中幀。幀圖/影片本機存 `extracted/title_re/dragonfx_compare/`
(未 commit,原版素材紀律)。

**發現 1:res5 tick 換算正確,兩邊即時播放的閃光時長同量級**——remake 揮劍白閃(video
frame152-154,~100ms)與 DOS 原版(video frame441-442,~67-100ms)像素比對 res5/frame12 皆
命中(remake 誤差 10/64000px、DOS 誤差 69/64000px,皆 <0.11%)。`tick=3`(=delayMs 50 的
60fps 換算,3/60=50.0ms 整除無誤差)本身沒有播太快的問題,閃光在原版裡本來就短(~2 幀)。

**發現 2:不是掉幀**——對 remake 連跑 3 次的 `Update()` tick 間隔量測(暫時 debug
instrumentation,驗完已 revert),全程只有 process 生命週期唯一一次 >25ms gap(idx0 第一幀
的 GPU/shader 暖機,~220-234ms,發生在整段過場最前面,與屠龍幕無關),res5 進場與播放
tick pacing 全程穩定 ~16.6ms/tick,3 次結果一致,沒有 catch-up burst。

**發現 3:remake 整段開場比原版快了近 3 倍,根因是「背景捲動沒有跟 AFM 幕交錯播放」**——
DOS 原版從開場到揮劍閃光實測 **~14.7s**(video frame441/30fps),remake 只要 **~5.0s**
(video frame152/30fps),比值 ≈2.9×。§10.5 記載的 esi 535→0 捲動迴圈(每步 30ms 複製一列)
是**貫穿整段開場、AFM/static 觸發點只是暫停它去播完整段再繼續**的背景機制,不是播完全部
AFM 幕之後才發生的獨立收尾動畫。remake 現行 `cutScript`(`title.go`)把 `scroll` 步驟(魔王
立繪捲動,`tick=220`)搬到所有 AFM/static 步驟**之後**單獨播一次,等於少掉原版穿插在
`res3→(esi 450 靜態)→res4→res5→…` 各段之間、由 esi 遞減累積出的 ~16s 背景延遲(§10.5 算出
的 2550/3600/3600/3000/2550/450/300ms 七段間隔)。這個落差**不影響屠龍閃光本身在螢幕上停留
的時間**(發現1已核實兩邊等長),只影響整體開場步調與各幕出現的時間點。

**結論**:「看不到閃光」不是掉幀、也不是 res5 自己播太快,兩邊即時播放閃光時長同量級——
閃光本身在原版設計裡就短暫(~2 幀、局部區域 ~14% 螢幕),比較可能是感知問題(旁邊索爾→
屠龍轉場與後段龍吐息都是更誇張的全螢幕大白閃,容易蓋過注意力)。發現 3 的「捲動未交錯」
是獨立的真實時序落差,值得列入 worklist,但修正前還有開放問題:原版 AFM 幕播放時直寫
