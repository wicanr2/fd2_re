# 91（歷史）— FD2 remake 的工作記錄

這是時間序列記錄，**不是待辦清單**。裡面的 `- [ ]`、`- [~]` 與各輪「有效佇列」
...
### 2026-09-10（拆成 JSON ＋ verify 時的第一輪核實）

下列條目在下面的記錄裡仍是 `- [ ]`，對照產品程式碼後確認早就做完了。它們沒有
進 `fd2-worklist.json`：

| 舊條目 | 實際狀況 |
|---|---|
| OGG 串流播放（M3） | `remake/internal/musiccatalog` 已是正式 BGM 來源 |
| 引擎 TTF 文字渲染（M2） | `remake/cmd/fd2/font.go` 已接，對白與 HUD 都在用 |
| 勝敗判定與回合推進（M1） | `on_turn_end` 事件鏈已在正式 runtime |
| headless 確定性回歸（M1） | `FD2_SHOT_DETERMINISTIC` 固定 60 Hz 虛擬時鐘 |
| 從原版資料生成線性 campaign（M4） | `tools/gen_campaign.py` 已存在並有測試 |
| ScenarioRunner 狀態機（M4） | campaign runner 已是正式節點驅動 |
| 全 33 戰場匯出為引擎資產（M5） | `tools/export_engine_assets.py` 已接進封包流程 |
| `decode_story_text --script-json`（M2） | 目的已由 `remake/assets/story/` 的 35 份 `chNN.json` 達成 |
...
- `phase-banner-glyph-asset`（2026-09-10 關閉）：字樣是 FDOTHER #5 的三格 LMI1
  cell，重製端已接上，落點改用原版的 (89,86)／(169,86)。原版逐幀比對走完滑入
  7 步、停留 33 步與滑出 5 步，每一步逐像素全中，收據
  [`fd2-phase-banner-glyph-parity-20260910.json`](../data/ui-traces/fd2-phase-banner-glyph-parity-20260910.json)。
  這一輪推翻了上一條「停住的位置照原版只會更不像」的判斷——兩塊是相向移動的，
  而且原版字模與分離素材逐像素相同，套原版座標之後反而完全對上。同時修掉一個
  重製端自己的差異：字樣要裁在 `sub_11EB0` 的 312×192 搬運窗格內。
- `remake-attack-missing-counterattack`（2026-09-10 關閉）：原版物理攻擊走
  `sub_28A6C` 而不是 `sub_2A6BD`——上一輪的方向是錯的，`-eip-watch` 實測那條路
  整段一次都沒進入。反擊條件（守方存活 ∧ 曼哈頓距離 1 ∧ 守方武器 record `+11`
  為 1）、傷害公式 `sub_29F72` 與三張靜態表都已解，重製端的正式路徑已接上並走
  原版 RNG。對拍用原版 checkpoint 的 record 當輸入：反擊傷害在全部 65536 個 RNG
  狀態下都是 17，與原版實測的 48→31 相同；主攻 20 也落在公式的值域內。收據
  [`fd2-physical-attack-parity-20260910.json`](../data/ui-traces/fd2-physical-attack-parity-20260910.json)。
  仍未做：反擊的第二段演出、原版的經驗鏈、`unit+38` 的語意。
- 2026-09-10 勘誤（106 寫入當下的兩句過期斷言）：`unit+38` 不是「重製端沒有
  對應欄位、語意未解」——它是 `+0x26`，六個暫時狀態剩餘回合數之一，重製端早有
  `NativeTransientDuration`；反擊判定已改讀真值。物理攻擊的經驗值也不是「本輪
...
- `remake-counterattack-presentation`（2026-09-10 關閉）：反擊接成同一段演出的第二
  半，照原版 `sub_2939D` 第二次呼叫的參數——攻方換成原守方的攻擊資源、守方換成原
  攻方的待機資源，資訊條與陣營一起換邊，時間軸重新開始而畫面不重新滑入。整段長度
  等於兩個攻擊資源的延遲和（測試釘住），與原版那 52 次同一個算法。第一段顯示的攻方
  HP 也修回反擊之前的值——結算在演出之前完成，`actor.HP` 當時已經是被反擊之後的。
...
> （拍下當時完整素材根下的 39 項已知失敗，該 39 項現已全部修完）。後續回歸仍以
> **與該清單的差異**判讀，不以總數判讀。
...
- RE-CLOSED／RUNTIME-E1：回合橫幅的整條呈現鏈解出來了。33 步是
  `sub_1F1CC` 進場 16 步（邊長 1..16）＋ Watcom `delay()`（敵方 20 毫秒
  `0x1A52F`、玩家 150 毫秒 `0x1A5D9`）＋ `sub_1F30A` 退場 17 步（邊長 17..1）；
  馬賽克核心是 `sub_4E809`（(4,4) 起 312×192、方塊左上角取樣）；**每一步以
  `sub_17AA9(1)` 等一個 BIOS tick ＝ 54.925 毫秒**，節奏因此換得回實機毫秒；
  畫面來自進場時 `memcpy(0xA0000, 0xFA00)` 的整幀快照，整段不重繪；每步另有
  `sub_11D40(0x10, 0xFF, 邊長 − 1)` 把 DAC 索引 0x10..0xFF 減暗，索引
  0x00..0x0F 不動所以字樣不變暗。重製端四處已照契約改：節奏、中段停留、
  進場凍結快照、DAC 暗化。量測與原始指令見
  [105](105-phase-banner-timing-20260910.md)、
  [收據](../data/ui-traces/fd2-phase-banner-timing-20260910.json)、
  [反組譯](../data/fd2_phase_banner_presentation_disasm.txt)。
- RUNTIME-E1：橫幅開始時左下 HUD 面板在不在，不由橫幅決定，而是進入那一刻
  螢幕上有什麼。敵方回合實測沒有面板、游標白框仍在；`0x1ACF3`／`0x1AD72` 在
  進入前的最後一次重繪執行過，與面板不在畫面上並不衝突。
- 待做（橫幅）：`sub_1F42D` 的字樣滑入 7 步與滑出 5 步（同樣每步一個 BIOS
  tick，位置 x ＝ `0x55 − 參數`、y ＝ `0x52`）尚未接進重製端；字樣資源
  `[0x53A81]` id `0x50`／`0x52` 尚未擷取，字模未比對；玩家回合橫幅只有靜態
  指令證據，沒有同一輪的執行期收據。
...
- RUNTIME-E1：回合橫幅已照收據接線——`MosaicNativeMapViewport` 對保留幀逐步
  重算馬賽克（方塊左上角取樣、格線原點即窗格原點），33 步的邊長曲線由
  `PhaseBannerBlock` 提供，字樣疊在最後且不再暗化地圖。每步停留幀數是重製端
  自訂節奏，不宣稱與原版一致。
...
- 測試基線：39 項已知失敗已全部修完（`tools/remake_go_test.sh <素材根>` 對
  `./...` 重跑，0 失敗、無新增）。其中 26 項是「手工 `&Game{}` 沒有語系」，
  12 處存檔訊息期待值沒跟著語言包走（對應 3 個測試），1 項是指令環展開最後一
  幀的期待值錯（以 `0x1741C` 的直接指令判定實作才是對的），其餘 9 項各有成因。
  逐項成因與修法見 [104](104-regression-baseline-review-20260909.md)。基線檔
  保留原樣，供之後仍以差異判讀。
...
- RE-CLOSED／RUNTIME-E1：地圖走行每格是**六幀**，跨格提交與下一段的第一拍
  ...
  提交後在同一幀接上下一段）。同一條線抓到
  `fdicon.NativePlacementOffset` 的上下位移寫成一整格高度（24×456），
  已閉合的數值是 `±0x720`＝四列＝四像素，四像素乘六拍剛好一格。寫成一整格
  高度的後果是垂直移動每拍跳一整格，單位整段離開可見範圍、抵達才回到終點格，
  也就是先前回報的「順移」；左右移動每拍只差 4 px，橫向樣本看不出來。量測、
  逐幀對照與出處見 [99](99-move-confirm-cursor-20260909.md)。
- RE-CLOSED／RUNTIME-E1：移動確認瞬間的原版契約已量到 5 毫秒粒度——游標由終點
  **瞬間**跳回單位所在格、鏡頭完全不動、之後逐格跟著單位走。重製端已照此接線：
  確認時走只寫絕對游標的 `0x149F8`（`jumpNativeMapCursorToUnit`），走行期間由
  `AdvanceNativeMapWalkStepView` 逐格跟隨，跨格不再多發佈一幀 `motion = 0`。
  收據與量測見 [99](99-move-confirm-cursor-20260909.md)。
- RUNTIME-E1：兩側都可以逐幀取畫面了。原版用 `FD2_ORACLE_FRAMES=1`，重製端用
  `TestDumpChapterOneMoveFrames`（`FD2_FRAME_DUMP`），輸出同格式的 320×200
  索引畫面。原版一幀約 13 萬指令；開機之後不再讀 `0x3DA`，沒有回掃可掛，用
  取樣加穩定閘門取回幀節奏。用法與收據見
  [96](96-parity-toolchain-20260909.md) 的逐幀擷取一節。
- RE-CLOSED／RUNTIME-E1：「快速順移殘影」已修。原版走行重繪是**獨立的一條
  路徑**，層集合只有地形（`0x11EEE`）＋單位（`0x127E0`×11）＋前景（`0x127A9`），
  整段完全不進 `0x11CAC`、`0x122DC` 與 `0x1AD72`，所以沒有游標白框也沒有左下
  HUD 面板——而 overlay selector `[0x51A83]` 整段仍是 1。重製端先前借用一般
  重繪，把白框留在終點。現在多一個 `ComposeNativeStepFrame`，`g.walk != nil`
  時改走它。收據與量測見 [99](99-move-confirm-cursor-20260909.md)。
- 引擎形狀的結論：原版的地圖重繪**依當下在做什麼挑不同的層集合**（一般重繪／
  走行重繪／`0x24618` 轉場）。層的原語與順序是共用的，重製端拆成獨立 blit 的
  形狀是對的；錯的是少了一個組幀入口。第 7 項預期同構，可用同一套 `-eip-watch`
  直接問。
...
- RE-CLOSED／RUNTIME-E1：對話觸發時的畫面位移與黑邊已修。原版**不為對白換
  世界層**：每個對白循環地圖照樣走 `0x11CAC`（`0x122DC`／`0x11EEE`／`0x127A9`
  各一次、`0x127E0` 十一次），四邊維持 4 px 黑邊、地圖不位移，只有對白框那一
  側因為框畫到邊界上而縮成 2 px。重製端先前在 `battleEvent` 非空時整個切到
  正規化管線，切換那一幀就會跳。現在只有 `storyBG` 走正規化管線。收據與量測見
  [100](100-dialogue-viewport-20260909.md)。
- 待查：故事場景（`storyBG`）的對白仍走正規化管線，本輪沒有取那條路徑的原版
  收據。
- RE-CLOSED／RUNTIME-E1：戰鬥後的傷害數字已從一般玩家路徑移除。五筆
  `battle.attack.*` 的 `source_string_id` 全部指向重製端自己的 Go 原始碼，且
  原版分離 FDTXT 全庫沒有「造成 N 傷害」型模板（僅有的字樣都在劇情對白裡）。
  字串保留作語言包完整性前置檢查，只有 F3 診斷模式才顯示。見
  [102](102-battle-result-message-20260909.md)。
- DATA-READY：原版物理攻擊演出的版面已取到（三段，`CONTINUE` 後每回合先把
  游標推到地圖角落再 END／YES，推進到雙方接戰）。右上攻方、左下守方，各為
  「姓名　LV·NN」＋ HP 條與三位數字 ＋ MP 條與三位數字，命中時 HP 同步變化，
  **整段沒有任何傷害量文字**。走 `0x29164`／`0x2A289→0x18C6D`，不經 `0x24618`。
  收據見 [102](102-battle-result-message-20260909.md)。
- RE-CLOSED／RUNTIME-E1：第 6 項前半的「戰鬥動畫退化」＝**我方被攻擊時腳步被
  台座遮住**，已修。左右由 FIGANI 幀內嵌座標按陣營分邊（亞雷斯 12／13 在
  x=89..178，盜賊 288／289 在 x=6..28），台座固定在右側 slice，所以會壓到台座的
  永遠是我方那張圖；原本固定的「守方→台座→攻方」在敵方攻擊時把我方畫在台座
  之前。現在規則是**台座緊接在我方 figure 之前**。敵方攻擊方向前後差 7564 像素，
  我方攻擊方向與 2026-08-25 存檔逐位元組相同。見
  [103](103-battle-pedestal-zorder-20260909.md)。
- 另外三項檢查沒有找到退化：既有命中 fixture 在目前 HEAD 重跑與 2026-08-25
  存檔 **AE = 0**；版面與原版擷取一致；分離包 409 筆 FIGANI 對 scenario 內
  96 個 `battle_fig` 的待機／攻擊資源**全部齊備**，完整包組裝也明文複製
  `animations/`。見 [102](102-battle-result-message-20260909.md)。

2026-09-08 前沿（部分已被上列取代）：

- RUNTIME-E1：普通物理 EXP 整數來源／寫回已修，v.1.0.14 普通操作已越過首次攻擊。
- RUNTIME-E1：JOIN 先於登場的永久名冊來源已修，逐動作與長鏈回歸通過；
  v.1.0.15 正從 START 重跑，尚待援軍後普通操作。
- 待驗：dosgolem 指令選單剩餘平台缺口、兩側首關勝利→城鎮的連續普通輸入收據。
- 待驗：重生完整尺寸對話／HUD／指令圖示對照；排除 v.1.0.14 第二輪裁切圖片。

唯一目前狀態與本機交付雜湊：[94](94-ch01-town-parity-20260908.md)。
不因歷史 60 格清冊合格而提高尚未通過的完整流程分數。

2026-09-08 測試修正：先前兩項抽測失敗已補齊選人資料並修正開場多語 ID 查詢，
重跑通過；結果與外部存檔略過限制見 [93](93-dialogue-cause-20260908.md)。
此為重製端回歸修正，不重開已完成介面接線，也不提升全段原版一致性。
...
2026-09-08 對話切片：[v.1.0.10 修正驗收](93-dialogue-cause-20260908.md)已補
...
> `RUNTIME-E1`。v.1.0.7 正常 START 已實際到 TURN 003，並補驗中立 Escape 循環、
> 回合聚焦及已行動角色資訊入口。v.1.0.9 本機候選另補第一關普通物品寶箱
> 問答；戰況面板解碼與零金錢修正一併保留。正式 dosgolem 同狀態收據、
> 精確輸入節奏／動畫相位／音訊，以及滿欄交換等其他寶物分支仍未提升；
> F5/F9 為既有節點重啟存檔，不等於原版目前戰況續存；不能用下方歷史發行紀錄宣稱已公開穩定版。
> 原地待機不再依賴移動預覽仍存在才能同步 raw bit7；此分支已另加回歸。
...
> **2026-09-07 v.1.0.3 發行批次：**`v.1.0.0-20260907` 因公開 CI 不可見私人
> 現代逐幀素材而三平台建置失敗，未建立 Release，標籤保留且不移動。
> `v.1.0.1-20260907` 又由更新的 AppImage 工具雜湊與 Windows checkout 換行揭露
> 兩個可重現封裝問題；`v.1.0.2-20260907` 在發行前發現完整原版素材會遮蔽現代
> action icon。三者均未建立 Release。玩家可見修正後唯一正式版號為
> `v.1.0.3-20260907`；三平台
> 公開封包固定為 Linux AppImage、Windows ZIP、macOS universal ZIP。本機
> `dist-all/v.1.0.3-20260907/full/` 另保存含完整原版資料的三平台完整版，禁止
> 上傳公開 Release。公開附件只含重製引擎與可散布資料；完成狀態以發行說明中的
> 雜湊與 GitHub Release 回讀收據為準。
>
> **發布完成：**三平台 Actions 均成功；GitHub Release 已建立並回讀六個附件。
> 本機三平台完整版亦已組裝，Linux 經啟動／解包驗證，Windows 與 macOS 經 ZIP
> 內容驗證。推廣片為 47 秒、1280×720、30 fps、H.264／AAC，不含原版音樂；
> 黑場、長凍結與長靜音檢查無事件。

> **2026-09-07 發行勘誤（取代上述「發布完成」的可玩性解讀）：**玩家實際啟動
> `v.1.0.3` 後直接落入第一關，沒有開場選單、片頭、完整第 0 章對話與音樂。
> 根因已重現為正式程式未設定預設 `FD2_CAMPAIGN`、標題素材載入錯誤後顯示預載
> 戰場、公開包手寫資料目錄漏掉 `cutscenes/` 等追蹤檔，以及本機完整版選錯封存型
> 素材包。該版只保留歷史收據，不再具有穩定版／可玩完成資格。當前最高優先工作是
> 新版正常啟動垂直切片：完整 Git 追蹤資產 → 正確執行期分離包 → 開頭／選單 →
> `story_ch00_handler` → 完整對話與音樂 → 第一關；另以 dosgolem 補真實原版開場
> 對拍。未完成這條路徑前不得再發布或稱為第一版完成。
>
> **修正進度：**提交 `8aac58f2` 已恢復未設定環境變數時的完整戰役預設、缺標題
> 素材阻擋畫面、完整 Git 資產複製與封存包拒絕閘門。後續測試另發現第 0 章截圖
> 快轉仍沿用舊 `dlgAdvance`，沒有消費原生逐字／收框相位；現已改走正式 progress
> 與 input owner，回歸可完整抵達 `battle_ch01`。尚未關閉的發行門檻只保留實際
> 封包正常輸入錄影、音訊錄製，以及 dosgolem 原版開場輸出／對拍；不可用測試快轉
> 截圖取代。

> **2026-09-07 玩家入口追加：**`F1` 為暫停式操作說明；`F2` 改為忠實原版／
> 現代手繪主題切換，原 F2 音源切換移至 `F3`，`F4` 四語、`F5` 存檔、`F6`
> 音樂開關、`F9` 讀檔、`F12` 除錯資訊。戰場系統選單既有音樂／音效／速度／
> 狀態欄按鈕維持正式 consumer。現代 HUD 與四張 action icon 已由 catalog loader
> 驗證並接入 `ringIcons`／HUD consumer；發行封包只收錄 `runtime_candidate`，不帶
> 高解析 concept／master。
...
| 順序 | 工作 | 現況 | 下一個可驗收結果 |
|---:|---|---|---|
| 即時勘誤 | A4／A5 數字 | 2026-09-05 現代地圖人物 catalog 已涵蓋 94 組可繪 selector、共 1,083 個可見影格：89 組為十二幀行走人物，48／66／67／74／75 為原版證據支持的三相靜態圖；39／57 是純目的地重映射，不是待補人物。四語內容各有 5,176 筆；繁中全為來源，簡中尚有 5 筆、日文 560 筆、英文 557 筆 `machine_draft`。兩語各有 170 筆源自單字或 `?` 的破碎說話者名稱，屬來源身分阻擋而非未翻譯；不可為追求全綠而猜測定稿 | selector 數字只由 catalog 重算；美術後續是依核准風格逐組審查辨識度，不再列成缺 62 組 |
...
| A4 | 現代美術主題 | `DATA-CONTRACT-READY／RUNTIME-E1-PARTIAL`：28 位身分已確認角色已有獨立閉嘴頭像母稿／80×80 候選；94 組可繪 FDICON 由正式 loader 原子預檢 1,083 個可見影格後取代正規化 sprite，其中 89 組為十二幀行走人物、5 組為三相靜態圖。2026-09-05 完成 `map0` 至 `map32` 共 33 張現代圖集：各自保留 `24×24` tile ID、地形／事件／碰撞座標，依第一關概念稿及獨立生成的高解析手繪參考統一色彩與材質；直接使用生成 atlas 曾由正式 map0 重組證明格線漂移，故 runtime 只取其風格，結構仍由原圖集逐格控制。96／192／288／384 格四種實際幾何、map ID 與 SHA-256 均由 loader 原子驗證；純黑洞窟／畫布外區域亦保持不變。現代戰鬥狀態面板是獨立無文字 `149×42` 母材，由既有 HUD owner 疊加動態數值；四向選單另有攻擊／法術／物品／待機四張 `28×26` 圖示，取代 `status` 冒充法術的舊 fallback，仍依選取方塊定位。所有素材均原子驗證雜湊、尺寸與不透明度；忠實原版呈現不變。完整 runtime 截圖仍受本機封裝啟動環境阻擋，不提升玩家畫面證據 | 依 selector 000 核准風格逐組抽查頭身比例、原版配色、輪廓與戰場尺寸可讀性；抽測早／中／晚期地圖的相鄰圖塊接縫、前景遮擋、人物、游標、HUD 與方塊附近選單；解決可重現封裝截圖入口後補正常畫面收據 |

> **2026-09-05 A4 核准風格批次：**selector 002、008、009、013 已按 selector 000／032
> 的大頭短身現代手繪母風格重新校稿，正式 48 幀已通過尺寸、二值透明度與 SHA-256
> 清冊驗證；下一批依序審查 015、021、022，再擴展至其餘高可見度 selector。
...
> **2026-09-05 A4 selector 31 追加：**最新統計是四十四組／528 幀，
> 尚缺 52 組。group 31 保留原版方正深藍頭部輪廓、棕膚藍眼小臉、灰綠蓬鬆領巾與藍色腰布；
> 頭部究竟是髮、帽或兜帽尚無證據，不強行命名，只以 selector 31 登錄。

> **2026-09-05 A4 selector 32 勘誤：**舊稿只看灰階匯出，誤判為圓角巨型全罩盔。
> 以 FDOTHER #0 正式 DAC 色盤重檢後，原版 384..395 幀顯示藍髮、紅額帶、裸露臉部、
> 灰綠肩甲、深藍衣裝與淡灰綠短刃；現代十二幀已依彩色證據重畫。身分未閉合，
> 只以 selector 32 登錄，不從外觀猜姓名。

> **2026-09-05 A4 selector 33 追加：**最新統計是四十六組／552 幀，
> 尚缺 50 組。原版彩色十二幀證實 group 33 是紅褐短髮、黑色額帶、灰綠輕甲與
> 淡冰藍短刃的年輕戰士；現代候選保留正面、左右與背向三步態。地圖資料沒有可直接
> 證實的角色身分，故不補姓名或職業，只以 selector 33 登錄。

> **2026-09-05 A4 selector 34 追加：**最新統計是四十七組／564 幀，
> 尚缺 49 組。轉職資料證實 current portrait 2 預設轉到 target 34；兩組彩色實幀
> 都保留棕髮、黑額帶、單一巨大背盾與銀白直刃。首版多畫正面第二面盾已拒收，
> 修正版恢復單一背盾與四方向三步態；姓名仍不從數值猜定，只以 selector 34 登錄。

> **2026-09-05 A4 selector 35 追加：**最新統計是四十八組／576 幀，尚缺 48 組。
> 轉職資料證實 current portrait 3 預設轉到 target 35；現代十二幀保留基礎圖組的
> 紅褐巨髮、濃密大鬍與矮壯身形，並依轉職彩色實幀加入銀灰戰斧、方形灰綠盾與重甲。
> 姓名不由轉職數值反推，只以 selector 35 登錄。

> **2026-09-05 A4 selector 36 追加：**最新統計是四十九組／588 幀，尚缺 47 組。
> 轉職資料證實 current portrait 4 預設轉到 target 36；現代十二幀延續基礎 selector 4
> 的棕髮、寬方形深藍帽與銀白直劍，只依轉職彩色幀增加藍色肩披／短背披及較厚護甲，
> 不新增原版沒有的盾牌或換人。

> **2026-09-05 A4 selector 37 追加：**最新統計是五十組／600 幀，尚缺 46 組。
> 轉職資料證實 current portrait 5 預設轉到 target 37；現代十二幀延續基礎圖組的
> 灰綠長形頭部披覆、銀劍、小型深藍方盾與纖細身形，只把紅褐背披改為原版轉職幀的
> 鈷藍背披並加深衣甲藍色；頭部披覆不在證據不足時硬命名為頭髮或兜帽。

> **2026-09-05 A4 selector 38 追加：**最新統計是五十一組／612 幀，尚缺 45 組。
> 角色表與轉職表共同證實萊汀的 selector 6 預設轉到 target 38；現代十二幀延續
> 深紅褐束髮、黑額帶與銀白槍刃，依轉職彩色幀改為深鈷藍重甲並加入金色腰胸飾。
> 一次誤指 selector 36 母稿的未登錄輸出已在清冊前拒收並刪除。

> **2026-09-05 A4 分母勘誤：**逐幀檢查固定雜湊的原版 `FDICON.B24` 後，96 組中
> selector 39 與 57 的 12 幀皆為 `source-write=0`、`destination-remap=576`，是純
> 目的地色彩重映射操作而非可繪人物。故現代地圖人物美術的有效分母為 94 組；目前
> 完成五十一組／612 幀，尚缺 43 組可繪圖組，另有 2 組特殊合成語意已分類但不以
> 透明 PNG 冒充新版美術。轉職表的 portrait 39／57 不等於 FDICON selector 39／57；
> 正式 runtime 若要在現代 renderer 消費這兩種操作，須另立 typed contract 與 consumer
> 證據，不能猜畫。

> **2026-09-05 A4 selector 40 追加：**最新統計是五十二組／624 幀；以 94 組可繪
> 圖組為分母，尚缺 42 組。角色表與轉職表共同證實希莉亞的 selector 8 預設轉到
> target 40；現代十二幀延續棕色長髮、紅額帶、木弓與箭袋，依原版轉職幀把深藍衣甲
> 改成深酒紅／黑褐色，背向加入淺青灰色斜向箭袋帶，不新增原版沒有的披風或盾牌。

> **2026-09-05 A4 selector 41 追加：**最新統計是五十三組／636 幀；以 94 組可繪
> 圖組為分母，尚缺 41 組。角色表與轉職表共同證實悠妮的 selector 9 預設轉到
> target 41；特殊道具另走 target 52，本組不混用。現代十二幀延續鮮紅高馬尾與
> 藍寶石額帶，依原版預設轉職幀改成炭黑／深灰甲與銀灰肩臂護甲。

> **2026-09-05 A4 selector 42 追加：**最新統計是五十四組／648 幀；以 94 組可繪
> 圖組為分母，尚缺 40 組。角色表與轉職表共同證實馬琳的 selector 10 預設轉到
> target 42；現代十二幀延續粉紫齊頸短髮與金色額飾，依原版轉職幀把紅金長衣改成
> 深藍綠／炭黑衣甲與淡青綠／銀灰肩臂護甲，不把模糊輪廓猜成武器。

> **2026-09-05 A4 selector 43 追加：**最新統計是五十五組／660 幀；以 94 組可繪
> 圖組為分母，尚缺 39 組。角色表與轉職表共同證實索菲亞的 selector 11 預設轉到
> target 43；現代十二幀延續灰金短髮、紅色髮飾與短杖，依原版轉職幀把深藍／紅衣裝
> 改成棕紅、赭橙、深褐與少量深綠金邊長衣。

> **2026-09-05 A4 selector 44 追加：**最新統計是五十六組／672 幀；以 94 組可繪
> 圖組為分母，尚缺 38 組。角色表與轉職表共同證實凱麗的 selector 12 預設轉到
> target 44；固定原版中兩組的十二幀，以及各幀 frame／mask／remap 三層，全部位元組
> 相同。因此新版 group 44 明確複用已核准 group 12，美術不硬造不存在的轉職差異。

> **2026-09-05 A4 selector 45 追加：**最新統計是五十七組／684 幀；以 94 組可繪
> 圖組為分母，尚缺 37 組。角色表與轉職表共同證實貝克威的 selector 13 預設轉到
> target 45；原版兩組的 mask／remap 完全相同但 12 張 frame 全部不同。現代稿保留
> 長金髮、黑額帶、木弓與箭袋，只依原版把深墨綠衣甲改成明亮黃綠／草綠。

> **2026-09-05 A4 selector 46 追加：**最新統計是五十八組／696 幀；以 94 組可繪
> 圖組為分母，尚缺 36 組。角色表與轉職表共同證實珊的 selector 14 預設轉到
> target 46；原版兩組的 frame／mask／remap 十二幀皆不同。現代稿保留鮮藍高馬尾與
> 綠寶石額帶，依原版重建深綠內衣甲、紅褐外袍／披肩及四方向輪廓。

> **2026-09-05 A4 selector 47 追加：**最新統計是五十九組／708 幀；以 94 組可繪
> 圖組為分母，尚缺 35 組。轉職表證實 current portrait 15 預設轉到 target 47，姓名
> 仍未閉合。原版 12 張 frame 全部不同、mask／remap 只有最後一幀不同；現代稿保留
> 深藍全罩重甲、窄眼縫與黃眼，只把紅色腰腿點綴改成冷藍紫，不猜角色身分。

> **2026-09-05 A4 selector 49 追加：**最新統計是六十組／720 幀；以 94 組可繪
> 圖組為分母，尚缺 34 組。轉職表證實 current portrait 17 預設轉到 target 49，
> class 15、移動力增加 1；姓名仍未閉合。原版 group 17／49 的 12 幀逐位元完全
> 相同，因此現代素材與 group 17 精確重用，不另畫未受證據支持的差異。

> **2026-09-05 A4 selector 50 追加：**最新統計是六十一組／732 幀；以 94 組可繪
> 圖組為分母，尚缺 33 組。角色表與轉職表證實這是索爾使用 item 89 的 optional
> target 50，class 17、移動力增加 2；職業名稱不猜。現代稿按 FDOTHER #0 正式色盤
> 與 12 幀原版輪廓，保留深鈷藍長髮、紅色披巾／短披風及深綠灰裝甲，未添加武器。

> **2026-09-05 A4 selector 51 追加：**最新統計是六十二組／744 幀；以 94 組可繪
> 圖組為分母，尚缺 32 組。角色表與轉職表證實這是哈諾使用 item 93 的 optional
> target 51，class 18、移動力不變；職業名稱不猜。現代稿保留栗紅短髮、深綠灰
> 斗篷式裝甲與方向性持物遮蔽，第二列 f03–f05 不顯示原版該方向沒有的短刃。

> **2026-09-05 A4 selector 52 追加：**最新統計是六十三組／756 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 31 組。角色表與轉職表證實這是悠妮使用 item 90 的
> 特殊 target 52，class 21、移動力增加 2。原版 f06 與 f08 逐位元相同；現代清冊
> 新增 `source_exact_repeats`，兩槽保存相同 PNG 位元組，不為湊足不同雜湊虛構動作。

> **2026-09-05 A4 selector 53 追加：**最新統計是六十四組／768 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 30 組。角色表與轉職表證實這是哈瓦特使用 item 93 的
> optional target 53，class 18、移動力不變。現代稿保留栗紅長髮／鬍鬚、灰綠大盾、
> 銀灰短柄斧與四方向盾斧側別；只有第三方向沿用 selector 35 的原版像素語意。

> **2026-09-05 A4 selector 54 追加：**最新統計是六十五組／780 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 29 組。角色表與轉職表證實這是亞雷斯使用 item 205 的
> optional target 54，class 19、移動力增加 2。現代稿按正式色盤保留大型鈷藍矩形
> 盔帽／護面、深藍甲衣、灰藍圓形肩背甲與方向性窄銀直刃，不猜職業名稱。

> **2026-09-05 A4 selector 55 追加：**最新統計是六十六組／792 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 28 組。轉職表證實 current portrait 5 使用 item 205 的
> optional target 55，class 19、移動力增加 2；姓名仍未閉合。現代稿保留灰綠大型
> 兜帽／髮罩、鈷藍甲衣、灰藍圓形肩背甲與方向性窄銀直刃，不猜角色身分。

> **2026-09-05 A4 selector 56 追加：**最新統計是六十七組／804 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 27 組。轉職表證實 current portrait 6 使用 item 205 的
> optional target 56，class 19、移動力增加 2；姓名仍未閉合。現代稿依正式色盤保留
> 深紅褐大型束髮、鈷藍／鋼藍甲衣與方向性窄銀直刃，不猜角色身分或職業名稱。

> **2026-09-05 A4 selector 58 追加：**最新統計是六十八組／816 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 26 組。原版 sprite 696–707 與 selector 40 的
> sprite 480–491 十二幀逐位元相同；現代清冊保留獨立 selector 58 索引並精確重用
> selector 40 的十二幀，不重畫不存在的差異，也不猜角色身分或職業名稱。

> **2026-09-05 A4 selector 59 追加：**最新統計是六十九組／828 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 25 組。原版 sprite 708–719 與 selector 41 的
> sprite 492–503 在圖像、透明遮罩及換色遮罩皆逐位元相同；現代清冊保留獨立
> selector 59 索引並精確重用 selector 41，不重畫不存在的差異或猜測身分。

> **2026-09-05 A4 selector 60 追加：**最新統計是七十組／840 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 24 組。轉職資料證實 current portrait 10 使用 item 88
> 的 optional target 60，class 22、移動力增加 1；原版 sprite 720–731 與 selector 42
> 三層素材全數逐位元相同，現代清冊保留獨立索引並精確重用 selector 42。

> **2026-09-05 A4 selector 61 追加：**最新統計是七十一組／852 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 23 組。轉職目標表證實 target 61 為 class 22、移動力
> 增加 1；原版 sprite 732–743 與 selector 43 的圖像、透明遮罩及換色遮罩全數
> 逐位元相同，現代清冊保留獨立索引並精確重用 selector 43，不猜角色身分。

> **2026-09-05 A4 selector 62 追加：**最新統計是七十二組／864 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 22 組。轉職資料證實 current portrait 12 使用 item 92
> 的 optional target 62，class 24、移動力增加 1；原版 sprite 744–755 與 selector
> 12／44 三層素材全數逐位元相同，現代清冊精確重用 selector 44。

> **2026-09-05 A4 selector 63 追加：**最新統計是七十三組／876 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 21 組。轉職資料證實 current portrait 13 使用 item 88
> 的 optional target 63；target 表直接記錄 class 20、移動力增加 1。原版只與
> selector 13 共用透明／換色遮罩，十二張可見圖均獨立；現代稿因此另繪大型深色
> 圓頂罩體、黑色面罩雙亮眼與銀灰側構件，不把 selector 13 的金髮外觀複製過來。

> **2026-09-05 A4 selector 64 追加：**最新統計是七十四組／888 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 20 組。轉職資料證實 current portrait 14 使用 item 88
> 的 optional target 64，class 22、移動力增加 1；原版 sprite 768–779 與 selector 46
> 三層素材全數逐位元相同，現代清冊精確重用 selector 46。

> **2026-09-05 A4 selector 65 追加：**最新統計是七十五組／900 個動畫槽位；以
> 94 組可繪圖組為分母，尚缺 19 組。轉職資料證實 current portrait 15 使用 item 91
> 的 optional target 65；target 表直接記錄 class 24、移動力增加 1。原版
> sprite 780–791 與 selector 47 三層素材全數逐位元相同，現代清冊精確重用。

> **2026-09-05 A4 selector 48／66／67 靜態契約追加：**最新統計是七十八組／909 個
> 可見槽位；以 94 組可繪圖組為分母，尚缺 16 組。三組原版都只有前三槽可見，
> 後九槽是零可見圖形加全幅換色遮罩；正式清冊與 Go loader 新增
> `fdicon_static_sprite_3x24_v1`，只接三張互異靜態相位，方向索引有界回落至相位，
> 不生成九張假步行幀。2026-09-05 重新以固定分離包的 `group×12+frame`
> 核對後，撤回 selector 48 的「藍帽、棕鬍、紅披風王者」斷言：那是把人物編號
> 誤投射成 FDICON selector 的舊便利輸出。原始 576–578 是方形全罩灰白機甲輪廓，
> 現代王者稿已拒收並另產生機甲修正版候選。66 保留紫灰
> 大型髮罩與橙金衣甲；67 保留灰藍蓬髮與墨綠肩衣。66／67 身分仍未知，不猜命名。

> **2026-09-05 A4 selector 048／050–056／063 重校：**九組已改由固定原版分離包的
> `group×12+frame` 輪廓校稿，正式同步至 runtime prototype 與私人素材包，並由
> 工具回算公開 catalog 雜湊。048 的王者錯稿已由方形全罩機甲取代；052 保留
> `f08=f06`；其餘各組維持可辨識的頭甲、面窗、盾劍與四方向差異。這是現代素材
> 忠實度修正，不改變 94 組／1083 個可見槽位統計。

> **2026-09-05 A4 selector 066–068 重校：**066／067 已依固定原版輪廓重新產生
> 三相靜態稿；068 已重製為保留方正全罩頭盔、白十字盾與直劍的普通量產士兵，
> 不再英雄化。三組正式同步至 runtime prototype 與私人素材包，catalog 與 manifest
> 由工具重建；可繪組及可見槽位總數不變。

> **2026-09-05 A4 selector 069–073 核准風格同步：**將先前已在戰場比例總覽檢視的
> 五組候選正式同步至 runtime prototype 與私人包；070 只採修正版，舊候選維持
> 拒收。十字盾重甲、深藍帽面罩、角狀重甲、背負輪廓及馬身兵器等玩家可見特徵
> 均與固定原版分離幀逐方向核對，catalog 與 manifest 由工具重建。

> **2026-09-05 A4 selector 077–079 核准風格同步：**077 已由偏寫實的高身比
> 改為大頭短身重甲；078 已由放大像素稿改為現代手繪深藍重甲；079 統一
> 兜帽遮面、短身與單一彎刃比例。三組均以固定原版十二幀輪廓及已登錄
> 彩色證據限制，未猜姓名或職業；24×24 二值透明總覽已通過人工檢視。

> **2026-09-05 A4 selector 082–083／085–086 核准風格同步：**四組已從方塊化或
> 放大像素舊稿重製為大頭短身手繪重甲，並依已登錄色盤保留各自盔形、眼光、
> 盾牌、兵器與下擺。085 首版擅加菱形徽飾已拒收，正式版回復素面盾與圓形盾釘。
> 四組 24×24 幀位的方向、遮擋與二值透明總覽已通過人工檢視。

> **2026-09-05 A4 selector 087／089–090／092–095 核准風格收尾：**七組已從
> 放大像素或比例不一舊稿改為大頭短身手繪風格。087 背側短筒不猜弓箭，089 側向
> 網格板不猜武器，090 不新增手持物；092–095 的面窗、格窗與燈號不人格化。各組
> 24×24 幀的四方向、遮擋、二值透明與縮圖辨識均已通過人工檢視。至此
> catalog 的 94 組可繪 selector 都有正式現代候選；39／57 仍依原版證據保留為
> 純調色重映射，不偽造人物圖。

> **2026-09-05 A4 selector 69 追加：**最新統計是七十九組／921 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 15 組。原版 sprite 828–839 為完整四方向三步態且
> 無整組重用；現代稿依正式色盤另繪大型灰綠矩形重甲、十字盾、窄銀直劍與深紅
> 關節，不補人臉或猜測角色／職業身分。

> **2026-09-05 A4 selector 70 追加：**最新統計是八十組／933 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 14 組。原版 sprite 840–851 為完整四方向三步態且
> 三層素材無整組重用；修正一次誤產生的 4×4 草稿後，正式現代稿以正確 3×4 保留
> 深藍軟帽、淺灰綠圓形護目鏡／面罩、窄銀直劍與深棕斗篷，不猜物件語意或身分。

> **2026-09-05 A4 selector 71 追加：**最新統計是八十一組／945 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 13 組。原版 sprite 852–863 是完整四方向三步態，
> 可見幀均為獨立圖像；現代稿依正式色盤保留大型藍灰角狀／獸首裝甲、深色面罩、
> 分段軀幹與棕色腰帶，但不猜為特定種族、角色或職業。

> **2026-09-05 A4 selector 72 追加：**最新統計是八十二組／957 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 12 組。原版 sprite 864–875 是完整四方向三步態且
> 可見幀無整組重用；正式色盤證實是深藍髮／頭巾、藍灰上衣、紅棕靴與大型棕色
> 背包的人形。現代稿依彩色證據重繪，並訂正先前灰階檢視造成的白灰重甲誤判。

> **2026-09-05 A4 selector 73 追加：**最新統計是八十三組／969 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 11 組。原版 sprite 876–887 是完整四方向三步態且
> 可見幀均為獨立圖像；現代稿依正式色盤保留棕色馬形／騎乘輪廓、直立長槍、
> 淺灰十字盾與紅色鞍具，不猜特定種族、角色或職業。

> **2026-09-05 A4 selector 74 追加：**最新統計是八十四組／972 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 10 組。原版 sprite 888–890 只有三張可見相位，
> 891–899 是空白保留槽；現代稿依正式色盤重繪紫帽紫袍、紅色腰腹布片、棕色
> 背負物與橙棕杖端，只輸出三張，不補造其他方向。

> **2026-09-05 A4 selector 75 追加：**最新統計是八十五組／975 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 9 組。原版 sprite 900–902 只有三張側向低矮人形
> 相位，903–911 是空白保留槽；現代稿保留紅髮／紅披風、藍色衣甲及棕／橙／紫
> 部件，不把姿態猜成特定劇情，也不補造其他方向。

> **2026-09-05 A4 selector 79 追加：**最新統計是八十六組／987 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 8 組。原版 sprite 948–959 是完整四方向三步態且
> 三層素材均無整組重用；現代稿依正式色盤保留深橄欖綠寬帽、象牙白面罩、深藍
> 上衣與斜持銀劍，背向不露面罩，也不猜角色或陣營。

> **2026-09-05 A4 selector 81 追加：**最新統計是八十七組／999 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 7 組。原版 sprite 972–983 是完整四方向三步態且
> 可見幀均為獨立圖像；現代稿依正式色盤保留深藍甲殼／重甲、雙角、紅色眼縫、
> 巨大圓肩與暗紅垂布，背面不露眼縫，也不猜種族或職業。

> **2026-09-05 A4 selector 84 追加：**最新統計是八十八組／1011 個可見槽位；以
> 94 組可繪圖組為分母，尚缺 6 組。原版 sprite 1008–1019 是完整四方向三步態；
> 正式色盤證實為鮮紅方正重甲、短米灰側角、黃色眼縫、橙紅旋紋方盾與直立銀劍，
> 現代稿保留左右裝備側別與背面大片紅甲，不猜角色或職業。

> **2026-09-05 A4 戰場人物風格定案：**使用者核准 selector 000 修訂候選作為全體
> selector 的共通方向：大頭約佔全高 55–60%、短身短腿、現代手繪光影，同時保留
> 原版髮色、頭帶、盔甲、武器、盾牌與主要配色。selector 000 與誤讀灰階的 032 已先
> 修訂。視覺稽核列出優先重畫 001、002、008、009、013、015、021、022、023；尚缺
> 素材的 087、089、092–095 仍按原版彩色證據新繪。其餘既有候選先保留並抽查四方向、
> 步態、背景對比與腳底基準，不為形式一致而無證據地全部推倒重畫。

> **2026-09-05 A4 核准風格首批擴展：**selector 001 已依 selector 000 核准母稿重畫
> 十二幀，保留原版栗紅短髮、黑額帶、灰綠斜向肩胸護具、深棕衣裝與深藍腰腿區，
> 並改為大頭短身比例；清冊雜湊與正式 loader 驗證後才列為可執行期候選。

> **2026-09-05 A4 核准風格第二批擴展：**selector 002 已依相同基準重畫十二幀，
> 保留棕髮、黑額帶、深藍衣甲、單一巨大淡灰綠塔盾與窄銀直劍；正面、側面及背向
> 都由同一面盾維持原版遮蔽關係，不把方向差異誤畫成多面盾。

> **2026-09-05 A4 核准風格第三批擴展：**selector 008（希莉亞基礎造型）已重畫
> 十二幀，保留棕色長髮、紅額帶、紫色頸巾、深綠衣甲、木弓與箭袋；背向不露臉，
> 四方向維持裝備側別並改為核准的大頭短身比例。

> **2026-09-05 A4 核准風格第四批擴展：**selector 009（悠妮基礎造型）已重畫
> 十二幀，以正式彩色幀保留朱紅高馬尾、藍寶石額帶、藍眼、深藍與紅棕布裝。
> 本輪拒收一次把灰階輪廓誤判為全罩頭盔的稽核結論；背向大片區域是紅髮，身分由
> 角色表閉合為悠妮，但手持小輪廓仍不猜成特定武器。

> **2026-09-05 A4 核准風格第五批擴展：**selector 013（貝克威基礎造型）已重畫
> 十二幀，保留巨大金黃長髮、黑額帶、深墨綠衣甲、木弓與箭袋；側面及背向的黃色
> 大輪廓維持為長髮，不誤畫成兜帽或披風，並採核准的大頭短身比例。

> **2026-09-05 A4 核准風格第六批擴展：**selector 015 已重畫十二幀，保留深藍
> 全罩重甲、黑色面罩、黃綠眼光、灰綠腹帶與鮮紅腰布；側面只露單眼光、背向不露
> 眼光，身分與武器均不猜。

> **2026-09-05 A4 核准風格第七批擴展：**selector 021 已依角色表與使用者勘誤重畫
> 為約拿：銀白蓬髮、白色大鬍鬚、棕褐長袍與木杖的聖者老人。十二幀維持四方向、
> 背向不露臉及大頭短身比例，不再出現年輕人或錯誤動物造型。

> **2026-09-05 A4 核准風格第八批擴展：**selector 022 已依原版十二幀重畫，保留
> 藍黑高馬尾、紅髮扣、灰綠護甲與深藍下裝；身分、職業與武器仍未知，不猜補。

> **2026-09-05 A4 核准風格第九批擴展：**selector 023 已依原版十二幀重畫，保留
> 巨大棕髮、深藍額帶與甲衣、金色飾帶、暗紅披布及側向短直刃。已拒收會把未定義
> 藍黑輪廓誤畫成箭袋的草稿；正式幀不含弓箭或箭袋。

> **2026-09-05 A4 核准風格第十批擴展：**先前缺少的 selector 087／089 已各新增
> 十二幀。087 對不明棕色背側輪廓採失敗即關閉，不畫箭矢、箭羽或弓弩；089 保留
> 灰綠方形金屬頭體、深紅圓頂、雙色面窗亮點與側向網格圓板。兩組均不猜姓名或
> 職業。catalog 目前涵蓋 90／96 組，尚缺 39、57、92、93、94、95。

> **2026-09-05 A4 核准風格第十一批擴展：**selector 092–095 已各新增十二幀並
> 通過原版輪廓抽查。catalog 現涵蓋全部 94 組可繪 FDICON 圖組；39／57 經原始
> mask 證實為純目的地調色重映射，沒有來源人物像素，故不生成透明人物圖冒充完成。

> **2026-09-05 A4 selector 30 追加：**最新統計是四十三組／516 幀，
> 尚缺 53 組。group 30 保留原版灰綠繃帶、鈷藍眼帶、深藍內衣、紫藍腰布與爪狀雙手；
> 背向不露正面眼縫，不補裸臉或武器。身分未閉合，只以 selector 30 登錄。

> **2026-09-05 A4 selector 29 追加：**最新統計是四十二組／504 幀，
> 尚缺 54 組。group 29 保留原版光滑藍灰橢圓頭部、小型淺藍眼光、厚灰綠環領與鈷藍長袍；
> 由於原小圖無法區分臉、面具或頭盔，不補鼻口或種族，只以 selector 29 登錄。

> **2026-09-05 A4 selector 28 追加：**最新統計是四十一組／492 幀，
> 尚缺 55 組。group 28 保留原版深藍圓頂全罩重甲、黑色面縫雙紅眼、肩側短刺與灰綠
> 腹腰護甲；背面無紅眼或面甲，不追加任何武器。身分未閉合，只以 selector 28 登錄。

> **2026-09-05 A4 selector 27 追加：**最新統計是四十組／480 幀，
> 尚缺 56 組。group 27 保留原版橄欖綠龍人、成對翼片、灰銀直刃與黑十字白色鳶形盾；
> 初版背向多畫劍、第二版意外刪除右向兩幀劍盾，均遭拒收；定稿背面無武器、其餘
> 三向皆保留劍盾。身分未閉合，只以 selector 27 登錄。

> **2026-09-05 A4 selector 26 追加：**最新統計是三十九組／468 幀，
> 尚缺 57 組。group 26 保留原版深藍灰龍首、灰綠甲身、成對摺疊翼片與金色直刃；
> 初版背向過多露刃遭拒收，修正後只有第三步右側露出金刃。身分未閉合，故只以
> selector 26 登錄。

> **2026-09-05 A4 selector 25 追加：**最新統計是三十八組／456 幀，
> 尚缺 58 組。group 25 保留原版深藍蒙面兜帽、窮亮眼與水平銀刃；初版母稿因
> 背面錯畫刀刃遭拒收，修正後第三列只顯示背向兜帽。身分未閉合，故只以
> selector 25 登錄。

> **2026-09-05 A4 selector 24 追加：**最新統計是三十七組／444 幀，
> 尚缺 59 組。group 24 保留原版禿頭老者、灰白鬍、金黃立領披肩與深紅長袍；
> 原圖沒有武器，也沒有足夠資料確定人名，故只以 selector 24 登錄。

> **2026-09-05 A4 selector 23 追加：**最新統計是三十六組／432 幀，
> 尚缺 60 組。group 23 保留原版巨大棕色長髮、深藍額帶與甲衣、金色飾帶、
> 暗紅披風及側向可見短直刃；因身分未閉合，只以 selector 23 登錄。

> **2026-09-05 A4 selector 22 追加：**上表 A4 的三十四組／408 幀統計
> 自本次起由三十五組／420 幀取代，尚缺 61 組。group 22 依原版
> 四方向三步態製作現代母稿與 `24×24` 候選，保留藍黑長髮、紅髮飾與
> 灰綠護甲；尚無足夠資料確認角色身分，故只以 selector 22 登錄，不硬編人名。
...
> **2026-09-07 勘誤：**本歷史批次的比較用了錯誤 codec，不能視為原版畫面一致。
> 四模式解碼修正與真正 AppImage 畫面補證見 `92`，不得據此重開其他已閉合圖塊。
