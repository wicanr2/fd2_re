...
0x25dd5  │   cmp [0x53ecc],1 ─ ==1 → call 0x22e5c(世界地圖/中場)→ [0x53ecc]=0 → 0x25e74
0x25e02  │   cmp [0x53ecc],2 ─ ==2 → 停曲; call [章節*4+0x51de9](戰後/勝利劇情);
0x25e2a  │                          call 0x2cad7(結局判定); test eax
0x25e3a  │                          ├ 非結局: call [章節*4+0x51d71](下一章戰前)+ play_bgm(下一章)
...
| 動作 | 處數 | 位置 | 意義 |
|---|---|---|---|
| `mov [0x53ecc], 1` | **22** | 全在 0x205c9–0x20c64 | 事件解譯器:觸發「需進世界地圖/中場」 |
| `mov [0x53ecc], 2` | **6** | 同區 | 事件解譯器:戰鬥勝利/章節結束達成 |
...
- **寫方**:**戰場/劇情事件腳本解譯器(0x205c9–0x20c64 一個大函式)**,依事件指令設碼。
- **讀方**:**戰役迴圈**(0x25dd5/0x25e02)據碼分派、處理後清 0;過場/讀檔模組讀 ==0 確認無 pending。
- **碼義**:`1` = 進世界地圖/中場(handler 0x22e5c:`play_bgm` + 載 FDOTHER 資源 #0x4f 全螢幕圖);
  `2` = 勝利/章節結束(→ 戰後跳表 0x51de9 + 結局判定 0x2cad7 + 下一章跳表 0x51d71);`0` = 閒置。
- **修正記憶**:上輪記「`[0x53ecc]` 只管戰後分支(1 事件/2 勝利)」**方向對但不完整**——它其實是事件解譯器與戰役迴圈間的通用 pending 碼,`==1` 精確語意是「進世界地圖/中場」(非泛稱事件)。已回填。
- **[阻]** 事件解譯器(0x205xx)入口無清楚 stack-probe prologue、`callers` 為空 → 它經事件指令 dispatch/間接呼叫進入(戰場迴圈 0x117e7 解譯 FDTXT 事件指令時觸發),精確 caller 鏈與 22 個 `=1`/6 個 `=2` 各對應哪種事件指令未逐條展開。
