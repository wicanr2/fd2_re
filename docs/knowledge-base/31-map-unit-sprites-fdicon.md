...
- 原版實機截圖(`real_pic/`)+ DATO face 當 oracle 佐證**地圖 sprite 組**(0→0)。註:此處 `fig` 指地圖小人組號;**敵方地圖組 ≠ 角色id**(§6 修正:盜賊組96/角色id70),戰鬥動畫 FIGANI 用角色id×3(doc06)。
...
> **敵方/通用(2026-06-29 修正)**:**地圖 sprite 組(FDFIELD)≠ 角色 id(DATO肖像/FIGANI)**,敵方不一定恆等!
> 反證:**盜賊地圖 FDICON 組96,但角色 id=70**(DATO_070;戰鬥動畫 FIGANI=70×3=210,反組譯驗證,doc06)。
> 士兵組68=DATO68 恰好恆等。即敵方「地圖小人組號」與「對話/戰鬥動畫角色 id」要分開看;只有**我方 0–31** 才三者(組=肖像=角色id)恆等。
