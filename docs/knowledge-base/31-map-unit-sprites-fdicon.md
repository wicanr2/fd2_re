...
> **敵方/通用**:地圖 sprite 組(`unit[+7]`)與 DATO 對話肖像(FA)**可不同**(盜賊 sprite組96、DATO肖像70)。
> 但 **FIGANI = sprite組 × 3**(恆等,盜賊組96→FIGANI288,doc06),用的是 sprite組,不是 DATO 肖像。我方 0–31 才 sprite組=肖像。
