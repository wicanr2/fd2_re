...
> **少數「龍人系/後期新增角色」跳號**:**portrait 67 = 凱拉斯(龍劍士)→ 組 49**(icon_588–599),不是組 67(跳號);組17(icon_204)是另一個長得幾乎一樣的「一般龍人兵」。
...
>
> **★ 重大考據:轉職當機 = 轉職後 sprite 組不存在**。青衫攻略警告「龍人轉職可能當機」——對應「廢案/沒畫滿 12 格的組」:**角色轉職會切換 sprite 組,若轉職後職業的組沒畫(空/缺),遊戲讀不到 sprite 而當機**。remake 必須**為轉職後職業備齊 sprite**(或避免缺組),否則重現原版 bug。詳見 doc 32(轉職)。
