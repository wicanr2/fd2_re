...
| 指令28／29／31校正 | 閉合caller分歧 | command29資產就緒；28／31 selector未知 | 校正後數值E1；indexed owner未接 | 缺E2 | `0x276EC`已固定28的first-target base／mode0／無`0x29C90`／分母8，以及29／31的actor base／mode1／逐target轉場／分母1。FIGANI全檔可達effect每target只有一個impact marker，故28正式delta是roll/8；舊完整roll交易已修正，29／31另有倍率回歸。主證據見[`fd2_command28_29_31_presentation_ida.txt`](../data/ida/fd2_command28_29_31_presentation_ida.txt)。 |
