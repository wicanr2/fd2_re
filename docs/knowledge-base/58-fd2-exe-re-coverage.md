...
| `0x16F55`、`0x1728C`、`0x117E7`、`0x4E8E1` | [`fd2_system_overlay_options_ida.txt`](../data/ida/fd2_system_overlay_options_ida.txt)、[`fd2_end_turn_command13_owner_ida.txt`](../data/ida/fd2_end_turn_command13_owner_ida.txt)、[`empty cursor owner`](../data/ida/fd2_117e7_empty_cursor_system_overlay_ida.txt)、[`END確認框`](../data/ida/fd2_end_turn_confirmation_ui_ida.txt)、[問句同狀態比較](../data/ui-traces/native-end-turn-confirmation-original-vs-remake-e1.json)、[逐字回覆動態配對](../data/ui-traces/native-end-turn-response-progressive-original-vs-remake-e1.json) | `0x12C0D==-1` 空游標 owner；direction2→`0x1728C` 四項設定已依 raw bytes、cells 54..75、音樂／音效／演出／HUD consumer與原生／JSON存檔接成正式 E1；direction3→END、DATO #75／FDOTHER #5/#2 indexed確認生命週期、FDTXT `0x1A3/0x1A4/0x19C`、YES／NO逐字形發布、來源復原、`0x1A30B` 與正式 battle E1；`0x4E8E1`由右往左寫入且`0x9017`是右端錨點 | 設定面板與END的精確 DOS tick／音訊及收合逐幀同相位、selector0巢狀current-runtime存讀檔、selector1全軍行動、逐章重製PLAYER-E2；nested selector3離場目的地已閉合，不重解；亦不重解已閉合設定writer／consumer、END分支與renderer |
| `0x19DF7`、`0x1B1E7` | [`fd2_nested_system_menu_ida.txt`](../data/ida/fd2_nested_system_menu_ida.txt)、[`fd2_system_exit_and_group_march_ida.txt`](../data/ida/fd2_system_exit_and_group_march_ida.txt) | `RE-CLOSED`、`DATA-READY`、`RUNTIME-E1`：nested cells `36、39/41、42/44、45`、save-file與raw `+5` table writers、資訊／存檔／讀檔／離場四分派；正式空游標鍵盤路徑現消費資訊畫面12次展開＋等待input／`0x4DFCC` palette pulse＋12次收合、entries `0x85..0x88`、章節／round／currency／三camp count與兩行FDTXT；selector3另依FDTXT `0x19F/0x1A0/0x19C`及完整`-1` caller chain，在接受回覆、停止BGM、200 ms與收合後正常終止程式，取消不改狀態；缺raw／VGA／DAC／資產時原子拒絕 | 原版current-runtime save不等於現有JSON battle restart；外層selector1的`sub_14B78`結果／間接effect仍未型別化。資訊／離場尚缺同狀態逐幀／tick／音訊E2。不得重解四分派、資訊schedule或selector3目的地，只有直接反證才重開 |
...
| `0x1D6C8`（command 17–23／25–27 玩家 palette owner） | [`fd2_command_modifier_palette_ida.txt`](../data/ida/fd2_command_modifier_palette_ida.txt) | 唯一caller `0x1CFF0`、#88 sub0、三張36-byte DAC table、四輪color／black；17–23與25–27玩家正式E1均先演出後交易，23並串接兩次`0x22253`離場／入場 | 補phase-expiry caller、status UI、command23同狀態camera／逐幀、精確tick與逐音訊E2；不重解palette loop或`0x22253` callee |
