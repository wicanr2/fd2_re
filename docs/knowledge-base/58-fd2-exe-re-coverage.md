...
| 對話、頭像與過場原語 | 部分偏高 | ch00_pre 97句、ch01_pre 20句與ch24_post 18句原生版面就緒 | 部分 E1 | 部分 | `0x15F84`、`0x1366A`、基本 pan／acting／spawn／join 等不應再從零重解。`ch00_pre`十九個caller的97句、`ch01_pre`四個caller的20句與`ch24_post`兩個lookup的18句，均由raw `FFEC..FFEF/FFFE/FFFD`建成typed pages；[`fd2_story_dialogue_layout_ida.txt`](../data/ida/fd2_story_dialogue_layout_ida.txt)閉合`sub_165AC`五階段開框、最終格網、文字座標、`0x1645F..0x1647F`逐字順序、`sub_16C57→sub_16559`完整頁等待期m0/m3嘴型，以及`sub_16B43`的16×5→12×4→8×3→4×2→原背景snapshot restore與可選游標尾段。序章正式輸入已消費97句並連續抵達`battle_ch01→town_ch02→preparation_ch02→story_ch02_pre→battle_ch02`；第2戰前冷讀路徑完成20句，第25戰戰後runner完成18句並抵達town26／祕密商店，三者達caller-specific `RUNTIME-E1`。序章map32兩筆缺constructor來源的特殊劇情角色只在私有背景clone採99%保守前景近似，不污染戰鬥或存檔；見[`ch00-pre-native-dialogue-e1.json`](../data/ui-traces/ch00-pre-native-dialogue-e1.json)。只重開E2或其他caller binding；不把重製更新分配冒稱DOS精確時鐘。 |
