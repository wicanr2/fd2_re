...
| 城鎮、祕密商店、商店、教會與整備 | 部分偏高 | 部分就緒 | 多個正式 E1 consumer | ch02 若干狀態 E2；其餘部分 | 個別 menu、購買、轉移、復活、轉職與整備已有窄切片。教會轉職與商店購買／賣出／獨立裝備（standalone-equip）／自身轉移（self-transfer）現均由正式 mutation 返回 town，再穿越重製 JSON 存讀檔保存金幣、緊密／原始背包、裝備、能力、typed roster 與隊伍拓撲；讀檔亦分別清除未序列化教會、商店及 item-panel 暫態。第25戰正式勝利、持續隊伍與存讀檔後，`town_ch26` 已以章節專屬 selection4＋Shift+F5（BIOS `0x58`）揭露selection5、進入variant5、驗證三項商品並經四幀離店返回；錯用ch02的`0x54`會拒絕。這些是正常戰間E1，不是未修改原版E2；其餘章節入口、native save、recipient scroll與完整交易仍缺。 |
