...
| 玩家指令、法術、物品與交易 | 部分 | 部分 | 部分 E1／部分失敗即關閉 | 缺完整 E2 | command mask、若干 ID、MP／物品交易與 selector 邊界已解；chapter0 current-runtime 的空游標 Down→END→YES 已沿未修改原版 E2 輸入接到四幀關閉、重製端確認、敵方回合及玩家回合返回。其餘三格 owner、未知 command、複合技、法術／物品完整效果、rollback、精確演出與 UI 尚未閉合。這是目前真正需要局部反組譯與實作並行的區域。 |
