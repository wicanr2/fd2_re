...
| 對話、頭像與過場原語 | 部分偏高 | ch24開框／穩定頁／逐字序列就緒 | 部分 E1 | 部分 | `0x15F84`、`0x1366A`、基本 pan／acting／spawn／join 等不應再從零重解。`ch24_post` 兩個 lookup 的18句已由 raw `FFED/FFEF/FFEE/FFFE/FFFD` 建成 typed pages；[`fd2_story_dialogue_layout_ida.txt`](../data/ida/fd2_story_dialogue_layout_ida.txt) 閉合`sub_165AC`五階段開框、最終格網、文字座標及`0x1645F..0x1647F`逐字寫入順序。正式第25戰戰後 runner 已消費原版五階段框、頭像、字模，以及每頁從空文字到逐字完成的indexed序列後抵達 town26／祕密商店，達 caller-specific `RUNTIME-E1`。只重開嘴型、closing中間幀、E2或其他 caller binding；不把重製更新分配冒稱DOS精確時鐘。 |
...
|---|---|---|---|
| `0x15F84` | [`29`](29-remake-extensible-event-system.md)、[`fd2_story_dialogue_layout_ida.txt`](../data/ida/fd2_story_dialogue_layout_ida.txt)、各 handler 直接指令 | 基本 renderer、四種故事開框碼、`FFFE/FFFD`、`sub_165AC`五階段opening、最終格網、逐字寫入順序與 ch24 index6/7 已 `RE-CLOSED`；ch24 opening／逐字indexed owner達`RUNTIME-E1` | 只開其他 caller-specific binding、嘴型／closing中間幀與 E2；不重解基本函式角色、opening或逐字順序 |
