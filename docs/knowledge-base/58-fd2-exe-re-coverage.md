...
| 對話、頭像與過場原語 | 部分偏高 | 部分就緒 | 部分 E1 | 部分 | `0x15F84`、`0x1366A`、基本 pan／acting／spawn／join 等不應再從零重解。`ch24_post` 的兩個 lookup 已由 raw `FFED/FFEF/FFEE` 固定18句上下框順序，typed binding 與正式第25戰戰後 runner 皆已消費，不再依角色編號猜位置。一般故事對話仍須處理其他 caller-specific layout、indexed renderer、完整控制碼與實際 chapter binding。 |
