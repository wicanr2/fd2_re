...
| `0x19DF7`、`0x1B1E7`、`0x16FF4` | [`fd2_nested_system_menu_ida.txt`](../data/ida/fd2_nested_system_menu_ida.txt)、[`fd2_system_exit_and_group_march_ida.txt`](../data/ida/fd2_system_exit_and_group_march_ida.txt) | `RE-CLOSED`、`DATA-READY`、`RUNTIME-E1`：nested資訊／存檔／讀檔／離場四分派、資訊indexed畫面與selector3程式出口均已接。外層selector1使用既有`0x51B91` 90-entry表且不消費`sub_14B78`回傳；`0x170C6`每輪重讀`dword_53BEB`，event61的`0x1100B`會遞增同一動態上限。正式鍵盤路徑完成FDTXT確認、私有逐格／事件投影、record順序移動與raw bit7；event61在對話／59幀／JOIN31後續行，event75在對話後發布turn chain。未知事件在發布前整批拒絕 | 原版current-runtime save不等於現有JSON battle restart；系統選單尚缺同狀態逐幀／tick／音訊E2。正式資料只有event61／75兩筆selector1 rule，兩者已接；不得把這外推成90-entry所有selector已完成，也不得重解四分派、資訊schedule、selector3目的地或`0x51B91`表身 |
