...
|---|---|---|---|---|---|
| 檔案版本、容器與主要資產格式 | 閉合 | 部分 | 部分 E1 | 部分 | `.DAT`、圖像、FDTXT／字型、AFM／FIGANI、XMIDI、地圖與多張 EXE 表已有雜湊與重生工具，不應重解容器格式。2026-08-28 全量匯出實測得到1,005個raw subresources、125張一般PNG、264／409組FIGANI、136組頭像與33張地圖；FIGANI另有2,118張indexed frame、2,118張mask與264份metadata，逐檔manifest現為6,268筆。`FDOTHER #0/#2`指令格、全DATO consumer、終局FIGANI montage／tail、通用攻擊及指令0–3／5–9／24／29／32–35的可達FIGANI已由分離pack載入，達各自窄`RUNTIME-E1`；FIGANI resource4七幀逐欄位／pixel／mask與archive一致。`FIGANI.DAT`直接邊界只剩兩個依零header退回的helper，須先替145個未接受resource登記空／非動畫／blocked狀態；另有16檔解碼`FDOTHER.DAT`內嵌動畫。production runtime也仍直接讀其他`.DAT`，且音效、AFM／ANI、UI用途未全量閉合，故整體仍是部分。契約與實測清冊見[`60`](60-editor-separated-assets-spec.md)及[`asset-export-audit-20260828.json`](../data/asset-export-audit-20260828.json)。 |
