...
達受限class19玩家`RUNTIME-E1`；ID34亦已接三段modifier正式owner，ID35仍只有state transaction。主證據與重開
...
| `0x27FC9..0x286BD`（玩家 commands32–35 共用演出） | [`fd2_command32_35_presentation_ida.txt`](../data/ida/fd2_command32_35_presentation_ida.txt)、[`fd2_command34_tail_presentation_ida.txt`](../data/ida/fd2_command34_tail_presentation_ida.txt) | `RE-CLOSED`／`DATA-READY`；ID32／33／34 `RUNTIME-E1`：唯一caller、#65..68效果、#91..94按ID音效、兩段滑入、main／11張可選tail、raw RGB插值、steady restore及四條command-specific tail已閉合。三個正式owner逐Draw消費共用段、0..40 map ramp及專用tail；ID34另逐段發布17→18→19 writer，中途失敗回復raw／RNG／indexed buffers | ID35仍須接正式indexed owner與三段application tail；不得只播共用前段後直接呼state executor；ID32／33／34一般玩家同狀態E2另列 |
