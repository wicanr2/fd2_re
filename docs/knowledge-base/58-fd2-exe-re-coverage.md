...
|---|---|---|---|---|---|
| 檔案版本、容器與主要資產格式 | 閉合 | 部分 | 部分 E1 | 部分 | `.DAT`、圖像、FDTXT／字型、AFM／FIGANI、XMIDI、地圖與多張 EXE 表已有雜湊與重生工具，不應重解容器格式。2026-08-28 清冊現為37,919筆（36,895 exported、1,005 intentionally raw、19 blocked）。除既有FIGANI、FDOTHER動畫、BG／TAI、FDTXT／字型、頭像與33張地圖外，FDICON已分離1,680張三層sprite；FDSHAP全33銀行另分離8,256張indexed frame、8,256張source mask、8,256張remap mask及33份controls metadata，全部與固定原檔逐層一致。正式FDTXT、FDICON、FDSHAP、`figani.DecodeResource`與TAI direct archive production caller歸零；戰場與第23戰重載在沒有FDSHAP archive時通過。FDFIELD#69可由`map23/map.json`逐byte重建，終局selector30的FDFIELD#90..92亦由單一具型別JSON逐byte重建。終局FDOTHER#56..#60及其FDOTHER#5對話格已轉成indexed surface／palette／animation／typed raw cells，party cycle與20段tail正式consumer不再回讀archive。production runtime仍直接讀其他`.DAT`；音效、AFM／ANI、其他FDFIELD selector與UI用途未全量閉合，故整體仍是部分。契約與完整數字見[`60`](60-editor-separated-assets-spec.md)及[`asset-export-audit-20260828.json`](../data/asset-export-audit-20260828.json)。 |
