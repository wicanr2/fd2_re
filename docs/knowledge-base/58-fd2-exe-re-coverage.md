...
| 敵方人工智慧 | 底層控制流部分偏高；高階交易部分 | mode／候選／部分 fallback 已資料化 | 多個窄 E1 consumer | 只有原版敵方回合邊界 E2 | `0x13FD4`、`0x14EF0`、mode 5／11 等既有函式邊界與窄 owner 不應反覆重解。command 9 現以 `0x15311` producer target 走raw-side-zero indexed owner；10–12走`funcs_1541F`的60幀owner；17–22及26–27也已由同跳表進各自wrapper內建的effect／mask／numeric tail，並在Draw邊界原子發布raw-selector target transaction。`0x15055` 現保存並原子消費 `0x1567E` winner 的完整 raw target list；正常正分 type 5／13／20／21／24 的數值交易已達 E1，不再借用玩家游標重算。type 5／13另已依`0x211A4`正式播放FDOTHER #6 `0x39..0x3F`、#80 sample12／1、五組mask與22張數字段，mask Draw後才發布交易。敵方25缺正常AI producer而維持失敗即關閉。剩餘缺口是 type 20／21／24 item indexed演出、精確tick／音訊、同一raw狀態的重製端配對與一般玩家效果。詳見 [`11`](11-enemy-ai.md)。 |
...
`RUNTIME-E1`。下一步只補type20／24的`0x1CD17`與type21的`0x1CAC7`，不重解
`0x211A4`。
