...
| 開機、標題、LOAD、CONTINUE、存檔 | 部分 | 部分 | 部分 E1 | 原版錨點部分 E2 | 四槽 envelope、checksum、名冊與部分戰間落點已接；標題 selector 的正式確認 owner 現以 checksum-valid 合成槽完整還原 `town_ch02`、typed/raw party、gold、chapter、HUD gate並清除舊 battle state，竄改 envelope 原子留在選槽。current-runtime CONTINUE 與巢狀 SAVE 已有正式 E1：SAVE 只接受合法 CONTINUE raw baseline，保留四槽與未命名 bytes，以 FDTXT `0x19A/0x19B/0x19C`、checksum、XOR envelope 及同目錄原子取代寫入明確可寫複本。尚缺巢狀 LOAD、動態增援／JOIN 後 persistent raw 同步、未修改有效四槽完整路徑與同 raw 狀態 E2。 |
...
| `0x19DF7`、`0x1B1E7`、`0x16FF4` | [`fd2_nested_system_menu_ida.txt`](../data/ida/fd2_nested_system_menu_ida.txt)、[`fd2_system_exit_and_group_march_ida.txt`](../data/ida/fd2_system_exit_and_group_march_ida.txt) | `RE-CLOSED`、`DATA-READY`、`RUNTIME-E1`：nested資訊／存檔／讀檔／離場四分派已閉合；資訊 indexed 畫面、selector1 SAVE 與 selector3 程式出口已接正式 E1，selector2 LOAD 仍未接。SAVE 從完整 raw baseline 複製未命名 bytes，只回填有 provenance 的 live 欄位；取消或不一致時原檔不變。外層selector1使用既有`0x51B91` 90-entry表且不消費`sub_14B78`回傳；`0x170C6`每輪重讀`dword_53BEB`，event61的`0x1100B`會遞增同一動態上限。正式鍵盤路徑完成FDTXT確認、私有逐格／事件投影、record順序移動與raw bit7；event61在對話／59幀／JOIN31後續行，event75在對話後發布turn chain。未知事件在發布前整批拒絕 | 尚缺current-runtime LOAD、動態增援／JOIN 後 persistent raw 同步與同狀態逐幀／tick／音訊E2。正式資料只有event61／75兩筆selector1 rule，兩者已接；不得把這外推成90-entry所有selector已完成，也不得重解四分派、資訊schedule、selector3目的地或`0x51B91`表身 |
