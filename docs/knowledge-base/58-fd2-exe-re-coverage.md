...
| `0x16F55`、`0x1728C`、`0x117E7`、`0x4E8E1` | [`fd2_system_overlay_options_ida.txt`](../data/ida/fd2_system_overlay_options_ida.txt)、[`fd2_end_turn_command13_owner_ida.txt`](../data/ida/fd2_end_turn_command13_owner_ida.txt)、[`empty cursor owner`](../data/ida/fd2_117e7_empty_cursor_system_overlay_ida.txt)、[`END確認框`](../data/ida/fd2_end_turn_confirmation_ui_ida.txt)、[問句同狀態比較](../data/ui-traces/native-end-turn-confirmation-original-vs-remake-e1.json)、[逐字回覆動態配對](../data/ui-traces/native-end-turn-response-progressive-original-vs-remake-e1.json) | `0x12C0D==-1` 空游標 owner；direction2→`0x1728C` 四項設定已依 raw bytes、cells 54..75、音樂／音效／演出／HUD consumer與原生／JSON存檔接成正式 E1；direction3→END、DATO #75／FDOTHER #5/#2 indexed確認生命週期、FDTXT `0x1A3/0x1A4/0x19C`、YES／NO逐字形發布、來源復原、`0x1A30B` 與正式 battle E1；`0x4E8E1`由右往左寫入且`0x9017`是右端錨點 | 設定面板與END的精確 DOS tick／音訊及收合逐幀同相位、selector0巢狀存讀檔／離場、selector1全軍行軍、逐章重製PLAYER-E2；不重解已閉合設定writer／consumer、END分支與renderer |
| `0x19DF7`、`0x1B1E7` | [`fd2_nested_system_menu_ida.txt`](../data/ida/fd2_nested_system_menu_ida.txt) | `RE-CLOSED`、`DATA-READY`、`RUNTIME-E1`：nested cells `36、39/41、42/44、45`、save-file與raw `+5` table writers、資訊／存檔／讀檔／離場四分派；正式空游標鍵盤路徑現消費資訊畫面12次展開＋等待input／`0x4DFCC` palette pulse＋12次收合、entries `0x85..0x88`、章節／round／currency／三camp count與兩行FDTXT；缺raw／VGA／DAC／資產時原子拒絕 | 原版current-runtime save不等於現有JSON battle restart，離場`-1`目的地與外層selector1仍未閉合；資訊畫面尚缺同狀態逐幀／tick／input flush E2。不得重解四分派與資訊畫面schedule，只有直接反證才重開 |
