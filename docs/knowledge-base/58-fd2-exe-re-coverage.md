...
1. 現已有 IDA Pro 9.4 重生的 1,305 筆函式清冊；Watcom FLIRT 分出170筆
   runtime，受版控語意索引分出37筆產品函式，其餘1,098筆仍未
...
2026-08-27 已由授權 IDA Pro 9.4 從固定雜湊原檔重新分析，完整匯出再經
`compact_fd2_function_inventory.py` 壓縮後與受版控清冊逐欄位完全相同：
`1,305 = 37 product + 170 runtime + 1,098 unknown`，語意註記38筆。`1,089` 是
數字次序誤寫，不是另一版清冊；`1,104` 則是語意索引尚未重生前的歷史數字。
