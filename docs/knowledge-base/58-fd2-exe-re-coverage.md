...
| 玩家指令、法術、物品與交易 | 部分 | 部分 | 部分 E1／部分失敗即關閉 | 缺完整 E2 | command mask、若干 ID、MP／物品交易與 selector 邊界已解；`0x16F55` direction3→END、FDTXT `0x1A3/0x1A4`、YES、`0xC8` ms 與 `0x1A30B` 已直接閉合，chapter0 current-runtime 正式入口會顯示原文後進敵方回合及返回玩家回合。command 13 的玩家游標交易回歸已接，`0x21AD9→0x21B18` indexed 演出 owner 已定位但未移植。其餘三格 owner、未知 command、複合技、法術／物品完整效果、rollback、精確演出與 UI 尚未閉合。 |
...
| `0x21AD9`／`0x21B18`（command 13–16 wrapper 家族） | [`fd2_end_turn_command13_owner_ida.txt`](../data/ida/fd2_end_turn_command13_owner_ida.txt) | 四個 command literal、wrapper 參數、共同 indexed presentation owner 與玩家／AI callers | 將既有 helper 轉成具型別演出播放器、逐幀逐音訊 E2；不重解 wrapper |
