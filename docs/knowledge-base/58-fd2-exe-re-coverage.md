...
| 對話、頭像與過場原語 | 部分偏高 | ch00_pre 97句、ch00_post 13句、ch01_pre 20句、ch17_post 21句、ch22_post 56句與ch24_post 18句原生版面就緒 | 部分 E1 | 部分 | `0x15F84`、`0x1366A`、基本 pan／acting／spawn／join 等不應再從零重解。六個已接切片均由raw `FFEC..FFEF/FFFE/FFFD`建成typed pages；[`fd2_story_dialogue_layout_ida.txt`](../data/ida/fd2_story_dialogue_layout_ida.txt)閉合五階段開框、逐字、完整頁嘴型、三列窗口捲動、snapshot收框與可選游標尾段。raw glyph上限須依control取13／15／14／14，且不能用Unicode rune數代替：`FDTXT_029 #12`的13格`ASR-07`會展開16個rune，現以可選`glyph_pages`保存多rune token並逐格畫出，見[`native-story-multirune-glyph-token-e1.json`](../data/ui-traces/native-story-multirune-glyph-token-e1.json)。序章97句後正式第1戰勝利進`ch00_post`，13句全部收框後才同步5人persistent roster並進`town_ch02`；第18戰後正式播放四caller／21句，保持index10跨場景順序，之後才JOIN21／7、進`town_ch19`並通過存讀檔；第23戰後正式分支另完成35句、同步、存檔冷讀及整備至`story_ch24`。只重開E2或其他caller binding；不把重製更新分配冒稱DOS精確時鐘。 |
