...
達受限class19玩家`RUNTIME-E1`；ID34／35仍只有state transaction。主證據與重開
...
| `0x27FC9..0x286BD`（玩家 commands32–35 共用演出） | [`fd2_command32_35_presentation_ida.txt`](../data/ida/fd2_command32_35_presentation_ida.txt) | `RE-CLOSED`／`DATA-READY`；ID32／33 `RUNTIME-E1`：唯一caller、#65..68效果、#91..94按ID音效、兩段滑入、main／11張可選tail、raw RGB插值、steady restore及四條command-specific tail已閉合。ID32／33正式owner逐Draw消費各自共用段、0..40 map ramp及專用tail；中途失敗回復HP／raw／RNG／indexed buffers | IDs34–35仍須接各自正式indexed owner與command-specific tail；不得只播共用前段後直接呼state executor；ID32／33一般玩家同狀態E2另列 |
