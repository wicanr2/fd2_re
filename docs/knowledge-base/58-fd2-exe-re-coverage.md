...
| 城鎮、祕密商店、商店、教會與整備 | 部分偏高 | 部分就緒 | 多個正式 E1 consumer | ch02 若干狀態 E2；其餘部分 | 個別 menu、購買、賣出、轉移、復活、轉職與整備已有窄切片；ch02 賣出已由正常商店輸入走完角色／物品／Yes-No、成功、向上金幣滾動及返回名冊，九組 route-patched 原版／正式重製畫面皆整幀 AE=0。獨立裝備 service2 另由正常商店輸入取得名冊與索爾面板，但動畫相位未同步，整幀仍為 AE=1389／1433。service3 物品轉移也由正常商店輸入取得來源提示／名冊、物品、目的提示／名冊；可見內容與幾何一致，整幀差異 AE=88／1391／2／88／321 來自翻頁箭頭或角色小圖動畫相位。兩者都只列 route-patched partial E2。裝備收件者以六名具完整 raw provenance 的 typed party，從正式 menu→purchase→Yes 進三列面板，走過 scroll、滿欄／無合適角色原子返回與成功裝備／扣款，達 `RUNTIME-E1`；這不是原版 E2。教會轉職與商店四種 mutation 也能返回 town，再穿越重製 JSON 存讀檔保存金幣、背包、裝備、能力、typed roster 與隊伍拓撲。第25戰後的 `town_ch26` 祕密商店 E1 亦已接通。這些不等於未修改一般玩家戰間 E2；其餘章節入口、原版存檔、recipient scroll／no-recipient／full、equip／transfer mutation／restore與完整交易仍缺。 |
