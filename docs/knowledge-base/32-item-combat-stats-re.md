...
- **轉職切換 sprite 組(反組譯實證)**:凱拉斯 sprite 組17(轉職前)→組49(轉職後),轉職換組。**推論**:轉職後職業組若沒畫(廢案/空組)可能讀不到 sprite。⚠「轉職當機」為**使用者口述,青衫攻略查無明文**(grep 無「當機/死機」),後果待實機驗證。remake 應為可轉職職業備齊 sprite。
