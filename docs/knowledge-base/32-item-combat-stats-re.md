...
- **★ 重大考據:轉職切換 sprite 組 → 缺組當機**。轉職會把單位 sprite 組換成轉職後職業的組;**若該組沒畫(FDICON 廢案/空組),遊戲讀不到 sprite 而當機** —— 正是青衫攻略警告的「龍人轉職可能當機」。例:portrait 67 凱拉斯(龍劍士)sprite 在組49(跳號);龍人系轉職後若指向未畫組即崩。**remake 必須為每個可轉職職業備齊 sprite,或攔截缺組**(見 doc 31 §6)。
