...
- [x] 戰鬥動態 index 表載入點已定位到真實位址(`0x028110`–`0x028156`,取代第 8 輪誤記的 `0x28f24`/
  ...
  已用 PCM 特徵掃描找出候選資源家族(`#48/#49/#50/#51/#52/#53/#64/#88`,見「戰鬥音效池」節)並
  ...
  位址證據鏈)。
  2026-08-29 勘誤：#78 已由 ANI #1 直接 caller／consumer 證實為標題 companion 音效，
  不再列入本戰鬥候選集合。
...
反組譯出的真實函式（入口 `0x027fc9`，標準
`push 0x6c; call 0x36cd7` Watcom stack-demand check 序言，兩個參數：
...
### 候選戰鬥 SFX 資源家族:`FDOTHER.DAT #48/#49/#50/#51/#52/#53/#64/#88`
...
| #53 | 5(4 有效) | |
| #64 | 7(6 有效) | |
| #78 | 2(1 有效) | **已排除於戰鬥候選**：ANI #1 第一幀的固定 companion 音效 |
...
在本段戰鬥路徑中，這是當時唯一**直接反組譯證實**的具體 index(`#88`)；其餘 `#48`–`#64`
是 PCM 特徵比對推斷。2026-08-29 另由 ANI 播放器 `0x20421` 的直接 caller／consumer 證實
`#78` 屬標題 ANI #1，故不再列為戰鬥候選；見
