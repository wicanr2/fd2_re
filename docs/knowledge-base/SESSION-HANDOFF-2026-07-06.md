...
- 2026-07-20 church RE：`0x3072f` 讀教會服務選擇並分派 `0..3` 到 `0x2ffa5/0x2f8ea/0x30dc3/0x31385`；`0x30dc3` 建立復活候選（FDTXT `0x24c`），`0x31385` 建立轉職候選（FDTXT `0x24f`），完成後可回教會選單。這證實目前 church placeholder 會漏掉復活／轉職服務；下一步先追 `0x31793` 與復活寫回欄位，再資料化 service nodes。
