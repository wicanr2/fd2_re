...
- 2026-07-20 finale #56 format boundary：玩家 raw `FDOTHER_056.bin` 為13609 bytes，前4 bytes直接是 little-endian 320×200，後接非 #54 frame-table layout；它不是 `ParseFrames` 可吃的 offset/RLE container。`0x2c405` 以 `0x4e63d` 直接使用此資源，故需另行 RE raw image/stream grammar，禁止硬套 #54 decoder。
