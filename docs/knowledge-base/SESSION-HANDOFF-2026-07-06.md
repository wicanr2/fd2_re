...
- 2026-07-20 ch24 transition slice：`0x24b4d(count)` 完整 RE 確認為先 terrain/main redraw，再以兩個 `0x1c8` buffer 交替 present、每幀 20ms；ch24 calls 為 20/20/20/60（400ms/1.2s）。compiler/runtime 已新增 `transition_reveal`，保留 count 與每幀 timing，不誤當黑幕 fade；indexed double-buffer visual adapter、`load_res`/SFX callsites 仍待接，故 `story_ch25` 尚未切到 binding。
