...
- 2026-07-20 ch02/ch03 handler audit：`ch02_pre` 的四組 dialogue index 已由 `count-aligned.json` 對到 `ch03.json` scene0 lines 0–13，並有 act18/17/19、spawn/pan/layout overrides；`ch02_post` 的 Tino 分支對到 scene1 lines1–5，else 分支對到 lines6–15、JOIN char2、sync/set_chapter3。`ch03_post` 僅有一段已證實對到 `ch04.json` scene3 lines0–3。`ch03_pre` 兩段 dialogue 尚無 binding/context 證據，暫不套用任何完整 story script；這是防止把下一章劇情錯播到戰前節點的明確 guard。
