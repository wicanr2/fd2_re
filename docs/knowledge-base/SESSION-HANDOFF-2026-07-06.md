...
- 2026-07-20 ch05_pre boundary：`FDTXT_006` string #0 雖已證實是 ch06 的 18 句，但 `count-aligned` 目標跨 scene0→1→2→3；現行 `HandlerDialog` 只接受單一 scene，compiler 會 fail-closed。未把它硬接成錯誤劇本；下一個引擎缺口是可編輯的跨-scene dialogue segment adapter。
