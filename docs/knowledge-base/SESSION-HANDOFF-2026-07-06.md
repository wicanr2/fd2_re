...
- 2026-07-20 `0x31602` stat-reset continuation：轉職後先以 `0x4e4d1(new portrait)` 取得成長列，透過 `0x1e529` 寫回 unit `+0x37/+0x39/+0x3e/+0x42/+0x46`（可對應 AP/DP/DX/MaxHP/MaxMP 候選欄位，需再核對欄位表）；`0x4e48d(new portrait)+1` 的 growth-group 若非零，額外加到 `+0x3b`。隨後呼叫 `0x1b750` 重算裝備，設 level `+0x21=1`、EXP/暫存 `+0x3c=0`，並複製 `+0x42→+0x40`、`+0x46→+0x44` 回滿 HP/MP。此段已具體鎖定，runtime mutation 仍待 Unit 欄位映射與 deterministic reset 測試。
