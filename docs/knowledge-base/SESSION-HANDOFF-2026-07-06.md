...
- 後續沿完整 back-edge 勘誤上述簡寫：前導實際是
  ...
  已同步修正，錯誤的地址對調與 mode8→mode7 說法不再有效。
- `sub_2BA22` (`0x2BA22..0x2BC9A`) 的唯一 caller、八參數 ABI 與九幀
...
- README 完成度已由68調整為73：戰役連續路徑加1、代表性戰鬥演出加1、戰場內容區
...
## 2026-08-27：標題後段色盤轉場勘誤與99%門檻閉合

- 授權IDA Pro 9.4重新核對`sub_1F894`後，舊稱「兩段wipe／logozoom」並不成立：
  `0x1FC6D..0x1FC7B`與`0x1FD52`之後都呼叫`sub_286BD`，語意是VGA DAC索引色盤
  內插。呼叫端傳入end=255，故實際半開區間為0..254；index255保持不變。
- 規格先將原版41個8ms相位投影為20個60Hz影格，保留phase 0／40端點；正式
  runtime再從玩家自備`FDOTHER #69..73/#101`及`#7/#8`建立索引影格，依序播放
  紅幕、真實`ANI.DAT #1`、近白至標題畫面。缺任何原始資源時不發布猜測交易。
- Docker／Xvfb擷取`titlefade tick=11`，與原版`dosbox_seq/frame_193.png`上方
  640×400比較為`AE=11712/256000`、每色頻道MAE=`0.076583/255`；相鄰tick12
  MAE為8.414，故採tick11為60Hz最近投影。紅幕、ANI #1與穩定選單既有錨點仍
  各為`AE=0/256000`。依使用者接受的99%玩家可見門檻，本轉場列`RUNTIME-E1`
  完成，不追DOS逐週期時序；剩餘只列精確音訊與原版完整啟動E2。

## 2026-08-27：第30戰終局正式畫面移除開發提示

- 程式盤點發現成功的角色蒙太奇與20段尾聲會在畫面底部疊加「來源約束 E1」及
  按鍵／定格說明。這些文字只屬研究診斷，沒有原版畫面證據，會直接污染正常玩家
  終局，故先在`56`明訂成功路徑的呈現隔離契約。
- 正式模式現只繪製來源約束的320×200原始畫布；上述兩種提示只在玩家主動開啟
  除錯HUD時顯示。素材／provenance不足而回到可編輯結語時仍保留必要錯誤說明，
  因那不是冒充原版成功畫面的路徑。
...
- 新回歸由`battle_ch21`正式勝利確認開始，清除16個出戰槽的`0xD1..0xD6`後，
...
- 目前18筆證據中有13筆符合正式重製路徑、正常輸入、無route patch／debug
  shortcut：戰役／戰間2、戰鬥／AI 4、介面3、存檔／持續隊伍3、終局／平台1。
  另5筆保留旁證但不計門檻；相同trace只有在輸入分支或驗收不變量不同時才拆成
  不同樣本。
- 這次盤點顯示下一個高價值缺口不是再反組譯，而是把既有早／中／晚期戰間與冷讀
  存檔consumer由普通作業系統輸入重播；下項已先關閉兩層皆為0的舊狀態。
- 新增`native-load-ch30-normal-input-remake-e1.json`：一次性無網路Docker以正式
  `campaign_full.json`、唯讀固定雜湊slot與普通X11鍵盤，從標題LOAD走過記錄提示
  NO、19人整備、21句／七次staging至`battle_ch30`。frame9000為45×40、camera
  `(16,14)`、cursor`(23,18)`，無殘留dialog／event／turn staging。音訊dummy與
  第三方存檔來源限制不變；這使戰役／戰間層由0增為2。

## 2026-08-27：全專案 Markdown 現況斷言勘誤

- 全專案唯讀稽核發現數份歷史規劃仍使用現在式，會讓後續工作重做已閉合RE。
  `REMAKE-STATUS`已刪除無可重跑評分規則的90／100自評；完成判定只看60格抽樣。
- 合法IDA Pro 9.4在固定雜湊原版上重新匯出1,305函式；現行38筆語意索引中37筆
  產品、1筆`0x37416`覆蓋同一FLIRT runtime分類，結果為產品37／runtime170／
  未知1,098。受版控清冊舊36／170／1,099是新增`0x26152`語意後未重生；`58`舊
  31／170／1,104則不符合現行匯出器分類契約，兩者均已訂正並保留本段原因。
- 原生故事正式切片由17筆訂正為19筆，加入`ch25_post`41句與`ch27_post`5句。
  `0x2BCE5`正式重製owner／handoff已接；舊「終局仍未接campaign」只保留為
  分段接線歷史，不得重開已閉合前綴、蒙太奇、20段尾段、定格與隊伍回顧。
- `20`／`21`／`22`／`04`／`07`／`18`／`09`／`53`／`30`已補現況邊界：Linux
...
## 2026-08-27：刪檔後第二次 Markdown 現況稽核

- 重新複核 81 份受版控 Markdown，確認六個已刪檔名及其知識庫編號沒有殘留引用；
  fenced-code-aware 本地連結檢查為 0 斷鏈，圖片引用均存在。目標總數會隨文件
  編輯自然變動，因此不再把它固化成現況指標。
- `57` 的 UI-01 表格仍把已接的 CONTINUE owner 列為缺口，shop service3 同一列也
  同時寫「正式輸入完成」與「empty／full／self／cancel 尚缺」。現已直接訂正表格：
  重製端為 `RUNTIME-E1`，剩餘的是未修改原版同狀態 `PLAYER-E2`，不再保留一行
  錯誤斷言再靠下一行勘誤。
- `57` 的 title「100%」與 tactical field「50–60%」容易把指定畫面／局部候選誤讀
  為整體完成度，已改成證據分級。`58` 的教會／商店缺口及 README 里程碑同步標明
  E1、E2、實體平台與 13／60 抽樣邊界。其餘零入度文件均具有獨有 RE、實測、
  文化保存、錯誤形成或操作契約價值，本輪不再刪除。

## 2026-08-27：第三次 Markdown 可重生斷言稽核

- 以機器清冊而非文字互抄核對數字，發現 `58` 檔首與後段仍殘留產品31、未知1,104；
  同檔較晚段、`56`、`91` 與 `fd2_function_inventory.json` 均直接反證。現已統一為
  函式1,305、產品37、runtime170、未知1,098。
- `56／91` 仍用現在式引用數個已在後續提交中拆分或刪除的 Go 測試名稱。Git 歷史
  證明那些測試當時確實存在，因此時間序列紀錄不是虛構；但權威文件現已改指目前
  原始碼存在的 planner、consumer、postbattle 與 montage 回歸，舊名稱只留歷史。
- 本檔保留的必要性是錯誤形成、提交脈絡與勘誤追溯；它已在檔首明示為歷史研究
  日誌，不是目前交接。搜尋命中本檔不可單獨重開工作，現況仍只看 `58／57／91`。
...
- 回到第一大項後，將既有 chapter0 current-runtime 命令格配對正式登錄抽樣清冊：
  原版與重製均由普通鍵盤 `CONTINUE→Return` 抵達，無 route patch／debug shortcut；
  清冊成為19筆登記、14筆合格。固定標題tick與截圖時鐘仍限制重製為E1。
- 同一筆樣本再以目前 `ac6e7cc4` 從原始碼冷建置實跑。第一次按鍵過早只停在開場
  捲動，分類為驗證腳本失敗；依既有契約分段略過後，普通X11鍵盤成功抵達
  `battle_ch01`、turn1、cursor(8,17)，Return開啟原版索引命令格，無殘留對話或
  battle event。人工檢視640×400擷取未見黑屏、錯誤節點或啟動阻擋；不外推全30關。

## 2026-08-27：開場到第一關原版／重製推廣片

- 依使用者指定把影片範圍縮為開場到第一關，使用既有原版錄影、重製執行期短片、
  標題／對話／戰場／命令格比較證據與三段貢獻字卡，沒有混入後期戰役大雜燴。
- 新增66秒可重生安全母版與Docker腳本。H.264／AAC為1280×720、30fps，兩串流均
  66.000秒；成片3,287,513 bytes、SHA-256 `289357b5…0869ae3`。底音為明示非原版
  配樂的中性合成音，mean −33.4 dB、max −27.5 dB；未直接散布原版音樂。
- 九格接觸表已人工檢查；命令格由原本過小的直式縮圖改成原版／重製／差異三欄，
  黑幀檢查未發現超過1秒事件。成片與接觸表留在被忽略的`dist/promo/`，Git只保存
  `docs/promo-opening-ch01-spec.md`、建置腳本與`docs/data/video/` metadata。

## 2026-08-27：第一輪抽樣既有證據補登

- 遵守「卡關即跳過」停止線，沒有為了新增樣本重打第一關或練功。依清冊允許的
  不同輸入分支／驗收不變量，把既有普通輸入證據補登為戰場HUD、command0調色盤、
...
## 2026-08-27：第2章酒店消息正常輸入往返

- 從前一批成功的 AppImage `LOAD→town_ch02` 繼續，普通 X11 鍵序實際抵達
  `rumor_ch02`，frame 1102 旁車顯示一筆對話、無 battle event／turn staging；
  再按 Return 後 frame 1250 回到 `town_ch02`，對話數歸零。
- 城鎮原生方向幾何沒有在本輪解讀；只保存實際鍵序與節點結果，不把四次 Down
  猜成線性索引。原本預定的教會路徑因此改收斂為酒店消息，不重試選單方向。
- 為測 `Shift+F1` 曾暫時擴充 XTest helper；該次 AppImage 以127結束且沒有旁車，
...
- Linux AppImage 普通標題 LOAD 的 `town_ch02` 旁車獨立登錄早期城鎮主畫面；
  受版控圖為同封包先前擷取，當前執行的PNG／state雜湊保存在trace，未冒稱同幀原版。
- 第30戰完整播放標題後普通 CONTINUE，再以普通Return開啟空游標indexed操作面板；
...
## 2026-09-07 原版主題開場操作複核與本機候選

- 使用者要求比較已編譯 AppImage 與原版從選單到第一關父子登場前，重點為
  操作感。唯一逐項現況、READY 規格與限制見 `92-opening-ch01-input-audit.md`。
- 實際原版 START／DOSBox 與 AppImage 揭露：舊包跳過王座廳；原生故事快取
  重建清掉朝向、地形誤套亮色；第一關初始游標採用另一份 CONTINUE 錨點；
  選人縮遠且缺資訊框、指令環間距錯誤、取消游標遭原生視圖蓋回。這些均已修正。
- 第一次實際 END 又揭露滿血索爾被舊姓名比對誤判死亡。原生角色的空姓名
  合法，須用來源實體身分辨識；舊完整開場測試人工補姓名掩蓋了缺口，現已移除。
- 戰況面板四張 PNG 的斜線破圖直接推翻舊 codec 假設。IDA Pro 9.4 固定版
  `0x1b41d` 呼叫證實應沿 `sub_16886→0x4e63d`，四筆改為既有四模式 Frame
  並重生 PNG／mask；舊文字證據保留並追加勘誤。另移除開局猜測的 1000 金幣，
  依原版正常 START 戰況與正式建構器回歸改為零。
- v.1.0.5 實際普通 START 完整序幕、選人、取消、移動一格、待機及兩次 END
  到 TURN 003。原版合法 CONTINUE 的 TURN 003 END 後已出現「老爸！老爸！」，
  所以不能宣稱第五回合前仍未登場；沒有為使用者記憶中的回合數修改事件。
- 最終本機候選為 `dist-all/v.1.0.6-20260907/full/`，AppImage SHA-256
  `45e46848427617ddacd58958b076d6da935aee46be3260e36140c705e87fbeaa`。
  該版另核對實際啟動、合法原版 CONTINUE、完整戰況面板、包內 codec／mask、
  二進位與素材清冊雜湊。前一版序幕收據沒有改名成最終版；對照頁及所有
  範圍限制保存於同目錄 `input-audit/index.html`／`receipt.json`。
- `internal/battle`／`internal/fdother` 全套與相關介面、來源身分四語系、
  完整開場、正式 START 金錢聚焦回歸通過。不宣稱整個 cmd/fd2 全綠：早期
  全套失敗及五個 HEAD 重現收據另存。F5/F9 補測座標差異已停止；回讀 SDD
  確認它是既有 JSON 節點重啟契約，不是原版目前戰況續存，故不將其定為新資料毀損。
  中立 Escape、回合聚焦、已行動角色資訊及 dosgolem 正式收據仍未關閉。
- 尚非完整一致或穩定版，未提交／推送，也未公開任何原版素材。
  所有本輪 `fd2-feel-*` Docker 容器已結束並移除；沒有新建工具映像。

### 2026-09-07 同輪續作：中立輸入與寶箱對話

- 上段 v.1.0.6 為當時候選，現由 `92` 的最新交付表取代，歷史收據不覆寫。
  `0x53AE9` 初始化／循環／回合重設、`0x12D7B`、`0x17AED` 已以固定
  FD2.EXE 和 IDA Pro 9.4 複核，證據保存於 `fd2_player_neutral_input_20260907.json`。
- v.1.0.7 由普通 START 播完完整序幕；Escape 依序聚焦索爾、悠妮、亞雷斯，
  F2 狀態／命令兩頁、取消移動不換人、移動待機、已行動狀態、循環跳過已行動者、
  兩次 END 與 TURN 003 聚焦均以實際 AppImage 通過。原版另有合法 CONTINUE
  的同按鍵輔助截圖；沒有將其拼接成同一條 START 收據。
- 補拍原版在 `(7,13)` 待機時揭露寶箱對話差異：重製原本立即領取並顯示
  `取得物品 C0h`。`0x190AC` 窄證據保存於 `fd2_treasure_input_20260907.json`，
  READY 後接入普通物品寶箱頭像、YES／NO、FFFC 物品名稱、取得後確認。
  v.1.0.8 實際驗證取消保留、下回合取得藥草；再補測原地待機後狀態時，
  發現移動預覽已釋放而漏設 raw bit7。v.1.0.9 改以原生戰場與選取角色
  provenance 同步；有／無移動預覽兩分支及寶箱、END、完整開場回歸通過。
- v.1.0.9 首次 GUI 重跑在容器執行約108秒收到 Docker `kill signal=15`，
  沒有 OOM 事件，遊戲日誌也無錯誤。原因尚未知，不定性為產品崩潰。
  原始紀錄另存 `v109-interrupted-*`，改將啟動／回放放在同一有界容器重跑。
- 第二次重跑正常回報驗證腳本逾時：方向鍵後取樣仍是按鍵當刻的游標，
  下一次確認實際已在另一格，沒有遊戲錯誤。保留 `v109-replay2-*`；回放改成
  短按鍵，並等待放鍵後的新週期取樣，未修改遊戲或重新打包。
  同一 v.1.0.9 AppImage 的乾淨重跑已通過寶箱 NO、下一回合原地 YES、
  藥草回覆及確認、已行動角色狀態（物品欄可見新藥草），再 END 到 TURN 003。
  這證明原地待機 raw bit7 修正已接到實際玩家入口；不提升精確按鍵重複時序。
- 最後戰況面板顯示 TURN 003、零金錢，沒有進入父子事件；回放正常退出。
  `tools/finalize_fd2_opening_audit.py` 已整理最終本機 `input-audit/index.html`、
  圖像／輸入收據與公開雜湊摘要。交付版本為 `v.1.0.9-20260907`，AppImage
  SHA-256 `8fe7fccaa7af553c7fdba45c030223652c92cdb2307a61da962eb0d970bfdd4e`。
  所有本輪容器已結束並移除；沒有新建映像，也未公開原版素材。

## 2026-09-08：王宮對話差異原因查明

本輪在 `fix/dialogue-speaker-focus-20260908` 保留既有變更，依使用者要求
優先查明差異原因；未改正式執行期、未重建修正版、未提交或推送。目前狀態見
[93 對話差異原因](93-dialogue-cause-20260908.md)。開框前說話者聚焦及逐字
頭像輪替為已證實漏接；DOSBox／dosgolem 第一頁的 1,298 像素差異均已按
來源畫格解釋，不能再稱文字解碼未知。實際 AppImage 雜湊回讀吻合。

研究比較工具在 `remake/tools/`；私有 PNG 與正常輸入日誌在
`work/dialogue-cause-20260908/`，不可將整個未追蹤 `work/` 加入公開版控。
IDA 初次 Python HOME 設定失敗屬環境問題，沿用既有映像設定後成功。
本輪命名 `fd2-dialogue-*` 容器以 `--rm` 有界結束，最後清單無本輪容器；
未清理其他專案，未重建映像。`git diff --check` 通過。

## 2026-09-08：對話缺口修正與 v.1.0.10 本機驗收

使用者續令修正後，補回正常對話開框前的說話者聚焦、聚焦完成後背景保存，
以及跨句持續的逐字嘴型計數。計數器載入初值與 writer 證據保存於
`docs/data/ida/fd2_dialogue_counter_20260908.json`。捲動中間幀不算普通字形；
等待期原有 frame 0／3 狀態機保持獨立。完整開場回歸揭露隱藏 HUD 仍驗證
父王戰鬥資料的前置條件誤用，已依既有顯示閘門修正，沒有猜補角色數值。

11 項對話／正常入口／完整開場回歸通過；新版 AppImage 普通 START 擷取
第二句完整 64,000 像素與 dosgolem 一致，第一句框內一致、框外仍差 482，
不宣稱鎖定人物動畫。逐字截圖完整頭像匹配 DATO frame 1，文字尚未寫完，
證明新接線已到實際玩家畫面。全段同狀態與音訊驗收未提升。

交付 `dist-all/v.1.0.10-20260908/full/`，AppImage SHA-256：
`aa9800de67385b25d3297af6d3b0a7483bbc70f55567b6de05adc5c2780df8a2`。
同目錄 `dialogue-review/index.html` 保存原版／修正前／修正後對照與輸入收據，
公開可追溯摘要為 `docs/data/ui-traces/dialogue-remake-fix-20260908.json`。
首次打包被舊工作素材快取雜湊阻擋，改用 v.1.0.9 已驗證包唯讀解出的素材，
清冊通過後重新建包；未跳過驗證，也未覆寫舊版本。

本輪容器均已結束且以 `--rm` 清除，沒有新增工具映像；原始碼及輸出擁有權
為 1000:1000。工作分支保留先前未提交變更，本輪未提交／推送或公開原版素材。
`.gitignore` 已排除本輪私有工作目錄，避免圖片、完整素材及 Go 快取被誤加入。

## 2026-09-08：延長對話前綴，鏡頭與跨頁修正
...
## 2026-09-08：補齊抽測資料與開場多語查詢

先前 `pan-tests.log` 的兩項失敗現已修正：選人測試沿用完整原版 record
測試載體並補齊六座標；多語測試正確區分姓名與對話 ID，避免錯誤追加
`/text`。姓名與本文仍逐條核對審核狀態，翻譯目錄及正式執行期未改動。

精確兩項重跑通過。補掛既有驗證包的分離素材後，同範圍及相關抽測為
37 項通過、3 項略過；多語套件 20 項通過，沒有失敗，計數包含子測試。
剩餘略過只涉及未提供的晚期／第29戰外部存檔，不能算通過。
命令、日誌雜湊與環境重跑原因見
[測試修正收據](../data/ui-traces/test-fixture-fix-20260908.json)。
舊失敗紀錄保留；本次不重建 AppImage，也不改寫 v.1.0.13 建包來源快照。
本輪一次性測試容器均以 `--rm` 結束清除，未新增工具映像。

## 2026-09-08 使用者暫停自動驗證、改人工測試

目前狀態唯一入口為 [94](94-ch01-town-parity-20260908.md)。已修EXP、JOIN、援軍32句原生接線、物品面板與哈瓦特陣營；相關7項通過。dosgolem檔案代號重用與完整32位元寫入修復首攻後黑塊，平台720項通過。r11雖到城鎮但模式表受污染；r12至第2回合；remake v.1.0.18至第5回合，完整雙側尚未通過。使用者要求打包v.1.0.19供人工測試。未提交或發布，保留全部未提交變更。

## 2026-09-09 勘誤：dosgolem 單位 sprite 並未缺失

2026-09-09 稍早在 `AGENTS.md`、[96](96-parity-toolchain-20260909.md) 與
[91](91-worklist.md) 記錄過一項 `BLOCKED`：「dosgolem 尚未繪製戰場單位
sprite」。該斷言錯誤，三處正文均已改寫。

錯誤來源是取樣了 `work/ch01-town-parity-20260908/oracle-live-r11/`。那一輪的
檔案模式表受污染，本檔前一段（2026-09-08）已記明「r11 雖到城鎮但模式表受
污染」，[94](94-ch01-town-parity-20260908.md) 也已記明「r12 首攻後人物已
正常」。以現行 oracle 重生的收據中，單位格唯一色數 30–46、平均亮度 55–71，
與空地格（12 色）明顯可分；r11 同位置只有 5–13 色、平均亮度 18。

規則層面的教訓已寫進 `AGENTS.md`：判斷 dosgolem 有無某項能力，必須用目前
程式產生的收據，不可拿保留下來的失敗樣本當現況。
