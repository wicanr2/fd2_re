...
- 2026-07-20 finale `0x2c405` phase-0 control-flow closure（Docker-only Capstone）：handler 先 `load_ch_text(30)`，alloc/clear `0x36b00` bytes staging buffer，並由 `0x15f84` 將 native text composite 寫至 `staging+0x12c30`；接著正好500次 loop。每次以 `0x11d40(0,255,BL)` 套 baseline palette，從 `staging + iteration*320` 複製 320×200 到 VGA，並 wait 1ms。`BL` 初始40；iteration≤300 時每5 tick 在非零時減1，之後每5 tick 加1。此為既知 palette/row-scroll phase，但其 staging 內容來源與 `0x15f84` renderer ABI 尚未恢復，故仍是 `0x2c172` gate 後的 fail-closed metadata，不能以空白 scroll 取代。
- 2026-07-20 finale phase-0 script closure：`0x15f84` entry 直接證實 arg1=FDTXT table、arg2=native selector、arg3=destination、arg4=stride；`0x2c469` 傳 `0x2c`。FDTXT_030 raw offset table 只有11實體 strings，故不得把它誤記作 raw string #44：count-aligned export 實證 selector #44 對應 logical utterance #44，也就是 raw string10 的第6段，並對位 editable `ch30.json` scene3 line6（悠妮「剛醒來的時候…忘記了以往的一切」）。已以 `native_2c405.json` 保存 native selector + physical locator、exact 9-arg layout 的可重製部分與 strict loader/test；因 glyph compositor、control codes 與 staging backdrop 仍未完整 lower，`Ready()` 固定 false。
