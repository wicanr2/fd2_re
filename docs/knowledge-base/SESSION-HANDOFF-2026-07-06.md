...
- 2026-07-20 ch29 post chapter-text transition：direct handler `0x25870 → 0x1088d` 已以 explicit `load_ch_text: ch30.json` binding lower；runtime 會替換當前 editable transcript，接著 `0x25921/0x25968` 的 FDTXT_030 dialogue 仍由明確 scene mapping 驗證。後續 Docker audit 更正流程判讀：internal chapter 29 是最終戰 handler，`0x2bce5` 返回後自迴圈，不會進 generic preparation；現行 final battle→generic ending 暫時略過此 handler。layout、dynamic pan、0x24618、0x2bce5 與 roster identity 完成前不得猜接。
