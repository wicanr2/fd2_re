...
- 2026-07-20 `0x1b750` synthesis continuation：raw `+0x3e` 是 DX/HIT/EV 共用基值，`0x1b750` 先讀 `+0x37/+0x39/+0x3e`，再對 equipped items 分別加 item `+1 AP/+5 DP/+3 HIT/+7 EV`，寫 derived `+0x48/+0x4a/+0x4c/+0x4e`；unit byte `+0x24` 非零時 DX accumulator 加 15，byte `+0x22/+0x23` 對 AP/DP 套 1.15 並 round。已新增 `campaign.RecomputeAfterClassChange` 與測試，class UI 不再把舊有效 HIT/EV 當新 base；race/multiplier raw bytes 尚未進 Unit。
...
- 2026-07-20 class-change equipment correction：發現 `ApplyClassChange` 先改有效 AP/DP/MV、再呼叫 `RecomputeAfterClassChange` 時，已有裝備會被重算兩次。現以既有 equipped item 貢獻反推 raw base，再套用 `0x1b750` 裝備重算；新增回歸測試證明 AP/DP 不會由 18/12 錯變 21/14，campaign/battle 測試通過。
- 2026-07-20 handoff reconciliation：本檔較早的 church/class-change 條目是歷史快照；其中「Lv=1」、「尚未接 UI／能力寫回」、「church 仍是 placeholder」等描述已由後續 RE 與實作更正。現行權威狀態是：保留 Lv、清 EXP、五組成長累加、target/item/UI/persistent roster 已接；仍待的是 `+0x22/+0x23/+0x24` raw 欄位資料化、原版實機數值回歸與完整 GUI 轉職操作截圖。
- 2026-07-20 class raw-field audit：直接以 `tools/disasm_le.py` 對 `FD2.EXE` 的 `0x1b750`（objdump 等價輸出）重現：`[unit+0x22] != 0` 時 AP accumulator 乘 1.15 並 round，`[unit+0x23] != 0` 時 DP accumulator 乘 1.15 並 round，`[unit+0x24] != 0` 時 DX/HIT base 加 15。這三個 bytes 在該 synthesis routine 的語意已確定；但攻略舊版 unit 表把 `+0x22..+0x26` 標為 M1–M5 spell bitfield，與 runtime direct trace 衝突，故 remake 不把它們猜測性併入 `Spells`，待完整 raw-unit pointer/schema 對齊後再資料化。
