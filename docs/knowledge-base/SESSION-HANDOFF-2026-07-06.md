...
- 2026-07-20 finale phase-0 script closure：`0x15f84` entry 直接證實 arg1=FDTXT offset-table、arg2=string index、arg3=destination、arg4=stride；`0x2c469` 使用 current `FDTXT_030` #44。依原始 strings 順序，#44 對位 editable `ch30.json` scene3 line6（悠妮「剛醒來的時候…忘記了以往的一切」）。已以 `native_2c405.json` 保存 exact 9-arg layout 的可重製部分與 strict loader/test；因 glyph compositor、control codes 與 staging backdrop 仍未完整 lower，`Ready()` 固定 false。
