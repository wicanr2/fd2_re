...
- 2026-07-20 `0x31602` stat-reset 定案：`0x4e4d1(portrait)=0x620a1+portrait*0x0b` 的 11-byte 成長列，五組 row pairs 經 `0x1e529` 寫 unit words `+0x37(AP),+0x39(DP),+0x3e(DX/HIT-EV base),+0x42(MaxHP),+0x46(MaxMP)`；`+0x40/+0x44` 由後段回填 current HP/MP。`0x4e48d(new portrait)+1` 的 mobility increment 會加到 raw `+0x3b`（即 Unit.MV，pathfinding movement budget）。接著 `0x1b750` 重算裝備衍生值，設 Lv=1、raw EXP `+0x3c=0`、HP/MP 全滿。row random 是 pair 的 `[min,max)` 取值。
...
- 2026-07-20 AI spell-family scoring：direct disasm `0x15B77..0x15DA1` 證實法術目標選擇不是通用物理評分：spell `0..12` 掃攻擊目標並累加 8/0x18 等優先分數，`13..16` 掃補血目標，`17..19` 走增益分支，`20..22`、`26`、`27` 走狀態／毒麻分支，部分狀態由 `0x1C269` 檢查。尚未把 `unit+0x25/+0x26/+0x27` 精確命名，remake 仍不猜接線。
