...
- 2026-07-20 finale `0x2c548` first party-cycle closure（Docker-only Capstone）：0x2c551/560/573 依序 alloc 0x1f400（320×400 staging）、0xfa00、0xfa00；`0x111ba(TAI.DAT,#3)`、`0x111ba(FDOTHER.DAT,#56)` 後先貼 backdrop。迴圈以 `[0x53bfb]-1` 向下走 party，unit base `[0x53a45] + slot*0x50`，byte `+7` 的 group 形成 FIGANI `group*3+1` 和 `group*3`，交給 `0x29164`/`0x2b9a1`；先有一段20×1ms的 320×200 present，再進 portrait/text/input branch。以上結構進 `assets/endings/native_2c548.json`，但不能據此把既有一般 battle scene 接上：slot selection、DATO/文字控制與 `0x29164` 全部 caller state 尚未合併驗證，gate 維持 `0x2c5e3`。
