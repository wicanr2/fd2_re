...
- 2026-07-20 ch29 post `0x1088d` correction：先前將 `0x25870 → 0x1088d` 縮窄 lower 成 `load_ch_text(ch30.json)` 已撤回，因 Docker/Capstone 證實本體不只載 FDTXT：它依 chapter 載三個 FDFIELD resource、讀 map control、重建 `0x1e00` runtime unit buffer、從 persistent `[0x53bf7]` 複製 own records、套 own-deploy coordinates 並 spawn groups。僅換 transcript 會錯過 map/roster 語意，故此 op 恢復 fail-closed；`0x25921/0x25968` 的 FDTXT_030 dialogue mapping 可保留為文字證據，但不可單獨視為完整 transition。internal chapter 29 是最終戰 handler，`0x2bce5` 返回後自迴圈，不會進 generic preparation；現行 final battle→generic ending 暫時略過此 handler。
...
- 2026-07-20 map29 final roster provenance：`0x1088d` 證實 `[0x53a45]+slot*0x50` slots 0..19 先由 persistent `[0x53bf7]` ordinal 0..19 複製，再寫 map29 own-deploy ordinal positions；`0x233c6` 只覆寫該同一 buffer 的 x/y/pose。`0x1b750` 是裝備／衍生能力 synthesis，不改 identity。尚未釘死 `[0x53bf7]` 的 persistent-record construction/order，故不能把 remake `PartyUnits` 或 map JSON row order 假定為 native slot order。
