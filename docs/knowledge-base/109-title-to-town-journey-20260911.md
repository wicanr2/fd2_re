...
- 種子固定 `FD2_SEED=20260911`。同一個種子跑兩次，`journey.json` 除了幀數
  以外的全部欄位，雜湊必須相同，否則測試失敗。實際兩次連幀數都相同（2026-09-12 為 24061 幀）。
...
| 死亡台詞 | 頭目 `death_effect` `[3, 8]`：`0x1AC2B..0x1AC49` 以 FDTXT_001（`[0x53C03]+1`）字串 8 呼叫 `0x15F84`；哈諾 `[2, 4]` 走事件 4（`0x343E2`）的字串 7 | 字串 8 恰好一次；字串 7 視哈諾是否倒下而出現，其餘字串不准出現 | 相符 |
...
| group 3 哈諾 | `[2, 4]` | 無（型態 2，走死亡程式：事件 4） |
| group 5 海盜頭目（頭像 97） | `[3, 8]` | 無（型態 3，走死亡程式：FDTXT_001 第 8 句） |

型態 2／3 不降成直接發放的獎勵，由劇本的死亡程式執行（事件 39／41 給的素材也在
程式裡），分派與轉寫見 [110](110-death-effects-and-level-cap-20260911.md)。頭目的
`[3, 8]` 只播一句台詞，不給金幣也不給物品，與原版進城金幣正好 1000（只算盜賊那一筆）
相符。
