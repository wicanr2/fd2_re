...
2026-09-07 修正後的 Docker 收據：正常啟動整合測試由未設定戰役變數的狀態載入
開頭與 `story_ch00_handler`，標題 `START` 由正式輸入 owner 揭示第 0 章；第 0 章
原生對話再逐拍消費開框、逐字、分頁與收框，最後抵達 `battle_ch01`。公開候選從
Git 清冊複製 441 檔並在封包目錄通過自我檢查；本機候選合併後為 42,720 檔，
640×400 標題選單已實際擷取。FM／MT-32 的 `FDMUS_004` 與 `FDMUS_018` 均完成
Vorbis 解碼及播放器建立。這些收據關閉重製端啟動缺陷，但 dosgolem 的 FD2 分支
目前只到 DOS/4GW／Watcom 初始化與 `FD2 main`，尚不能輸出原版 VGA 開場；因此
原版開場動態對拍與新版本發布仍保持未完成。
