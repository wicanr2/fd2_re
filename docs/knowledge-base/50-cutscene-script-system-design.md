...
待物品給予 UI 完成後再還原原版提示。材料取得後的五個關鍵戰場（ch11/13/15/17/20）已各有 editable
`postbattle_chNN_persist`，順序固定為 `sync_party → set_chapter → town`，分別回 town12/14/16/18/21；
因此不會為保存物品而跳過商店／教會／整備。ch21 既有配方兩臂則仍共同 sync 後回 town22。
