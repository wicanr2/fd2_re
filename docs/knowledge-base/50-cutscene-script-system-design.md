...
2026-07-20 追加 church direct trace：`0x3072f` 並非單純 placeholder return。它建立教會場景後呼叫 `0x2d669/0x2d7bd` 讀取服務選擇，依選項 `0..3` 分派到 `0x2ffa5`、`0x2f8ea`、`0x30dc3`、`0x31385`，完成後重畫教會並可再次選擇。`0x30dc3` 先由 `0x309ff` 建立符合條件的角色清單，再顯示 FDTXT `0x24c`（「誰要復活呢？」）相關文字；`0x31385` 呼叫 `0x31793` 建立轉職候選，顯示 FDTXT `0x24f`（「誰要轉職呢？」）相關文字。`0x2f8ea` 逐角色掃描 8 個 inventory bytes 並取未裝備 item，`0x2ffa5` 走角色選擇／能力服務路徑；這兩項的最終語意仍待追 callee。remake 目前 church 仍是 placeholder，下一步需先把 revive/class-change 兩個可證實服務資料化，不能再把 church 視為單純返回 town。
