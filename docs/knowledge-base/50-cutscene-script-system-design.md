...
**[校正 2026-07-05]acting 的移動語意(兩種走位機制)**:反查 0x1366a 本體 + 0x11cac,acting 播放器寫
unit **+3(pose=方向)/+4(tick=拍數)**,**不寫 +0/+1 邏輯格**;0x11cac 也是純重繪不搬格。**但**繪製公式
`0x127e0 = 格 + tick × f(pose)`,故 acting 的 **(pose, 拍數) = (方向, 視覺位移量)** —— 能做**視覺滑動**。
⟹ **兩套走位並存**:①**長程邏輯走位**(如王座 28 格 + 捲鏡頭)= §1.1 step 家族/0x13488 搬邏輯格;
②**短程視覺走位**(如草地)= acting 的 (pose+拍數) 經繪製公式視覺滑動。
⚠ 先前「acting 只做面向、不是走位」講太死,已校正:acting **不做邏輯搬格,但能做視覺位移**。
`拍數→格數`換算(f(pose) 每拍幾 px / 持續或遞減)與草地 roster(unit16/17=誰)**待校準**——
用影片對 doc55 逐幀距離反推,再把 0x65~0x69 轉錄成草地腳本。
...
- **草地「亞雷斯撞見」走位 = acting 視覺滑動**(§1.2 校正):doc47 §3 Part1 beat10-11 = ACT `0x65~0x69`,其 (pose,拍數) 即方向+視覺位移量,是原版腳本出處。**待校準**:拍數→格數換算 + roster(unit16/17=誰),用影片對 doc55 距離反推後可逐筆轉錄。remake 目前用 storyWalk grid-walk 近似 doc55 量測值。
