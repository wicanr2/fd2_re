...
**[定論 2026-07-05,dump 佐證]acting 的移動語意**:反查 0x1366a 本體 + 0x11cac,acting 播放器寫
unit **+3(pose=方向)/+4(tick=拍數)**,**不寫 +0/+1 邏輯格**;0x11cac 也是純重繪不搬格。
繪製公式 `0x127e0 = 格 + tick × f(pose)` 使 tick 產生視覺偏移(次格滑動),但 **acting 是否搬多格未證**。
