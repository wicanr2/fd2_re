...
**序章 ACT inventory（2026-07-15）**：`ch00_pre` 共 20 個 `0x1366a` call，其中 **16 個**已有完整
resource frame 解碼，不能再以「只解出部分 header」概括。王座／草地階段是 `0x32343/0x323f5/
...
不能從 30-slot projection 外推的依據。尚未取得 runtime resource bytes 的只有 map0 階段四 call：
`0x3283a→0`、`0x328a5→1`、`0x328c5→2`、`0x3290d→5`；必須在各自 entry 時點 dump table entry 與
resource，不能拿 0x50–0x99 的王座資源集猜低 ID 語意。
...
這些 target 確實全在 active count 外。map0 的 0/1/2/5 則是另一個尚未載入的低-ID resource context，
仍待其獨立 runtime dump。
...
**已淘汰的 entry breakpoint 路徑（保留作除錯警示，2026-07-15）**：不要把 `0x1366a` 的 runtime 位址
...
一律拒絕。
   ...
   pan、對話群組、spawn group、map31 timing-only acting 與 player JOIN；map0 acting 與其他未證實 op仍是**刻意不完整**的縱切，不能被當成全章可播放。
