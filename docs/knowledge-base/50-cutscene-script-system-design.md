...
2. 編譯層：`campaign.CompileHandlerScript` 將 editable handler script + 已驗證的章文本／
   ...
   `bgm`（`track=-1` → `bgm_stop`）；`pan/dialog/act` 一律要求**以 `source.addr` 鍵控的**
   ...
   JSON 表示；其 override 以 call-site 位址為 key。`ch00_pre.json` 先只收入已驗證的開場
   pan 與第一句對話，故仍是**刻意不完整**的縱切，不能取代現行手工場景或被當成全章可播放。
