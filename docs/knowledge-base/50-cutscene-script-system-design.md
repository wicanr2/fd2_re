...
ch00 的 map32 首段 99..105 與 map31 次段 90..98 均已改由這個庫引用；binding 也完整 transcription
三次 `loadch`（32→map32/ch00_palace，31→map31/ch00_meadow，0→map0/ch01），使每段 decoded
acting 都在它的原始 roster state 後才可能播放。0/1/2/5 尚待 runtime dump，仍 fail-closed。
