...
1. **`tools/dump_chapter_beats.py` + `tools/export_handler_scripts.py`(2026-07-15)**：走
   ...
   `loadch/pan/dialog/act/spawn/join/bgm/step/palette_fade/delay/reveal` 等可讀 op。
   尚未證實的 native call **保留為 `unknown`**，不可靜默丟失或猜譯。ch0(序章)仍逐項
   對照 doc47 §7；其中迴圈由 parser 自動辨識，精確匯成 `step(direction:2,repeat:15)`
   與 `repeat:13`。
2. 編譯層（進行中）：editable handler script + 已驗證的章文本／FDFIELD roster mapping
   → map-specific runtime `Beat`。這層只編譯有證據的 op；`unknown` 必須顯式回報，不能
   假裝成可執行效果。
