...
| **演出** | EXE acting 資源目錄 + context getter window，0x1366a(local id) 播放 | 序章 global 0x50~0x99 的 74 筆及 map0 local 0/1/2/5 已 dump+解碼 |
...
**acting 靜態來源與 context table base（2026-07-15 定位）**：已知 raw bytes 不在外部 DAT；
`FD2.EXE file+0x565d8` 是 106×u32 offset directory，offset 加 `file+0x53e00` 指向連續 acting data。
序章 context 的 getter `0x204801` 已 relocation 成 `[eax+0x207718]`，故 local32 對 global ID 0x50；
map0 context 則在同一條 `.text` 指令實測為 **`[eax+0x2077d8]`**，剛好 `+0xc0=48×4`，把 local0
重新對齊 global ID 0。這解釋低 ID 為何不能從序章 table dump 直接讀：不是另一種 decoder、也不是
LOADCH0 新讀某個 DAT，而是 loader/context 切換後修補 getter 的 disp32。I/O trace 亦證實 map31→map0
只新讀 FDTXT/FDFIELD/FDSHAP/FDICON，沒有新讀 FDOTHER/ANI acting 容器。
...
`decode_acting.py` 的文字解碼報告機械轉為 `assets/cutscenes/acting/map32.json`（歷史檔名；現為 ch00
共用 bank），74 個 `0x50..0x99` 資源加上 map0 的 `0/1/2/5`，共 **78 個** editable
`{beats,special,units:[{slot,pose}]}`，沒有原始 pointer。
...
這裡的 20/20 僅指 ACT calls；`ch00_pre` 整體仍是 partial binding，其他未 lower 的 handler 原語
會維持 compiler issue／runtime fail-closed，不能宣稱整段 handler 已可播放。
...
（ID 90–98）。後者依序會觸及 slots 25–32、33–37、39/41–44、8、61–71、67、64、65、52，正是
不能從 30-slot projection 外推的依據。map0 四 call 已由 `0x2077d8` table window 直接取得：
...
72-slot roster」猜出角色移動。後續靜態 SPAWN 資料流與王座／密林 acting-table byte compare 已把
此矛盾解完：map31 的 runtime `unit_count` 依序為 2→4→5，而 ID 0x50–0x99 table 在兩幕完全相同；
這些 target 確實全在 active count 外。map0 的 0/1/2/5 使用同一 EXE directory、但 getter table
base 改成 `0x2077d8`；其 bytes 與 slots 已由上節獨立 runtime dump 解完。
...
**map31 ACT(90..98) 最終裁決（2026-07-15）**：doc48 已保存王座與悠妮甦醒對話中兩份完整
`0x207718..+0x400` table dump；ID 0x50–0x99 的 74 個 pointer **逐 byte 完全相同**，排除 LOADCH 重新綁
resource。normal frame 的 `0x13882..0x13985` 會直接寫 target memory（不做 unit_count bounds check），
special/nonzero 亦直接寫；但主單位 renderer `0x127a9` 只迴圈 `0..<[0x53beb]`，所以 slots 8、25–71
不會畫出人物。播放器仍按 frame 呼叫 redraw／wait；special/zero 更會走
`delay(1)→terrain/unit composite→delay(2)→redraw`，不是空 terminator。因此這九個 call 的可見語意是
**保留原 frame scheduler／轉場節奏，不移動任何 active actor**。binding 以 call-site-local
`timing_only:true` lower ID 90–98，會清除 inactive targets 但保留 beats/special；這不是一般越界逃生門，
runtime 對其他未 materialize slot 仍 fail-closed。
...
是有效方法；本輪正是以此停在 ACT0/ACT1 context，再直接反組譯 getter，捕到 disp32 從
`0x207718→0x2077d8` 的 context relocation。淘汰的是「猜下一個 caller runtime 位址」，不是 entry 本身。
...
- ~~acting 原始靜態容器未定位~~：已於 2026-07-15 定位為 `FD2.EXE file+0x565d8`
  的 106×u32 offset directory，資料起點 `file+0x53e00`；context getter relocation 決定 local ID 視窗
  （序章 `0x207718`、map0 `0x2077d8`）。見 §1.2 與 doc48 §6.1。
