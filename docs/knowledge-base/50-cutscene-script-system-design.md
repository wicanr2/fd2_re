...
來源的 `spawn_intro` 現已承接 FDOTHER #9 的12個項目、舊／新增槽位邊界與第6／7／8次
快照重建；每次必須真正呈現後才前進，完整預檢成功才發布 roster。caller 後續 ACTING
仍是下一個獨立 beat，不併入 wrapper。缺素材或原始欄位時會在 roster 變更前失敗即關閉。
`0x32975(slot)`
...
2026-07-20 追加 church direct trace：`0x3072f` 並非單純 placeholder return。它建立教會場景後呼叫 `0x2d669/0x2d7bd` 讀取服務選擇，依選項 `0..3` 分派到 `0x2ffa5`、`0x2f8ea`、`0x30dc3`、`0x31385`，完成後重畫教會並可再次選擇。後來的 IDA E0 補讀已固定 selector 僅接受左右鍵、四項循環；此段的 `0..3` 是 raw index，不能據此替四項命名。`0x30dc3` 先由 `0x309ff` 建立死亡角色清單；count=0 顯示 FDTXT `0x24c`「隊伍中沒有須要復活的！」，有候選才顯示 `0x24d`「誰要復活呢？」並進入角色選擇。選定角色後從 `0x52669 + class*2` 讀 fee word，以 `fee * level` 計費；`0x30f39` 先驗金，`0x2d516` 扣款，再清死亡 flag 並把 max HP 寫回 current HP。`0x31385` 呼叫 `0x31793` 建立轉職候選，無候選顯示 `0x24f`、有候選顯示 `0x250`，確認使用 `0x252`。`0x2f8ea` 逐角色掃描 8 個 inventory cells，只保留 signed flag 非負者；FDTXT510/511/512與`0x1b8e7→0x1bb8c` writer把此服務閉合為物品轉交。舊文件誤把別的 shop callee 的`0x2f4c6/0x2d516`掛到`0x2f8ea`；完整 body沒有這兩個 call，也不改 gold，該斷言已刪。`0x2ffa5` 則走 caller-owned roster／唯讀`0x17aed`角色狀態 path。remake 已接 raw index0 完整 presentation、raw index1 roster＋indexed item list＋atomic transfer loop、復活核心與 class-change mutation；目的欄滿的 indexed feedback、FD2.SAV與 command effect/target仍 fail-closed，不能再把 church 視為單純返回 town。
- 2026-07-20 class-change direct trace：`0x3151a..0x3152d` 依 portrait 分支取轉職道具（特殊 portrait 0x34 固定查 item 0x5a；其他 promoted portrait 由 `0x526a7+portrait` byte table 查），`0x31860` 掃角色 8 個 inventory slot 找該 item。成功後 `0x1b8e7` 移除對應 inventory slot，`0x2a2e8` 執行能力/成長重算，最後 `0x31571..0x3157a` 寫回 class byte(+0x20) 與 portrait(+7)。item branch 與 class mapping table 仍需完整導出後才接 runtime。
