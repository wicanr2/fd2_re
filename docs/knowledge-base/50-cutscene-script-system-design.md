...
**[定論 2026-07-06,反組譯播放器逐 byte 證明]acting 只設面向、不搬格子**:反組譯演出播放器 `0x1366a`
兩個模式的**使用碼**(逐指令證明見 `scene-decode/ch1-throne.md` 附錄):
- 格式 = `u8 幀數 + 幀×{ u8 拍數(bit7=顯示模式旗標), u8 N, N×(u8 單位idx, u8 pose) }`。
- **正常模式**(bit7=0,0x13882):`mov [單位[a]*0x50+3], pose`、`+4(tick)=1`。
- **特殊模式**(bit7=1,0x137c7/0x1370b):同樣寫 `+3=pose`,不寫 +4,多做重繪。
- **關鍵:兩模式、所有子路只寫 +3(pose)/+4(tick),從不寫 +0/+1(X/Y 格子)。**
⟹ **acting = 逐幀設單位「面向/姿態」,不搬格子。(unit,pose) 解碼完整正確;bit7 只是顯示模式旗標,不改變兩 byte 意義。**
**+4(tick)的視覺效果(獨立事實,doc47 §10/§11)**:繪製公式 `0x127e0 = 格 + tick × f(pose)` 會把單位畫在
「格座標 + tick×方向」的偏移位置,tick 遞減→滑回定格 = **同一格內的次格平滑滑動(進格 polish)**。正常模式寫
tick=1 就是給一個最小滑動。⚠ 這是**單格內的小滑動,不是多格位移**——所以「acting 只設面向 + 微滑,不搬格」才是完整圖像。
⟹ **確定的走位機制只有一套:長程邏輯走位 = §1.1 step 家族(0x13185 等)/0x13488 搬邏輯格 + 捲鏡頭**(王座已驗)。
先前「acting 能做草地視覺走位(pose+拍數)」的假設**已被此證明排除**——拍數只是「這幀顯示幾個 tick」= 幀持續時間,非位移量。

**⚠⚠ 兩次校正的最終定論(別再翻案)**:先前「acting 只做面向」與「acting 能做草地視覺走位」**都下太快**。
