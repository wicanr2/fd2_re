...
- 母稿：`jonah-portrait-style-a-v1-master.png`，`1254×1254`；角色身分由
  ...
  眼神及少量棕金聖職服飾。2026-09-01 經角色資料與人工圖像再次核對後，
...
- 五組都具有 12 個不同 SHA-256、`24×24`、二值 alpha 與三個不同週期，列為
...
- `loadGame` 在完整預檢後，才以各組 12 張真彩色圖原子取代正規化
  `g.sprites[0]`／`g.sprites[1]`／`g.sprites[2]`／`g.sprites[3]`／`g.sprites[68]`；其他 group 不變，忠實主題
  預設路徑也不變。
