...
- 八十四組目前共有 972 個可見 `24×24`、二值 alpha 槽位：80 組一般圖組各有
  12 幀，selector 48、66、67、74 各有 3 張靜態相位。除 selector 52 依原版證據保留
  一組相同雜湊外，一般圖組維持 12 個不同 SHA-256；三個靜態圖組各維持三個
  不同 SHA-256，全部列為
...
- 原版 selector 48、66、67 都只有槽位 0–2 具有可見 `frame`／`mask`；槽位 3–11
...
- 正式繪製端既有有界索引規則在方向索引超出三幀時回落至 `phase % 3`，因此三張
  ...
  都失敗即關閉。
- selector 48、66、67 的身分與前三幀語意仍分別依自身原版素材記錄；本契約只
