...
第四章 r10／r52 的結果（原版側 r10 由 dosgolem `3c54de6` 乾淨樹重生，逐點與 r9 相同）：
行為、節點、交易三個 gate 通過（LOAD → 出口 → 五回合 → 增援 → 清場 → 戰後城鎮出售／
四棟建築／酒店存檔／祕密商店，單位、HP、回合、金額、鏡頭逐點相同，酒店存檔整檔 sha256
相同）；畫面 gate 56 張比 3 張未過：seq 892（第 5 回合 stay，839 像素：畫面上只有 (4,17)
那一隻單位是 idle 相位 1、其餘全是相位 2，單一相位的變體對不到，成因待查）、seq 934
（END 後的橫幅馬賽克，1035 像素：回復單位上的 `0x1DA16` 圖示演出未接，#29）、seq 1198
（酒店存檔畫面，#27）。

尚未閉合：酒店 `0x2FC85` 原生介面（資源 13、四圖示、存檔槽列表、「記錄儲存完畢」）
未實作，選項 0 目前是重製端自創的傳聞（#27）；升級與 END 回復演出（#29，這一章的
樣本沒有抽到升級幀，但 END 回復圖示影響 seq 934）；seq 892 的單一單位相位差（#31）。
