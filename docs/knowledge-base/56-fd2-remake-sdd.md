...
正式逐動作路徑不可由同步 `Scenario.Fire` 測試取代；失敗時不得發布半套永久名冊。
唯一目前狀態、驗證限制與後續門檻見 [94](94-ch01-town-parity-20260908.md)。
尚未完成首關勝利→城鎮的雙側連續對拍，不沿用後文歷史完成宣稱。
...
2026-09-08 對話勘誤與修正：v.1.0.10 已接開框前說話者聚焦及逐字頭像輪替。
舊有「逐字固定 frame0」推論撤回；[查證與驗收](93-dialogue-cause-20260908.md)
優先於後文歷史敘述。王宮第二句整張零差異，仍不宣稱完成全段對拍。

> **2026-09-07 操作驗收修正：** 開場地形、人物姿勢、第一關初始視圖、
> 玩家移動資訊框／路徑、圖示動畫、取消游標、戰況背景與來源身分勝敗接線，
> 依 [`92` 的 READY 規格](92-opening-ch01-input-audit.md) 修正。
> 正常 START 的角色姓名可以為空；可編輯 `protect` 必須由來源實體身分辨識，
> 不得以目前顯示語言判定死亡。這不提升完整戰役或 dosgolem 對拍的證據等級。
> 同輪追加修正 #5 entries 133–136 應走 `sub_16886→0x4e63d` 四模式，
> 以及普通 START 的零金錢；原始呼叫、畫面與未關閉項目仍集中於 `92`。
> 後續 v.1.0.7 接入中立 Escape／F2、原生回合聚焦與待機 raw bit7 同步；
> v.1.0.8 接入有空欄位的普通物品寶箱問答。兩者均依 `92` 追加 READY 契約，
> 不提升滿欄交換、其他寶物、精確 DOS 時序或音訊。
...
### 1.0 第一輪重製的抽樣完成門檻（2026-08-27）

第一輪 remake 不再以全30關逐幀、逐音訊或 DOS 硬體逐週期一致作完成條件。
後續採分層代表性抽樣；戰役／戰間、戰鬥／AI、介面、存檔／持續隊伍及終局／平台
五層合計至少60個正常輸入樣本，若全部通過、每層達最低配額、沒有重大已知失敗，
即可用約5%未發現重大缺陷率的單側近似支持95%信心，標記第一輪 remake 完成。

95%是產品抽樣信心，不提升個別證據的E0／E1／E2等級，也不表示整支EXE已翻譯或
每幀逐像素一致。除錯直達、route patch及第三方存檔必須明列，不能取代核心正常
輸入樣本；公開發行所需的實體平台、簽章、公證與宣傳素材另設門檻。可重跑矩陣及
目前差距見[`REMAKE-STATUS.md`](../REMAKE-STATUS.md)。
...
|---|---|---|---|
| UI-01 | Title/main menu | 上下選擇、確認、取消、save/load、游標音效與 focus state；開場按鍵只可中斷原始第三參數標為 `skippable` 的當前 AFM 幕，並繼續下一幕，不得以 ESC 無條件略過整段 | partial；`TitleMenuState`／`TitleSlotState` 已與 Ebiten 輸入共用並有 deterministic trace；`0x20421` 的 `(index, delay, skippable)` 與既有 index 3／1 可中斷證據是逐幕輸入契約。仍缺原版逐幀 E2 對照與完整 boot/campaign 接線 |
...
| UI-05 | Dialog | 上／下框、portrait anchor、文字避讓、控制碼、分頁／捲動、嘴型、輸入鎖 | partial；ch24兩個lookup已用原版indexed frame接通開框、逐字、完整頁等待期m0/m3嘴型與收框；`internal/dato.MouthState`保存`0x16D00` post-decrement。2026-09-14 dosgolem 三臂另證實第一關戰鬥事件與第二關回合起手對白都把 Escape 當成與 Enter 相同的單次推進，正式兩分支已共用 typed gate；無輸入臂維持等待，見[`battle-dialogue-esc-vs-enter.json`](../data/ui-traces/battle-dialogue-esc-vs-enter.json)。其他caller layout與同狀態E2仍未閉合 |
...
ID1–8的entry現已由IDA線性位址重新固定為`0x262EF/0x26528/0x26795/
0x269D3/0x26BFD/0x26E39/0x272B8/0x274B0`。`0x523B9`表內raw dword少
`0x10000`，不可直接當線性位址；早期把ID5–8讀成`0x16xxx`並解讀其他函式的
結論全部撤回。八個entry各有不同mode回傳、channel/counter、offset table與
sample marker，typed schedule必須逐entry保存，不能共用ID0七元素排程。
完整校正與mode契約見
...
schema填入這三張表。`0x2673F`現已閉合為單一frame/repeat helper：在caller
提供的原位destination畫`effect[state]`，依descriptor delay推進，不另算座標。
mode0清狀態並給29張front；mode3給12張target，frame16/17交替並在
step`0,2,4,6,8,10`產生六個HP／sample2 marker；九張boundary沿用同一狀態，
...
2026-08-25 補齊 IDs10..12 的正式演出規格。typed schedule 必須保存 command ID、
可選的前導 `Native2189A{actorSlot,initialRadius,radiusStep}`（ID10無；ID11為
`15,10`；ID12為`30,16`）、前導 #80 selector2、三組 signed sampling triplet
`X={128,0,-128}`／`Y={128,0,128}`／`increment={131,128,125}`、四 surface循環、
60個 Draw-ack frame、delay10、#80 selector13 markers
`{0,6,12,18,24,30,36,42}`，以及共用 #5 命中／未命中結果佇列。正式 owner
必須先驗證 actor／target raw provenance、地圖view／HUD、pointer-grid來源、
FDOTHER #3 LUT0..9、#5 result descriptors、#80 selector2／13與所有 work/VGA
buffers；再完整預建 ID11／12 前導、三個 `sub_1F558` 等價 surface、60張主段及
22張結果。原版在 `sub_21548` 主段開始即扣 MP；重製只可在相應第一張主段完成
Draw後發布，target HP在60張主段結束及steady redraw後一次發布，結果hold完成才
...
| 35 | `27FC9` 依序以 IDs26/22/27 呼 `22D1B`；每段都自帶effect／mask／damage或skipped queue／22幀數字與尾停 | 私有三階段plan在各段mask邊界發布該段HP／marker／RNG，取消時整批回復 | 受限class19玩家正式indexed owner達`RUNTIME-E1`；精確逐幀／逐音訊E2待補 |
...
只定義ID34的三段狀態契約；ID33／35契約見下段。ID32的正式演出擁有者已由
後文規格與實作取代此處早期限制；ID33／34／35皆已接正式owner，AI／其他
...
`0x7E`起12張／sample7且raw frame index4重播／mask`0x48`；`0x93`起11張／sample8且
raw frame index3、6重播／mask`0xD8`。這些index從0開始，不可減一。每段接#80 sample1、五組snapshot→mask與最後
...
raw bytes、RNG與`Acted`；任一失敗保持actor／targets零修改。score／EXP、AI、
其他BattleFig與高階狀態名仍失敗即關閉。主證據見
[`fd2_command33_transaction_ida.txt`](../data/ida/fd2_command33_transaction_ida.txt)。

2026-08-25 補齊指令33專用尾段規格。`0x211A4`在函式內硬編碼command
`0x0D`，所以正式owner在`0x27FC9`共用段及0..40地圖漸入後，只接第13號的
raw尾段：FDOTHER #6 `0x39..0x3F`七張（#80 sample12）、#80 sample1、五組
snapshot→raw mask `0xC0`切換、固定`0x320`回復、FDOTHER #5 bias `0x69`
結果queue、22張數字段及500 ms尾停。不得重播第13號的`0x21EB1`前置，也不得
在這五條已證實class19來源猜扣MP。mask切換最後一張完成繪製後才原子發布
HP、raw `+0x25..+0x27`與RNG；尾停後才發布`Acted`。首幀前須驗證#66、#92
selector1、#5／#6、#80 selector1／12、actor／target indexed base、palette、
panel與私有交易；任一缺件零修改。主證據見
[`fd2_command33_tail_presentation_ida.txt`](../data/ida/fd2_command33_tail_presentation_ida.txt)。

正式`nativeCommand33PresentationJob`現已依此規格接入玩家grid→confirm：所有
共用與專用畫面在首幀前建妥，#92 selector1位於effect frame6，#80 selector12／1
分別位於七張效果與mask切換入口；發布邊界、尾停完成、讀檔／節點切換取消及晚期
錯誤均由私有plan保護。非靜音原始資產端到端與完整Docker／Xvfb回歸已通過，故
提升為`RUNTIME-E1`；score／EXP、敵方owner、精確逐幀／逐音訊與E2仍未閉合。
...
保存三個可分段發布且可整批回復的私有快照。正式owner只可在各段五組mask完成後
發布該段HP、marker與RNG；第三段數字段及尾停完成後才發布actor `Acted`。缺class／BattleFig、target
raw provenance、MP gate、target geometry或任一stage時，actor與所有targets保持
零修改。ID35玩家正式indexed owner已由下段規格接通；score／EXP、AI、其他
BattleFig及高階狀態名稱仍失敗即關閉。
主證據見
[`fd2_command35_transaction_ida.txt`](../data/ida/fd2_command35_transaction_ida.txt)。

2026-08-25 補齊指令35三段專用尾段規格。`0x22D1B`每次都先執行
`0x1C4CC→0x1C2DA`，再以實際傷害走bias`0x5E`數字queue，或以`0x1E1DC`
輸出固定skipped queue。正式owner在共用段及地圖漸入後依26→22→27消費
FDOTHER #6：起點`0x8A／0xAA／0x9E`、幀數`9／13／12`、首幀sample`3／3／2`、
mask`0xC0／0x23／0xC0`；command22另在零起算frame index7重播sample3。
每段再播放sample1、五組snapshot→mask、22張數字段與500 ms尾停。正式`Game`
owner已消費#68／#94共用段、三列raw schedule、damage／skipped queue與三次Draw
發布邊界。每段mask後才發布該段HP／marker／RNG，第三段尾停後才發布`Acted`；
失敗或取消整批回復。
主證據見
...
來源約束、近似等級與失敗原因屬開發診斷，不是原版終局畫面的一部分。正式模式
播放角色蒙太奇、20 段尾聲及永久定格時，不得在原始 320×200 畫布上疊加
「來源約束 E1」、「完成後停留」或按鍵說明。這些提示只允許在玩家主動開啟的
除錯 HUD 中顯示；素材不足而進入可編輯結語的失敗回退仍可顯示必要說明，因為它
不是冒充原版畫面的成功路徑。驗收需鎖定正式模式無提示、除錯模式有提示，避免
研究用標籤污染第 30 戰正常玩家終局。

終局輸入必須由單一 `Game` 消費端處理，正式 `Game.Update` 與回歸不得各自改寫
內部旗標：角色蒙太奇期間的任意新按鍵只保存原版已證實的 raw-change pending，
直到 portrait boundary 消費；永久定格的 Enter／Space 才啟動重製端隊伍回顧，
回顧中的 Enter／Space／Escape 則回復同一終端影格。輸入處理不得直接推進蒙太奇
或尾聲，也不得把特定 DOS scan code 語意猜進正式路徑。

## 2026-08-25：第29戰後至第30戰的連續 E1 合約

正式回歸不得在 `preparation_ch30` 停止，也不得直接指定 `story_ch30`。它必須由
第29戰勝利確認進 raw ch28 post，同步持續隊伍，在終局整備走19人選擇與存讀檔，
再消費 ch29 pre 的 LOADCH、21句對話與七次 `0x33F78` staging，最後由正式節點
物化 `battle_ch30`。可見等待可由隔離測試快速推進，但 handler 副作用、資產驗證、
選人、存檔與節點邊界不得略過。

此回歸暴露三個原先被直接節點測試遮蔽的錯誤：LOADCH 只歸零專用視圖而未歸零
一般游標；`nativeFocusEndpoint` 把 story 可見游標與舊一般絕對游標混成不一致的
六欄狀態；LOADCH 建立的新 scene actor 未初始化已證實的原生 `+0/+1/+3/+4`
呈現載體。現行實作把六個視圖欄位視為同一 typed state、在 LOADCH 同步歸零，並
只對尚未具備原生載體的新 actor 物化 constructor 初值。任一來源或資產失敗仍整段
失敗即關閉。這是 `RUNTIME-E1`，不提升未修改原版一般玩家 `PLAYER-E2`。

### 2026-08-25：冷讀檔持續隊伍至終局的收尾規格

本切片不新增原版語意，也不重開已閉合的終局函式。它把兩個既有正式契約接成
同一條可重跑交易：`preparation_ch30` 節點存檔必須先在私有候選中驗證隊伍拓撲，
再原子還原 `party_members`、JOIN 首次出現順序、部署選擇與 roster。JOIN 順序不得
重複，每個順序 ID 都必須存在於 membership；每個已部署 ID 還必須有 roster 紀錄，
已物化 roster 也不得游離於順序／membership。尚未在本戰部署的 reserve 可保留為
chronology-only，等後續具證據的 LOADCH／JOIN constructor 物化；不得為了驗證方便
猜造 raw 紀錄。失敗時目前節點、隊伍、金錢與介面狀態均不得改變。

冷讀檔後必須走19人整備確認、正式 `story_ch30` handler 與 `LOADCH` 物化
`battle_ch30`。最終戰勝利確認要在清除戰場前，把具原始身分的最後數值同步回
同一 persistent roster；終局蒙太奇及重製端回顧必須依完整 JOIN chronology 消費
這批紀錄，而不是只沿用第30戰的暫時部署選擇。出戰者取得最終戰更新，未出戰者
保留冷讀檔時的持續狀態，確保「每位隊員的最終狀態」不會因最後整備被刪掉。
同步為零、身分不一致或任一 ending 資產 admission 失敗時保持第30戰結果與節點，
...
- 2026-08-29 素材消費端已改為分離的FDOTHER #34完整101格bank與ANI #0；固定原檔
  只留在匯入器及逐格oracle。`0x24336`正式owner不再查找或解碼`FDOTHER.DAT`。
...
2026-08-27 依新增的 `0x2415B` caller 證據，正式節點已在第一段對話前以三張
25-byte表配置slots 0..24及special slot25，並發布camera `(336,240)`；成功臂又在
FDTXT #7／#8／#9之間依序消費 ACT63／64 的4／5幀typed資料。回歸要求layout的
normalized／raw pose一致、兩段ACT實際啟動，之後才允許`0x24336`、JOIN、城鎮與
存讀檔完成。layout、ACT與`0x24336`各自在自身發布邊界驗證slot、原生presentation
與資產；缺件時停止該階段，不把局部驗證誇大為整條配方交易的單一原子預檢。

尚未解除的只剩未修改原版同狀態逐幀／音訊配對，以及`0x4DFCC`進入本函式前的
程序內第一相位。重製端只保存已證實的相對相位與順序，不宣稱DOS計時逐拍或
逐像素一致；這些證據限制不得觸發重新反組譯`0x24336`本體。
...
此驗收只使用上述已閉合的資料與 runtime owner，不重開新的 EXE 語意。
起點必須是全新 `Game` 冷讀 `preparation_ch30`，並保留完整
`partyJoinOrder`、`partyRoster` 與 raw `+6/+7/+8/+0x20` provenance。其後必須
依序經過19人選取、`story_ch30`、`battle_ch30`、END／YES、勝利確認、
兩個終局對話門、`0x2C548` 角色蒙太奇與20段尾段，最後到 FDOTHER #59
永久定格。測試不得在終局入口重建另一份名冊，或以獨立終局 fixture
取代冷讀後的持續隊伍。

蒙太奇與尾段的測試推進必須有固定上限。為節省回歸時間，可使用已證實的
原版「輸入改變後在目前角色結束時跳至最後一輪」契約，但仍須證明蒙太奇
開始時的 `Units` 數量與冷讀 `partyJoinOrder` 相同、尾段真正抵達定格、
定格在時間推進後不變，以及選用隊伍結果回顧進入後能回到同一定格。
任一原始資產、raw provenance、蒙太奇或尾段深層預檢失敗時，整條驗收必須
失敗即關閉，不得把編輯後備結語當成原生終局完成。這關閉的是重製
...
複合技中ID32／33／34／35皆已具受限class19玩家的正式indexed owner與原子交易；
尚無正式owner的其他法術仍各自失敗即關閉。
...
畫面後再送 Return，必須開啟原版指令環。同一程序的後續回合驗收必須再以普通輸入
完成 `END→YES→ENEMY PHASE`，等待敵方演出結束後，以 Return 開啟索爾狀態面板。
2026-08-26 的有界擷取已通過這條鏈，因此原版側達成一輪控制權交接的單次連續候選
`PLAYER-E2`。這仍不證明第29戰勝利、戰後、整備、存讀檔與第30戰已形成同一原版
連續程序，也不提供停用音訊容器中的音訊證據。
...
選擇一個合法圖像相位。這個入口必須在擷取前凍結指定相位；舊實作只設初值，
仍讓正式時鐘推進，所以得到的9255／3763／9255不是三個固定相位的有效比較。
修正後只有固定雜湊截圖凍結；一般玩家時鐘仍依原週期推進。缺值預設0，
越界或非整數則失敗即關閉。

`0x31EA9..0x31EC6` 與 `0x31EDB..0x31EFB` 對 `sub_187D6` 的第四參數
不是色盤值，而是 FDOTHER #5 數字字形起始項目：第一組 quota 使用
`0x1F` 即 entries 31..40，第二組 `quota-selected_count` 使用 `0x2A`
即 entries 42..51。名冊圖像的 `0x1088D→0x11019→0x31FBB` 消費鏈另見
[`fd2_preparation_gray_roster_ida.txt`](../data/ida/fd2_preparation_gray_roster_ida.txt)。固定槽的28個
可選格在相位0逐格 `AE=0`；接通第二套數字字形後，同狀態320×200
整幀達 `AE=0/64000`。這只是固定第三方槽、初始選人狀態與相位0的
重製 `RUNTIME-E1` 像素錨點，不外推其他狀態或完整 `PLAYER-E2`。

### 標題主選單 99% 穩定畫面契約（2026-08-26）

未修改原版的 [`title-original-dosbox.png`](../figures/title-original-dosbox.png) 與從目前
原始碼重新建置、以 `FD2_SHOT_TITLE_MENU=1` 擷取的 640×400 畫面，在最近鄰縮回
320×200 後只有選單區域 `x=129..190、y=162..189` 不同；標題美術與背景沒有差異。
現行選單起始列 `y=162` 的整幀差異為 `AE=917/64000`。以既有六張透明選單素材、
START 選中相位及共同向下兩列的幾何投影重建同一狀態後，最佳候選為
`y=164／173／182`。正式 Ebiten renderer 接入後重新擷取，整幀達
`AE=0/64000`；先前離線透明合成候選的249像素差異不是正式執行結果。

正式 renderer 應把三列共同起點定為原生座標 `y=164`、間距9，水平仍依素材寬度
相對320像素畫面置中；不得個別微調字樣或修改已相同的背景。決定性測試須鎖定這三個
...
存讀檔。材料不足臂也由第21戰勝利邊界開始，以正式輸入完成共同#5十句與#6四句；
它不執行ACT63／64或`0x24336`、不授予`0x64`，但仍完成JOIN24／23、`town_ch22`
及存讀檔。原始位址、雜湊與控制碼見
[`fd2_ch20_sky_key_sequence_ida.txt`](../data/ida/fd2_ch20_sky_key_sequence_ida.txt)。
...
- **酒店存檔寫回原版槽（`0x30012`，已證實；#24 關閉）。** 原版把 32 筆持續紀錄整段
  抄進 `0x312B + slot×0xA28`，metadata `+0` 章節、`+1` 筆數、`+2..+5` 金幣。重製端
  `BuildNativeChapterSlot` 以標題 LOAD 讀進來的 plaintext 為 byte 基底，每筆只覆寫
  已證實欄位（`EncodeNativePersistentRecord`：座標、`+2` runtime 索引、`+3/+4` 歸零、
  `+5`、`+6..+8`、物品欄、指令位元、種族／職業／等級、暫時狀態、`+0x34..+0x36`、
  `+0x37/+0x39` 基礎攻防、`+0x3B..+0x4F`），`fdsave.Encode` 後原子取代
  `FD2_NATIVE_SAVE`。r9 原版酒店存檔與重製端 r38 整檔 sha256 相同；達到相同要一併
  對齊的三條原版規則：`0x1B8E7` 出售左移後第 8 格只寫旗標 0x80、item byte 留舊值
  （空格的 item byte 是殘值，佔用看旗標 bit7）；`0x1E529` 升級加在 `+0x37/+0x39`
  基礎值上（`BaseAP/BaseDP`）；反擊把攻方打倒時 `0x1562F 0x1B6B7→0x1AA1D` 的掉落給
...
- **敵方回合的游標／鏡頭協定（#30 關閉；r10 收據 eip-trace `0x12CEA`，已證實）。** 敵方回合
  裡沒有獨立的鏡頭規則，全部是 `0x12D7B`／`0x12CEA` 把游標先 X 後 Y 逐格走過去，鍵盤
  處理器的安全帶帶著鏡頭；順序是：要走路的單位先聚焦自己（`0x13BA5`／`0x14203`）→
  `0x13488` 走行跟視圖（與玩家同一支）→ 攻擊前 `0x1548E` 再聚焦自己（`0x154AD`）與目標
  （`0x154DE`）→ 演出；指令路徑 `0x15311` 聚焦自己（`0x15360`）後在 range mode 下把游標
  移到選中的目標格（`0x153C2`）；有說話者的對話（`0x165AC`）聚焦說話者；敵方回合結束
  `0x1A7AB` 聚焦記錄 0。重製端 `aiFocusCursor`、`walkAnim.followView`；r52 起 seq 720 之後
  每個檢查點的 camera 逐點相同。
- **END 回復標記與清除（`0x1A477`→`0x13512`、`0x13536`，已證實）。** 回復過的記錄 `+5`
  bit7 設起來（橫幅下畫成灰色，r10 seq 934），橫幅播完、敵方 AI 之前 `0x13536` 把全部
  記錄的 bit7 清掉；重製端 `ApplyNativeEndTurnRecovery`／`aiPhaseClearBit7Pending`。
  戰鬥若在敵方回合中結束，持續紀錄裡不會殘留這個 bit（否則酒店存檔 sha256 會差）。
- 行動收尾（`finishSuccessfulUnitAction`）把 raw `+3` 回靜止值 0：玩家攻擊（r9 seq 132）與
  敵方攻擊（seq 920→921）之後的檢查點都是 0。
...
- **酒店 `0x2FC85` 原生介面（#27 關閉）。** 酒店是商店框架的 hub 變體 0：`0x111BA` 載入
  FDOTHER 資源 13（LMI1 目錄、28 個條目，版面與資源 12 相同，沒有購買成功演出）、
  DATO `0x81` 店主頭像（表 `0x52659`：`[0x81,0x80,0,0x82,0x83,0x84]`）、提示文字 `0x249`、
  四個圖示（`0x2D669`／`0x2D7BD`）；服務 1 存檔（`0x30012`）進 `0x30550` 的四槽列表（與
  標題 LOAD 同一支繪製器，框是資源 13 條目 16），存完顯示文字 `0x294`「記錄儲存完畢！」；
  服務 0 是打聽消息（story 節點）、3 離開。重製端 `native_hotel_ui.go`（`nativeHotelMode`
  menu／slots／saved），戰役資料每章多一個 `hotel_chNN` 節點（`rumor` 欄位指向傳聞
  story），城鎮 0 號建築進酒店而不是直接進傳聞。近似：傳聞播完回城鎮（原版回酒店
  選單）；服務 2 讀檔（`0x301F4`）未接。
- **原版側 checkpoint 的撕裂與重播的 idle 相位。** oracle 的 checkpoint 若落在整幀
  memcpy（`0x373C4`）或 `0x11EB0` 視窗複製中間，PNG 會上半新、下半舊（r10 seq 892 的
  熊）；dosgolem `caa9ee8` 起 EIP 在這三段（含 `0x4E809` 馬賽克）裡時只延後 PNG、其餘
  狀態照舊拍，時間線不變（r13 逐點與 r10 相同）。重播端出 idle 變體時，`0x1297D` 在
...
- **事件尾段 `0x13CCE`／`0x13CD6`／`0x122xx`。** `[0x53AD5+event]=1` → `0x25B45` 音效（資源 31、
  ...
  **這條路徑影響戰役分支，不只是金錢**：敵人撿走的東西要能被打掉拿回來，天空之鑰那組道具
  （`0x24B14(0x64)` 的 gate，見 ch20／ch26 兩列）才湊得齊（使用者 2026-09-18 指出；
  哪一格的箱子會被哪隻敵人撿走仍待逐章收據確認）。
...
第六章（`parity-ch06.json`，remake-r11，58 點、18 點有差）加進來：
