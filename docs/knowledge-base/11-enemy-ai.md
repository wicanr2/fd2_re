...
- spell id `17..19` 走增益術分支；`20..22`、`26`、`27` 各自進入狀態／毒麻相關分支，部分條件透過 `0x1C269` 檢查目標狀態旗標。
- 這些分支證實 AI 不只是「會把 SpellID 傳給 Cast」；它有依 spell family 選目標的獨立優先級。精確欄位名稱（例如 `unit+0x25/+0x26/+0x27` 的狀態旗標）仍需配合單位結構與實機案例命名，暫不在 remake 猜測接線。
