...
4 筆未知後，四語內容目錄各有 5,176 筆玩家內容出現位置；2026-09-16 依當時清冊
（5,166 筆，語言包鍵名字面不再列入）重綁為 5,099 筆，作法見
[`docs/data/localization/README.md`](../data/localization/README.md)。其中 3,331 筆劇情欄位
...
### 現代 selector 綠幕輸入契約（READY）

現代 selector 的 3×4 動態母稿與 3×1 靜態三相母稿若無真實 alpha，均可明示 `--chroma-key #RRGGBB` 與
`--chroma-tolerance N`。明示綠幕代表角色不得使用該 key 色；處理器將所有 RGB
歐幾里得距離不大於 `N` 的像素標為透明，包含腋下、腿間與髮絲間被人物輪廓包圍
的背景空隙。對高亮且單一通道明顯佔優勢的 key，處理器另移除同一通道仍至少為
其他通道1.3倍、亮度至少20且色彩飽和度至少55%，但因抗鋸齒變暗而落在距離容差外的溢色；低飽和灰綠裝備不得因此
消失。未指定綠幕時維持既有「只移除邊界連通高亮中性背景」規則；輸入已
含透明像素時，仍以原有 alpha 作為二值遮罩，不再依 RGB 猜測背景。

綠幕色與容差是母稿處理參數，不是 runtime 資料。正式採用前必須驗證：母稿及
12張輸出皆為 RGBA、alpha 僅含0／255、每幀固定24×24、黑底與白底合成均無可見
色邊，而且深青色服裝等非背景區不得被侵蝕。最大連通元件裁切仍屬既有輸出契約；
若角色包含與身體分離的投射物、武器或特效，不得使用此處理器，必須另立具型別
動畫資產契約並失敗即關閉。
