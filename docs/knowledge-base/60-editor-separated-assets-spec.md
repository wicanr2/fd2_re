...
2026-08-31 `RUNTIME-E1-PARTIAL`：非繁中的 greeting、購買問句、金額不足、無可裝備者、
裝備確認、轉移來源與目的提示已接正式 compositor；三個翻譯語系的固定提示及所有正式商店
商品＋價格問句均通過字形與三列矩形測試。506／511 仍需要先完成角色名稱 ID 語系契約，
因此角色滿欄與空來源提示目前不冒險接入；服務按鈕圖像文字及 E2 截圖亦未完成。
...
繁中仍走原版 indexed compositor。正常玩家截圖 E2、角色名冊與戰鬥 HUD 尚未由本批關閉。
...
出售／轉移持有物預定重用商品清單既有 `67×16` 名稱矩形，價格模式仍由 caller 保持
原版四分之三價，語系層不得改價；但目前實體目錄只涵蓋商店供應的120個ID，尚未證明涵蓋
所有可進入玩家背包的物品。因此持有物名稱不得先接正式路徑，必須先建立完整有效物品ID
清冊及四語名稱；缺名稱、缺字或超界時整個 frame 拒絕發布。
