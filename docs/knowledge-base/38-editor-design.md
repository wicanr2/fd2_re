...
### 2.3 地形屬性:成本已接,編輯器只顯示不編輯

`remake/internal/battle/move.go` 的 `MoveCost` 讀 `map.json` 的 `cost` 陣列(doc01 §5
地形控制表換算),不可通行地形給 `BLOCKED_COST=99`,`Reachable`/`Path` 的
`nco <= u.MV` 判斷天然把它篩掉。**地形會影響移動,不是視覺層。**

> 勘誤(2026-09-11):本節原本寫「引擎目前完全不讀地形屬性,`MoveCost` 是全平地
> 成本 1」。那在 worklist 第 8 輪「地形屬性接線」之後就不成立了,只是沒有人回頭
> 改這一節。原版側也印證成本確實生效——第二關移動力 6 的單位實測只走得了 4 格。

編輯器據此的分工:
- **`cost` 由 tileset 元資料換算,不逐格填。** 編輯器畫 `tiles[]`,`cost[]` 跟著
  tile index 查表產生;讓使用者逐格填移動成本會讓同一張 tile 在不同地圖上行為不同。
- **屬性面板是唯讀的**:選中一格顯示它的 tile index 與換算出的成本,用來檢查畫出來
  的地形走不走得通,而不是用來改數字。
