...
- **尚未證實**：其他 command ID 的完整 FIGANI／SFX mapping，以及 command24
  的`0x29164／0x2B659`完整indexed source base、精確 PCM 取樣率與一般玩家 E2。
  `0x29C90`兩段各10次的BG viewport與`0x2A289→0x18C6D` actor／target狀態欄
  已接，不再列為整體未知。
...
- `0x2a6bd` 其餘 command-specific presentation branch（尤其 `id>=0x20` →
  ...
  未知」；`0x29C90`的20次viewport與indexed狀態欄已閉合並接線，但完整
  `0x29164／0x2B659`source、精確音訊／E2仍待。
