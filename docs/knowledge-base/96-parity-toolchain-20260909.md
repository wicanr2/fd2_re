...
`sweep_round` 原本只處理「走得到敵人旁邊」的單位（貼敵格在一般移動力加射程之
內），走不到就整個回合不動。第一關兩軍一開始就貼在一起，這個限制看不出來；第二關
我方在東側 `(20..24, 13..16)`、敵方在西側，相距十幾格，結果是每一回合都「處理了
0 個單位，結束回合」，敵方 AI 也沒過來，三個回合下來雙方 HP 一點沒變。

修法不是放寬那個距離，而是**分成兩種情況**：有單位走得到就照舊只處理那些；一個都
走不到才改成整隊朝最近的敵人推進。

逼近格的產生方式也一起換掉。原本沿 origin 到最近敵人的直線取路徑格，直線被地形
擋住時那幾格會十試十敗——第二關 `(23,16)` 的單位往西整列都走不了，一路試到
`(13,16)` 全被拒絕，一個回合白花三億指令。改成掃 origin 周圍移動力內的所有格、留下
比原地更靠近敵人的，再**按距離分桶、每桶取最靠近敵人的兩格**：先試最遠的一桶，走
不到就退一格再試。實際移動力連同地形成本從狀態層看不出來（第二關實測只走得了三
格，而 `typical_move` 是 6），全押在同一個距離上猜錯就整輪報銷。

同一次改動修掉另一個相關的坑：`sweep_round` 記「這一輪處理過誰」原本用**座標**，
而推進過的單位座標就變了——下一圈重算 `pending` 時它的新座標不在集合裡，於是同一個
單位被反覆選中直到預算用完。第一關看不出來，是因為攻擊會設 record `+5` bit7，
...
補法依專案定案：**把 dosgolem 補到涵蓋**，不是退回 DOSBox。往新關卡推進時這會連續
發生——2026-09-11 從第三關推到第四關就撞了三次，每一次的表徵都一樣：

| EIP | opcode | 缺什麼 |
|---|---|---|
| `0x1E1C1` | `02` | `ADD r8, r/m8` 的記憶體形式（暫存器形式本來就在）|
| `0x1C2CB` | `00` | `ADD r/m8, r8` 整個方向都沒有 |
| `0x15BBF` | `DB /0` | x87 的 `FILD m32int`（骨架本來就在，缺這一個載入）|

補第二個時順手盤了整組 byte 運算，發現缺的是**「目的為 `r/m8`」那一半**（`00` ADD、
`08` OR、`20` AND、`28` SUB；反方向 `02`／`0A`／`22`／`2A` 本來就在，只差結果寫回哪
裡），一次補完五個。逐個撞、逐個補要多跑好幾輪四十分鐘。

第三個（x87）乾脆不再一輪撞一個：**線性掃過整個 code 段**，把所有 `f` 開頭的指令
統計出來。FD2 總共只用 17 種、75 條，已支援 7 種，一次補齊剩下的 10 種就再也不會
在 x87 上停下來。

```bash
# capstone 線性反組譯 obj1，收集 mnemonic 以 f 開頭的指令
# 會在資料區錯位，所以這是**上界**不是精確清單：出現的不一定會執行，
# 沒出現的幾乎不可能被執行——用來盤「要補哪些」剛剛好。
```

掃出來的形式一樣要逐條看：`fld` 全是 `DD /0`（m64fp）不是 `D9 /0`（m32）、`fimul`
是 `DA /1` 讀有號 32 位元整數、`faddp`／`fmulp` 都只用 `st(1)` 那一個編碼。補的時候
只認實際出現的形式，其餘明確拒絕——放行沒驗過的延伸碼會安靜地算出錯的數字。

補的依據是 Intel 語意加既有 helper（`decodeAddress32` 解位址、`add8`／`sub8`／
`setLogicFlags8` 設旗標），不是推測。x87 的記憶體形式要特別小心：ModRM 的 reg 欄位
是**指令延伸碼不是暫存器**，`DB /1` 是 FISTTP、`/2` 是 FIST、`/3` 是 FISTP，行為都
不同，一起放行會安靜地算出錯的數字。

每次換基底都照紀律先跑重現對照：三次補完後 `ch01-move-attack.jsonl` 的 24 個登錄
檢查點都全同，補指令沒有動到既有路徑。

### 走不到就原地行動，不要中止整場

單位四周被自己人或敵人佔滿、或地形不通時，候選格會一個都走不到——第二關第 7 回合
的 `(13,12)` 就是這樣。原本這種情況讓整場中止，五十分鐘的進度連同還沒存的續跑點
一起沒了。

正確的處置是回到單位自己那一格原地確認、把行動結束掉，回合才推得動。中止要留給
「介面停在猜不出來的狀態」那種真的無法繼續的情形。

### 移動力只能從「哪一格走成了」學回來

實際移動力連同地形成本在狀態層看不到，所以落腳格能不能到一律靠送 `enter` 之後看
介面有沒有進指令環裁決。第二關的單位實測走得了四格，而 `typical_move` 是 6——每個
單位每一回合都要從六格一路試到四格，白花四五次試探，一次試探要送十幾個鍵、跑三千
萬指令。

驅動端因此記住每個 identity 上次走成功的距離（`MOVE_SPAN`），下一次從那個距離開始
試再往兩邊擴。這是學回來的觀測值不是假設：沒學到之前照舊從移動力上限往下掃，學錯了
下一次的實測會把它換掉。

### 收尾等待要夠一段演出加一輪敵方回合

`engage` 收尾（把介面退回地圖游標）原本給 30 格預算。第二關第 3 回合一次攻擊觸發
增援，敵方從 9 個變 15 個，演出期間介面一直是 `unknown`，30 格等不完就判成「收尾
之後介面停在 unknown」而整場中止——其實只是還沒播完。預算改成 90 格、每格 1500 萬
指令。

`unknown` 在戰場上幾乎都不是壞掉：不是演出就是敵方回合（AI 在走位，遊戲根本沒在等
鍵盤，堆疊上自然沒有輸入路徑）。這兩種都只能等，而且要等得夠久。

### 指令環上的 `esc` 是「取消行動」，不是「關掉選單」

單位移動之後原版會開指令環（攻擊／法術／物品／待機）。驅動端原本在「走到了但射程
內沒有敵人」時送 `esc` 離開指令環，理由是待機要在六格 grid 上選對格子、狀態層看不
出選中了哪一格。那個理由成立，但結論錯了：**`esc` 會把單位送回移動前那一格**。

實測（第二關第 1 回合，同一次執行的相鄰兩個檢查點）：

| 檢查點 | 動作 | identity 0 的座標 |
|---|---|---|
| 151 | 移動到 (17,15) 成功，介面是 ring | (17, 15) |
| 152 | 送 `esc` | (20, 14) |

推進因此從來沒有生效——每一回合看起來都有在走，回合結束時全隊還在原地。第一關看
不出來，是因為那裡兩軍一開始就貼在一起，單位走一步就打得到人，幾乎不會走到這個
分支。

所以戰場上**只有這幾種情況可以送 `esc`**：選移動格時取消選取、退出系統選單、退出
狀態面板或指令 grid 這類子面板。指令環本身不行。`stand_by` 送完 `down` 之後如果
介面還停在 ring，也只能換下一個方向重試，不能 `esc`。三個方向都不是待機就回報失敗
中止，讓人看 checkpoint 判斷——默默抹掉一次移動比中止難查得多。

`ensure_cursor_mode`（把介面退回地圖游標）同理：遇到 ring 先用待機把行動好好結束，
而不是一路 `esc` 退回去。
...
### 回合結束用系統選單的 END，不用「逐一讓單位待機」

回合收尾一律送系統選單的 END：在空地按 `enter` 開面板、`down` 三次選 END、`enter`
確認、再 `enter` 選 YES。那是 `ch01-phase-banner.jsonl` 既有收據走過的路徑。全員行動
完原版有時會自己換手，但「有時」不夠——打不到人而待機的單位、推進到一半的單位，換手
條件不一定成立，那時等回合推進會空等到預算用完。

單位本身的行動還是要待機結束（`esc` 會取消行動，見上一節）。待機是指令環的 `↓`，
但可選項隨單位而變：實測第一關第 3 回合加入的哈瓦特按 ↓ 開的是六格 command grid
不是待機。所以 `stand_by` 每送一個方向就檢查介面——開了 grid 或狀態面板就 `esc`
退回指令環換下一個方向（那個 `esc` 退的是子面板，行動還在），還停在指令環就直接換
方向。`down`／`right`／`left` 都不是待機才回報失敗。

實測第二關：六次待機全部是 `down` + `enter` 一次就成，沒有需要換方向的情況。
哈瓦特那種開 grid 的是個別單位的差異，不是通例。

同時「這一輪處理過誰」改由驅動端自己記，不再讀 record `+5` bit7：那個位元在本回合
最後一個單位行動完（換手後整批清零）、攻擊接的升級對白結束前、以及「退出指令環
沒行動」這三種情況下都不成立，會讓同一個單位被反覆選中。

實測（第一關）：連續五個回合都正常執行——每回合挑打得到人的單位接戰、其餘留在
原地、END 收尾換手，中途的加入過場與升級對白都推得過去。**但第 5 回合我方全滅、
戰敗**：驅動器的戰術只有「走到最近的敵人旁邊打」，敵方到第 4、5 回合有兩波增援，
單兵貼上去會被圍毆。要在原版側真的通關，缺的是戰術（集中火力、留在地形上、撤退
治療），不是操作能力。

### 長關卡跑不完時的作弊：鎖定我方 HP（修改路徑，證據要降級）

自動對拍的戰術只有「走到最近的敵人旁邊打」。第一關有六波增援，單兵貼上去會被
圍毆——連續五個回合的操作都正確，第 5 回合我方全滅、戰敗，後面的關卡也就取不到
收據。

`FD2_ORACLE_LOCK_ALLY_HP=1` 開啟 oracle 的 `-lock-ally-hp`：把 camp 2 單位 record
`+0x40` 的 HP 每 2000 指令壓回該 identity 看過的最高值。用 identity（record `+8`）
當 key 而不是陣列索引，因為增援與死亡會讓陣列重排。間隔要密——一次攻擊演出就可能
把單位從滿血打到 0。

**這是修改路徑。** CLAUDE.md 的規則是「使用修改、略過戰鬥、第三方存檔或直接進場時
必須降低證據等級，不能冒稱完整一般玩家 `PLAYER-E2`」，所以收據自己會講：
...
每測一次第二關就重跑第一關那 73 億指令（約 20 分鐘）是不可行的。原版自己的存檔
就是答案：`FD2_ORACLE_STATE` 指的覆蓋層是可寫的，遊戲寫出來的 `FD2.SAV` 落在那裡，
下一輪掛同一份檔案、從標題走 **LOAD** 就回到存檔當時的位置，只要 2 億指令。

判準不是畫面而是**檔案**：探測每按一項就列一次覆蓋層目錄，多出 `FD2.SAV` 才算數。
先前在系統選單與建築裡盲試了三輪都只看畫面，而那些畫面全長一樣（同一個店員、
同一句「還有事嗎？」），看不出誰寫了檔。

三件實測結果：

- **存檔在酒店**（0 號建築）的第二個圖示：進去之後 `right` 一次、`enter`、再
  `enter` 確認，畫面回「記錄儲存完畢！」，覆蓋層就多出 `FD2.SAV`。右下角那排
  圖示是**建築內部**的功能列，和城鎮的建築切換是兩層不同的選擇。
- **LOAD 要按兩次 enter**：第一次選 LOAD，接著跳出四槽的存檔選擇畫面（槽 1 顯示
  「第 二 章 羅德鎮」，其餘「無記錄」），第二次才真的載入。只送一個 enter 會停在
  槽位選擇，後面所有方向鍵都餵給那個畫面——症狀是 `steps` 幾乎不動、eip 一直是
  `0x36D98`。
- **CONTINUE 不是存檔**。它讀的是 `FD2.TMP`，回到的是關卡開場。三份 `FD2.TMP` 的
  雜湊互異、遊戲確實一直在寫它，但那不是玩家存檔。

存檔本身是遊戲自己寫、由標題的 LOAD 讀回的，不是第三方存檔；但產生它的那一輪開了
HP 鎖定，所以**由這個續跑點延伸出去的收據一樣不是 `PLAYER-E2`**。

每一關打完都要在城鎮存一次，續跑點才會往前推；`{"town_save": true}` 做這件事。
判準從第二關起要換：覆蓋層本來就有一份 `FD2.SAV`（上一關的續跑點載進來的），
「多出一個檔」只在第一次成立，之後看的是**內容雜湊**變了沒。只比大小不行——存檔
大小固定，新進度看起來會像沒變。存不成功一律失敗收場：默默往下走會讓下一輪從舊
存檔起跑，而 log 看起來完全正常。

### 城鎮是五格循環，不是走動畫面

戰間城鎮（`0x2CE08`）長得像一張可以走的營地圖，實際上不是：五棟建築排成一圈，
`left` 把編號加一、`right` 減一、超出 0..4 就繞回去，**`up` 與 `down` 完全無效**。
先前「四個方向各走 10~14 步都留在城鎮」不是沒找到出口，是半數按鍵沒有意義、另外
半數在原地繞圈。
...
目前選到哪一棟只能看畫面右下角那塊標籤（`0x2cd46..0x2d05a` 畫的不透明面板），
狀態層沒有這個欄位——`view.cursor` 是戰場用的，在城鎮不動。

這個順序早就在重製端的 `remake/assets/scenarios/campaign_full.json` 裡：每個
`town_chNN` 節點的 `options` 就是這五項，第三項寫得清清楚楚是「出口：出戰整備」。
**盲試之前先 grep 自己的 repo**——重製端已經實作過的東西，原版側的操作順序通常
也一起記在那裡了。第六項（`selection 5`）是神秘商店，要在該章指定的建築上按專屬的
BIOS 掃描碼才會出現（`native_secret_gate`），一般切換到不了。
