...
原本的作法是把每個單位推進到敵人附近再待機，讓原版自己換手。待機要在指令環上選
「↓ 待機」——但**指令環實際上是六格的 command grid**，不是四向十字：方向鍵在格子
之間移動，按幾次 `down` 會選到哪一格從狀態層看不出來。而且可選項隨單位而變，實測
第一關第 3 回合加入的哈瓦特按 ↓ 開的是 grid，四個方向都試過仍停在那裡。

改用系統選單：在空地按 `enter` 開面板、`down` 三次選 END、`enter` 確認、再 `enter`
選 YES。那是 `ch01-phase-banner.jsonl` 既有收據走過的路徑。打不到人的單位就 `esc`
退出指令環、這一回合停在原地。
