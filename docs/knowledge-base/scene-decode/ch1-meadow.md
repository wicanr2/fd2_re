...
> ⚠ **這一段呼叫的原語只有:PAN / BGM / ACT(0x65~0x69) / 對白。沒有 step 家族(0x13185等)、沒有 0x13488。**
> 也就是說「索爾/亞雷斯在草地走動」的位移**不在 handler 的顯式呼叫裡**——若在 acting,就是藏在 ACT(0x65~0x69) 內。
...
> 格式:`u8 幀數 + 幀×{ u8 拍數(bit7=模式旗標), u8 N, N×(u8 單位idx, u8 姿態) }`。pose 0下1左2上3右。
> **每個 byte 意義的反組譯證明(播放器 0x1366a 逐指令)見 `ch1-throne.md` 附錄**——已證實 (unit,pose) 解碼正確、
> bit7 只是顯示模式旗標、兩模式都只寫單位 +3(pose)/+4(tick)、**從不寫 +0/+1(格子)**。
...
## 5. ★為什麼卡★(2026-07-06 反組譯到底後的定論)
1. **handler 草地段呼叫的演出(0x65~0x69)只設面向**(unit 0-15 面上 / unit16-17 守衛 / unit1 王后),
   **沒有一筆動 unit 3/4(草地主角)、也沒有搬任何單位的格子**。但原版影片(doc55)明明看到主角在草地走多格。
2. **先前的懷疑「bit7 解碼錯/藏著位移」已排除**(見 ch1-throne 附錄:反組譯播放器 0x1366a 兩模式使用碼,
   **只寫 +3 pose/+4 tick,從不寫 +0/+1 格子**;decode_acting.py 那句「待實測」也已改)。⟹ **acting 解碼是完整正確的,acting 就是只管面向。**
3. **所以草地主角走位確實用「另一個機制」,而且目前沒定位到**:
   - 全 EXE 沒找到動 slot3/4 的 step 家族(0x13185等)/0x13488 呼叫;
   - handler 草地段只有 PAN/BGM/ACT/對白。
   ⟹ **草地主角走位驅動源 = 深層未解**(比王座難:王座是顯式 step,草地找不到顯式位移呼叫)。
4. **下一步候選(擇一,待使用者定)**:
   - (a) 全 EXE 搜「誰寫 slot3/4 的 +0/+1(X/Y)」(找 `mov [base+idx*0x50+0/1]` 且 idx=3/4 或動態)——直接抓位移的寫入點。
   - (b) 森林 context 另有 acting dump(`out/acting_resources_forest.bin`+`acting_table_forest.bin`,未解碼);
     若草地/森林另有一套資源,值得解(但機制已知只設面向,期望值低)。
   - (c) 接受 doc55 影片量測重建(remake 已對齊,可玩),把精確原版驅動長期擱置。
