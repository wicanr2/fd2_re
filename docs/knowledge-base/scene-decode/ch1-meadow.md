...
## 5. ★可疑點 / 為什麼卡★(供使用者檢查——你懷疑的核心)
1. **handler 草地段呼叫的演出(0x65~0x69)動的是 unit 0-15(全體面向)+ unit 16/17(守衛)+ unit 1(王后),
   沒有一筆動 unit 3/4(草地主角索爾/亞雷斯)。** 但原版影片(doc55)明明看到索爾/亞雷斯在草地走多格。
   ⟹ **草地主角走位的驅動源,不在這幾筆演出的 (unit,pose) 解碼結果裡。**
2. **兩個可能(未定,要你幫判斷)**:
   - (a) **我的 bit7 解碼是錯的**:0x65~0x69 全是 bit7 模式;若 bit7 幀的兩 byte 不是 (unit,pose) 而是別的
     (如鏡頭/位移/目標),那「動守衛16/17」是誤讀,真正內容可能是主角位移。decode_acting.py 自己標
     bit7 語意「待實測、不臆測」+ bit7 分支會 `push 鏡頭座標; call 0x11eee(地形重繪)`——**高度懷疑 bit7 跟鏡頭/位移有關**。
   - (b) 解碼沒錯,主角走位是**另一個機制**(非這段 handler、非這幾筆演出)——但目前全 EXE 沒找到動 slot3/4 的 step/0x13488 呼叫。
3. **建議下一步**:反組譯 bit7 分支(0x1370b~0x1385f)看那兩 byte 到底被當什麼用(是 (unit,pose) 還是鏡頭/位移參數)。
   這是「acting 是否解乾淨」的關鍵——也可能一舉解開草地走位。
4. **森林 context 另有 acting dump**:`out/acting_resources_forest.bin` + `acting_table_forest.bin`(尚未解碼);
   若草地/森林用不同 acting 表,值得一併解。
