...
- **狀態欄用到的 sub-resources(視覺驗證)**:
  - **框 = #22(149×42)**:深藍底 + 左上亮白 raised bevel + 「LV‧」+「HP」「MP」標籤 + 血條紅槽,**全燒在這張素材**(`[0x53a81]+0x5e` → #22)。框內槽 native:HP y22–26、MP y31–35、槽 x≈26–145。
  - **血條 cell = #27–30(1×5)**:純色漸層欄(#27/28 黃=HP、#29/30 紅=MP/空槽);raw(值全 ≤0xC0)。
  - 數字 digit cell、其他 UI 片散在 138 個 sub-resources。
