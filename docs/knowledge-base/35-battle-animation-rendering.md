...
- **左右是資料決定的**：FIGANI 幀標頭內嵌的絕對螢幕座標按陣營分邊，同一單位
  不論當回合攻或守都留在自己那一側。分離包實測（`assets/figani/meta.json`
  與 `figani.LoadSeparatedResource` 解出的 `Frame.X`）：

  | 資源 | 角色／用途 | 幀內嵌 x 範圍（@320） |
  |---|---|---|
  | 12 / 13 | 亞雷斯 待機／攻擊 | 89–178 |
  | 0 / 1、3 / 4、9 / 10 | 我方其他職業 待機／攻擊 | 85–206 |
  | 288 / 289 | 盜賊 待機／攻擊 | 6–28 |

  台座固定畫在右側 slice（模板匹配 orig 台座左上 =(165,157)@320），所以會壓到
  台座的**永遠是我方那張圖**；重製端的圖層順序必須讓台座緊接在我方 figure
  之前（見 `remake/cmd/fd2/battle_scene_layers.go`）。
...
最佳frame76由`AE=4436`降至`AE=1330`；再修正原版字模索引載入後降至`AE=903`；
普通攻擊重用既有indexed bar／digit核心後全幅為519。519全部位於從舊比較圖
反向裁切oracle帶入的左邊與底邊；排除合成邊框後319×199內容區`AE=0／RMSE=0`。新對照圖為
[`battle-impact-compare-20260825.png`](../figures/battle-impact-compare-20260825.png)。
這是固定fixture的`RUNTIME-E1`畫面改善，不是一般玩家E2或完整DAC／音訊時序證據。
