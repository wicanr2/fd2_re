...
### 3.2.5 [設計鐵則] 我方=背影+土台 / 敵方=正面(使用者確認的原版視角設計)

- **原版固定視角**:玩家從**我方背後**看戰場 → **我方角色一律畫背影**(背對鏡頭)、**敵方角色一律畫正面**(面對鏡頭)。
- **土台只在「我方」角色腳下**(標示我方站位),不是「攻方 / 守方」的概念——**與當回合誰攻誰守無關**,純粹「我方 vs 敵方」陣營。
- 故 orig_05:亞雷斯(藍衣**背影**、腳下大 dither 土台)= **我方**;盜賊(紅頭巾**正面**)= **敵方**。
  (先前 doc 用「攻方 / 守方」描述左右是**誤框架**,正名為「我方(背影,右,有土台)/ 敵方(正面,左)」。)
- remake 對映:我方單位用背影 FIGANI 幀 + 自帶土台;敵方用正面 FIGANI 幀。

### 3.3 我方腳下「圓圈 / 土台」= 我方背影 FIGANI 幀自帶的大 dither 橢圓陰影,**不是 BG 層、不是程式畫純色**(視覺確認)

- **靜態前提**:整條戰鬥合成沒有任何 fillrect / 畫圓 / 畫線原語——畫面只走 `0x4e63d`(sprite RLE blit)、
  `0x11eb0`(逐列 memcpy present)、`0x11d40`(色盤寫)。→ 圓圈必是素材,非程式生成。
- **視覺確認(放大 orig_05 + 人眼定位用幀)**:攻方亞雷斯用的是**攻擊動作幀 `FIGANI_013_f01`**(= FDICON 組4 ×3 **+1**,
  見 §1「組×3 / 組×3+1 兩攻擊動作」),這張**圖底自帶一個大的 dither 網點橢圓陰影**(`01xxxxxx` 隔位寫模式,見 doc06)。
  - 它是**半透明**的:疊在綠草 BG 上 → 透出綠色,所以 orig 看起來像「綠色草地圈」;其實不是綠色實心素材,是灰 dither 透草。
  - 攻 / 守各自的攻擊幀 FIGANI 都自帶;**幀別差很大**——`FIGANI_012`(動作0)腳下陰影小,`FIGANI_013`(動作1)才是這個大橢圓。
- **反組譯鐵證(前景層 = 藍漸層,非土台)**:攻方合成路徑 `0x29ded` 確實有 blit 前景層
  `0x29e27 0x4e63d([0x5410b+edx*4], X=0, Y=50, ...)`,但這三層 `[0x5410b]/[0x5410f]/[0x54113]`
  由 `0x28daf/0x28dc9/0x28de3` 的 `0x111ba("BG.DAT"@0x52381, slot, entryIndex=0/1/2)` 載入
  = **BG.DAT entry 0/1/2 = `BG_000/001/002`**;dump 出來是**垂直藍色漸層**(body `3f4f→3f4e→3f4c` 每列遞減)
  ——是戰鬥畫面的藍底 / 捲簾漸層,**不是土台**。故前景層說推翻。
- **dither 半透明驗證**:`FIGANI_013_f01` 底部橢圓區(y150–185)~67% 透明(隔位網點),y174–180 帶灰土台像素
  → 確認半透明 dither,疊綠草 BG 才會透出綠(orig 綠土台的成因)。
- **刪除的錯誤推測**(本輪推翻,省 token):① 「圓圈 = BG.DAT 前景層 `[0x5410b/0f/13]`」(上述反組譯證實該三層 = 藍漸層 `BG_000/1/2`);
  ② 「圓圈 = `FIGANI_012` 自帶小 dither」(用錯幀,實為 `FIGANI_013` 動作1 的大橢圓)。
- **remake 對映**:攻方用**攻擊動作1**(`figaniIndex+1` = 組×3+1)的 FIGANI;**不要 drawCircle / drawEllipse 自畫**;
  正確解碼保留 dither 透明,畫 figure 時土台陰影自動帶出。**注意 BG 須延伸到 figure 腳下**(orig 草地到 y≈175),
  否則 dither 疊黑底會變灰而非透綠(remake 已知:BG 只貼 y50–150 時亞雷斯腳下 y175 土台偏灰)。
...
- **血條長度來源釘死**:`unit+0x40 = 當前 HP`、`+0x42 = 最大 HP`、`+0x44 = 當前 MP`、`+0x46 = 最大 MP`(由 0x18795 的 `cur*101/max` 推得;spawn 時 0x10fe9 把 +0x40=+0x42、+0x44=+0x46 設成同值=滿血滿魔)。
  > ⚠ **與 §2.2/§7 衝突**:那邊把 `+0x40/+0x42/+0x44/+0x46` 標成「戰場格 current/home X/Y」。0x18c6d 的血條算式不容反駁地證明這四個 word = HP/MaxHP/MP/MaxMP。**§2.2 figure lunge(0x29f72 讀 +0x40)的「格 X」解讀需重新查證**(figure 位移很可能是讀 +0x48… 螢幕投影,不是 +0x40)。本輪只釘死 HP 欄義,§2.2 待複查不動。
...
- **"HP" / "MP" / "LV" 標籤字**:未見獨立 blit → 內含在 ① 的面板底圖 sprite([0x53a81]+0x5e)裡(待視覺對照確認,合理推定)。
...
| unit[+0x40]/[+0x42] (word) | **當前 HP / 最大 HP**(0x18c6d 血條算式釘死;spawn 0x10fe9 設 cur=max)。⚠ 推翻舊「戰場格 X current/home」標法 — §2.2 figure 位移解讀需複查 |
