...
## 7.5.1 序章主角隊進場 staging:直接定位,無行軍動畫(2026-07-03,反組譯+dosbox 複驗收尾)
...
**結論**:
1. **remake 現行做法(main.go focusOnParty 純鏡頭對準 + event.go spawn_party 直接定位)已忠實**,#3 **不是 bug**,不需要補行軍動畫。
2. **玩家記憶「一行人走到地圖中央」查無實據**:本次完整重跑序章開場(含所有對話分支)未發現任何世界地圖/道路移動段落,也未發現任何單位行走動畫。最可能的解釋是把 `0x13185`/`0x32999` 的**攝影機平移 reveal 效果**(鏡頭移動 + 單位隨之「浮現」的視覺印象)誤記成「角色在走」——這在原版視覺上確實容易造成「有東西在移動」的錯覺,只是移動的是鏡頭不是人。若後續其他章節真的出現世界地圖/道路移動機制,需另開新證據鏈,不可用本節結論套用到其他章節。
3. 先前 event.go 註解一度誤引「doc 25 §7.5.1 dosbox 實機證實[世界地圖走位]」,但該小節當時並不存在——本節即補上正確內容並修正該註解的引用鏈(§7.5.1 = 本節,無世界地圖佐證)。
