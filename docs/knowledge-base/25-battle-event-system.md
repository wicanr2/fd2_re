...
| 原語 | linear | 次數 | 作用 | 狀態 |
|---|---|---|---|---|
| **查單位陣亡** | `0x3453e` | 36 | `unit_dead?(idx)`:位址 `[0x53a45] + idx*0x50 + 5` 的 bit0(每單位 0x50B;bit0=陣亡,doc 26 §3 證實) | [驗] |
| **handler prologue** | `0x205be` | 15 | 預設 `[0x53ecc]=2` → 清 `[0x53ecc]=0` → `call 0x1088d`(載該章 FDTXT 文本/腳本) | [驗] |
| **繪事件畫面** | `0x15f84` | 6 | 全螢幕圖繪製(過場 / 事件畫面) | [驗] |
| 各章特有動作 | 0x33499/0x1d4cb/0x1b722/0x4e56c/0x211a4/0x1b8e7/0x22af6/0x1df58/0x21082/0x1c4cc/0x1c2da… | ×1 | 增援登場 / 觸發對話 / 特殊效果,逐章不同 | [推] |

關鍵狀態變數:
- **`[0x53a45]`** = 戰場單位陣列基底(每單位 0x50 byte;doc 23)。事件條件多半在查它。[驗]
- **`[0x53ec8]`** = 戰場計數(每 tick `clamp` 到 0x63=99;0x11959)→ 推定**回合數**,供「第 N 回合觸發」類事件。[推]
...
```
即:**章 1 的腳本邏輯 = 「若單位 5–10 群都達到某狀態(到達/陣亡)→ 觸發劇情事件;再依單位 50/51 狀態分支」**。
這就是一條「事件指令」的真身——一段檢查單位狀態的硬編碼條件 + 設 `[0x53ecc]`。

> 單位狀態 byte(+5)bit0 的確切語意(陣亡 / 抵達指定點 / 已行動)未隔離驗證 → **[推]**「到場/存活」類旗標。
...
- **[阻]** 18 個特殊 handler 的逐章完整事件語意(各 ×1 動作函式 0x33499/0x1b722/0x22af6… 是增援還是對話還是特效)未逐一展開——量大,留待逐關重製時按需反組譯。
- **[推]** 單位狀態 byte(+5)bit0、`[0x53ec8]` 的確切語意需隔離驗證。
