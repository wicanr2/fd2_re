...
| `0x32999(group_id)`(序章呼叫 2 次,group 1/2)`spawn_group_with_intro` | 先用 `0x111ba` 載頭像+`0x1366a` 播演出（非對白；acting 機制見 doc50 §1.2）,內部再呼叫 `0x10b4e` spawn,**接著逐一掃描新 spawn 的單位**,若座標落在目前攝影機視窗([0x53aa9]/[0x53aad] ± [0x51a87]/[0x51a8b])內就用 `0x4e85b` blit 出來,最後 `0x11eb0` present | 無——同樣是單位**已經**定位好,只是「有沒有進攝影機視窗」決定畫不畫;視覺上像「隨鏡頭捲動逐個冒出來」,但每個單位本身沒有位置插值 |
