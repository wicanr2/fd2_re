...
| 原語 | linear | 次數 | 作用 | 狀態 |
|---|---|---|---|---|
| **查單位旗標** | `0x3453e` | 36 | `unit_flag?(idx)`:`[0x53a45] + idx*0x50 + 5` 的 bit0(狀態旗標,初始=1;語意未定,**非陣亡**,見 doc 26) | [驗 欄位] |
...
> 單位 byte(+5)bit0 **初始化=1(非陣亡)**;確切語意(存活/在場/啟用)未隔離驗證 [推]。bit7=0x80 推定「已行動」。
