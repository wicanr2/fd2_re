...
| UI-05 dialog | dialog Draw 約 3590–3686；`dlgAdvance` 有 page/scroll state；ch01 original oracle `docs/figures/ch01-dialogue-original-dosbox.png` 固定左肖像下框、文字、page indicator。2026-09-05 另以獨立 dosgolem 複本擷取王座廳第一句，重製旁車證實相機`(3,20)`、索爾`(8,21)`、`0x32382`、`FDTXT_033#0`；修正 `scroll_step` 完成後未發布最後 story view 的 consumer 錯誤。320×200 near-state 比較為33223／64000相同像素、RGB平均絕對誤差25.055，見[`story-ch00-audience-near-state-e1.json`](../data/ui-traces/story-ch00-audience-near-state-e1.json)。動畫相位與完整 DAC 時序未鎖定，仍不得升為 same-state／E2 | partial | 鎖定原版與重製動畫相位及 DAC 狀態，再補每種上／下框、肖像錨點與控制碼同狀態差分；不得以目前 near-state 色差猜補 renderer |
