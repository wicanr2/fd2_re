...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合 `0x16F55` direction3→END、FDTXT `0x1A3/0x1A4`、YES、`0xC8` ms 與 `0x1A30B`；chapter0 current-runtime 正式 E1 以十二個60 Hz幀近似延遲後進敵方回合。command 13–16 的玩家與敵方 mode 11 現先走 `0x21EB1→0x22046` 16張 FDOTHER #3 LUT 演出，再做 MP／HP／action；AI 在移動後依 raw selector 重建 target array，缺 raw baseline、record 或任一 LUT 時不 mutation。 | partial | END 同狀態畫面、command 13–16 後段 indexed／數字 queue、其餘三格 owner、其他 effect presentation與缺archive UX |
