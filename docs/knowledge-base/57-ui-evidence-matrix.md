...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合共用 `0x117E7` 的 `0x12C0D==-1→0x16F55` 空游標 owner、direction3→END、FDTXT `0x1A3/0x1A4`、YES、`0xC8` ms 與 `0x1A30B`；所有正式 battle 現要求完整 FDOTHER #2 後開啟四格面板，以十二個60 Hz幀近似延遲後進敵方回合，缺圖格不建立不可見熱區。command 13–16 的玩家與敵方 mode 11 已接完整 E1：`0x21EB1` 16張 FDOTHER #3 LUT、FDOTHER #6七張、五組 snapshot→`0xC0` mask、MP／HP transaction後 steady redraw、FDOTHER #5四欄數字22張與500 ms尾停；AI 只沿 raw selector target array，缺 #3/#5/#6、baseline或record時零 mutation。 | partial | END 確認框 indexed renderer與逐章E2、command 13–16 同狀態逐幀／逐音訊 E2、其餘三格 owner、其他 effect presentation與缺archive UX |
