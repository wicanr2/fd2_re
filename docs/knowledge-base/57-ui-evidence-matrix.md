...
| preparation | 45–55% | 舊「兩欄文字核取方塊」與「確認框仍是重製殼層」斷言已失效。城鎮 FDTXT `0x201` 出發提示會保存／還原實際 town frame；無城鎮 FDTXT `0x19a` 記錄提示使用原版黑色來源，存檔延至完整關框後。兩者與 `0x31d3c` 最終確認都接上 6＋4＋兩 tick 脈動＋4＋5＋還原。`0x318ad/0x31e80` 選人主畫面、`0x17fc0` 狀態與 `0x1297d` 待機週期亦已接正式路徑。2026-08-26 外部 checksum-valid 晚期槽已由未修改原版普通 LOAD 走過 save-NO、19人選擇、最終確認與戰前劇情至可操作第30戰，形成候選E2。仍缺同狀態重製像素差分、跨畫面初始相位與第29戰勝利寫槽的完整來源鏈；45–55%不可外推到所有整備章節 |
...
> 重製節點間的路由差分；逐像素整備比較、writer 來源及其他章節差分仍未閉合。
