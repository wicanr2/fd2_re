...
| story dialogue | 75–85%（ch00_pre、ch01_pre與ch24_post原生生命週期 E1） | `ch00_pre`十九個caller的97句、`ch01_pre` index0..3的20句及`ch24_post` index6/7的18句，均已直接由raw speaker control與`FFFE/FFFD`建立可編輯pages，不再從現代文字猜框位。序章正常節點以正式故事輸入完成97句後，連續走過`battle_ch01`、戰後、城鎮／整備、第2戰前劇情至`battle_ch02`；raw slot／identity取得speaker owner，四列raw頁面在三列窗口內採10幀捲動近似，見[`ch00-pre-native-dialogue-e1.json`](../data/ui-traces/ch00-pre-native-dialogue-e1.json)。第2戰前冷讀路徑完成20句，見[`ch01-pre-native-dialogue-cold-load-e1.json`](../data/ui-traces/ch01-pre-native-dialogue-cold-load-e1.json)；`ch24_post`的18句則由正式第25戰戰後runner抵達`town_ch26`／祕密商店。三者共用已閉合的五階段opening、原生字形座標、完整頁嘴型及snapshot closing；更新分配與序章特殊角色前景是有界99%近似，不是DOS精確時鐘。同狀態DOS畫面與其餘caller仍缺，未綁定caller仍走既有RGBA路徑；README舊runtime圖不是本切片成果圖 |
