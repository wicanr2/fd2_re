...
| story dialogue | 十七個正式切片（E1；全章仍partial） | 十七個切片均直接由原始說話者控制碼與`FFFE/FFFD`建立可編輯頁面，完整清冊以`58`為準。2026-08-27新增`ch20_post`六組分支呼叫共30句：固定版`FDTXT.DAT`第21項為4,660 bytes，逐句control／operand與編輯行由原始資料等值驗證；成功臂以正式輸入播放26句，依序穿過26-slot layout、ACT63／64、天空之鑰動畫、隊伍同步、`town_ch22`與存讀檔。材料不足四句已有資料與展開回歸，尚缺獨立正式輸入長鏈。`ch22_post`56句則在正式可達分支播放35句並通過86-slot同步、冷讀與整備，見[`ch22-post-native-dialogue-e1.json`](../data/ui-traces/ch22-post-native-dialogue-e1.json)。多字元字形仍以`glyph_pages`保存，見[`native-story-multirune-glyph-token-e1.json`](../data/ui-traces/native-story-multirune-glyph-token-e1.json)。各切片共用已閉合的開框、逐字形、嘴型及快照收框；同狀態DOS畫面與其餘呼叫端仍缺，未綁定呼叫端仍走既有RGBA路徑 |
