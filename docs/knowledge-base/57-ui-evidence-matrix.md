...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合共用 `0x117E7` 的 `0x12C0D==-1→0x16F55` 空游標 owner。direction3→END 現以原版 DATO #75、FDOTHER #5/#2、FDTXT `0x1A3/0x1A4/0x19C` 跑6＋4展開、YES／NO、4＋5收合與來源復原；只有YES在`0xC8` ms近似後進`0x1A30B`，缺任一資產即在關閉命令框前拒絕。同一存檔的原版一般玩家與重製決定性擷取已證實肖像／問句子區域精確重合，並修正`0x9017`右端錨點與三畫面別名缺陷；[比較證據](../data/ui-traces/native-end-turn-confirmation-original-vs-remake-e1.json)保留全幀1692差異像素及其動畫相位限制。command 13–16 的玩家與敵方 mode 11 已接完整 E1：`0x21EB1` 16張 FDOTHER #3 LUT、FDOTHER #6七張、五組 snapshot→`0xC0` mask、MP／HP transaction後 steady redraw、FDOTHER #5四欄數字22張與500 ms尾停；AI 只沿 raw selector target array，缺 #3/#5/#6、baseline或record時零 mutation。 | partial（END原版 E2＋重製同狀態 E1；未全幀逐像素） | END接受／取消／收合逐幀E2與精確tick／音訊、command 13–16 同狀態逐幀／逐音訊 E2、其餘三格 owner、其他 effect presentation與缺archive UX |
