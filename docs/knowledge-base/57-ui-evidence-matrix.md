...
| story dialogue | 75–85%（ch00_pre／post、ch01_pre與ch24_post原生生命週期 E1） | `ch00_pre`97句、`ch00_post`13句、`ch01_pre`20句及`ch24_post`18句，均已直接由raw speaker control與`FFFE/FFFD`建立可編輯pages。序章正式輸入完成97句後進第1戰；勝利後的13句以繼承的戰況視圖完成多頁、四列窗口與游標收框尾段，才執行隊伍同步，再連續通過城鎮／整備冷讀、第2戰前20句至`battle_ch02`，見[`ch00-post-native-dialogue-e1.json`](../data/ui-traces/ch00-post-native-dialogue-e1.json)。`ch24_post`的18句則由正式第25戰戰後runner抵達`town_ch26`／祕密商店。四者共用已閉合的五階段opening、原生字形座標、完整頁嘴型及snapshot closing；更新分配與序章特殊角色前景是有界99%近似，不是DOS精確時鐘。同狀態DOS畫面與其餘caller仍缺，未綁定caller仍走既有RGBA路徑；README舊runtime圖不是本切片成果圖 |
