...
| story dialogue | 五個正式切片（E1；全章仍partial） | `ch00_pre`97句、`ch00_post`13句、`ch01_pre`20句、`ch24_post`18句及raw `ch22_post`56句，均已直接由raw speaker control與`FFFE/FFFD`建立可編輯pages。第23戰後十個caller的56句由raw equality完整驗證；單次正式可達分支依原始互斥條件播放35句，再通過隊伍同步、存檔、全新`Game`冷讀與15人整備至`story_ch24`，見[`ch22-post-native-dialogue-e1.json`](../data/ui-traces/ch22-post-native-dialogue-e1.json)。多rune glyph不再以Unicode長度誤判：`FDTXT_029 #12`的13個raw格會形成16個rune，現在以`glyph_pages`逐token前進16px，見[`native-story-multirune-glyph-token-e1.json`](../data/ui-traces/native-story-multirune-glyph-token-e1.json)。五者共用已閉合的opening、逐glyph、嘴型及snapshot closing；同狀態DOS畫面與其餘caller仍缺，未綁定caller仍走既有RGBA路徑 |
