...
| full-screen battle presentation | 40–50% | FIGANI/AFM、部分 status frame與局部 pixel-equal slices可重播；2026-08-10 戰鬥狀態欄姓名已由 TTF fallback 改接 FDOTHER#4 16×16 glyph，固定命中 fixture 已把 HP 顯示改為 impact 邊界立即提交 post-hit 值、守方剪影 E1 色值對齊原版 RGB `(190,0,0)`，守方待機幀也改消費 descriptor `+6` 延遲而不再固定 `(prog/6)`；正規化對照圖 [`battle-impact-compare-20260810.png`](../figures/battle-impact-compare-20260810.png) 仍有 3933 個差異像素。command29玩家多目標indexed owner已接。ID33／34／35正式command grid與state transaction亦已接，但`0x27FC9` indexed presentation仍缺，不能把綠色交易測試當成畫面完成。28／31無已證實正常取得來源；敵方command29、ID32、其他可達command/spell/item完整presentation、精確音效與palette/timing sequence、一般玩家同狀態DOSBox E2仍缺 |
