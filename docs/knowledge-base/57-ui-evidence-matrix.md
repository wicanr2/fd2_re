...
| story dialogue | 65–75%（ch01_pre與ch24_post原生生命週期 E1） | `ch24_post` index6/7 的18句及`ch01_pre` index0..3的20句，均已直接由raw speaker control與`FFFE/FFFD`建立可編輯pages，不再從現代文字猜框位。第2戰前正常節點現可在`preparation_ch02`存檔，以全新`Game`冷讀後用正式整備／故事輸入完成20句開框、逐字、換頁、嘴型等待與收框，再物化`battle_ch02`；上／下框為12／8句，見[`ch01-pre-native-dialogue-cold-load-e1.json`](../data/ui-traces/ch01-pre-native-dialogue-cold-load-e1.json)。`ch24_post`的18句仍由正式第25戰戰後runner抵達`town_ch26`／祕密商店。兩者共用已閉合的五階段opening、原生字形座標、完整頁嘴型及snapshot closing；更新分配是可重現近似，不是DOS精確時鐘。同狀態DOS畫面與其餘caller仍缺，未綁定caller仍走既有RGBA路徑；README舊runtime圖不是本切片成果圖 |
