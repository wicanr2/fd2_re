...
| ending | 20–30% | prefix已跑到 `0x2c548`，portrait compositor已閉合一段；campaign generic 結局頁現在會在沒有已證實曲目時停止前一曲，但 party montage、確切終局音訊與 native terminal route仍未完成 |

2026-08-10 的音訊邊界：戰鬥節點使用原版 `0x51e63` 章節曲表，城鎮／商店節點使用
已證實的 `FDMUS_010`；這些是資料回歸，不代表每章一般玩家 E2。`ending` 沒有猜測
曲目，空白 BGM 只呼叫已證實的 `play_bgm(-1)` 停曲；`0x2BCE5` 的 indexed
結局播放器仍只在 `FD2_ENDING_PREFIX` 預覽路徑執行，未接正式 campaign。
