...
| weapon/item/secret shop | ch02 examined slice 92–95%；全章 partial | 69個shop節點已用variant1/3/5啟用indexed owner，但DOSBox E2只覆蓋ch02若干狀態。購買主選單／清單／Yes-No／不足金與收件者selection0↔1、賣出名冊／物品／Yes-No／成功／加款／返回已有多組整幀AE=0。service2名冊／面板與service3五個穩定子面板另有route-patched partial E2，內容與幾何一致但動畫相位未同步。service2裝備現先在私有 unit 完成 raw transaction、重算及完整 panel 重建，最後才一次發布；刻意移除 palette 的深層失敗回歸證實 roster、能力、selection與既有 panel 全部不變，達`RUNTIME-E1`。service3目的名冊取消已接正式五幀收合→來源提示六幀展開 owner，取消保持角色與金幣原子不變；取消後 self-transfer 仍由 raw remove→append／重算 owner 消費，亦達production-input `RUNTIME-E1`。六名synthetic typed party另由正式menu→purchase→Yes consumer進三列裝備收件者，驗證selection3時start0→1、horizontal no-op、滿欄／無合適角色零交易及正常裝備成功／扣款；每個opening／closing／restore與timeline都經Draw acknowledgment。測試未注入OS鍵盤，也未從完整campaign抵達商店，不能替代原版四人以上存檔或玩家E2。這些E2仍依screenshot-only LOADCH bootstrap；正常JOIN→LOADCH roster另有runtime regression。仍缺四人以上recipient scroll、no-recipient/full、service2動畫相位與原版mutation／restore畫面、service3 empty／full，以及self／destination-cancel與其他章節的原版同狀態E2；92–95%不可外推成69店全覆蓋率 |
...
| save/load | 45–55% | 四槽 input、native save envelope、原版 indexed loadslots 與 chapter-slot→typed party→town/preparation restore owner 已接；標題確認 owner 又以 checksum-valid 合成槽完整還原 town、party、gold、chapter、HUD gate並清除舊 battle state，竄改 envelope 原子留在選槽。空槽及修改存檔 chapter1 有效槽畫面均與 DOSBox 全幀 RGB 相同。一般玩家有效槽 E2、CONTINUE current battle、delete/overwrite 仍缺 |
| ending | 55–65%（E1；來源約束終局） | prefix 已跑到 `0x2c548`；正式 `battle_ch30→ending` 現以 persistent JOIN roster 的 raw `+6/+7/+8/+0x20` 同步最後隊伍並執行原資源角色蒙太奇。缺戰場、零筆身分符合、缺任一原始資產或 raw provenance 時不發布部分終局，而是整批回到可編輯結語。其後 `MontageTailPlayer` 預檢 20 組共80個 TAI／BG／FIGANI selector；實檔全部 header byte1=0，因此正式 E1 現逐 raw `+6` inner present、`+7 bit0` 層序、base scheduler、`+4` 位移與 palette33 override執行兩次交叉配對，第二次在最後 effect frame 結束；再依 caller 的20／78 ticks疊 FDOTHER #58，完成後保持 #59。[20 組總覽](../figures/ending-tail-20-segments-approximate-remake-e1.png)仍只是 E1，因為 `0x1088d(0x1e)` 的31-record baseline 穿過 `0x2c548` 的位元連續性、`0x2939D` 3% RNG重播、聲音 owner、原版輸入時序與一般玩家路徑尚未閉合。外部片尾錄影把 #59 對應為 `THE END` 是**強推論**，見 [`ch30-ending-youtube-visual-side-evidence.json`](../data/ui-traces/ch30-ending-youtube-visual-side-evidence.json)，不是一般玩家 E2。Enter／空白鍵開啟、Enter／空白鍵／Esc 關閉的角色回顧循環是重製版延伸；一般玩家 E2 仍未完成 |
...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合共用 `0x117E7` 的 `0x12C0D==-1→0x16F55` 空游標 owner。direction3→END 現以原版 DATO #75、FDOTHER #5/#2、FDTXT `0x1A3/0x1A4/0x19C` 跑6＋4展開、YES／NO、4＋5收合與來源復原；接受／取消回覆已逐字形發布，完整句後才等待十二個60 Hz畫格，只有YES才進`0x1A30B`；缺任一資產即在關閉命令框前拒絕。同一存檔的原版一般玩家與重製決定性擷取已證實肖像／文字子區域精確重合，並修正`0x9017`右端錨點與三畫面別名缺陷；[問句比較](../data/ui-traces/native-end-turn-confirmation-original-vs-remake-e1.json)保留全幀1692差異像素及其動畫相位限制，[逐字回覆配對](../data/ui-traces/native-end-turn-response-progressive-original-vs-remake-e1.json)則以普通鍵盤正式路徑固定兩條分支的字數遞增與結果邊界。command 13–16 的玩家與敵方 mode 11 已接完整 E1：`0x21EB1` 16張 FDOTHER #3 LUT、FDOTHER #6七張、五組 snapshot→`0xC0` mask、MP／HP transaction後 steady redraw、FDOTHER #5四欄數字22張與500 ms尾停；AI 只沿 raw selector target array，缺 #3/#5/#6、baseline或record時零 mutation。command 17–22 的玩家grid已接E1；玩家由`0x1D6C8`證據播放#88 sub0及八個DAC phases，最後一幀前不發布交易；17–19敵方mode 11則直接走raw selector transaction。20／21清`+0x25/+0x26`與record10 restore、22的class／RNG gate與`+0x27` application均先完整preflight。command24的selector32玩家路徑現消費FIGANI資源98：frame4繪製後發布MP／#53 sub3，frame10繪製後發布完整傷害／sub2，尾幀才完成行動；中間依raw terrain control選BG並播放`0x29C90`兩段各10次的640-stride viewport滑動。actor／target base現使用原生BG、TAI、`0x2B659` frame0..8、扣MP後source snapshot與`0x2A289→0x18C6D`狀態欄；轉場後以`0x2B9A1`契約把target idle重設frame0／repeat0。`0x29164`九段雙分支角色／TAI滑入與DAC減算已接；精確音訊與一般玩家E2仍缺，故只列`RUNTIME-E1 partial`。 | partial（END原版 E2＋重製同狀態 E1；未全幀逐像素） | END收合逐幀同相位E2與精確tick／音訊、command 13–24同狀態逐幀／逐音訊E2、17–22 phase-expiry／status UI、command24精確音訊與一般玩家「轉職→Lv4學會→施放」連續路徑、其餘三格 owner、其他 effect presentation與缺archive UX |
| UI-04 target/range | `0x1cff0` + `0x149f8` 證實 command record `+3/+4/+6` 參與 target-candidate geometry；`0x1bbdc` item case 0 的 two-stage targets、observed type5–24 effect dispatch 已閉合。item entry materialize `row[+0x12]+2`；first selector return後grid reset且selector回1。type23 destination把literal target code6傳給`0x115b6`，不是global selector6；兩層取消都回item panel。remake已接tracked transaction、occupancy/class/race/29×20 cost/terrain gate。2026-08-22 同一未修改 `FD2.SAV` 已由標題 CONTINUE 正常操作至悠妮 command 0：原版四相位擷取為窄 `PLAYER-E2`，重製普通 X11 路徑的sidecar明列 `native_command_targeting=true`／ID0／游標(10,15)，為 `RUNTIME-E1`；[四相位圖](../figures/native-command0-target-original-phase-sequence.png)、[原版／重製／差異圖](../figures/native-command0-target-original-vs-remake-e1.png)與[擷取紀錄](../data/ui-traces/native-command0-target-original-vs-remake-e1.json)已固定動態LUT與modal presentation。兩側時鐘相位未同步，不列逐像素一致。command 0 的正式 Game confirm 數值交易另由回歸證實游標 gate→MP／HP／acted→target field／range／modal 清理，列 `RUNTIME-E1 numeric`。`0x26152` 已閉合為 28 幀／7 元素錯開排程，並將 FDOTHER #18／#20 實檔簽章、#82/sub1 子資源 gate、back→target→front indexed primitive 與七段 HP plan 實作為 `DATA-READY`；正式 `Game` 尚缺 generic actor／target battle scene owner，故 indexed 演出、音訊與結果 E2 仍缺。物品第一階段 raw target field 與 selector 1..5 已由正式索引組合器消費；缺原生素材時不再顯示無證據的綠／青／橘半透明後備層。另以同一未修改存檔正常選到索爾草藥：四名我方皆滿 HP 時，Return 後 4.3 秒仍保留物品面板、未進 target modal，形成 type5 無可回復候選的窄 `PLAYER-E2`；[三狀態圖](../figures/native-item-herb-fullhp-original-e2.png)與[擷取紀錄](../data/ui-traces/native-item-herb-fullhp-original-e2.json)已固定 | partial | 受傷角色的 type5 目標 modal、native argument↔weapon min/max mapping、AOE/LOS、不可用目標灰化、command0 generic battle-scene owner、其餘 indexed command／item effect presentation、物品取消鍵原版語意、其他command同狀態比較；global selector6的production owner仍待 |
