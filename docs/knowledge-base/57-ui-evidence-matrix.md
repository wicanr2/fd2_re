...
| story dialogue | 五個正式切片（E1；全章仍partial） | `ch00_pre`97句、`ch00_post`13句、`ch01_pre`20句、`ch24_post`18句及raw `ch22_post`56句，均已直接由raw speaker control與`FFFE/FFFD`建立可編輯pages。序章正式輸入完成97句後進第1戰；勝利後的13句以繼承戰況視圖完成多頁、四列窗口與游標收框尾段，才同步隊伍，再連續通過城鎮／整備冷讀、第2戰前20句至`battle_ch02`。第23戰後十個caller的56句由raw equality完整驗證；單次正式可達分支依原始互斥條件播放35句，再通過隊伍同步、存檔、全新`Game`冷讀與15人整備至`story_ch24`，見[`ch22-post-native-dialogue-e1.json`](../data/ui-traces/ch22-post-native-dialogue-e1.json)。五者共用已閉合的五階段opening、原生字形座標、完整頁嘴型及snapshot closing；更新分配與序章特殊角色前景是有界99%近似，不是DOS精確時鐘。同狀態DOS畫面與其餘caller仍缺，未綁定caller仍走既有RGBA路徑；README舊runtime圖不是本切片成果圖 |
