...
| story dialogue | 30–40% | 原版 oracle 固定左下 80×80 portrait、native frame/text/page marker；`ch24_post` index6/7 的18句已依 raw `FFED/FFEF/FFEE` 保存精確上／下框順序，並由正式第25戰戰後 runner 送入一般輸入生命週期，達窄 `RUNTIME-E1`。目前一般 runtime 仍有 RGBA/font/layout path，其他 caller 的 upper/right anchor、FFxx、scroll/clipping 未逐類驗收。README 的 `dialogue.png` 是文字解碼圖，不是 remake 對話 runtime screenshot |
