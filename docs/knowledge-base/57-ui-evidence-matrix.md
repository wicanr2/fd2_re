...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。chapter0 current-runtime 的空游標 Down→END→YES 已沿未修改原版 E2 輸入接成重製端 E1：四幀關閉後才確認，YES 進敵方回合並返回玩家回合；其餘三格仍失敗即關閉。`0x19953` 是另一 selector | partial | END 確認提示的原版 indexed renderer／同狀態重製畫面配對、其餘三格 owner、indexed effect presentation與缺archive UX |
