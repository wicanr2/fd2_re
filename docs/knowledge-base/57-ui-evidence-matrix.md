...
| weapon/item/secret shop | ch02 examined slice 92–95%；全章 partial | 69個shop節點已用variant1/3/5啟用indexed owner，但DOSBox E2只覆蓋ch02：三種variant service menu、variant5 wrap/return、weapon purchase-list四個selection、Yes/No、gold0不足金及gold1000裝備收件者selection0/cycle1全幀相同。selection0按Down到selection1亦以exact-pixel同步人物動畫相位後，remake三cycle各取得不遮罩的整幀AE=0；Down/Up與horizontal no-op input已觀測。賣出角色名冊selection0↔1、索爾物品selection0、短劍Yes／No、五個成功原子影格、向上金幣滾動的11／36元畫面及返回名冊cycle0／1，均由正常商店輸入取得route-patched原版／正式重製整幀AE=0。`0x2D3FF`向上排程已由IDA固定並接正式擁有者；背包移除延至滾動完成。購買成功的25個原子樣本與`0x2D516`扣款16個原子樣本也各有整幀AE=0。recipient／sell E2依screenshot-only LOADCH typed-party bootstrap；正常JOIN→LOADCH roster另有runtime regression，但尚無完整campaign/save input E2。仍缺四人以上recipient scroll、no-recipient/full、equip/transfer child panel與其他章節E2；成功演出的原版精確停留時間、未修改戰役與原版存檔亦未由本切片證明；92–95%不可外推成69店全覆蓋率 |
...
| UI-09 shop | purchase、sell、standalone equip與transfer均有original-resource regression及production owner；strict adapters在raw projection不完整時fail-closed。secret gate保存23筆normal selection與Shift/Ctrl/Alt-F1..F10 BIOS scan；chord只揭露selection5，後續confirm才進variant5。ch02 variant1/3/5 service0 selected phase、variant5四service/wrap/return、purchase list／Yes-No／不足金／收件者，以及賣出名冊／物品／Yes-No均有同狀態整幀AE=0。賣出短劍後的五個成功原子影格、`0x2D3FF`向上金幣滾動11／36元與返回索爾名冊cycle0／1又新增九組AE=0；正式擁有者在成功演出後發布新金額，滾動完成後才發布背包移除。這些E2使用screenshot-only LOADCH typed-party bootstrap，不是完整campaign/native save E2。購買25個成功原子樣本與`0x2D516`扣款16個樣本亦有AE=0。正式四種mutation可經`leaveShop→town→JSON save/load`保存金幣、背包、裝備、能力與隊伍拓撲；第25戰後`town_ch26`祕密商店E1也已接通。 | partial（E1 + ch02 多個商店狀態與賣出成功／加款／返回 route-patched E2） | ch26與ch02仍缺未修改原版完整玩家路徑；recipient scroll、no-recipient/full、equip/transfer child panel E2；其他章節route/state、原版存檔與成功演出的精確停留時間 |
