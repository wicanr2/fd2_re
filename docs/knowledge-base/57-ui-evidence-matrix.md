...
| story dialogue | 40–50%（ch24_post stable-page E1） | `ch24_post` index6/7 的18句已由 raw `FFED/FFEF/FFEE` 固定上／下框，再逐字對照 `FFFE/FFFD` 投影成可編輯 pages；正式第25戰戰後 runner 現使用原版 FDOTHER #5 `(5,2/112)` 框格、DATO portrait、FDOTHER #4字模及 `(15,9)/(95,119)` 文字座標，完整預建後才發布 indexed 穩定頁，並走到 `town_ch26`／祕密商店，達窄 `RUNTIME-E1`。逐字發布、嘴型、開／關框中間幀、同狀態DOS畫面與其他 caller 仍缺；其他 caller 仍走既有 RGBA路徑。README 的舊 runtime 圖不是本切片成果圖 |
