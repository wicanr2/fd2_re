...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合 `0x16F55` direction3→END、FDTXT `0x1A3/0x1A4`、YES、`0xC8` ms 與 `0x1A30B`；chapter0 current-runtime 正式 E1 以十二個60 Hz幀近似延遲後進敵方回合。command 13 的玩家 cursor→MP／HP→action→range reset 回歸已接，原版 `0x21AD9→0x21B18` indexed 演出 owner 已定位但未移植。 | partial | END／command13 原版 indexed renderer與同狀態重製畫面配對、其餘三格 owner、其他 indexed effect presentation與缺archive UX |
