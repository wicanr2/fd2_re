...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；tracked item Enter transaction已接。IDA 9.4／Capstone 已直接閉合共用 `0x117E7` 的 `0x12C0D==-1→0x16F55` 空游標 owner。direction2→`0x1728C` 設定面板現使用原始 cells 54／57、60／63、66／69、72／75，四次opening／closing present後進出；音樂、音效、完整演出gate、HUD與原生／JSON存檔均有正式consumer，替代的 abbreviated battle presentation尚未閉合時保持失敗即關閉。direction3→END 現以原版 DATO #75、FDOTHER #5/#2、FDTXT `0x1A3/0x1A4/0x19C` 跑6＋4展開、YES／NO、4＋5收合與來源復原；接受／取消回覆已逐字形發布，完整句後才等待十二個60 Hz畫格，只有YES才進`0x1A30B`；缺任一資產即在關閉命令框前拒絕。同一存檔的原版一般玩家與重製決定性擷取已證實肖像／文字子區域精確重合，並修正`0x9017`右端錨點與三畫面別名缺陷；[問句比較](../data/ui-traces/native-end-turn-confirmation-original-vs-remake-e1.json)保留全幀1692差異像素及其動畫相位限制，[逐字回覆配對](../data/ui-traces/native-end-turn-response-progressive-original-vs-remake-e1.json)則以普通鍵盤正式路徑固定兩條分支的字數遞增與結果邊界。command 13–16 的玩家與敵方 mode 11 已接完整 E1；command 17–22 玩家grid已接E1；command24 selector32玩家路徑已接來源約束 E1。 | partial（設定與END為重製E1；END另有原版窄E2，未全幀逐像素） | 設定面板同狀態逐幀／逐音訊E2、END收合逐幀同相位E2與精確tick／音訊、selector0巢狀存讀檔／離場、selector1全軍行軍、command 13–24同狀態逐幀／逐音訊E2、17–22 phase-expiry／status UI、其他 effect presentation與缺archive UX |
