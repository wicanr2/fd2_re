...
## 全螢幕戰鬥演出 → 完整反組譯見 [doc 35](35-battle-animation-rendering.md)

原版攻擊是全螢幕戰鬥演出(320×200,非地圖小格)。**繪圖機制已完整反組譯**(攻守雙圖演出主函式 0x28a6c、blit 0x4e63d、狀態欄 0x29164、階段 [0x540ff]、戰場→BG 索引表 0x52363),見 doc 35。
- **關鍵**:blit **無 runtime 縮放**——守方小/攻方大是 **FIGANI 美術本身畫成不同尺寸**(景深燒進素材),不是 scale。共用螢幕錨點 (164,157),figure X = `word[unit+0x40]`,每幀內嵌 (dx,dy)。
- 閃紅/HP抽乾 = **VGA DAC 色盤操作**(0x11d40 埠 0x3c8/0x3c9),非重畫像素。
