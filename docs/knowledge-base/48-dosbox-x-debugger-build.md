...
## 6. §5 懸案已解(序章 context 的執行期位址 = `0x207718`)
...
3. ~~海島幕(id 0~8)的容器載入時機另外驗證~~：已於 2026-07-15 解決，見 §6.1。

### 6.1 2026-07-15 更正：getter 的 disp32 是 context window，不是全程固定 base

上述 `0x207718` 只適用於序章 context，不能當成整個程式固定不變的 acting table base。以 normal core
在真正的 ACT entry `0158:1C966A` 精確中斷後，map0 的 getter runtime code 為：

```asm
002047F8 ...
00204801 8B80D8772000  mov eax,[eax+002077D8]
```

亦即 LOADCH0 後 loader 把同一條 getter 指令的 disp32 從 `0x207718` 改成 `0x2077d8`，剛好前進
`0xc0 = 48 * 4` bytes；map0 的 local ACT 0 因而對到該 context 的第一筆，而不是去讀序章視窗的
global 0。這解釋了先前為何在 `0x207718` 查 map0 ID0/1/2 只會看到「無效／殘留」內容。

靜態交叉驗證已定位 `FD2.EXE file+0x565d8` 的 106×u32 offset directory，offset 以
`file+0x53e00` 為資料起點；已知 ACT 0x50 bytes 也逐 byte 命中 EXE。另以 `strace` 包住原版執行：
序章會讀 `FDOTHER.DAT`、`ANI.DAT`，但 map31→map0 的 LOADCH0 只新讀 FDTXT/FDFIELD/FDSHAP/FDICON，
沒有新讀 acting DAT。故海島低 ID 不是外部檔案剛載入，而是 **EXE 內同一資源目錄經 context relocation
重新開窗**。map0 ACT0/1/2/5 的 runtime bytes、slot 身分和最終座標均已 dump 並接入 remake；詳見
doc50 §1.2、doc53 §D。
