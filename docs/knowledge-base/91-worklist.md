...
| A4 | 現代美術主題 | `PROTOTYPE-SET／DATA-CONTRACT-READY／RUNTIME-E1-PARTIAL`：28 位身分已確認角色已有獨立閉嘴頭像母稿／80×80候選，第一關戰場與戰鬥HUD概念稿亦已產生；受版控`themes/modern/catalog.json`保存穩定asset ID、來源、尺寸與SHA-256。28 位speaker候選已由通用`FD2_THEME`正式對話consumer載入；身分／speaker／雜湊／尺寸／不透明契約任一不符即原子拒絕。約拿已依`DATO_021_m0`與聖者職業資料撤換錯誤尖帽巫師稿。FDICON selector 0–21、68、76、77、78、80、82、83、85、86、88、90、91現各有12張`24×24`二值alpha候選與逐格雜湊；三十四組由正式 loader 原子預檢408幀後才取代正規化sprite。2026-09-05 新增第一關 `384×432`、288 格現代圖集：逐格保留 `24×24` 索引、地形／事件／碰撞座標，只從概念稿取得色彩與平滑手繪方向；正式 loader 驗證 map ID、幾何與雜湊後取代 map0，並禁止原版完整 indexed frame 回頭覆蓋現代圖層。規格見`61-modern-theme-runtime-spec.md`。戰鬥 HUD 仍是 concept，其餘 32 張戰場尚未製作 | 以同一圖塊契約逐張建立其餘戰場；再建立 FDOTHER HUD cell 基準與真彩 overlay。抽測第一關人物、游標、選單、前景與相鄰圖塊接縫 |
