...
- RUNTIME-E1：oracle 可以逐幀取畫面了（`FD2_ORACLE_FRAMES=1`）。原版一幀約
  13 萬指令；開機之後不再讀 `0x3DA`，沒有回掃可掛，用取樣加穩定閘門取回幀
  節奏。用法與收據見 [96](96-parity-toolchain-20260909.md) 的逐幀擷取一節。
- 待查：使用者回報的「快速順移殘影」本身。原版走行期間**不畫游標白框也不畫
  左下 HUD 面板**（15 張逐幀畫面），重製端的游標狀態停在終點；下一步是讓重製
  端吐出同節奏的畫面逐張比對。
