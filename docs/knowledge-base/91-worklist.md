...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 底層 RE 與多個窄 `RUNTIME-E1` 已有；共用 `0x117E7` 空游標 `0x16F55` 的 selector2 設定面板已接四組原始圖格、opening／closing、音樂／音效／演出／HUD consumer與原生／JSON存檔；selector3 END 已接原版6＋4展開、YES／NO、兩段逐字形回應、4＋5收合與來源復原。2026-08-22 同一未修改存檔已由標題正常走到悠妮 command 0 目標模式；command 0、13–27多個正式 indexed owner與原子交易已達窄 `RUNTIME-E1`，未知效果仍失敗即關閉 | 系統 selector0巢狀存讀檔／離場與selector1全軍行軍仍缺；設定的 abbreviated battle presentation owner仍缺，故關閉完整演出後交易前失敗即關閉。command0仍需#82精確取樣率的人耳驗證、同時鐘相位逐幀／逐音訊 E2及其他target contracts；物品仍缺受傷角色原版 target modal、indexed effect、disabled target外觀、原版取消鍵與global selector6 owner；command 17–27仍缺phase-expiry、status UI與一般玩家E2；其他command family仍待 |
