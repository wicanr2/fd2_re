...
- [~] 敵方 AI 回合:flood-fill + 評分選目標(擊殺×2),對齊 `11`(0x15140)：已補地形 AP/DP 與原版 `dmg≤2` 跳過門檻；情境加成、狀態倍率待 RE，且已證實原版 `0x157B5/0x150F1` 有 SpellID 評分／執行、`0x15B77` 依 spell family 分流目標。remake 已建立 `State.SpellBook`/`AIPlan.SpellID`、item raw K4 (`0x11`) command inventory、`AIAvailableSpells` 與 `AISpellCandidates`（攻擊／補血／增益／狀態 family camp filter）；尚未接原版評分與實際施法行動。
