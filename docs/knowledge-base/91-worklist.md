...
| 7 | 三平台打包與推廣片 | 目前 HEAD 已通過 Linux Docker／Xvfb 建置、啟動與城鎮擷圖；Windows x86_64 現由無網路、指定UID/GID、單一一次性容器產生GUI PE與可公開資產ZIP，並通過格式、內容、擁有權與SHA-256驗證，不再由主機操作檔案或事後`chown`。macOS workflow已改產真正`.icns`，但尚無執行紀錄，派送受失效GitHub權杖阻擋。全戰役長程遊玩由使用者人工進行 | 補 Windows 真機啟動／輸入／存檔／音訊抽測；把Linux AppImage的執行期下載／root寫入舊流程改成可重現映像；修復GitHub驗證後執行macOS CI並做相同抽測；三平台成功後才產最終封包與推廣片 |
