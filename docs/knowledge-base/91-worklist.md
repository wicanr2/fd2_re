...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`已輸出palette JSON＋78 action-cell PNG並改接正式loader；136組DATO／544張頭像也已重生為indexed PNG，story dialogue consumer在archive不可讀時通過且resource26四幀逐pixel一致。教會／整備／商店／暫態／group march／終局DATO caller及title、battle presentation等其他archive家族仍未移除 | 先把剩餘DATO production callers全部改接同一loader，再依玩家價值逐家族遷移；每批保留archive不可讀與缺分離資產fail-closed測試，最後production decoder caller歸零 |
