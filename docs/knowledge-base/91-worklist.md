...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0、玩家／敵方ID1／2／3／5／6／7／8／9／10／11／12均已有caller-specific indexed owner；受限class19玩家ID32–35亦達`RUNTIME-E1`。敵方IDs10–12現依`0x15311→funcs_1541F`走既有60幀逐Draw owner，不再同步跳過畫面；ID35則完整消費#68／#94共用段與26→22→27三個專用tail。ID4無已證實正常玩家producer；28／30／31沒有已證實正常玩家取得來源，均不以猜測列阻擋。 | IDs10–12敵方owner與IDs32–35玩家owner已關閉，不重做；下一個玩家價值缺口依`58`轉向其他仍有正常producer的敵方caller-specific演出／commit順序，同狀態逐幀／逐音訊另屬E2 |
