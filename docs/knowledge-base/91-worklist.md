...
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `DATA-READY（5,176筆／語）＋RUNTIME-E1（12-key）`：目前清冊為5,225筆候選／2,489個互異繁中字串；排除內部、開發與未知項後，四個內容目錄各含5,176筆玩家內容，其3,333筆劇情欄位使用canonical穩定ID。簡中為OpenCC初稿、英文為Argos初稿、日文為固定NLLB revision的直譯初稿；四包key／變數／來源一致、英文CJK殘留、日文污染／膨脹／敘事英文殘留閥門已通過。F4、settings、save隔離與12個正式runtime key仍通過；不冒稱其5,176筆已被runtime消費或經人工校譯 | 把其餘劇情、商店、戰鬥HUD、角色／物品／法術名稱改為正式semantic key並接runtime；對英文／日文做人工校譯，再驗證字形、最長字串與對話／商店／HUD排版。4筆身份歧義解決前不合併角色翻譯鍵 |
