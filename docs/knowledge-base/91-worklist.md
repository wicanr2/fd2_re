...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 空游標設定、END、nested資訊／SAVE／LOAD／離場皆達正式E1。LOAD 先在私有 `Game` 完成 current-runtime typed handoff，YES 後才原子替換；SAVE 會保留四槽與未知 bytes，區分非我方增援及具 constructor／identity 證據的新我方 JOIN，並同步 persistent raw。`0x20C6F` 已恢復的物品 type 5–24 數值交易均有正式消費端。`0x1A866` 的 selector 1→0 已依原呼叫順序接在合併的非玩家掃描前，selector 2 接在玩家輸入前；倒數、歸零重算及 raw 同步採單批原子發布 | 下一步補未知法術／複合指令、原版 status icon，以及一般玩家同狀態逐幀／逐音訊 E2；不得把 event61／75 接通外推成90-entry所有 selector 已完成，也不得再重做已接的物品 type 5–24 transaction |
...
3. **戰鬥交易完整性**：物品 type 5–24 已恢復的數值交易不再列缺口；selector 1→0／2 的狀態到期呼叫端與 FDTXT 481..486／DATO indexed 提示已接通，不再列為 owner 缺口。仍須關閉未知指令／法術／複合效果、原版狀態圖示，以及代表性玩家／敵方回合的精確演出順序。
...
- [~] **transient command duration lifecycle**：official IDA/Capstone 固定 `0x1A866` gate 為 `record+6 == selector` 且 `(record+5 & 1)==0`，direct callers 為 `0x1a4d1→push1`、`0x1a55e→push0`、`0x1a797→push2`；另有 `0x1a30b` 內部 `record+6==2` sweep，不能混成同一 phase。通過 raw gate 後，六個 bytes `+0x22..+0x27` 才逐一 decrement，歸零才發 expiry feedback 並 `0x1B750` 重算 derived stats。remake 現以 `NativeRecordByte5/6` 保存 provenance，`TickNativeTransientsRaw` 失敗即關閉；selector 1→0 已按原順序接在合併的非玩家掃描前，selector 2 接在玩家輸入前，歸零時才重算並同步 raw，整批任一失敗不發布。舊 `TickNativeTransients(camp)` 不再猜測映射。FDTXT 481..486／DATO indexed 到期訊息已達 `RUNTIME-E1`；尚缺 status icon、精確 tick／音訊與一般玩家 E2，故不可稱完整介面復刻。
