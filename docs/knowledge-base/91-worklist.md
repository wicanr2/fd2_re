...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT、字型、共用道具panel、FDICON、FDSHAP與ANI正式玩家consumer已遷移；第23戰FDFIELD#69與終局#90..92均可逐byte重建。終局、商店、城鎮及標題FDOTHER正式consumer均已改用分離pack；標題不再讀舊`assets/title/*.png`，標題／第20戰／結局也不再讀`ANI.DAT`。2026-08-29再將18條戰鬥、地圖、LOAD與class／church的FDOTHER#0 palette owner統一改讀分離JSON，正式`ReadResource(...,0)`歸零。FDOTHER sound與其他UI仍未全數遷移 | 下一批處理nested sound catalog與其餘UI家族；保留archive不可讀與缺分離資產失敗即關閉測試 |
