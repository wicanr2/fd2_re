...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 空游標設定、END、nested資訊／SAVE／LOAD／離場皆達正式E1。LOAD 先在私有 `Game` 完成 current-runtime typed handoff，YES 後才原子替換；SAVE 會保留四槽與未知 bytes，區分非我方增援及具 constructor／identity 證據的新我方 JOIN，並同步 persistent raw。`0x20C6F` 已恢復的物品 type 5–24 數值交易均有正式消費端。selector 0 狀態倒數已接在敵方 AI 前的已證實 owner，其他 selector 不猜接 | 下一步補未知法術／複合指令、selector 1／2 phase owner、原版 status icon／indexed presentation，以及一般玩家同狀態逐幀／逐音訊 E2；不得把 event61／75 接通外推成90-entry所有 selector 已完成，也不得再重做已接的物品 type 5–24 transaction |
...
3. **戰鬥交易完整性**：物品 type 5–24 已恢復的數值交易不再列缺口；仍須關閉未知指令／法術／複合效果、selector 1／2 狀態到期 owner、原版狀態介面與代表性玩家／敵方回合的演出順序。
