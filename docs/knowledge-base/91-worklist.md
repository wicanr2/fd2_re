...
| A4 | 現代美術主題 | `PROTOTYPE-SET／DATA-CONTRACT-READY`：索爾頭像、第一關戰場與戰鬥HUD三張style A本機私有稿均已產生；受版控`themes/modern/catalog.json`保存穩定asset ID、來源、尺寸與SHA-256，本機嚴格validator逐檔通過。戰場稿保留第一關隊形與地形關係；HUD稿保留功能選單貼近選取格、狀態板在左下的操作契約。三張仍是concept，不冒稱可直接替換的sprite／tileset／九宮格 | 由使用者檢視整組style A；方向保留後，依catalog逐張切成透明portrait、12幀索爾sprite、map0 tileset及HUD元件，完成縮放／透明邊界／runtime合成驗收後才升為`runtime_ready`並批量生產 |
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `DATA-READY（5,176筆／語）＋RUNTIME-E1-PARTIAL`：35份劇本共1,564句具穩定 `line_id`；正式 story、handler dialog及事件61／75／76均查詢四語內容。非繁中原生 indexed 對話已重建安全分頁並依裁決固定閉嘴。FDTXT清冊現涵蓋200個可顯示物品與15個confirmed-empty ID；四語全部名稱通過67×16矩形，出售／轉移持有物已接正式ID重畫。32名角色目錄供商店動態訊息、兩欄共用名冊與三列裝備比較名冊；100×16及80×16兩種版面全量通過。戰鬥raw `+8`另有94筆名稱及45筆confirmed-empty ID；玩家／敵方共用指令0系27個呼叫與指令24系18個呼叫皆由共用owner在120×16矩形重畫。原生指令格另有35筆四語名稱、ID31空槽及MP欄前動態縮放；七個高頻戰鬥提示的九個正式寫入點已改查官方pack，法術選單／目標／阻擋與玩家／敵方共用結算由七個鍵接通，寶物及擊破獎勵六種分支也已四語化；教會轉交空值／成功與復活成功再接三鍵且mutation前預檢語系。F4會原子同步正式grid與SpellBook相容顯示。英文／日文其餘內容仍是機器初稿，尚無正常玩家截圖 E2 | 商店服務格已證實是純圖示，不列翻譯缺口；教會服務格的亂碼紋理改列UI codec／palette缺陷，須先補原版同狀態證據。轉職成功待職業名稱四語目錄，之後做四語正常玩家畫面及人工語文審校。4筆劇情身份歧義不與0–31隊伍顯示目錄合併；raw戰鬥姓名也不以`native_identity`替代 |
