...
- 重現不出來：第 6 項前半的「戰鬥動畫退化」。三項檢查都沒有找到退化——既有
  命中 fixture 在目前 HEAD 重跑與 2026-08-25 存檔 **AE = 0**；版面與原版擷取
  一致；分離包 409 筆 FIGANI 對 scenario 內 96 個 `battle_fig` 的待機／攻擊
  資源**全部齊備**，完整包組裝也明文複製 `animations/`。要繼續查需要更具體的
  症狀或指定發行包。見 [102](102-battle-result-message-20260909.md)。
