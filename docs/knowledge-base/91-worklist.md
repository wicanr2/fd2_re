...
- [~] **ending compositor asset preflight**：FDOTHER #36 已確認是 408×138 sprite/RLE source（不是 full-screen PNG），ANI #2 已可由 `internal/afm` 解出 26×320×200 frames；下一個實作前置是 player asset loader + 原版 `0x2935b` frame rectangle/transparent/placement adapter，再資料化 palette/branch schedule。未完成前禁止 generic ending fallback。
- [~] **ending #36 frame decoder contract**：`0x2935b` 已確定以 `base+8+frame*4` offset table取 descriptor、讀 `u16 w/h`、payload `+9` transparent RLE blit；`0x2bce5` 的 frame0/9/12..108 與 double-buffer call sequence已可逐段轉錄。尚缺完整 branch/text/ANI schedule與 runtime player-asset bridge。
