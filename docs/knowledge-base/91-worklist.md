...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：既有戰鬥surface／animation／portrait切片不變；FDTXT第一批另把LOAD、職業／教會共用資產與party montage改接分離#0/#31及FDOTHER#4字型。#31逐word、字型1,824 glyph逐bit相同，stale UTF-8/token文件會被拒絕。道具／轉移面板、終局phase0、title、FDFIELD及FDOTHER palette／sound／其他UI仍有archive consumer | 下一批先遷移共用道具／轉移面板與終局phase0的FDTXT／font consumer，再處理FDFIELD及FDOTHER palette／sound／UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
