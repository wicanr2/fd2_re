...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整 manifest 為6,268筆：5,248 exported、1,005 intentionally_raw、15 MIDI blocked。264組 FIGANI 已輸出264份 metadata、2,118張 indexed frame PNG＋2,118張 mask PNG，來源／frame 可回查；raw零遺漏，但TAI／BG／UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；為145個未接受 FIGANI 及其餘 raw逐家族判定非動畫、已由標準輸出涵蓋或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`與全部DATO production consumers已改接分離loader；FIGANI resource4七幀逐欄位／pixel／mask與archive一致。終局、通用攻擊、指令0–3／5–9的角色與目標待機，以及24／29／32–35的FIGANI均已改接；24／29／32有archive不可讀回歸。FIGANI只剩兩個依零header退回的helper，另有16檔直接解碼FDOTHER內嵌動畫；title、FDTXT、BG、TAI、FDFIELD等家族也未全移除 | 先讓 importer替145個未接受FIGANI登記空／非動畫／blocked狀態，才能無歧義遷移兩個header-fallback helper；再把FDOTHER內嵌effect輸出成同契約animation metadata。每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
