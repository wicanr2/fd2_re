...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；穩定標題主選單與320×200原版 oracle 達整幀AE=0。完整開場的舊串列已由 `sub_1F894` canonical證據取代：535列捲動、六個插播點、八段AFM與兩張靜態幕現在由正式runtime交錯播放。2026-08-27再接漢堂發行商畫面及後段兩次`sub_286BD`索引色盤內插；紅幕→真實ANI #1→近白標題已達99%玩家可見門檻，撤回wipe／logozoom舊說。ch02賣出成功／加款／返回已有九組AE=0。正式`church_ch02`主選單也以旁車證明selection0／pulse2／gold1000，與現行原始資源oracle達`AE=0/64000`；舊oracle的320點假差異已勘誤。獨立裝備service2與物品轉移service3也都有正常route-patched子面板證據；service3更已完成索爾短劍→悠妮、返回loop、雙方交易後物品清單及跨`town_ch02`重製JSON冷讀檔。物品清單舊記AE=2已定位為店員背景動畫兩點相位差。service2交易已改為私有 unit→raw裝備／重算→完整panel重建→一次發布，達`RUNTIME-E1`；service3現由正式menu Right×3進入typed input consumer。標題 LOAD 的正式確認 owner、外部晚期有效槽到第30戰、固定初始整備AE0、裝備收件者正式輸入及`ch24_post`原生indexed對話均已接。 | ch24只剩未修改同狀態E2；其他故事caller仍需各自binding。標題只剩精確音訊與原版完整啟動E2，不再追DOS逐週期差異；教會只留DOSBox caller E2，不重做正式menu。商店下一步是service3與service2原版mutation／restore畫面。第29戰已依99%門檻結案，不再阻擋remake。 |
