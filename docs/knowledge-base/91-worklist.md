...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1–8的`funcs_2AC25`線性entry與mode契約已`RE-CLOSED/DATA-READY`。表內raw值需加`0x10000`，早期ID5–8的`0x16xxx`語意已撤回。ID1另已完成#19/#21實檔驗證與mode4→target→mode5純indexed compositor；勘誤：`0x262EF`沒有本地sample marker。正式AI的ID1仍是state-only數值路徑，不能冒稱畫面E1 | 先閉合ID1外層MP／HP／音訊發布owner與正常producer，再接正式presenter；其後依序完成有正常producer的ID2–8。ID9–12與command29仍須先閉合各自路由／owner |
