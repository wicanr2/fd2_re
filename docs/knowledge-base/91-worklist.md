...
| 7 | 三平台打包與推廣片 | 最新`a5bbaf3f`已由鎖定映像重建AppImage與Windows ZIP。AppImage 5,306,872 bytes／SHA-256 `f085dc58...acadfb`，從空工作目錄配唯讀XDG玩家資產在Debian系及Ubuntu 24.04都抵達`town_ch02`，跨映像畫面AE=0。Windows ZIP 4,895,851 bytes／SHA-256 `931397d8...eb286`，含GUI PE與公開資產72項；Wine 9.0實跑已接收城鎮輸入、進商店，並以F5寫出可解析`town_ch02`存檔。兩包均未含原版EXE／DAT／存檔，UID/GID正確。macOS workflow已改產真正`.icns`，但尚無執行紀錄，派送受失效GitHub權杖阻擋。全戰役長程遊玩由使用者人工進行 | 補Windows真機啟動／輸入／存檔／音訊抽測；修復GitHub驗證後執行macOS CI並做相同抽測。Wine與Ubuntu容器驗收不取代真機。三平台成功後再產最終版號封包與推廣片 |
