...
| 7 | 三平台打包與推廣片 | 最新`a5bbaf3f`已由鎖定映像重建AppImage與Windows ZIP。AppImage 5,306,872 bytes／SHA-256 `f085dc58...acadfb`，在Debian系及Ubuntu 24.04抵達`town_ch02`且跨映像AE=0。Windows ZIP 4,895,851 bytes／SHA-256 `931397d8...eb286`；Wine 9.0已接收城鎮輸入、進商店並以F5寫出可解析存檔。macOS run 32997009964亦在真實`macos-14`成功完成雙架構、lipo、bundle、DMG／tar.gz及19,299,516-byte artifact上傳。公開API無憑證不能下載artifact，尚未做實體Mac啟動。兩個Linux產物未含原版資產，UID/GID正確。全戰役長程遊玩由使用者人工進行 | 補Windows與macOS實體機啟動／輸入／存檔／音訊抽測；取得並解包macOS artifact、處理簽章／Gatekeeper；之後產最終版號封包與推廣片。Wine、Ubuntu容器與CI build均不取代真機 |
