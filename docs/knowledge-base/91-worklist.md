...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；教會轉職與商店獨立裝備（standalone-equip）／自身轉移（self-transfer）的正式 mutation→town→重製 JSON 存讀檔均已保存完整 typed／raw roster 與隊伍拓撲，並修正讀檔殘留教會、商店及 item-panel 暫態 | 依 `57` 逐個輸入狀態機取得同狀態原版／重製證據；下一個可實作存檔切片是商店 purchase／sell→town→JSON save/load，視覺則優先補 equip／transfer child panel E2，不用單張 layout 圖代替 |
