...
| A4 | 現代美術主題 | `DATA-CONTRACT-READY／RUNTIME-E1-PARTIAL`：28 位身分已確認角色已有獨立閉嘴頭像母稿／80×80候選；三十四組 FDICON 由正式 loader 原子預檢 408 幀後取代正規化 sprite。2026-09-05 完成 `map0` 至 `map32` 共 33 張現代圖集：各自保留 `24×24` tile ID、地形／事件／碰撞座標，依第一關概念稿及獨立生成的高解析手繪參考統一色彩與材質；直接使用生成 atlas 曾由正式 map0 重組證明格線漂移，故 runtime 只取其風格，結構仍由原圖集逐格控制。96／192／288／384 格四種實際幾何、map ID 與 SHA-256 均由 loader 原子驗證；純黑洞窟／畫布外區域亦保持不變。現代戰鬥狀態面板已由概念稿生成獨立無文字 `149×42` 母材，正式 loader 驗證雜湊、尺寸與完全不透明後，由既有左下 HUD owner 疊加動態姓名、LV、HP、MP、AP、DP、MV；忠實原版面板不變。完整 runtime 截圖仍受本機封裝啟動環境阻擋，不提升玩家畫面證據 | 抽測早／中／晚期地圖的相鄰圖塊接縫、前景遮擋、人物、游標、HUD 與方塊附近選單；解決可重現封裝截圖入口後補正常畫面收據。接著補齊其餘 62 組地圖人物與未登錄頭像 |
