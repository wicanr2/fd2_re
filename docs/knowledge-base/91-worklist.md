...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1–8的`funcs_2AC25`線性entry與mode契約已`RE-CLOSED/DATA-READY`。表內raw值需加`0x10000`，早期ID5–8的`0x16xxx`語意已撤回。八個entry的channel／counter／sample均不同 | 依校正證據建立逐entry typed schedule與共同mode0–8 renderer，再接有正常producer的敵方ID；ID9–12與command29仍須先閉合各自路由／owner |
