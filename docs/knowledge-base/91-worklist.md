...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1的#19/#21每目標31張、八個HP marker、九張boundary與共同tail已由正式owner接入玩家／敵方。ID2的單frame/repeat helper、29張front、每target12張／六HP marker、每boundary九張、10張handler tail、共同tail與#83 sample0..3亦已由同一正式owner接入玩家／敵方。ID6的#32/#33/#87、7張front＋每target12張＋九張boundary＋七張tail同樣完成；三者的MP、HP、`Acted`與RNG皆依Draw發布並可整批回復，達indexed `RUNTIME-E1`。ID5已閉合#24/#25/#86、六通道持續狀態、1／12／9／8排程、九個raw marker／六段HP與indexed compositor，達`RE-CLOSED`／`DATA-READY`，尚未接正式owner | 下一個結果是把ID5接入玩家／敵方正式owner並保留跨target RNG交錯的明示近似；之後依正常producer頻度處理ID4／7／3。ID1／2／6只留逐呼叫音訊與未修改原版同狀態E2，不重做已閉合handler |
...
1. **戰鬥交易完整性**：物品 type 5–24與ID32／33／34／35受限玩家state transaction不再列數值缺口；狀態到期owner與raw indicators已接。command28／31無已證實一般玩家取得來源，不列阻擋。剩餘阻擋是正常敵方回合實際可達command的caller-specific演出／commit順序、其他有producer的未知法術／複合效果、狀態高階名稱，以及代表性玩家／敵方回合的精確演出順序。
...
> **command30敵方候選勘誤（2026-08-23）**：map13 index0雖有bit30，但raw AI
> mode低四位是8；原版`0x13A9A`直接分派`0x1317D`，不進command scorer或
> `0x15311`。它不是正常敵方command30 producer，不列AI阻擋。
