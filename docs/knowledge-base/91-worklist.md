...
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `DATA-READY（5,176筆／語）＋RUNTIME-E1-PARTIAL（12個介面鍵＋story／handler劇情查詢）`：目前清冊為5,225筆候選／2,489個互異繁中字串；四個內容目錄各含5,176筆玩家內容。2026-08-30 runtime 已逐句核對35份 legacy／canonical story，共1,564句具穩定 `line_id`；正式 story 與 handler dialog 已查詢四語內容，F4會原子切換介面包與內容包，下一句使用新語言。缺鍵、身分漂移及 blocked 一律停止，不回退繁中。戰場內嵌事件61／75／76仍直接建立繁中 action；非繁中的原生 indexed 對話也尚未完成分頁／字形重排，刻意維持失敗即關閉。英文／日文仍是機器初稿，不能冒稱完整玩家語系或人工校譯 | 先接通戰場內嵌事件61／75／76的 canonical 查詢，再完成原生 indexed 對話四語安全矩形、分頁與 progressive frame 重建；之後處理商店、戰鬥HUD、角色／物品／法術名稱 semantic key。最後做英文／日文人工校譯及實機排版；4筆身份歧義解決前不合併角色鍵 |
