...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整manifest為7,914筆：6,890 exported、1,005 intentionally_raw、19 blocked。除既有FIGANI、FDOTHER內嵌動畫及BG／TAI外，FDTXT 35筆現為34份lossless typed JSON＋1份zero-byte blocked；FDOTHER#4亦有1,824 glyph二值atlas與metadata。raw零遺漏，但UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；其餘raw逐家族判定已由標準輸出涵蓋、需新typed輸出或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：既有戰鬥surface／animation／portrait切片不變；FDTXT第一批把LOAD、職業／教會共用資產、戰場姓名、party montage與終局phase0改接分離#0/#31及FDOTHER#4字型。#31逐word、字型1,824 glyph逐bit相同，stale UTF-8/token文件會被拒絕。道具／轉移面板、title、FDFIELD及FDOTHER palette／sound／其他UI仍有archive consumer | 下一批遷移共用道具／轉移面板的FDTXT／font consumer，再處理FDFIELD及FDOTHER palette／sound／UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
