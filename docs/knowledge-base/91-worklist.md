...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整manifest為7,877筆：6,854 exported、1,005 intentionally_raw、18 blocked。409筆FIGANI均有resource status；264組FIGANI共2,118幀，另23組FDOTHER內嵌動畫共408幀。BG／TAI各57筆也全數具狀態，#0..55輸出indexed PNG＋mask，兩個0-byte #56維持blocked。raw零遺漏，但UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；其餘raw逐家族判定已由標準輸出涵蓋、需新typed輸出或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`、全部DATO、FIGANI與23組FDOTHER effect已改接分離loader；戰鬥指令0–3／5–9／24／29／32–35及終局20段tail的BG／TAI也只讀分離surface。112筆surface逐pixel／mask／final blit一致，正常指令0在BG／TAI archive不可讀時通過；tail四項測試覆蓋20筆transaction與失敗即關閉。production已無BG／TAI single-frame decoder caller，但party montage仍直接核對TAI#3 raw placeholder；title、FDTXT、FDFIELD及FDOTHER palette／sound／其他UI家族仍未全移除 | 下一批依玩家路徑處理TAI#3 raw placeholder、FDTXT／FDFIELD與FDOTHER palette／sound／UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
