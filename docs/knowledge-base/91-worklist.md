...
| 順序 | 工作 | 現況 | 下一個可驗收結果 |
|---:|---|---|---|
| 0 | 原始碼註解與 Markdown 現況斷言稽核 | `DATA-READY`：本輪再撤回「合成有效槽 fixture 已證明未修改原版四槽 LOAD E2」、「轉職系統尚未實作」與「所有 `0x13FD4` 消費端均未接入」三項過時說法；並修正growth本體殘留註解、Beat型別等同handler一比一閉合、以及把購買success/debit E2誤讀成sell成功E2的範圍混淆。較早`0x9017`／END文字層／終局／FIGANI／第29戰／chapter0-only／Tab／商店收件者勘誤均保留可追溯。這是持續性品質閘門，不是一次掃描即永久完成 | 每個玩家功能關閉時，以程式、測試與 `58` 現況核對完成度、節點、slot、handler、renderer；錯誤現況直接訂正，歷史證據追加勘誤 |
...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；ch02賣出名冊selection0↔1、索爾物品selection0及短劍Yes／No selected現為route-patched正常商店輸入E2；教會轉職與商店四種正式 mutation→town→重製 JSON 存讀檔均已保存金幣、完整 typed／raw roster 與隊伍拓撲 | 依 `57` 逐個輸入狀態機取得同狀態證據；商店下一步為sell成功／返回、recipient scroll、no-recipient/full、equip/transfer child panel，不重做已閉合交易或 JSON persistence |
...
- [~] **UI-VIS-SHOP**：四個callee與secret selection+BIOS-scan gate皆已接production；ch02 variant1/3/5主選單、purchase list／Yes-No／不足金／recipient selection0↔1、success與debit的既有E2維持。賣出正常商店輸入現閉合角色名冊selection0↔1、索爾物品selection0及短劍Yes／No selected，均與production完整320×200 RGB AE=0；證據見[`shop-sell-child-ch02-e2.json`](../data/ui-traces/shop-sell-child-ch02-e2.json)、[`shop-sell-selection-confirm-ch02-e2.json`](../data/ui-traces/shop-sell-selection-confirm-ch02-e2.json)及兩張對照圖。route patch與screenshot-only typed/raw bootstrap限制仍有效。下一gate為sell成功／返回、四人recipient scroll、no-recipient/full、equip/transfer child panel與其他章節。
