...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；故事對話現接十三個切片。新增`ch09_post`35句：60／61兩個frontier均以正式輸入完成DAC淡出、sparse patch、淡入、35句、JOIN11／6、`town_ch11`與存讀檔；第7、8、16、17戰後亦保持各自分支與城鎮邊界。 | 十三個已接故事切片只剩未修改同狀態E2；下一個候選從已有正式路徑且canonical RE充分的未接caller挑選，不重做已接切片；第29戰已結案。 |
