...
| 7 | 三平台打包與推廣片 | 最新`3dd0bd20`已重建Linux AppImage 5,327,352 bytes（SHA-256 `32239daa…091f5`）與Windows ZIP 4,910,912 bytes（`c85e466b…ee11`），兩者均在一次性無網路Docker由空白工作目錄通過正式封包自我檢查；Windows 使用既有Wine 9.0／Xvfb。macOS run 33037065272由原生`macos-14`完成arm64／amd64、`lipo` universal、bundle自我檢查、DMG／tar.gz與artifact。封包只含受版控scenario／story／spells，不含原版EXE／DAT／存檔。尚未做實體Windows／Mac操作；全戰役長程遊玩由使用者人工進行 | 補Windows與macOS實體玩家桌面的視窗／輸入／存檔／音訊抽測；處理簽章／Gatekeeper；之後產最終版號封包與推廣片。Wine、Docker與CI自我檢查均不取代真機 |
