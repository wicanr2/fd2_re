...
- [x] **finale `0x2c548` first party-cycle map**：Docker Capstone 切出三個 native buffers（128000、64000、64000）、TAI#3 platform 與 FDOTHER#56 backdrop；按 `[0x53bfb]` persistent party count、unit stride80／visual group `+7` 載 `FIGANI group*3+1` 與 `group*3`，`0x29164`／`0x2b9a1` compose 後初始跑20×1ms frames。已入 `assets/endings/native_2c548.json`，但 unit choice、portrait/text/input 與完整 battle renderer composition 未解，保持 fail-closed。
