...
- [~] **ch29 post terminal handler**：`0x25870 → 0x1088d` 已 lower 為可編輯 `load_ch_text(ch30.json)`；compiler 與 runtime regression 鎖住 transcript 切換，確保後續 FDTXT_030 對話不是沿用 FDTXT_029。`0x25970 → 0x2bce5` 返回後是 self-loop，這是 internal ch29／map29 最終戰的終局路徑，**不是** map28 戰後可接 `preparation_ch30` 的 handler。現行 final battle→generic ending 暫略過它；待 layout roster identity、動態 pan、0x24618 indexed adapter、`0x2bce5` renderer 完成後，以 terminal node 接入。
- [~] **ch29 post layout frontier**：`0x257b4 → 0x233c6` 的 20 個 native slot X/Y/pose 與 camera `(16,18)` 已由 Docker RE 完整讀出；但 remake 尚未證明 postbattle 的 20-slot runtime roster identity，`campaign_full` 也尚未進入 native post handler。座標不能先接到 `handlerUnitAt` 猜角色，故維持 fail-closed，下一步是 roster frontier/slot identity evidence。
...
- [~] **native `0x22253` renderer adapter**：已釘死 `0x22547→0x22046` indexed off-screen blit 呼叫鏈與 FDOTHER #81 sub1 資源（9782 bytes）；仍缺 indexed buffer／resource decoder／PNG or Ebiten adapter，`unit_present` 暫維持 fail-closed。
- [~] **chapter ending renderer (`0x2bce5`)**：已釘死 FDOTHER #36、320×200 雙 buffer、palette 0→63/4ms、2000ms hold、chapter26/29 分支文字與 fade-out；仍缺 ANI/FDOTHER compositing adapter，禁止把它吞成 generic ending。
