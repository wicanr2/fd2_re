...
- [~] 敵方 AI 回合:flood-fill + 評分選目標(擊殺×2),對齊 `11`(0x15140)：已補地形 AP/DP 與原版 `dmg≤2` 跳過門檻；情境加成、狀態倍率待 RE，且已證實原版 `0x157B5/0x150F1` 有 SpellID 評分／執行、`0x15B77` 依 spell family 分流目標。remake 已建立 `State.SpellBook`/`AIPlan.SpellID` plumbing，但 command inventory、施法目標與優先級尚待接入
...
- [~] 戰後 town/整備流程：preparation 與 church selector UI 已接；`docs/figures/church-selector.png` 為 xvfb 實機畫面。revive 與 class-change branch/mutation 已可保存 roster/gold；尚待完整 xvfb 轉職操作與原版 DX/race/multiplier 欄位接線。
...
- [~] class-change synthesis：直接追 `0x1b750/0x1145a` 已確認 raw `+0x3e` 同時作 HIT/EV base，裝備 item `+3/+7` 分別累加；class byte `+0x24` 非零時 DX/HIT base 另加 15，AP/DP multiplier byte `+0x22/+0x23` 為 1.15。`RecomputeAfterClassChange` 現在會先扣除既有有效裝備貢獻再重建 raw base，避免轉職後 AP/DP/MV 把裝備重算兩次；`+0x24` race bonus 與 AP/DP multiplier 尚待 Unit 欄位資料化。
