...
| 4a | 玩家／敵方指令 `28／29／31` 原版演出 | `RE-CLOSED`（caller分歧）／`RUNTIME-E1`：28已校正為`roll/8`；29玩家正式confirm現以selector34／resource104播放九段前導、一次actor phase，再為每個final target各播20張轉場、重設idle並在marker發布HP；全部完成才`Acted`／獎勵，任一晚期錯誤整批回復。28／31及敵方仍只有數值交易 | 先閉合28／31一般玩家actor selector，再分別接演出；command29下一步是敵方owner與未修改一般玩家同狀態逐幀／音訊E2。不得把玩家owner直接外推到敵方caller |
