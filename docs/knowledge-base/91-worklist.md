...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1–8的`funcs_2AC25`線性entry與mode契約已`RE-CLOSED/DATA-READY`。表內raw值需加`0x10000`，早期ID5–8的`0x16xxx`語意已撤回。ID1已完成#19/#21九張target純compositor，且`0x262EF`沒有本地sample marker；ID2已完成#26/#27單張target compositor與#83 sample邊界，且撤回把ID3三張table誤掛ID2的斷言。正式AI的ID1/2仍是state-only數值路徑，不能冒稱畫面E1 | 先閉合ID1/2外層MP／HP／音訊發布owner與正常producer，再接正式presenter；其後依有正常producer的實際頻度處理ID6／5／4／7／3。ID8現行map無敵方mask，僅新producer證據才提升；ID9–12仍須閉合各自owner |
