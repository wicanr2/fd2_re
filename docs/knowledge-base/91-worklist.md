...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 多項正式E1維持；ID32／33／34／35已有受限class19玩家state owner：selector4／5／6／7／20只作76／52／28／36 MP gate不扣款；32依`0x2111A→0x1C75E`原子發布HP／RNG，33清raw後固定回復，34與35的三段私有stages全成才發布，四者正式grid→confirm已驗證。這不包含`0x27FC9`演出／score／EXP | 下一步選一個正常敵方回合實際可達的state-only command，補caller-specific indexed presentation與commit／rollback；command29敵方owner缺producer／selector／resource證據，維持fail-closed，不得把AI套入玩家owner |
