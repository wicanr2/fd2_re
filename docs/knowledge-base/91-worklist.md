...
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `DATA-READY（5,176筆／語）＋RUNTIME-E1-PARTIAL`：四個內容目錄各含5,176筆玩家內容；35份劇本共1,564句具穩定 `line_id`。正式 story、handler dialog及事件61／75／76均查詢四語內容，F4原子切換內容；缺鍵、漂移及 blocked 一律停止。非繁中原生 indexed 對話已重建安全分頁與 progressive frames，依使用者裁決固定閉嘴。2026-08-31 新增120個穩定 `item.<id>.name`，購買清單會以無原文殘影的原版索引列重畫簡中／日文／英文名稱；全量字形與67×16矩形測試通過，6筆過長機器初稿已有受版控人工短名稱。英文／日文其餘內容仍是機器初稿，尚無正常玩家截圖 E2 | 接續商店固定提示與出售／轉移持有物名稱，再處理戰鬥HUD、角色與法術名稱 semantic key；最後抽測四語正常玩家畫面並人工校譯英文／日文。4筆身份歧義解決前不合併角色鍵 |
