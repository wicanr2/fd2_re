...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT、字型、共用道具panel與FDICON正式玩家consumer已遷移；戰場、整備、職業／教會、城鎮與終局tail baseline均讀完整分離FDICON bank，FDICON production caller歸零。第23戰重載另由`map23/map.json`逐byte重建FDFIELD#69，缺原封存檔仍可原子提交。終局FDFIELD#90..92、FDSHAP、title、FDOTHER sound／其他UI仍未全數遷移 | 下一批分離FDSHAP戰場圖塊／control，再處理終局FDFIELD#90..92與FDOTHER palette／sound／其餘UI家族；保留archive不可讀與缺分離資產失敗即關閉測試 |
