...
> **2026-09-07 v.1.0.2 發行批次：**`v.1.0.0-20260907` 因公開 CI 不可見私人
> 現代逐幀素材而三平台建置失敗，未建立 Release，標籤保留且不移動。修正版唯一
> `v.1.0.1-20260907` 又由更新的 AppImage 工具雜湊與 Windows checkout 換行揭露
> 兩個可重現封裝問題，兩者均未建立 Release。修正後唯一正式版號為
> `v.1.0.2-20260907`；三平台
> 公開封包固定為 Linux AppImage、Windows ZIP、macOS universal ZIP。本機
> `dist-all/v.1.0.2-20260907/full/` 另保存含完整原版資料的三平台完整版，禁止
