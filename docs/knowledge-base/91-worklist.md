...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；教會轉職與商店購買／賣出／獨立裝備（standalone-equip）／自身轉移（self-transfer）的正式 mutation→town→重製 JSON 存讀檔均已保存金幣、完整 typed／raw roster 與隊伍拓撲，並修正讀檔殘留教會、商店及 item-panel 暫態 | 依 `57` 逐個輸入狀態機取得同狀態原版／重製證據；商店下一步轉為 recipient scroll、no-recipient/full、sell/equip/transfer child panel 的正常輸入與同狀態 E2，不再重做已閉合交易或 JSON persistence |
