...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；穩定標題主選單與320×200原版 oracle 達整幀AE=0。完整開場、發行商畫面與兩次索引色盤內插已達99%門檻；ch02城鎮／商店、正式教會、service2／3正常輸入與多組AE0證據維持。故事對話現接`ch00_pre`97句、`ch00_post`13句、`ch01_pre`20句、`ch24_post`18句及raw`ch22_post`十個caller／56句；舊89句是整份FDTXT_023，不是post子集。第23戰後正式可達分支以具型別輸入播放35句，再完成同步、存檔、冷讀與15人整備。 | 五個已接故事切片只剩未修改同狀態E2；下一個未接切片必須重新依完整caller資料、正式玩家路徑與收益排序，不沿用舊89句盤點。標題只剩精確音訊與原版完整啟動E2；教會只留DOSBox caller E2。service2／3依99%門檻不再列runtime阻擋。第29戰已結案，不再重開。 |
