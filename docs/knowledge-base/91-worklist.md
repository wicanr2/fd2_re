...
- [~] 敵方 AI 回合:flood-fill + 評分選目標(擊殺×2),對齊 `11`(0x15140)：已補地形 AP/DP 與原版 `dmg≤2` 跳過門檻；情境加成、狀態倍率待 RE，且已證實原版 `0x157B5/0x150F1` 有 SpellID 評分／執行。remake 已建立 `State.SpellBook`/`AIPlan.SpellID` plumbing，但 command inventory、施法目標與優先級尚待接入
