...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；故事對話現接`ch00_pre`97句、`ch00_post`13句、`ch01_pre`20句、`ch24_post`18句及raw`ch22_post`十個caller／56句；舊89句是整份FDTXT_023，不是post子集。第23戰後正式可達分支以具型別輸入播放35句，再完成同步、存檔、冷讀與15人整備。raw glyph格數已與Unicode rune數分離，後段`ASR-07`等多rune原版glyph可由token層正確進位。 | 五個已接故事切片只剩未修改同狀態E2；下一個候選優先審核已有正式路徑的`ch17_post`四caller／21句，但先核對binding實檔的跨scene context數，不直接採信舊盤點。標題、教會與service2／3只留既有E2限制；第29戰已結案，不再重開。 |
