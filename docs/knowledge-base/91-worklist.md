...
3. **戰鬥交易完整性**：物品 type 5–24 已恢復的數值交易不再列缺口；selector 1→0／2 的狀態到期呼叫端、FDTXT 481..486／DATO indexed 提示與 `sub_17FC0` status colors／entries `0x37..0x39` 圖示已接通，不再列為 owner 缺口。command28／31目前沒有已證實的一般玩家取得來源，亦不列阻擋。仍須關閉實際可達的未知指令／法術／複合效果、狀態高階名稱，以及代表性玩家／敵方回合的精確演出順序。
