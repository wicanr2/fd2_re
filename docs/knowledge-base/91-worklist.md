...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整manifest為8,007筆：6,983 exported、1,005 intentionally_raw、19 blocked。除既有動畫、surface、FDTXT及字型外，FDOTHER#5共用panel新增58張indexed PNG、34張mask與1份metadata；58筆准入entry依三種已證實codec逐pixel／mask一致。raw零遺漏，但其餘UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；其餘raw逐家族判定已由標準輸出涵蓋、需新typed輸出或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT#0..34與FDOTHER#4已分離，LOAD、職業／教會、戰場姓名、終局及共用道具／轉移panel正式consumer均改接typed loader；FDTXT正式玩家archive consumer歸零。FDOTHER#5 panel集合亦由分離pack供應，但同archive其餘80筆entry不可泛化codec；title、FDFIELD、FDOTHER sound／其他UI仍有archive consumer | 下一批處理FDFIELD及FDOTHER palette／sound／其餘UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
