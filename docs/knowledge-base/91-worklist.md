...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`、全部DATO、FIGANI與23組FDOTHER effect已改接分離loader；戰鬥指令0–3／5–9／24／29／32–35、party montage TAI#3 gate及終局20段tail的BG／TAI也只讀分離surface。112筆surface逐pixel／mask／final blit一致，正常指令0與正式campaign montage均在TAI archive不可讀時通過；tail四項測試覆蓋20筆transaction與失敗即關閉。production TAI direct reader及BG／TAI single-frame decoder caller均歸零；title、FDTXT、FDFIELD及FDOTHER palette／sound／其他UI家族仍未全移除 | 下一批依玩家路徑處理FDTXT／FDFIELD與FDOTHER palette／sound／UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
