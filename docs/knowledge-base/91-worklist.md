...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL／LEDGER-DATA-READY`：manifest v2現有39,820筆asset與1,005筆source-resource ledger；900 standardized、11個零長度confirmed-empty、0 blocked、94 unknown。FDSHAP control、終局FDFIELD #91/#92、map23 FDFIELD #69、FDOTHER #1／#6完整bank、標題FDOTHER #77／#78音效、#102短暫調色盤及15首音樂已建立可驗證關聯；可版控摘要綁定完整manifest SHA-256。94筆unknown分布為FDFIELD 79、FDMUS 5、FDOTHER 10，不等同94個decoder缺口 | 將94筆unknown與production consumer及既有runtime標準資料交叉比對；FDOTHER #47／#49尚無可證實正式consumer，維持unknown，不為減數量猜接；改選有玩家consumer的下一筆跨容器缺口 |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT、字型、FDICON、FDSHAP、ANI、FDFIELD主要玩家路徑，以及城鎮、商店、標題、LOAD、整備、教會與戰場初始化均已遷移。19個巢狀音效bank已分離；標題#77選單音與ANI #1的#78 companion均由正式runtime消費，#78在第一幀啟動並於結束／略過停止。BGM只從完整驗證的30份OGG catalog解析，物理攻擊亦由分離FIGANI provider原子預檢 | 以version 2 ledger的94筆unknown交叉核對production caller；已具標準資料但缺manifest bridge者只補provenance，不重做decoder。只有確定仍有正式archive consumer且沒有標準輸出，才開新RE→spec→runtime切片 |
