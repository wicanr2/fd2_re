...
> **抽樣清冊現況**：已登記18筆證據，13筆符合門檻（戰役／戰間2、戰鬥／AI 4、
> 介面3、存檔／持續隊伍3、終局／平台1）。正常標題LOAD已由普通X11鍵盤走過
...
| 1 | 維護 handler 三態與 IDA 函式清冊 | `RE-CLOSED`、`DATA-READY`：2026-08-27由合法IDA 9.4重生1,305函式；全專案unknown位址足跡已成為可重跑清冊。兩批人工複核共接受22筆具caller／consumer／writer的窄語意，語意索引現60筆，結果為產品58／runtime171／未知1,076；剩餘486個精確起點、502個range命中，325筆同時有canonical與direct artifact而列第一優先。舊83 unknown call sites已拆為80已分類、3已知但未閉合、0真未知 | 依 `fd2_unknown_footprints.json` 審完325筆第一優先候選；每批只接受直接證據閉合者並由IDA重生，raw／history-only及無玩家影響者後置，不為歸零猜名 |
...
> 以合法IDA 9.4重生並完成兩批足跡回填後，函式清冊為產品58／runtime171／未知1,076，將故事切片由舊17筆訂正為
