...
- [~] **ENDING-AUDIO-WIRING**：戰鬥 BGM 以原版 `0x51e63` 30-entry table、
  ...
  會以 #57/#58/#60/#59 的形狀驗證呈現並保持 #59 終局靜態圖，且把 `FDMUS_018`
  接到此近似 static tail。原版是在未還原的 20-entry loop 前啟動該曲，因此其
  timing 仍是觀測、不得宣稱相同；素材或 raw provenance 失敗才確認回到可編輯結語。
  完整 `0x2BCE5` indexed renderer、`0x28a6c` loop、raw terminal owner、一般玩家
  路徑與 E2 仍失敗即關閉；證據見
...
- [x] **玩家第17戰／raw ch16 post 垂直切片**：已把 raw handler `0x23b5f`
  ...
  layout、PAN、ACTING50–53、FDTXT_017 index5–8、JOIN16 與
...
不要把它們當成目前覆蓋率。現況一律以本文件頂端的 121／9／50／62 稽核為準。
...
- [~] **raw ch24 post evidence（玩家第25戰候選；owner 勘誤）**：固定版 FD2.EXE 的 IDA Pro 9.4／Docker Capstone 已共同證實 table index24 的 raw entry `0x24df2`、FDTXT_025 index6/7、PAN raw `(4,16)`→`(96,384)`、raw `0x10b4e(2)`、ACT resource75 與共享 `0x112a5(0x1a)` 建構器呼叫；Docker acting exporter 解碼為4 beats、slot70/pose2。Capstone 另固定 `0x24e7b push 0x1d→jmp 0x237c8` 會跳過 direct-entry 的 `push 0x0e`，所以共享尾段實際消費角色29，不是角色14；ch12 的 `push 3→jmp 0x237c8` 是同型交叉證據。map24 group0 唯一我方 raw unit為26、group2唯一敵方 raw unit為29，後續 authored party也同時列出26／29；可編輯 handler 已保存為 `join` 26／29。尚未閉合的是候選 handler 的70-slot map control frontier與重製 `ch25.json` 86-slot battle runtime、`Roster`／selector 的戰後交接；因此 `postbattle_ch25_persist→town_ch26` 仍 blocked。完整位址與雜湊見 [`fd2_ch24_post_ida.txt`](../data/ida/fd2_ch24_post_ida.txt)。
- [x] **ch24 raw role-argument correction（非正式接線）**：`remake/assets/cutscenes/handlers/ch24_post.json` 不再保留錯誤的 `raw_append(14)`；它以原始 `source.addr` 保留 `join 26`（`0x24e6c`）與堆疊參數 `join 29`（`0x237c8`）。這只修正可編輯腳本與證據索引，不解除 `postbattle_ch25_persist` 的 runtime handoff gate；正式 campaign 仍失敗即關閉。
...
- [~] **chapter ending renderer (`0x2bce5`)**：IDA／Capstone 已完整固定 `0x2bce5..0x2c39b` 之前綴（FDOTHER `#0x36`、320×200雙 buffer、frame0/9、palette ramp、三輪重複 ramp、frame12..108、40/200次 composite）與 `0x2c405` 的 `0x1088d(0x1e)`、500次 raw staging、`0x2c548` montage 入口；`0x2c548` party-cycle 與 `0x2c194` raw tail 現已有原資源 executor／schedule。`0x29164` 已更正為 raw `+6==0`／非零分支；`0x2c194` 的三組表也已校正為每輪寫 record0、record1 各自的 `+7`、並以 `<0x4c` 即時計算各自 `+6`，第三組寫 `[0x540ff]`，不能再誤寫成單一 record 的高位欄位或首輪例外。`0x10620→0x4e031` 已證實在 portrait loop 讓當前角色完成後改走 final loop，但尚無確切按鍵映射。最終 `ending` 節點現在只在 `FD2_APPROXIMATE=1` 以嚴格前綴合約播放前綴與原資源 party montage，從 persistent JOIN roster 取得 raw records，精確 gate 消費唯一允許的 `FDMUS_004`；cycle 成功完成後驗證 #57 調色盤、#58 frame table、#60/#59 單影格，呈現並保持 #59，只有 admission 失敗才等待確認回到可編輯結語。`ch29_post` 的 `0x25970→0x2bce5` 仍保留編譯問題（compile issue），截圖隊伍輔助程式拒絕拿不完整的 LOADCH 偽造此 roster；它不是 raw ch29 terminal owner。未閉合的 `0x28a6c` 20-entry renderer、精確輸入、一般玩家 E2 與正式 raw campaign handoff 仍不可吞成 generic ending；完整 body 證據見 [`fd2_ch29_terminal_body_ida.txt`](../data/ida/fd2_ch29_terminal_body_ida.txt)。
...
- [~] **indexed ending compositor core**：`internal/ending.IndexedCompositor` 現提供原版尺寸的 VGA/offscreen/work buffers、透明 `fdother` in-place blit、64000B copy、baseline-derived DAC、ANI、frame12..108、40/200-pass schedule與Ebiten獨立preview。`internal/ending.MontageCycle` 現可在原資源上執行 `0x2c548` 的九段 fade、20 次 secondary、primary FIGANI、DATO/FDTXT portrait 與 64 段 palette 收尾；`MontageTailAssets` 另驗證 #57 768-byte palette、#58 20-frame table、#60/#59 單影格，並只呈現 #59 的穩定終局圖。`MontageTail.Plan` 已保存 `0x28a6c(0,1)` 前 record0／record1 的精確 `+7` bytes、`<0x4c` 推得的 `+6` 與 `[0x540ff]`，但沒有 renderer adapter 或 runtime mutation。近似最終節點以 persistent raw roster／shared native RNG 接線，且只在 portrait loop 消費新輸入的 raw-change 效果。證據見 [`fd2_ch29_montage_ida.txt`](../data/ida/fd2_ch29_montage_ida.txt)。精確 BIOS key mapping、`0x28a6c` 20-entry renderer 與一般玩家 E2 尚未解除。
- [~] **ending compositor asset preflight**：正確圖源是 `FDOTHER_054.bin`（263655B、111-frame table），不是 `FDOTHER_036.bin`（408×138 的無關資源）；ANI #2 已可由 `internal/afm` 解出 26×320×200 frames。`internal/fdother` 已有 fail-closed raw table parser、原版透明 RLE in-place blitter，及 player-provided `FDOTHER.DAT` 的 `#0x36` archive loader；後者有與 raw #054 byte-for-byte 的 regression。`native_2c548.json` 描述的 FDOTHER#56、TAI#3、FIGANI/DATO party montage 現已由 `LoadMontageCycleAssets` 實際載入並交給近似終局 executor；它只接受 persistent raw roster，缺任一資料／資產即回到可編輯結語，不可偽造 native owner。精確輸入與 raw handoff 仍是獨立 gate。
...
- [x] **0x2bce5 可播放前綴（受限近似接線）**：`internal/ending.Player` 現以毫秒 clock 依原序執行 frame0/copy/hold、ANI#2、baseline-derived ramp、兩段 native text、frame12..108、40/200-pass composite，再銜接 `0x2c405` 的500-pass scroll。它精確抵達`0x2c548`；只在 `FD2_APPROXIMATE=1`，才由 persistent raw roster 啟動原資源 `MontageCycle`。預設忠實模式、direct preview、`0x2c194` tail、精確按鍵與 raw terminal owner 仍不接 campaign，也不改用 generic ending。
...
> **2026-08-12 勘誤：**上述描述的是當時的獨立預覽入口。現況另有一條狹義近似戰役接線：
> 最終 `ending` 節點只有在 `FD2_APPROXIMATE=1` 與嚴格 `native_ending_prefix` 合約下才建立
> 相同的前綴播放器；它精確抵達 `0x2c548` 時消費 `FDMUS_004`，近似模式隨即播放
> 原資源 party montage。成功完成後會驗證 #57/#58/#60/#59 並停在 #59；只有 admission
> 失敗才等待確認回到可編輯結語。`FDMUS_018` 已接到這個近似定格，但不主張其原版時序。
> 這不接 raw ch29 terminal handler，也不改變一般玩家 E2、精確 BIOS key mapping 或
> `0x28a6c` 20-entry renderer 未完成的狀態。
- [~] **下一個 ending gate**：`MontageCycle` 已完成 FDOTHER#56 backdrop、FIGANI/DATO、dialogue-frame grid、mirror/non-mirror figure fade、primary descriptor delay 與 64 段 palette 收尾的 standalone indexed adapter。IDA 已閉合 `0x10620` 的兩個 raw word 比較與 `0x4e031` 的 `0x41a→0x41c` 複製；`fdsave.NativeBIOSKeyboardState` 只提供這個原語，不解碼按鍵。`0x28a64` 已校正為共用清理 epilogue，不是返回 owner；`MontageTailAssets` 現會驗證 `0x2c194..0x2c39a` 的四項資源並呈現 #59 穩定終局圖。`MontageTail.Plan` 的三組 20-byte table 現已正確表示為 record0／record1 各自的 `+7`、`<0x4c` 推得的 `+6` 與 `[0x540ff]`。原版非零 `0x28a6c` 的 TAI／FIGANI／BG 載入、合成、VGA 輸出與略過一般戰鬥結果解析器現已達 E0；`MontageTailLoaderBaseline` 也會以真實 FDFIELD／FDICON 建立 `0x1088d(0x1e)` 後的 31-record 基線。但 `0x2c548` 後的呼叫時 records/globals、重製 renderer adapter、按鍵映射、一般玩家 E2 與 campaign terminal route 仍保持失敗即關閉。證據見 [`fd2_ch29_input_cleanup_ida.txt`](../data/ida/fd2_ch29_input_cleanup_ida.txt) 與 [`fd2_ch29_post_montage_tail_ida.txt`](../data/ida/fd2_ch29_post_montage_tail_ida.txt)。

- [~] **終局隊伍回顧循環（重製延伸）**：預設終局仍停在來源 #59 靜態圖；玩家以 Enter／空白鍵才啟動已 admission 的 `MontageCycle`，每次完整隊伍最終狀態播放後重新開始，Enter／空白鍵／Esc 回到 #59。它不改寫戰役、不模擬 DOS BIOS input，也不宣稱原版存在該循環。後續僅可在 `0x28a6c` 的 20-entry renderer 與一般玩家終局 owner 都有 E0/E2 證據後，評估是否將原版尾段插在兩者之間。
...
- [~] **RE-RAW-BUFFER-LATCH-24D22**：Docker Capstone 重讀 `0x24d22(arg)`：`arg!=0` 只把低 byte 寫入 global `0x51a10` 後返回；`arg==0` 配置 `latch*0x138` bytes，從 `0x53aff+(0xc0-latch)*0x138` 複製，接著以 `0xbf-latch` 向下做 `0x138` bytes row copy，最後再 copy 一列並經 `0x37416` free。沿 `0x11cac→0x11eee` case 23 的間接呼叫已證實 `0x24d22(0)` 是 312-byte staging 列旋轉消費端；`0x11EB0` 的共用尾端另固定將 312×192 bytes 複製到 `0xA0504`，重製端加入 `RotateNativeCh23Rows`、`ApplyNativeCh23PaletteCycle` 與 #42 staging 解碼原語。tick gate 已證實為 `[0x46c] != [0x539f8]` 的 BIOS tick 變化條件；固定版 raw seed 已證實為 `0x01`，但入口 latch 的執行期值、`0x53AED`／`0x53AF5`／`0x53AF1` 中間偏移生命週期、`dword_53C03` 生命週期與完整 handler 工作仍未閉合，故 production renderer adapter 仍 fail-closed。
