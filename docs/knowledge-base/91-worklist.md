...
- [~] **editable ending prefix timeline**：新增 `assets/endings/native_2bce5.json`，把已證實的 #054 blit、copy、delay、palette ramps、兩個 chapter branch 的 opaque `0x2c39b` arg pairs，以及兩段 native composite loops 存成可編輯 IR；`internal/ending` 只接受並驗證 `recovered_prefix_only_fail_closed`，絕不將它視為可播放 ending。缺失的 text semantics、buffer/palette helpers、ANI/戰鬥段落 bridge 仍是明確 gate。
