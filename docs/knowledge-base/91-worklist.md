...
| A4 | 現代美術主題 | `PROTOTYPE-SET／DATA-CONTRACT-READY／RUNTIME-E1-PARTIAL`：索爾頭像、第一關戰場與戰鬥HUD三張style A本機私有稿均已產生；受版控`themes/modern/catalog.json`保存穩定asset ID、來源、尺寸與SHA-256。索爾獨立閉嘴頭像母稿及80×80候選已接入顯式`FD2_THEME`正式對話consumer；身分／speaker／雜湊／尺寸／不透明契約任一不符即原子拒絕。本輪勘誤`0x9017`為下框右緣錨點，實際左上角是`(8,115)`，不再沿用舊的`(87,115)`斷言。`FDICON.B24`第68組另已建立12張`24×24`二值alpha候選與逐格雜湊；它不是`FIGANI_068`，且第11格目前明確重用第9格，所以只列prototype，不提前接runtime。規格見`61-modern-theme-runtime-spec.md`。戰場與HUD仍是concept | 先以一致比例的獨立右向第三步態取代重複格，通過12格總覽審查後再接native／normalized兩條地圖人物consumer；同時抽測索爾上／下框正常故事路徑，再以穩定speaker ID逐角色產生獨立頭像。map0 tile ID與FDOTHER HUD cells仍須各自建立基準，不能從合成概念圖反切 |
