...
2026-08-25 戰場命中畫面量測勘誤：以目前原始碼重建完整序列後，最佳 frame76
仍為 `AE=4436`，差異主要集中於守方命中剪影。下一步先補 `0x2939D` 進度／
疊層對守方 idle descriptor 的消費證據；禁止由單張影像猜接 `-14,-10` 位移。
可重跑入口為 `tools/docker/fd2-battle-series-compare.sh`。
