...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 多項正式E1維持；ID34新增受限class19玩家state owner：selector4／5／6／7／20、28 MP gate不扣款、17→18→19私有raw stages全成才發布，正式grid→confirm已驗證。這不包含`0x27FC9`演出／EXP。ID32／33／35仍關閉 | 下一步依證據補ID32／33／35交易，或取得ID34 indexed presentation／一般玩家E2；不得用legacy combo外推，也不得把AI套入玩家owner |
...
3. **戰鬥交易完整性**：物品 type 5–24與ID34受限玩家state transaction不再列數值缺口；狀態到期owner與raw indicators已接。command28／31無已證實一般玩家取得來源，不列阻擋。仍須關閉實際可達的ID32／33／35、其他未知法術／複合效果、狀態高階名稱，以及代表性玩家／敵方回合的精確演出順序。
