...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1–8的`funcs_2AC25`線性entry與mode契約已`RE-CLOSED/DATA-READY`。ID1已完成#19/#21九張target純compositor；ID2已完成#26/#27單張target compositor與#83 sample邊界。ID6已以完整direct body修正mode6回傳7，完成五值side table、#32/#33、#87 samples1／2／3及`fcos/fsin`五點座標純函式；六槽誤讀已由Capstone推翻且未入版控。正式AI的ID1/2/6仍是state-only數值路徑，不能冒稱畫面E1 | 先閉合ID1/2外層MP／HP／音訊發布owner；ID6下一步是mode4／5 counter與secondary effect完整圖層，再接有正常producer的presenter。其後依頻度處理ID5／4／7／3。ID8現行map無敵方mask，僅新producer證據才提升；ID9–12仍須閉合各自owner |
