...
| 順序 | 工作 | 現況 | 下一個可驗收結果 |
|---:|---|---|---|
| 即時勘誤 | A4／A5 數字 | 2026-09-04 現代地圖人物為 selector 0–21、68、76、77、78、80、82、83、85、88、91，共 32 組／384 幀；76 是第三章普通追兵，77 是同章未知姓名的重裝隊長，80／91 是職業欄混合的中性敵方單位，78／82／83／85／88 是具不同原版輪廓的敵方劍士。四語內容各有 5,176 筆；繁中全為來源，簡中尚有 5 筆、日文 2,184 筆、英文 1,200 筆 `machine_draft`，不可只以劇情本文審校數冒充全包完成度 | 後續數字直接由 catalog 與語系檔重算，不手抄舊快照 |
...
> 法術結果、商店交易、轉職錯誤與終局提示；另閉合第一章章名、地點、場景標籤、
> 初始隊伍與說話者名稱。依繁中來源將全內容中的哈瓦特錯譯 `Howard／ハワード`
> 統一勘誤為 `Hawat／ハワット`。其後再審校 40 筆物品、法術、攻擊、寶物與獎勵等
> 玩家可見戰鬥介面文字，接著審校 32 筆商店、戰後／出戰整備、物品轉交與讀檔介面，
> 並由正式 `entities.json` 回填 38 筆法術／指令及結果標籤，避免同一法術在兩套目錄
> 使用不同譯名；其餘 22 個正式城鎮的 132 筆固定選項則沿用已審校的第二章六項選單，
> 避免同功能出現 `酒場／宿屋` 或 `Tavern／Inn` 漂移。全量 validator 通過後，日文為
> 2,186 reviewed／2,990 machine draft。再核對物品轉交、轉職確認、教會與早期戰役選項
> 的實際消費端後，追加 12 筆審校；現況為日文 2,198 reviewed／2,978 machine draft，
> 英文 3,368 reviewed／1,808 machine draft；四語資料完整與
> 正式切換已成立，但日英全內容人工審校尚未完成。

> **2026-09-04 A4／A5 士兵頭盔與第三章說話者勘誤：**新增 selector 85 的
> 12 幀現代候選，依 36 筆敵方劍士與原版逐幀保留大型全罩桶盔、紅眼、方盾及
> 直劍，不把制式士兵美化成露臉英雄。`FDTXT_003` 的 operand 7／77 亦重新核對：
> 「約」只是 77 被誤當 glyph 的研究投影，不是約拿姓名；故事、canonical、四語
> speaker-name、阻擋清冊與靜態解碼器已同步改用場景角色描述並保留原始定位。
> 經全量驗證後，簡中剩 5 筆、日文 2,972 筆、英文 1,802 筆 `machine_draft`。

> **2026-09-04 A4 selector 78 重甲士兵追加：**`map7／8／17` 的 29 筆敵方
> 劍士一致使用 selector 78。現代十二幀保留封閉低盔、面罩紅眼、巨大圓肩與
> 敦實深藍甲；因原版小圖無可靠可見武器或盾，沒有自行補入。清冊與公開總攬
> 因此增為 32 組／384 幀，仍按中性 selector 身分管理。

> **2026-09-04 A5 既有審校詞彙精確復用：**以完整相同的繁中來源為鍵，僅在
> 日文與英文各自都存在唯一已審校譯法時，將該譯法回填到仍為 `machine_draft`
> 的重複目標；259 筆涵蓋戰役整備提示、商店商品、隊伍名稱與少量正式介面。
> 這批沒有送入機器翻譯，且所有目標在套用前均由腳本確認狀態。完成後日文為
> 2,463 reviewed／2,713 machine draft，英文為 3,633 reviewed／1,543 machine draft。
> 第二次改以各語系獨立尋找唯一已審校譯法，再安全回填 529 筆重複日文角色名及
> 343 筆重複英文實體名；五筆來源阻擋明確排除。現況進一步成為日文 2,992
> reviewed／2,184 machine draft，英文 3,976 reviewed／1,200 machine draft。

> **2026-09-01 A4 哈諾戰場人物追加：**角色表 `index=1` 已證實哈諾的基礎
> `sprite_group=1`；依原版 `fig_001_f00..f11` 四方向／三步態生成現代稿，經
> 連通背景分割、最大人物元件保留與最近鄰縮放輸出 12 張獨立 `24×24` PNG。
> `modern.fdicon.group_001.style_a` 已登錄逐幀 SHA-256、二值 alpha 與
> `fdicon_map_sprite_12x24_v1`，任一幀缺失即整組失敗即關閉。私人 allow-list
> 現為 96 檔，包含生成母稿；私人素材庫提交 `57a88ce0` 已推送並逐檔核對 manifest。
> 這仍是 `runtime_candidate`，不冒稱原生 indexed
> 戰場已顯示現代圖或具有正常玩家 E2。

> **2026-09-01 A4 鐵諾戰場人物追加：**角色表 `index=2` 已證實鐵諾的基礎
> `sprite_group=2`；原版 12 幀的巨盾、直劍、灰綠兜帽與四方向遮擋關係已作為
> 現代母稿姿勢基準。新增通用 `process_modern_map_sprite_sheet.py`，以連通背景
> 分割、最大人物元件、最近鄰縮放與二值 alpha 確定性輸出 group 2 的 12 幀；
> catalog 登錄逐幀雜湊後，私人 allow-list 為 109 檔；私人素材庫提交
> `2a468063` 已推送並逐檔核對 manifest。這仍是
> `runtime_candidate／RUNTIME-E1-PARTIAL`，原生 indexed 戰場 consumer 尚未閉合。

> **2026-09-01 A4 哈瓦特戰場人物追加：**角色表 `index=3` 已證實哈瓦特的
> 基礎 `sprite_group=3`。現代母稿依原版巨大紅棕頭髮／鬍鬚輪廓、黑灰重甲、
> 披肩與戰斧生成；通用處理器新增原生透明通道分支後，確定性輸出 12 張
> `24×24` 二值 alpha 獨立幀。公開目錄只保存契約與逐格雜湊，私人素材庫清冊
> 現為 122 檔；私人素材庫提交 `ccbc7178` 已推送並逐檔核對 manifest。此組仍為
> `runtime_candidate／RUNTIME-E1-PARTIAL`，不提升為原生 indexed 或正常玩家
> E2 證據。

> **2026-09-01 A4 圖組 7 戰場人物追加：**角色表 `index=7 / sprite_group=7`、
> 四語隊伍姓名與多章地圖角色資料共同支持蘭斯洛特的基礎地圖圖組投影。母稿
> 依原版巨大橙紅長髮、遮住口鼻的深藍高領巾與深色裝甲生成，沒有加入原版小圖
> 未顯示的披風或武器；通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材
> 清冊現為 174 檔，提交 `2a5570a4` 已推送並逐檔核對 manifest。這只關閉 map
> sprite group 7；`ch03`／`ch18` 的 `speaker=7` 名稱衝突仍存在，現代頭像繼續
> 失敗即關閉。

> **2026-09-01 A4 希莉亞戰場人物追加：**角色表 `index=8 / sprite_group=8`、
> 四語隊伍姓名、多章地圖資料與 `DATO_008 / speaker=8` 一致確認希莉亞。現代
> 母稿依原版大片棕色波浪髮、藍色弓兵服裝、側／背面木弓與少量箭袋輪廓生成，
> 通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 187 檔，提交
> `16318f8f` 已推送並逐檔核對 manifest。此組仍為 `runtime_candidate`，不把
> group／portrait／identity 的本次一致性泛化成全域 ABI 等價。

> **2026-09-01 A4 悠妮戰場人物追加：**角色表 `index=9 / sprite_group=9`、
> 四語隊伍姓名、多章地圖資料與 `DATO_009 / speaker=9` 一致確認悠妮。現代
> 母稿依原版深紅長髮與頭帶、藍色上衣、紅色下身及四方向輪廓生成；通用處理器
> 輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 200 檔，提交
> `5b148a37` 已推送並逐檔核對 manifest。此組只代表正常地圖行走；第一次見到
> 悠妮時的倒地昏迷姿勢仍須由場景 acting 的獨立證據與資料驅動演出負責，不可
> 用一般行走圖組冒充已完成。

> **2026-09-01 A4 瑪琳戰場人物追加：**角色表 `index=10 / sprite_group=10`、
> 四語隊伍姓名、多章地圖資料與 `DATO_010 / speaker=10` 一致確認瑪琳。現代
> 母稿依原版粉紫短髮、金色額飾、紅色僧侶上衣與深色下身生成，通用處理器
> 輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 213 檔，提交
> `c53ec7ff` 已推送並逐檔核對 manifest。這只支持 group 10 的地圖投影；本次
> 欄位一致性不得泛化成 selector、persistent identity 與 portrait 的全域 ABI 等價。

> **2026-09-01 A4 索菲亞戰場人物追加：**角色表 `index=11 / sprite_group=11`、
> 四語隊伍姓名、多章地圖資料與 `DATO_011 / speaker=11` 一致確認索菲亞。現代
> 母稿依原版灰綠短髮與長鬢髮、紅色頭飾、深色上衣、紫袖、紅腰飾與短金色
> 法杖生成，通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為
> 226 檔，提交 `9c78ca2e` 已推送並逐檔核對 manifest。短法杖依原版方向與步態
> 顯示或遮擋；不從頭像或 FIGANI 猜補其他裝備。

> **2026-09-01 A4 凱麗戰場人物追加：**角色表 `index=12 / sprite_group=12`、
> 四語隊伍姓名、多章地圖資料與 `DATO_012 / speaker=12` 一致確認凱麗。現代
> 母稿依原版巨大橙金側馬尾、深藍武者服、藍灰腰帶與淺灰綠護腕生成，通用
> 處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 239 檔，提交
> `8a495605` 已推送並逐檔核對 manifest。逐幀檢視確認橙色長輪廓是頭髮，沒有
> 可證實武器；早期把它讀成長柄武器的描述不得再作重繪依據。

> **2026-09-01 A4 貝克威戰場人物追加：**角色表 `index=13 / sprite_group=13`、
> 四語隊伍姓名、多章地圖資料與 `DATO_013 / speaker=13` 一致確認貝克威。現代
> 母稿依原版巨大金色長直髮、黑色寬頭帶、深綠弓兵服、樸素棕弓與箭袋生成，
> 通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 252 檔，
> 提交 `932a429f` 已推送並逐檔核對 manifest。原表種族仍是「精靈？」；沒有
> 直接證據前不加入尖耳，也不把問號抹成定論。

> **2026-09-01 A4 珊戰場人物追加：**角色表 `index=14 / sprite_group=14`、
> 四語隊伍姓名、多章地圖資料與 `DATO_014 / speaker=14` 一致確認珊。現代
> 母稿依原版巨大深藍不對稱長髮、綠色額帶與額側飾物、深藍法師袍、棕橙
> 肩領及紫紅腰胸飾生成；通用處理器輸出 12 張 `24×24` 二值 alpha 幀。
...
> **2026-09-01 A4 凱拉斯戰場人物追加：**角色表 `index=16 /
> sprite_group=16`、四語隊伍姓名、多章 roster 與 `DATO_016 / speaker=16`
> 一致確認凱拉斯。現代母稿依原版棕色龍首、深藍甲胄、紅色腰飾、藍灰圓盾
> 及紅白長刃生成，通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材
> 清冊現為 291 檔，提交 `69e1cb58` 已推送並逐檔核對 manifest。頭像只協助
> 角色識別，不把其細節冒充地圖小圖逐像素證據，也未追加翅膀或第二件武器。

> **2026-09-01 A4 米亞斯多德戰場人物追加：**角色表 `index=17 /
> sprite_group=17`、四語隊伍姓名、多章 roster 與 `DATO_017 / speaker=17`
> 一致確認米亞斯多德。現代母稿依原版暗紅龍首與身體、灰綠圓盾、深色三角
> 盾紋及藍白長刃生成，通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人
> 素材清冊現為 304 檔，提交 `c024619d` 已推送並逐檔核對 manifest。頭像只
> 協助確認角色識別，未把尖耳、鬚棘等細節冒充地圖小圖證據，也未追加翅膀。

> **2026-09-01 A4 蜜蒂戰場人物追加：**角色表 `index=18 / sprite_group=18`、
> 四語隊伍姓名、多章 roster 與 `DATO_018 / speaker=18` 一致確認蜜蒂。現代
> 母稿依原版蓬鬆金橙長髮、紅色長袖外衣、淺灰胸甲、藍灰腰部與深色下裝生成，
> 通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 317 檔，
> 提交 `dfb05bd3` 已推送並逐檔核對 manifest。原版小圖看不清武器，因此沒有
> 依劍聖職業名稱猜加長劍；頭像只協助角色識別，不冒充地圖逐像素證據。

> **2026-09-01 A4 羅德曼戰場人物追加：**角色表 `index=19 / sprite_group=19`、
> 四語隊伍姓名、多章 roster 與 `DATO_019 / speaker=19` 一致確認羅德曼。
> 現代母稿依原版棕色長髮與鬍鬚、藍灰甲胄、深色十字圓盾及短直銀劍生成，
> 通用處理器輸出 12 張 `24×24` 二值 alpha 幀。私人素材清冊現為 330 檔，
> 提交 `2a779568` 已推送並逐檔核對 manifest；沒有追加頭盔、披風或雙手大劍。

> **2026-09-01 A4 莎拉戰場人物追加：**角色表 `index=20 / sprite_group=20`、
> 四語隊伍姓名、多章 roster 與 `DATO_020 / speaker=20` 一致確認莎拉。現代
> 母稿依原版深綠長髮、暗灰綠衣甲與紅色腰肩點綴生成，輸出 12 張 `24×24`
> 二值 alpha 幀。私人素材清冊現為 343 檔，提交 `f958a57e` 已推送並逐檔核對
> manifest；因小圖手部物件不明，未依龍騎士職業猜加長槍、龍、翅膀或坐騎。

> **2026-09-01 A4 約拿戰場人物追加：**角色表 `index=21 / sprite_group=21`、
> 四語隊伍姓名、多章 roster 與 `DATO_021 / speaker=21` 一致確認約拿。現代
> 母稿依原版蓬鬆白髮、巨大白鬍鬚與棕色長袍生成，輸出 12 張 `24×24` 二值
> alpha 幀。私人素材清冊現為 356 檔，提交 `8bf18eb8` 已推送並逐檔核對
> manifest；未再帶入狼族、年輕尖帽法師或武器造型。

> **2026-09-01 A5 第三章第二場景首批審校：**scene1 第1–5句的說話者與本文
> 已完成日文、英文、簡中人工審校；人工審校總數現為日文280句、英文271句、
> 簡中248句。第0句仍由身分衝突的「約」發言，因此說話者與本文都維持
> `machine_draft`，不以場景脈絡猜選蘭斯洛特。

> **2026-09-01 A5 第三章第二場景第 6–10 句審校：**第 6、7、9、10 句的
...
> **2026-09-01 A5 第三章第二場景收尾：**第 16 句的「什麼碗糕」依台灣口語
> 語意翻為「什麼鬼東西」，說話者與正文，以及第 17 句萊汀姓名，已完成日文、
> 英文、簡中人工審校。人工審校總數現為日文 302 筆、英文 293 筆、簡中 270 筆。
> 第 17 句「卡蘿娜」與前文「卡蘿那女孩」仍有專名切分衝突，三語正文維持
> `machine_draft`；第三章沒有 scene2，不再以不存在的場景規劃審校工作。

> **2026-09-01 A5 第四章第一場景審校：**實檔 scene0 只有 4 句，均有完整
> 繁中來源與明確說話者；日文、英文、簡中說話者與正文全部完成人工審校。
> 人工審校總數現為日文 310 筆、英文 301 筆、簡中 278 筆。scene0 line2 的
> `..` 是語氣停頓，不是截斷；未虛構不存在的第 5 句。

> **2026-09-01 A5 第四章第二場景審校：**scene1 第 0–4 句的繁中來源完整，
> 說話者依序為豹、索爾、豹、亞雷斯、豹，沒有身分衝突或截斷。日文、英文、
> 簡中說話者與正文均完成人工審校；人工審校總數現為日文 320 筆、英文 311 筆、
> 簡中 288 筆。此處「豹」是敵方角色名稱，不與謝多的豹人種族設定混用。

> **2026-09-01 A5 第四章戰後場景審校：**scene3 四句繁中來源與索爾、希莉亞、
> 哈諾、希莉亞的說話者均完整，日文、英文、簡中說話者與正文完成人工審校。
> 人工審校總數現為日文 328 筆、英文 319 筆、簡中 296 筆。scene2 的連續
> `speaker=24..27` 與龍人吼聲映射仍有疑點，未因本批而升格。

> **2026-09-01 A5 第四章短句審校：**scene4 兩句來源完整；「什麼碗糕」依
> 台灣口語語意翻為「什麼鬼東西」，沒有按字面誤譯為糕點。索爾與敵方「豹」
> 的說話者及兩句正文均完成日文、英文、簡中人工審校；累計日文 332 筆、
> 英文 323 筆、簡中 300 筆。

> **2026-09-01 A4 萊汀戰場人物追加：**角色表 `index=6` 與多章角色身分資料
> 已證實萊汀的基礎 `sprite_group=6`。現代母稿依原版深棕束髮、深色頭帶、
> 灰綠重甲、高聳銀色肩甲輪廓與貼身短直刃生成，通用處理器由透明母稿確定性
> 輸出 12 張 `24×24` 二值 alpha 幀。萊汀肖像目錄同時移除亞雷斯的錯誤角色
> 來源，只保留 `DATO_006_m0`。私人素材清冊現為 161 檔；私人素材庫提交
> `c72a1fb8` 已推送並逐檔核對 manifest。此組仍為
> `runtime_candidate／RUNTIME-E1-PARTIAL`，不提升為原生 indexed 或正常玩家
> E2 證據。

> **2026-09-01 A4 洛娜戰場人物追加：**角色表 `index=5`、多章 `fig=5` 與
> `native_identity=5` 已證實洛娜的基礎 `sprite_group=5`。現代母稿依原版
> 銀灰長髮、藍灰騎士裝甲、直立長劍與背面紅棕披風生成，通用處理器由透明
> 母稿確定性輸出 12 張 `24×24` 二值 alpha 幀。洛娜肖像目錄同時移除亞雷斯
> 的錯誤角色來源，只保留 `DATO_005_m0`。私人素材清冊現為 148 檔；私人素材庫
> 提交 `24e5c37c` 已推送並逐檔核對 manifest。此組仍為
> `runtime_candidate／RUNTIME-E1-PARTIAL`，不提升為原生 indexed 或正常玩家
> E2 證據。

> **2026-09-01 A4 亞雷斯戰場人物追加：**角色表 `index=4`、多章 `fig=4` 與
> `native_identity=4` 已證實亞雷斯的基礎 `sprite_group=4`。現代母稿依原版
> 深藍軟帽、暗綠頭帶、藍色輕甲、直立長劍、小型圓盾與四方向遮擋關係生成，
> 通用處理器由透明母稿確定性輸出 12 張 `24×24` 二值 alpha 幀。公開目錄只
> 保存契約與逐格雜湊，私人素材清冊現為 135 檔；私人素材庫提交 `afab7bc9`
> 已推送並逐檔核對 manifest。此組仍為
> `runtime_candidate／RUNTIME-E1-PARTIAL`，不提升為原生 indexed 或正常玩家
> E2 證據。

> **2026-08-31 A5 商店四語追加：**除購買／出售結果四條交易路徑外，
> **2026-08-31 A5 審校筆數勘誤：**上表的日文 24 句是上一批快照；本批再完成
> 王城與第一章後續審校後，當時現況為日文 100 句、英文 91 句、
> 簡中 68 句；第二章首批已由上方 A5 現況取代。本段只保留數字演進原因，
> 不可再引用 24／100 句作目前進度。
>
...
>
> **2026-09-04 第三章追兵視覺分流：**原版戰場普通追兵使用 `fig／selector 76`，
> 敵方隊長使用 77；兩者都不是角色表的蘭斯洛特 `group 7`，也不是第十八章才登場的
> 約拿 `speaker 21`。現代主題新增 76 的十二幀候選，依使用者決定保留封閉式深鋼盔、
> 護頰、盾牌與制式軍裝，使其一眼可辨識為普通士兵。隊長 77 已另依原版十二幀
> 建立全罩盔、厚板甲、大盾、直劍與暗紅腰帶候選；其姓名仍未知，故只採中性 selector
> 身分，不與 76、蘭斯洛特 group 7 或約拿 speaker 21 共用。使用者檢視初版後指出
> 普通士兵過度俊美；現版已改以原版低圓頂鋼盔、額鼻護片與護頰為辨識核心，移除
> 十字徽記並改用無徽記木盾，降低精銳騎士感而保留一眼可辨識的士兵輪廓。
>
> **2026-08-31 A4 希莉亞追加：**`DATO_008`、86 筆故事 `speaker=8` 與繁中
> 實體目錄三方一致確認希莉亞；原版影像的正面棕色波浪長髮、藍色額飾、中央
> 紅寶石與灰藍高領已由人工檢視記錄。現代主題現有 8 位私有頭像候選。
>
> **2026-08-31 A4 悠妮追加：**`DATO_009`、107 筆故事 `speaker=9` 與繁中
> 實體目錄三方一致確認悠妮；原版影像的正面深紅長髮與頭帶、中央藍寶石、
> 狹長眼及暗紅高領已由人工檢視記錄。初遇倒地昏迷仍由場景演出負責；現代
> 主題現有 9 位私有頭像候選。
>
> **2026-08-31 A4 瑪琳追加：**`DATO_010`、18 筆故事 `speaker=10` 與繁中
> 實體目錄三方一致確認瑪琳；原版影像的正面紫紅短髮、中央金色髮飾、狹長眼、
> 紅唇及金紅高領已由人工檢視記錄。現代主題現有 10 位私有頭像候選。
>
> **2026-08-31 A4 索菲亞追加：**`DATO_011`、19 筆故事 `speaker=11` 與繁中
> 實體目錄三方一致確認索菲亞；原版影像的灰綠短髮、兩側長鬢髮、紅唇、黑色
> 細頸圈及藍灰肩甲已由人工檢視記錄。現代主題現有 11 位私有頭像候選。
>
> **2026-08-31 A4 凱麗追加：**`DATO_012`、16 筆故事 `speaker=12` 與繁中
> 實體目錄三方一致確認凱麗；原版影像的微向右側、金色短捲髮、垂眼、紅唇、
> 金色耳飾及深藍高領已由人工檢視記錄。識別字依英文實體目錄採 `Kelly`；
> 現代主題現有 12 位私有頭像候選。
>
> **2026-08-31 A4 貝克威追加：**`DATO_013`、60 筆故事 `speaker=13` 與繁中
> 實體目錄三方一致確認貝克威；原版影像的正面金色長直髮、寬深藍頭帶、微瞇眼、
> 淡笑及深藍頸圈已由人工檢視記錄。識別字依重製英文實體目錄採 `Beckway`；
> 現代主題現有 13 位私有頭像候選。
>
> **2026-08-31 A4 珊追加：**`DATO_014`、23 筆故事 `speaker=14` 與繁中實體
> 目錄三方一致確認珊；原版影像的微向左側、深藍長髮、橙金細額飾與紅寶石、
> 狹長眼、紅唇及藍色高領已由人工檢視記錄。識別字依重製英文實體目錄採
> `Shan`；現代主題現有 14 位私有頭像候選。
>
> **2026-08-31 A4 speaker 15 停止線：**原版角色清冊與 15 筆故事寫
> 「賽可邦勒」，繁中實體目錄卻寫「塞可邦勒」。字形衝突校訂前不產生或接通
> `speaker 15` 現代頭像，英文 `Saikobunler` 也不能消除繁中原始證據衝突。
>
> **2026-08-31 A4 凱拉斯追加：**`DATO_016`、30 筆故事 `speaker=16` 與繁中
> 實體目錄三方一致確認凱拉斯；原版影像證實它是向左的棕黑龍首，具長吻、尖牙、
> 綠眼、後掠角與深色頸部。識別字採 `Kylas`；現代主題現有 15 位私有頭像候選。
>
> **2026-08-31 A4 米亞斯多德追加：**`DATO_017`、22 筆故事 `speaker=17` 與
> 繁中實體目錄三方一致確認米亞斯多德；原版影像證實它是正面暗紅龍／魔獸首，
> 具亮紅眼、多根尖角、尖長側耳、黑色鬚棘與灰藍肩部。識別字採 `Miasdord`；
> 現代主題現有 16 位私有頭像候選。
>
> **2026-08-31 A4 蜜蒂追加：**`DATO_018`、29 筆故事 `speaker=18` 與繁中
> 實體目錄三方一致確認蜜蒂；原版影像的正面蓬鬆金色長捲髮、深色眉毛、紅唇
> 與沉著表情已由人工檢視記錄。識別字依重製英文實體目錄採 `Mitty`；現代
> 主題現有 17 位私有頭像候選。
>
> **2026-08-31 A4 羅德曼追加：**`DATO_019`、12 筆故事 `speaker=19` 與繁中
> 實體目錄三方一致確認羅德曼；原版影像的向左三分之二側面、棕色後梳長髮、
> 濃密八字鬍與下巴鬍鬚、銳利眼神及灰藍肩甲已由人工檢視記錄。識別字依重製
> 英文實體目錄採 `Rodman`；現代主題現有 18 位私有頭像候選。
>
> **2026-08-31 A4 莎拉追加：**`DATO_020`、6 筆故事 `speaker=20` 與繁中
> 實體目錄三方一致確認莎拉；原版影像的正面精靈尖耳、灰綠長直髮、細紅色
> 頭帶、纖細眉眼、紅唇與深色高領已由人工檢視記錄。識別字依重製英文實體
> 目錄採 `Sarah`；現代主題現有 19 位私有頭像候選。
>
> **2026-08-31 A4 約拿追加：**`DATO_021`、84 筆故事 `speaker=21` 與繁中
> 實體目錄三方一致確認約拿；原版影像呈現老年人類聖者、貼合頭部的白色頭巾／
> 白髮、巨大白色鬍鬚與鬍髭、飽經風霜的面孔、堅毅眼神及少量棕金聖職服飾。
> 識別字依重製英文
> 實體目錄採 `Jonah`；現代主題現有 20 位私有頭像候選。
>
> **2026-08-31 約拿勘誤：**先前現代稿把藍色頭巾與白鬍錯讀成尖耳與獸毛，
> 因而誤畫為狼族。角色職業表原已正確記為聖者；經使用者指出並重新檢視
> `DATO_021_m0` 後，錯圖已撤換且素材契約、雜湊與公開總攬同步更正。
>
> **2026-09-01 約拿造型二次勘誤：**上一版雖已撤掉狼族誤圖，仍把原版頭部
> 淺色輪廓誤畫成深藍尖帽，成為一般巫師造型。使用者確認約拿是留著巨大白鬍鬚
> 的老年聖者後，再以 `DATO_021_m0` 與角色表 `index=21 / cls_name=聖者` 核對；
> 現代稿已改為白色頭巾／白髮、巨大白鬍鬚與素色聖者衣袍，並移除尖帽及華麗
> 魔法師服飾。公開 catalog 雜湊與 28 人總攬已同步；私人母稿／80×80 候選已由
> 素材庫提交 `8cb2e7df` 保存並推送。舊深藍尖帽描述不得再作造型依據。
>
> **2026-08-31 A4 卡里斯追加：**`DATO_022`、16 筆故事 `speaker=22` 與繁中
> 實體目錄三方一致確認卡里斯；原版影像的正面略向左、深藍黑高馬尾、兩側
> 長鬢髮、冷峻微瞇眼、黑色高領與暗紅肩部已由人工檢視記錄。識別字依重製
> 英文實體目錄採 `Carlos`；現代主題現有 21 位私有頭像候選。
>
> **2026-08-31 A4 私人素材庫同步：**`fd2-assets-private` 提交 `8df7b770` 已保存
> catalog allow-list 內 69 個現代重繪檔案（約 44.8 MB），包含 21 位頭像母稿／
> 執行期候選、兩組 FDICON、第一章戰場概念與戰鬥 HUD。私人 manifest 逐檔
> SHA-256 與公開 catalog 一致；公開 README 另放壓平的 21 位頭像總攬，不提供
> 可重組原版的完整私人包。
>
> **2026-08-31 A4 羅蘭追加：**`DATO_023`、3 筆故事 `speaker=23` 與繁中
> 實體目錄三方一致確認羅蘭；原版影像的正面略向左、灰棕長捲髮、寬深藍頭帶、
> 狹長眼、淡紅唇色與小型金色耳飾已由人工檢視記錄。識別字依重製英文實體
> 目錄採 `Roland`；現代主題現有 22 位私有頭像候選。
>
> **2026-08-31 A4 私人素材庫續同步：**`fd2-assets-private` 提交 `fefdcdec`
> 已保存更正後的約拿與新增羅蘭，現為 catalog allow-list 內 71 個檔案、22 位
> 頭像候選；私人 manifest 已逐檔核對大小與 SHA-256。公開總攬亦由同一份
> catalog 確定性重建，不再顯示錯誤的狼族約拿。
>
> **2026-08-31 A4 希爾法追加：**`DATO_024`、10 份故事檔共 41 筆 `speaker=24` 與繁中
> 實體目錄三方一致確認希爾法；原版影像的完全禿頭、高額頭、兩側長尖耳、
> 細黑八字鬍、中央尖山羊鬍及金橙高聖職衣領已由人工檢視記錄。識別字依重製
> 英文實體目錄採 `Sylpha`；現代主題現有 23 位私有頭像候選。
> `fd2-assets-private` 提交 `d1873505` 已保存母稿與執行期 PNG；私人 manifest
> 現為 73 個 catalog allow-list 檔案，逐檔大小與 SHA-256 驗證通過。
>
> **2026-08-31 A4 謝多追加：**`DATO_025`、2 份故事檔共 5 筆 `speaker=25`
> 與繁中實體目錄三方一致確認謝多。2026-09-01 勘誤：謝多是豹人，不是鼠人；
> 原版低解析頭像不足以單獨判定種族，先前把輪廓斷言為圓鼠耳、鼠吻與門牙
> 已撤銷。現代稿已改為深藍灰毛豹人忍者，保留向右側面、淺色眼與深色高領，
> 改用貓科耳、短寬口鼻、強壯顴骨及低調豹斑。
> 識別字依重製英文實體目錄採 `Seymour`；現代主題現有 24 位私有頭像候選。
> 私人素材庫提交 `7e72b282` 保存的鼠人版已由本勘誤取代；目前母稿與執行期
> PNG 已由提交 `368eaaab` 保存，新雜湊以 catalog 及私人 manifest 為準。
>
> **2026-09-01 A4 聖寇拉斯追加：**`DATO_026`、角色資料表的龍人／龍劍士欄位
> 與四語隊伍姓名清冊共同確認聖寇拉斯；原版頭像呈現向左的深鋼藍龍首、狹長
> 金色眼、後掠長角、頰頸棘刺、外露尖牙與藍色護甲。故事資料的 `speaker=26`
> 存在跨場景身分混用，因此不以其總筆數支持角色身分。識別字依英文清冊採
> `Saint Colas`；現代主題現有 25 位私有頭像候選。`fd2-assets-private` 提交
...
> allow-list 檔案，逐檔大小與 SHA-256 驗證通過。
>
> **2026-09-01 A4 身分 28／29 停止線：**`DATO_028` 的原始名稱「達克賽」
> 與四語顯示清冊「達克塞」衝突；`DATO_029` 的「亞奇梅吉」與「亞齊梅吉」
> 亦衝突。兩者均保留 legacy alias，字形裁決前不產生或接通現代頭像；未知種族
> 欄位也不以故事描述代填。
>
> **2026-09-01 A4 蓋亞追加：**`DATO_030`、機兵職業、四語隊伍姓名與故事中的
> 機能／導航敘述共同確認蓋亞；原版頭像呈現向右的白灰分段裝甲機械頭、深色
> 水平視窗與藍色關節／線路。現代主題現有 27 位私有頭像候選。
> `fd2-assets-private` 提交 `f73c6751` 已保存母稿與執行期 PNG；私人 manifest
> 現為 81 個 catalog allow-list 檔案，逐檔大小與 SHA-256 驗證通過。
>
> **2026-09-01 A4 渥德追加：**`DATO_031`、機兵職業、四語姓名與故事中的
> 系統／記憶庫／機甲戰鬥團敘述共同確認渥德。原版頭像呈現向左的仿人機械臉、
> 古銅色人造表皮、黑色硬質頭冠、藍黑側板、紅橙眼與強壯下顎；不與蓋亞的
> 全罩機械頭混畫。現代主題現有 28 位私有頭像候選。`fd2-assets-private`
> 提交 `37a4765b` 已保存母稿與執行期 PNG；私人 manifest 現為 83 個 catalog
> allow-list 檔案，逐檔大小與 SHA-256 驗證通過。
>
...
> 不把未證實音譯升格為正式英文。
>
> **2026-09-01 A5 第二章故事續審：**第二章第一個場景的前六句及其說話者
...
> 均沿用既定身分。其餘機器初稿仍不升格。
>
> **2026-09-01 A5 第二章故事第三批：**同一場景第 12–17 句及其說話者名稱已
> 完成日文、英文與簡中人工審校，再新增每語 12 筆 `reviewed`。本批修正日文
> 「放我們一馬」誤譯、英文漏句與 `Sole`，並固定 `Bandit B`／`盗賊B`；其餘
> 機器初稿仍不升格。
>
> **2026-09-01 A5 第二章場景接續：**第一場景最後第 18–19 句及第二場景前
> 0–5 句已完成日文、英文與簡中人工審校，本批每語新增 16 筆 `reviewed`。
> 第一場景實檔沒有第 20–23 句，不建立不存在的穩定鍵；日文無來源人名與英文
> `leaking fish` 等錯譯已移除。其餘機器初稿仍不升格。
>
> **2026-08-31 A5 商店四語追加：**除購買／出售結果四條交易路徑外，
...
> **2026-08-31 A5 戰間四語追加：**旅館與出戰整備的 8 個 renderer 鍵已接入
> 四個官方語系；包含 fallback 標題、操作提示、記錄戰況詢問、部署計數、未知
> 角色、防呆進戰確認與保存提示。Go 與 Python 契約現同時固定 75 鍵；節點本身
> 已提供的 `Text／Prompt` 仍保留資料內容，不由 fallback 翻譯覆蓋。

> **2026-08-31 A5 戰鬥結果四語追加：**一般戰鬥勝利、敗北、Enter 繼續與
> 城鎮戰後整備標題四條正常玩家 renderer 已接官方 pack。Go 與 Python 契約現
> 同時固定 79 鍵；戰後標題保留節點 `Town` 作 `%s` 參數，不把地名寫死進句型。

> **2026-08-31 A5 標題 LOAD 四語追加：**正規化 LOAD fallback 的操作提示、
> 槽位編號、空槽與已有存檔四個 renderer 鍵已接官方 pack；原生 indexed LOAD
> 素材不被文字 fallback 覆蓋。Go 與 Python 契約現同時固定 83 鍵。

> **2026-08-31 A5 教會與結局四語追加：**正規化教會 fallback 的四項服務、
> 主選單操作、轉交來源／物品／目的、空清單、物品標籤、選擇與執行提示，以及
> 結局標題共 13 個 renderer 鍵已接官方 pack。缺 key 時停止整個畫面發布，不
> 混回繁中。Go 與 Python 契約現同時固定 96 鍵。
