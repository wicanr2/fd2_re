...
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `DATA-READY（5,176筆／語）＋RUNTIME-E1-PARTIAL`：四個內容目錄各含5,176筆玩家內容；35份劇本共1,564句具穩定 `line_id`。正式 story、handler dialog及事件61／75／76均查詢四語內容，F4原子切換兩種目錄；缺鍵、漂移及 blocked 一律停止。2026-08-31 非繁中原生 indexed 對話已用13／14／15 glyph＋實際像素寬雙閘門重建三列分頁與 progressive frames，保留開收框、閉嘴頭像及鏡頭；所有正式可編譯 native utterance 的簡中／日文／英文字形與矩形測試通過。依使用者裁決非繁中不做嘴型。英文／日文仍是機器初稿，尚無正常玩家截圖 E2 | 依序處理商店、戰鬥HUD、角色／物品／法術名稱 semantic key；再抽測四語原生對話、商店與HUD畫面，最後人工校譯英文／日文。4筆身份歧義解決前不合併角色鍵 |
