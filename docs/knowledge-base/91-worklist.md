...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 空游標設定、END、nested資訊／SAVE／LOAD／離場皆達正式E1。LOAD 先在私有 `Game` 完成 current-runtime typed handoff，YES 後才原子替換；SAVE 會保留四槽與未知 bytes，區分非我方增援及具 constructor／identity 證據的新我方 JOIN，並同步 persistent raw。`0x20C6F` 已恢復的物品 type 5–24 數值交易均有正式消費端。`0x1A866` 的 selector 1→0 已依原呼叫順序接在合併的非玩家掃描前，selector 2 接在玩家輸入前；倒數、歸零重算及 raw 同步採單批原子發布 | 下一步補未知法術／複合指令、狀態高階名稱，以及一般玩家同狀態逐幀／逐音訊 E2；不得把 event61／75 接通外推成90-entry所有 selector 已完成，也不得再重做已接的物品 type 5–24 transaction |
| 4a | 玩家／敵方指令 `28／29／31` 原版演出 | `RE-CLOSED`（caller分歧）／`RUNTIME-E1`（校正後數值）：IDA已固定28的first-target base／mode0／無`29C90`／分母8，及29／31的actor base／mode1／逐target轉場／分母1；FIGANI全檔證實可達effect每個target只有一個impact marker，故28已由錯誤完整roll修正為`roll/8`，29／31另有倍率交易回歸。正式玩家與敵方路徑仍略過indexed演出 | 先閉合28／31一般玩家actor selector；再接command-specific schedule → indexed UI → marker交易 → 玩家與敵方owner，缺任一原始資產或raw provenance時保持零修改；command29可先用已證實growth selector34／resource104形成首條完整演出切片，最後才做同狀態逐幀／音訊E2 |
...
2. **存檔與持續隊伍**：current-runtime LOAD 與動態增援／JOIN 後的 persistent raw 同步已達窄 `RUNTIME-E1`；剩餘交付門檻是長程玩家路徑驗證章節、金錢、HP／MP、物品、裝備與入隊順序不漂移。
3. **戰鬥交易完整性**：物品 type 5–24 已恢復的數值交易不再列缺口；selector 1→0／2 的狀態到期呼叫端、FDTXT 481..486／DATO indexed 提示與 `sub_17FC0` status colors／entries `0x37..0x39` 圖示已接通，不再列為 owner 缺口。仍須關閉未知指令／法術／複合效果、狀態高階名稱，以及代表性玩家／敵方回合的精確演出順序。
