...
| A5 | 繁中／簡中／日文／英文與可調文字顯示 | `RUNTIME-E1（12-key；切換／保存契約完成，內容遷移中）`：四語官方包現在具有精確相同key／變數／provenance集合；F4原子切換、`fd2_settings.json`持久化、戰役save隔離與封包自檢不變。除物理攻擊外，語言／音源切換及SAVE／LOAD成功、禁止與錯誤訊息已移除硬編碼繁中並通過四語正式路徑。官方包多一或少一key都拒絕；完整候選仍為5,233筆來源／2,497個互異繁中字串，不把12-key冒稱完整翻譯 | 以本輪canonical line／character身份取代來源行號，批次完成其餘對話、商店、戰鬥HUD、角色／物品／法術名稱四語內容；驗證日文字形與最長字串排版。4筆身份歧義解決前，不建立可能綁錯角色的正式翻譯key |
