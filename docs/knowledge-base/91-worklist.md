...
- [x] **finale phase-0 editable script node**：新增 `assets/endings/native_2c405.json` 和嚴格 loader；`0x2c469` 前的 `load_ch_text(30)` 依 loader 規則選 FDTXT_031，故 `0x2c` 是其合法實體 string #44（46 strings）。內容是後日談跑馬燈前言，對位跨資源重用的 `ch32.json` scene0 line0；staging/layout/timing/palette cadence 均資料化。asset 可編輯但 `Ready()==false`，直到 glyph compositor 恢復。
...
- [~] **FDTXT archive provenance**：`FDTXT.DAT` resource30 與 extracted `FDTXT_031.bin` 實測不相同（5762 vs 6756 bytes），不可假設 archive index = extracted suffix；preview 接線保持封閉，需先追 `0x1088d/0x111ba` 的實際 FDTXT descriptor/resource mapping。
