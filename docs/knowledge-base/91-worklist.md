...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 底層 RE 與多個窄 `RUNTIME-E1` 已有；空游標 selector2設定與selector3 END已接正式E1。selector0的`0x19DF7` nested cells、`0x1B1E7` 四panel／六數值／兩行FDTXT、12開／等待任意鍵＋`0x4DFCC`循環／12關已接正式鍵盤與indexed畫面，達`RUNTIME-E1`；存檔／讀檔／離場仍只`RE-CLOSED`。command 0、13–27多個正式 indexed owner與原子交易已達窄 `RUNTIME-E1` | 原版current-runtime存檔與現有JSON battle restart語意不同，未決前不得冒稱相容；先補離場`-1` caller目的地及外層selector1，再決定是否實作原生current-runtime存讀檔。其後補abbreviated presentation owner、物品effect、phase-expiry／status UI及一般玩家逐幀／逐音訊E2 |
