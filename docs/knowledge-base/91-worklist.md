...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0、玩家／敵方ID1／2／3／5／6／7／8／9／10／11／12，以及玩家／敵方IDs17–22均已有caller-specific indexed owner；受限class19玩家ID32–35亦達`RUNTIME-E1`。玩家17–22保留專用`0x1D6C8` palette前段再進handler tail；敵方省略palette但仍呈現wrapper內建tail並原子發布。ID4無已證實正常玩家producer；28／30／31沒有已證實正常玩家取得來源，均不以猜測列阻擋。 | IDs10–12／17–22與IDs32–35正式owner已關閉，不重做；下一個缺口依`58`轉向其他仍有正常producer的敵方command／item，同狀態逐幀／逐音訊另屬E2 |
