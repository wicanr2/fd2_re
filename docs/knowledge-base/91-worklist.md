...
- DATA-READY：使用者回報的「快速順移殘影」已由逐幀對照定位。原版走行期間
  **不畫游標白框也不畫左下 HUD 面板**，重製端兩者都還畫著，白框就停在終點。
  **這個差異在狀態層看不出來**：原版 overlay selector `[0x51A83]` 整段是 1，
  重製端 `NativeMapRangeMode` 也是 1；原版是走另一條不呼叫 `0x122DC` 的繪圖
  路徑。收據見 [99](99-move-confirm-cursor-20260909.md)。
- 待查：原版移動動畫的繪圖路徑（進入點、疊哪幾層、何時交還 `0x11CAC`）。
  找出來才知道重製端該怎麼收；把 selector 改成 0 是錯的，原版就是 1。
