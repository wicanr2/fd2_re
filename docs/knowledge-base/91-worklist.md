...
- [~] **BATTLE-VISUAL-GAP-CH01**：重新稽核 GitHub 上的戰場對照圖，確認
  ...
  右欄只標出這 22 個左下邊界像素；舊不同狀態圖片仍只作歷史參考。
...
- [~] **raw ch24 post evidence（玩家第25戰候選；owner 勘誤）**：固定版 FD2.EXE 的 IDA Pro 9.4／Docker Capstone 已共同證實 table index24 的 raw entry `0x24df2`、FDTXT_025 index6/7、PAN raw `(4,16)`→`(96,384)`、raw `0x10b4e(2)`、ACT resource75 與共享 `0x112a5(0x1a)` append 呼叫；Docker acting exporter 解碼為4 beats、slot70/pose2，map24 raw frontier=70。後續 IDA 追查確認 `0x10b4e` 逐筆比對 FDFIELD row `+0x15` 並呼叫 `0x10c50` 建立 runtime record；map24 resource073 的 70 筆列中 group2 恰有1筆，因此 binding 的 `spawn_groups["2"] = 1` 已有可重生的列數證據。`0x112a5` 的兩個 immediate 也已由固定角色表辨識為 26「聖寇拉斯」與 14「珊」；這只證明建構器角色索引，不證明永久 JOIN 或戰後隊伍操作。table index不直接等於玩家戰次，先前誤接 `postbattle_ch24_persist→town_ch25` 已撤回。`postbattle_ch24_persist` 仍 blocked，完整位址與雜湊見 [`fd2_ch24_post_ida.txt`](../data/ida/fd2_ch24_post_ida.txt)。
- [x] **ch24 candidate binding withdrawal**：`0x112a5(0x1a)` 與共享尾段 `0x112a5(0x0e)` 已保存 raw append 及角色索引 26「聖寇拉斯」／14「珊」，但沒有已證實的永久 JOIN 或戰後隊伍用途；因此 `postbattle_ch25_persist→town_ch26` 仍維持 blocked，`ch24_post.json` 的兩個 raw append 操作由編譯器失敗即關閉。
...
- [~] **UI-03 原版 command grid E2 blocker（2026-08-09）**：目前沒有未修改、可正常載入且已知戰鬥前／戰鬥中狀態的 `FD2.SAV`，也沒有可回放的進場按鍵時間線；不能從 title、CONTINUE 或 remake fixture 猜按鍵後宣稱原版指令格畫面。先保留 E0 layout／input contract 與重製端截圖，待取得合法有效存檔後再用 Docker `waitpixel`／逐步 `shot` 建立同狀態 oracle。
