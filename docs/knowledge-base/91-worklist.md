...
| 7 | 三平台打包與推廣片 | 最新`a5bbaf3f`已由鎖定映像重建AppImage與Windows ZIP。AppImage 5,306,872 bytes／SHA-256 `f085dc58...acadfb`，在Debian系及Ubuntu 24.04抵達`town_ch02`且跨映像AE=0。Windows ZIP 4,895,851 bytes／SHA-256 `931397d8...eb286`；Wine 9.0已接收城鎮輸入、進商店並以F5寫出可解析存檔。macOS run 32999934526另在真實`macos-14`成功完成雙架構、lipo、bundle、DMG／tar.gz，並從空白cwd執行bundle binary具型別驗證全部campaign轉場、36個唯一法術ID及所有story引用；舊`Contents/MacOS/assets`錯路徑與只抽首末章的弱檢查均已修正。尚未做實體Mac操作。全戰役長程遊玩由使用者人工進行 | 補Windows與macOS實體機視窗／輸入／存檔／音訊抽測；處理簽章／Gatekeeper；之後產最終版號封包與推廣片。Wine、Ubuntu容器與CI自我檢查均不取代真機 |
