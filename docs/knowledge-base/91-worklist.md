...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 多項正式E1維持；ID32／33／34／35已有受限class19玩家state owner。敵方17–19已確認由`0x15311`直接進effect table，不套玩家`0x1D6C8` palette；mode2與mode11 `0x1548E`物理入口現先完整預檢FIGANI／delay，再消耗RNG、HP與死亡獎勵，缺素材零交易 | 下一步關閉正常敵方回合實際可達的command0–12 caller-specific `0x15311→0x2A6BD`演出與提交順序；command29敵方owner缺producer／selector／resource證據，維持fail-closed |
