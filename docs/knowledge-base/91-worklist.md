...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0、玩家／敵方ID1／2／3／5／6／7／8／9均已有caller-specific indexed owner；玩家IDs10–12與受限class19玩家ID32–35亦達`RUNTIME-E1`。ID35正式owner已完整消費#68／#94共用段與26→22→27三個`0x22D1B`專用tail，並以三階段私有plan保護HP／raw／RNG／Acted。ID4無已證實正常玩家producer；28／30／31沒有已證實正常玩家取得來源，均不以猜測列阻擋。 | IDs32–35正式玩家owner已關閉，不重做；下一個玩家價值缺口依`58`轉向仍有正常producer的敵方caller-specific演出／commit順序，同狀態逐幀／逐音訊另屬E2 |
