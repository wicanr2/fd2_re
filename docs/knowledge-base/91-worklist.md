...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整 manifest 為6,677筆：5,657 exported、1,005 intentionally_raw、15 MIDI blocked。409筆FIGANI均有resource status：264 decoded、144 confirmed zero-header、#408 blocked；另有264份animation metadata、2,118張indexed frame PNG＋2,118張mask PNG，來源／frame可回查。raw零遺漏，但TAI／BG／UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；其餘raw逐家族判定已由標準輸出涵蓋、需新typed輸出或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`與全部DATO production consumers已改接分離loader；全部FIGANI資源具狀態契約，終局、通用攻擊、指令0–3／5–9／24／29／32–35及兩個zero-header fallback helper均只讀分離pack，正式FIGANI decoder caller為0；0／24／29／32有archive不可讀回歸。仍有16檔直接解碼FDOTHER內嵌動畫；title、FDTXT、BG、TAI、FDFIELD等家族也未全移除 | 先把FDOTHER內嵌effect輸出成同契約animation metadata並遷移16檔；再依玩家路徑處理BG／TAI／FDTXT／FDFIELD等家族。每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
