...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；ch02賣出名冊selection0/cycle1與索爾物品selection0現為route-patched正常商店輸入E2；教會轉職與商店四種正式 mutation→town→重製 JSON 存讀檔均已保存金幣、完整 typed／raw roster 與隊伍拓撲 | 依 `57` 逐個輸入狀態機取得同狀態證據；商店下一步為sell selection1／問句／成功、recipient scroll、no-recipient/full、equip/transfer child panel，不重做已閉合交易或 JSON persistence |
...
- [~] **UI-VIS-SHOP**：四個callee與secret selection+BIOS-scan gate皆已接production；ch02 variant1/3/5主選單、purchase list／Yes-No／不足金／recipient selection0↔1、success與debit的既有E2維持。新增賣出正常商店輸入：角色名冊selection0/cycle1及索爾物品selection0經人物相位同步後均與production完整320×200 RGB AE=0，證據見[`shop-sell-child-ch02-e2.json`](../data/ui-traces/shop-sell-child-ch02-e2.json)與[對照圖](../figures/shop-sell-child-ch02-original-vs-remake.png)。route patch與screenshot-only typed/raw bootstrap限制仍有效。下一gate為sell selection1／問句／成功、四人recipient scroll、no-recipient/full、equip/transfer child panel與其他章節。
