...
- DATA-READY：移動確認瞬間的原版契約已量到 5 毫秒粒度——游標由終點**瞬間**
  跳回單位所在格、鏡頭完全不動、之後逐格跟著單位走。重製端游標全程停在終點，
  且跨格會多發佈一幀 `motion = 0`。兩處都還沒改：`visible` 游標的寫入端未解出
  （它不等於 cursor 減 camera），依失敗即關閉先不憑推測寫進畫面組成。
