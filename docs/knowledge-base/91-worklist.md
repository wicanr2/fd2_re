...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；ch02賣出已從名冊／物品／Yes-No走到成功、向上金幣滾動與返回原名冊，九組新增route-patched原版／正式重製畫面AE=0；教會轉職與商店四種正式mutation→town→重製JSON存讀檔均已保存金幣、完整typed／raw roster與隊伍拓撲 | 依 `57` 逐個輸入狀態機取得同狀態證據；商店下一步為recipient scroll、no-recipient/full、equip/transfer child panel、其他章節與未修改一般玩家路徑，不重做已閉合賣出成功／加款／返回或JSON persistence |
