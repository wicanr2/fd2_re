...
| A4 | 現代美術主題 | `DATA-CONTRACT-READY／RUNTIME-E1-PARTIAL`：28 位身分已確認角色已有獨立閉嘴頭像母稿／80×80候選；三十四組 FDICON 由正式 loader 原子預檢 408 幀後取代正規化 sprite。2026-09-05 完成 `map0` 至 `map32` 共 33 張現代圖集：各自保留 `24×24` tile ID、地形／事件／碰撞座標，依第一關概念稿統一色彩與平滑材質；96／192／288／384 格四種實際幾何、map ID 與 SHA-256 均由 loader 原子驗證。現代主題禁止原版完整 indexed frame 回頭覆蓋現代圖層，公開總覽則由正式 map 資料重組。戰鬥 HUD 仍是 concept | 抽測早／中／晚期地圖的相鄰圖塊接縫、前景遮擋、人物、游標與方塊附近選單；再建立 FDOTHER HUD cell 基準與真彩 overlay。接著補齊其餘 62 組地圖人物與未登錄頭像 |
