...
- [~] **editable ending prefix timeline**：新增 `assets/endings/native_2bce5.json`，把已證實的 #054 blit、copy、delay、palette ramps、兩段 native composite loops 存成可編輯 IR。`0x2c39b` 第二 arg 現已依 `0x15f84` direct ABI 定案為 current-FDTXT string index；final-route else branch 的 indexes 2..7 已 count-align 到 `ch30.json` scene1 lines0..13。第一 arg 只證實為 archive `0x51a70` resource index，故中性保存、不猜成 portrait。`internal/ending` 仍只接受 `recovered_prefix_only_fail_closed`，絕不將它視為可播放 ending；chapter26 branch、buffer/palette helpers、ANI/戰鬥段落 bridge 仍是明確 gate。
