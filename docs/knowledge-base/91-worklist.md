...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 多項正式E1維持；ID33／34／35已有受限class19玩家state owner：selector4／5／6／7／20只作52／28／36 MP gate不扣款；33清raw `+0x25..+0x27`再固定`0x320`回復，34的17→18→19與35的26→22→27私有stages全成才發布，三者正式grid→confirm已驗證。這不包含`0x27FC9`演出／score／EXP。ID32仍關閉 | 下一步補ID32的`0x2111A→0x1C75E`直接差異證據，或取得`0x27FC9` indexed presentation／一般玩家E2；command29敵方owner缺selector／resource證據，維持fail-closed，不得把AI套入玩家owner |
...
1. **戰鬥交易完整性**：物品 type 5–24與ID33／34／35受限玩家state transaction不再列數值缺口；狀態到期owner與raw indicators已接。command28／31無已證實一般玩家取得來源，不列阻擋。仍須關閉ID32、其他未知法術／複合效果、狀態高階名稱，以及代表性玩家／敵方回合的精確演出順序。
