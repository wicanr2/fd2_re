...
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT、字型、共用道具panel、FDICON、FDSHAP與ANI正式玩家consumer已遷移；第23戰FDFIELD#69與終局#90..92均可逐byte重建。天空之鑰#34完整101格、終局#54完整111格、#56..#60與#5對話格，以及商店、城鎮、標題、LOAD四槽、整備#1／#2／#5、共用#5 dialogue／digits及教會／轉職#14正式consumer均已改用分離pack；#34／#54逐格pixel／mask／placement oracle一致，兩個正式owner均移除archive path。標題不再讀舊`assets/title/*.png`，標題／第20戰／結局也不再讀`ANI.DAT`。2026-08-29將18條FDOTHER#0 palette owner統一改讀分離JSON，並將UI #31、command29 #50、command24 #53、共用#80、command 0..8／敵方9、commands32–35與增援#95共17個巢狀音效bank改讀分離OGG；正式`ReadResource(...,0)`及`ReadNestedResource`均歸零 | 下一批整合map初始化#1／#3／#5／#6／#9／#55與一般物理攻擊動態bank；保留archive不可讀與缺分離資產失敗即關閉測試 |
