...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；穩定標題主選單與320×200原版 oracle 達整幀AE=0。完整開場的舊串列已由 `sub_1F894` canonical證據取代：535列捲動、六個插播點、八段AFM與兩張靜態幕現在由正式runtime交錯播放。2026-08-27再接AFM 3前的漢堂發行商畫面；runtime第60幀與`FDOTHER #74 + #76` oracle達640×400整幀AE0，119幀後續排程仍於frame1884自然抵達AE0選單。舊`dosbox_seq/frame_000`實為AFM 3而非發行商標誌的斷言已勘誤；標題只剩兩段wipe／logo揭示、精確音訊與原版runtime E2。ch02賣出成功／加款／返回已有九組AE=0。獨立裝備service2與物品轉移service3也都有正常route-patched子面板證據；service3更已完成索爾短劍→悠妮、返回loop、雙方交易後物品清單及跨`town_ch02`重製JSON冷讀檔。物品清單舊記AE=2已定位為店員背景動畫`(175,90)/(176,90)`兩點相位差，靜態清單內容在該狀態一致；其他可見內容／幾何一致，但動畫相位未同步，只到partial E2。service2交易已改為私有 unit→raw裝備／重算→完整panel重建→一次發布，palette深層失敗保持roster／能力／panel原子不變，達`RUNTIME-E1`；目前原版CONTINUE快照只進戰場名冊／狀態面板，故mutation／restore E2未冒稱完成。service3現由正式menu Right×3進入同一typed input consumer，逐Draw驗證empty返回、full原子拒絕，以及目的取消後重入self-transfer，不再由測試直接改mode或呼叫transaction helper，達production-input `RUNTIME-E1`。標題 LOAD 的正式確認 owner 現以 checksum-valid 合成原版槽還原 `town_ch02`／typed party／gold／HUD gate，竄改 envelope 原子留在選槽，亦為`RUNTIME-E1`。外部晚期有效槽又由未修改原版普通LOAD走過save-NO、19人整備與戰前劇情至可操作第30戰，達候選E2；重製端同一固定槽的正式LOAD初始整備已凍結相位0，並依IDA直接指令分開FDOTHER #5 entries 31..40／42..51兩套數字字形，同狀態整幀達AE=0/64000。裝備收件者另以六人typed party沿正式menu→purchase→Yes production input完成三列scroll、滿欄／無合適角色原子返回與正常裝備／扣款`RUNTIME-E1`。`ch24_post`兩筆文字 lookup 的18句上下框、`FFFE/FFFD` pages、原版indexed五階段開框／頭像／字模、stable-page、逐字、完整頁等待期m0/m3嘴型與`sub_16B43`保存畫面收框，以及正式town26／祕密商店runner均達caller-specific `RUNTIME-E1`。 | ch24只剩未修改同狀態E2；其他故事caller仍需各自binding。穩定標題選單與發行商畫面、整備固定初始狀態均已達100%同狀態像素，不再追DOS逐週期差異；商店下一步是service3 empty／full與self／destination-cancel原版畫面；service2尚需原版mutation／restore畫面。第29戰勝利writer建槽來源降為證據限制，不再阻擋remake。 |
...
| 7 | 三平台打包與推廣片 | Linux AppImage現由雜湊鎖定工具映像與單一無網路、指定UID/GID容器產生；實際封包已由空工作目錄配唯讀XDG玩家資產抵達`town_ch02`並驗證畫面、格式、擁有權及SHA-256。Windows x86_64同樣由單一無網路容器產生GUI PE與可公開資產ZIP並完成格式／內容／雜湊檢查。macOS workflow已改產真正`.icns`，但尚無執行紀錄，派送受失效GitHub權杖阻擋。全戰役長程遊玩由使用者人工進行 | 補Windows真機啟動／輸入／存檔／音訊抽測；修復GitHub驗證後執行macOS CI並做相同抽測；抽測Linux其他發行版。三平台成功後再產最終版號封包與推廣片 |
