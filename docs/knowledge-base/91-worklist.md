...
- [x] **finale `0x2c548` first party-cycle map**：Docker Capstone 切出三個 native buffers（128000、64000、64000）、TAI#3 platform 與 FDOTHER#56 backdrop；loop index 從 `[0x53bfb]-1` 向下，但必做 `i=0→slot1、i=1→slot0` swap，才以 unit stride80／visual group `+7` 載 `FIGANI group*3+1` 與 `group*3`。`0x29164` 後先有 `0x2b9a1` 的20×1ms loop，再跑 primary FIGANI descriptor frames。已入 `assets/endings/native_2c548.json`，但 DATO/text/input 與 dedicated indexed renderer 未解，保持 fail-closed。
- [x] **finale party portrait/text map**：DATO=`unit+7`；FDTXT_031 的 #10/#11/ending epilogue 與 FDTXT_000 的角色名／職業名，五個 destination 與 CD/4C glyph style 均已直接對齊 `0x2c7ed..0x2c967`。`Montage.PlanPartyCycle` 現將 native slot swap 與兩組 FIGANI index 資料化並 regression；DATO countdown/anchor、special epilogue slot 與 native indexed renderer 尚未可執行，仍 fail-closed。
