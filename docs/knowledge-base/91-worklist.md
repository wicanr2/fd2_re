...
| 2 | 玩家第21戰天空之鑰固定演出 | `RE-CLOSED`、`DATA-READY`、`RUNTIME-E1`：`0x2415B`的三張25-byte表與special slot已接26-slot layout；ACT63／64及`0x24336`的真實`FDOTHER #34`、`ANI #0`、調色盤循環均由正式戰後路徑依序消費，再進城鎮與存讀檔 | 只取得未修改原版同狀態 E2並核對第一個動態調色盤相位；不重解函式本體、layout或ACT resources |
...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；故事對話現接十六個切片。新增`ch23_post`11句：86-slot正式輸入保持跨scene映射、stage2..14的BIOS tick／DAC演出、同步與`preparation_ch25`存讀檔；並修正舊fixture把86筆selector批次重複成172筆。 | 十六個已接故事切片只剩未修改同狀態E2；目前沒有canonical RE充分且可直接生成的未接caller。下一個故事切片必須先替`ch18_post`或`ch20_post`補caller／consumer canonical evidence，再寫規格與runtime；第29戰已結案。 |
