...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；教會轉職正式 mutation→town→重製 JSON 存讀檔已補完整 typed roster／隊伍拓撲回歸，並修正讀檔殘留教會暫態 | 依 `57` 逐個輸入狀態機取得同狀態原版／重製證據；下一個可實作存檔切片是商店裝備／轉移→town→JSON save/load，不用單張 layout 圖代替 |
