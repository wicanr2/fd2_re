...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1的#19/#21每目標31張、八個HP marker、九張boundary與共同tail已由正式owner接入玩家／敵方。ID2的單frame/repeat helper、29張front、每target12張／六HP marker、每boundary九張、10張handler tail、共同tail與#83 sample0..3亦已由同一正式owner接入玩家／敵方。ID6的#32/#33/#87、7張front＋每target12張＋九張boundary＋七張tail同樣完成。ID5的#24/#25/#86、六通道跨目標持續狀態、1張front、每target12張、每boundary9張、8張handler tail與共同4張tail現亦由玩家／敵方正式owner消費。四者的MP、HP、`Acted`與數值RNG皆依Draw發布並可整批回復，達indexed `RUNTIME-E1`；ID5跨target process-wide RNG交錯保留明示近似 | 依正常producer頻度處理ID4／7／3；ID1／2／5／6只留明示的音訊／RNG近似與未修改原版同狀態E2，不重做已閉合handler |
