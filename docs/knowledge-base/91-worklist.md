...
| 5 | 戰場與戰間 UI 收尾 | UI-03、UI-07～12 仍 partial；ch02賣出成功／加款／返回已有九組AE=0。獨立裝備service2亦有正常route-patched名冊／索爾面板證據，但動畫相位未同步，只到partial E2。裝備收件者另以六人typed party沿正式menu→purchase→Yes production input完成三列scroll、滿欄／無合適角色原子返回與正常裝備／扣款`RUNTIME-E1`；教會轉職與商店四種mutation→town→重製JSON存讀檔也已保存完整狀態 | 依 `57` 取得原版同狀態證據；商店下一步同步service2動畫相位並驗mutation／restore，或做transfer child panel；recipient scroll／no-recipient／full、其他章節與未修改一般玩家路徑仍缺，不重做已閉合重製端input、賣出成功或JSON persistence |
