...
| A4 | 現代美術主題 | `PROTOTYPE-SET／DATA-CONTRACT-READY`：索爾頭像、第一關戰場與戰鬥HUD三張style A本機私有稿均已產生；受版控`themes/modern/catalog.json`保存穩定asset ID、來源、尺寸與SHA-256，本機嚴格validator逐檔通過。戰場稿保留第一關隊形與地形關係；HUD稿保留功能選單貼近選取格、狀態板在左下的操作契約。三張仍是concept，不冒稱可直接替換的sprite／tileset／九宮格 | 由使用者檢視整組style A；方向保留後，分別以原始portrait、fig_068十二幀、map0 tile ID與FDOTHER HUD cells作獨立重繪基準，不能從合成概念圖反切。完成縮放／透明邊界／runtime合成驗收後才升為`runtime_ready`並批量生產 |
