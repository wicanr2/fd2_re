...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL`：現行完整 manifest 為7,539筆：6,519 exported、1,005 intentionally_raw、15 MIDI blocked。409筆FIGANI均有resource status；264組FIGANI共2,118幀，另23組FDOTHER內嵌動畫共408幀，全部輸出indexed PNG＋mask＋metadata且來源可回查。raw零遺漏，但TAI／BG／UI／AFM用途與OGG仍未全量閉合 | 將15首MIDI轉成具cue／loop metadata的OGG；其餘raw逐家族判定已由標準輸出涵蓋、需新typed輸出或blocked，補齊用途關聯，輸出包不入Git |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：`FDOTHER #0/#2`與全部DATO production consumers已改接分離loader；全部FIGANI資源具狀態契約，終局、通用攻擊、指令0–3／5–9／24／29／32–35的FIGANI及23組FDOTHER effect均只讀分離pack，正式`figani.DecodeResource` caller為0。0／24／29／32有FIGANI archive不可讀回歸，FDOTHER#18與archive逐幀一致。title、FDTXT、BG、TAI、FDFIELD及FDOTHER palette／sound／其他UI家族仍未全移除 | 依玩家路徑處理BG／TAI／FDTXT／FDFIELD與FDOTHER palette／sound／UI家族；每批保留archive不可讀與缺分離資產失敗即關閉測試，最後production archive decoder caller歸零 |
