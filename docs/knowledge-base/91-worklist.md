...
| A2 | 原版素材全量分離與清冊 | `DATA-PARTIAL／LEDGER-DATA-READY`：manifest v2現有39,818筆asset與1,005筆source-resource ledger；899 standardized、11個零長度confirmed-empty、0 blocked、95 unknown。FDSHAP control、終局FDFIELD #91/#92、map23 FDFIELD #69、FDOTHER #1／#6完整bank、標題FDOTHER #77音效／#102短暫調色盤及15首音樂已建立可驗證關聯；可版控摘要綁定完整manifest SHA-256。95筆unknown分布為FDFIELD 79、FDMUS 5、FDOTHER 11，不等同95個decoder缺口 | 將95筆unknown與production consumer及既有runtime標準資料交叉比對；FDOTHER #47／#49尚無可證實正式consumer，維持unknown，不為減數量猜接；改選有玩家consumer的下一筆跨容器缺口 |
| A3 | runtime 移除 `.DAT` 即時讀取 | `RUNTIME-E1-PARTIAL`：FDTXT、字型、共用道具panel、FDICON、FDSHAP、ANI、FDFIELD#69／#90..92、天空之鑰、終局、商店／城鎮／標題／LOAD／整備／教會及戰場初始化正式玩家consumer已遷移。map初始化#1 range、#3 LUT、#5 HUD與完整bank、#6完整bank、#9增援與map28／29 #55只讀分離pack；一般與晚期地圖在原版FDOTHER不可讀時通過。19條palette owner、18個巢狀音效bank與FM／MT-32 BGM亦已分離；標題捲動現依14筆原版列值切換#102並於11列後回復#101，主選單與LOAD槽位消費原版#77 selector2／1。BGM只從完整驗證的30份OGG catalog解析，不再靜默fallback。普通玩家與敵方物理攻擊現在均在交易前由同一分離FIGANI provider原子載入attack／idle pair；缺pair零狀態修改。相關direct archive reader與過期原檔存在gate歸零 | 以version 2 ledger的95筆unknown交叉核對production caller；已具runtime標準資料但缺manifest bridge者先補provenance，不重做decoder。只有確定仍有正式archive consumer且沒有標準輸出，才開新RE→spec→runtime切片 |
