...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 底層 RE 與多個窄 `RUNTIME-E1` 已有；共用 `0x117E7` 空游標 `0x16F55` END 已接原版6＋4展開、YES／NO、兩段逐字形回應、4＋5收合與來源復原；同一存檔問句畫面的肖像／文字子區域已與原版相同，整幀仍受選項與戰場動畫相位影響。command 13–16 的玩家與敵方 mode 11 已走完整 #3前段、#6七張、mask、交易後 redraw、#5數字22張與尾停。command 17–19 已由玩家grid與敵方raw selector接到原子modifier交易，正確區分ID17由record18扣MP；玩家依`0x1D6C8`播放#88 sub0與八個command-color／black DAC phases，第八幀Draw確認後才發布交易 | 補 END接受／取消／收合及command 13–16逐幀逐音訊 E2與精確tick；command 17–19下一缺口是phase-expiry正式caller、status UI與同狀態逐幀／逐音訊E2；其他family仍須先有原版owner證據 |
