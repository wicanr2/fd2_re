...
| 4 | 玩家指令／法術／物品與敵方 AI 完整交易 | 敵方ID0已接完整presenter；ID1修正後的#19/#21每目標31張、八個HP與八個sample1 marker、九幀boundary及common tail現已由正式Game owner接入玩家與敵方，達indexed `RUNTIME-E1`。ID2的`0x2673F`已閉合為單frame/repeat helper，29張front、每target12張／六HP marker、每boundary九張、10張tail及#83 sample1/2/3已完整`DATA-READY`。ID6的#32/#33/#87、`fcos/fsin`座標、7張前導＋每目標12張＋相鄰目標9張過場＋7張尾段，亦已由正式Game owner接入玩家與敵方，達indexed `RUNTIME-E1` | ID2下一步直接建立全批次Game job並接玩家／敵方continuation，不再重解`0x2673F`。其後依正常producer頻度處理ID5／4／7／3。ID1／6只留音訊與同狀態E2 |
