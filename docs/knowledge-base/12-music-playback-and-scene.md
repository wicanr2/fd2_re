...
- 全 32 處 `play_bgm` 呼叫中,`track=0x12`(字面 push,非查表動態值)只出現在**兩處**:
  上述 boot 呼叫(`0x025db5`),以及結局 caller `0x02c1f5`（詳見下方勘誤）。
  **沒有任何一個店鋪/城鎮場景的呼叫點 push 18**——動態查表呼叫(依章節取曲,呼叫點見
  ...
  `{0x01,0x06,0x08,0x0c}` 亦無 `0x12`。因此 track18 的已證實 caller 是 boot／標題
  與結局 `0x2c1f5`，沒有任何一條路徑會在「進商店」時 push 18。
...
### 2026-08-10 結局音訊勘誤（IDA Pro 9.4）

此前以 `0x2bcea` 作為函式名稱、並宣稱「呼叫端未找到」的段落已被新函式
邊界與交叉引用取代，不能再當成現況斷言。IDA Pro 證實 `sub_2BCE5`
（`0x2bce5..0x2c39b`）在 `0x2c18f` 呼叫 `sub_2C405`
（`0x2c405..0x2c9ec`），因此三個結局音訊事件分屬兩個明確 caller：

| 位址 | 原始呼叫 | 資源／時機 |
|---|---|---|
| `0x2c5cf` | `0x25977(4,0)` | `FDMUS_004`；`0x2c405` phase-0 後、party cycle 前 |
| `0x2c1ac` | `0x25977(-1,1)` | 停曲；`0x2c405` 返回後、`FDOTHER#60` 前 |
| `0x2c1f5` | `0x25977(18,0)` | `FDMUS_018`；`FDOTHER#60` 畫面後、尾段循環前 |

這只證實原始位址與順序，不把 `FDMUS_004`／`FDMUS_018` 改名成標題、城鎮或
商店曲。可回查的完整 IDA 證據與重製資料契約見
[`fd2_ending_audio_ida.txt`](../data/ida/fd2_ending_audio_ida.txt) 及
`remake/assets/endings/native_2bce5.json` 的 `audio_cues`。

### 給 remake 的結論值

開場／標題的 `FDMUS_018` 結論仍只由 boot caller 的 `0x25db5` 支持；本節新增
的 `FDMUS_018` 是結局 `0x2c1f5` 的另一個已證實消費端，不能互相覆蓋。
`FDMUS_004` 現在有結局 `0x2c5cf` 的直接證據；一般玩家結局仍在
`0x2c548` montage owner 邊界失敗即關閉，未宣稱已可播放。
