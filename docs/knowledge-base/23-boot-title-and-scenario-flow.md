...
| 變數 | 含意 | 寫入點 | 讀取/調度 |
|---|---|---|---|
| `[0x53c03]` | **目前章節(核心 game-state)** | 新遊戲=0(0x25ed6)、讀檔=存檔值(0x10147/0x26067)、戰後+1(0x25855/0x22f2c)、cutscene 暫借(0x32326) | 章節跳表 0x51d71/0x51de9、BGM 表 0x51e63、戰場公式 0x10b9e、完整章節 loader 0x1088d |
| `[0x53c57]` | 選單目前選擇 index(各選單共用,進選單即歸 0) | 0x16f83/0x172ba/0x2600c | `cmp [0x53c57],0/1/2/3` 串接分派 |
| `[0x53ecc]` | 章節事件 handler↔戰役迴圈的 raw pending 碼；0=無 pending，1／2 的玩家可見名稱依外層路徑另證 | 章節事件 handler(0x51b19→0x205xx,28 處設 1/2,見 doc 25/26);戰役迴圈清 0 | 1→固定 `0x22e5c` 資源 #79 呈現；2→章節索引戰後表與 `0x2cad7` gate。完整控制流見 doc 24 §6 |
...
而 doc23 §2.1/§2.2 描述、以 FDOTHER #0x45–0x49 五幀直疊還原出的「藍紅色調肌肉立繪」,視覺特徵(藍/紅交錯尖爪紋 + 兩側淺灰色蜿蜒邊緣)與上述幀序中**反覆穿插出現的背景紋理**(几乎每個故事分鏡之間都會插入,例如 frame_015–026、060–076、108–123、132–143)高度吻合——**但它不是「一次性由下往上捲過整個畫面」的單一事件,而是貫穿整段開場的背景 layer**,不同分鏡會以不同 palette 呈現。完整分鏡來源現已由 doc39 §6 對應：ANI.DAT 驅動①②③④⑤⑦⑧，**item⑨ 惡魔臉是例外**，它是 §2.1 的 FDOTHER #0x45–0x49 捲動立繪抵達頂端後露出的頭部，見 §2.4⑦。不得再保留「ANI.DAT 尚未拆／另有未 RE 資料來源」的舊推測。
...
2026-07-29 以合法 IDA Pro 9.4 重核 read side：`0x2602c..0x26056`
先把選中 record 的完整 `0xa00` roster 複製至 `[0x53bf7]`，
`0x2605e..0x26098` 才載入 metadata `+0..+9`；chapter `0xff` 會回到
selector。非空槽關框後先進 `0x2cad7`，只有 gate 回 0 才由 chapter
索引 `funcs_25e3a` pre-handler。四槽路徑不呼叫 `0x10010`。
`fdsave.InspectChapterSlot` 已接 32×`0x50` raw records、完整 opaque
metadata、空槽及 count 超容量的失敗即關閉；它尚未把 record 猜成
normalized `battle.Unit`。可重現證據見
...
章節0 處理器 0x3231b = 開場序章 cutscene  ←★「與前代主角對話」在此
   ...
   ├ push 0x5a..0x69; call 0x1366a (逐段)             ;★ 演出資源(王城/草地場次)
...
> - `0x1366a(id)` 不是「逐段播對白」——是**演出(acting)播放器**；資料格式與 normal/special
>   位移語意以 **doc50 §1.2（唯一主檔）** 為準。
...
### 戰後流向 [部分驗]
raw pending 碼 2 → 章節索引戰後表 `0x51de9[章節]` →
`0x2cad7` gate；只有回傳零時才走第二張章節表 `0x51d71[章節]`、選擇後續
BGM 並恢復 driver。raw pending 碼 1 則固定呼叫 `0x22e5c` 的資源 #79
呈現後清碼；舊稱「世界地圖／中場」及「一定是下一章」均已撤回。各章
town/shop/preparation/ending 與完整勝敗分歧仍待逐條 E0／E2 驗證。
...
| `0x51de9` | **章節→phase-2 戰後處理器跳表**(ch0=0x22ef6) | [驗] |
