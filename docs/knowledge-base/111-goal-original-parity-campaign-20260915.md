...
**亂數的比法**：`0x4E893` 的唯一狀態字組 `0x627B8` 是行程級的（開機為 0、不進
存檔），從標題畫面起就被對白嘴型、戰鬥演出等以 tick 為單位消耗，兩側不可能逐
tick 對齊。所以 oracle 把每個控制邊界的字組寫進 `view.rng_word`，重製端重播在
每個會擲骰的決定點（攻擊確認、END 換手）同步該值，行為 gate 比的是「同一個字組
下的結果」。同步點之間的漂移（敵方回合多次攻擊、演出消耗）由 verifier 分開列成
`between_sync_drift`；它一樣算失敗，但歸因時要先看是消耗次數不同還是公式不同。
