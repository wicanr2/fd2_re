...
| 敵方 AI:**施法決策**(法師/僧侶主動用攻擊術/補血術) | direct disasm 已證實：`0x15688` 枚舉 command，`0x1579A–0x157B5` 對 `command>0x0F` 以 `spell_id=command-0x10` 評分；`0x150D3–0x150F1` 執行同一 spell command，`0x15168→0x28784` 播放施法演出 | ❌ | `aiActUnit`/`NextAIPlan` 尚未保存或執行 SpellID，敵方法師/僧侶在 remake 仍恆為純物理攻擊 | 高；需接 command inventory、法術目標／效果優先級 |
