...
| 物理攻擊:**命中率 (HIT−EV)%** | doc02 §4.1 | 🟡已補(HIT/EV 來源／戰鬥全鏈仍部分) | `model.go EffectiveHIT/EffectiveEV` + `combat.go rollsHitPct`。早期敵／友單位匯出仍用 `DEFAULT_HIT=90/DEFAULT_EV=5`，因為 doc03 已記錄它們是衍生值，不是單位 10B 表的原始欄位。2026-08-22 勘誤：remake 已有 `Inventory`/`Equipped`/`InventorySlots`、`EquipItem`、`RecomputeEquipment` 與 shop equip owner，不可再說「尚無裝備系統」；真正尚缺的是完整 native HIT/EV 基礎來源與 battle integration。 | 中（補 native stat source 與 battle integration） |
| 物理攻擊:**傷害隨機化(0.9×max~max−1)** | doc02 §4.1 | ✅ | `combat.go AttackWithRNG` 呼叫 `magic.go randomizeAmount`(與法術共用同一公式) | — |
| native IDs24/28/29/31 derived strike | SDD56 UI-03 | 🟡 strict state-only | `ExecuteNativeCommand24`／`ExecuteNativeCommandDerivedStrike` 已依 `0x276EC` 寫 final HP delta。ID24 的 selector32 resource98、FDOTHER #53 samples3／2與 denominator1 已閉合；舊「等份多段傷害」撤回。正式 indexed presenter／完整背景與E2仍待；legacy `CastArea` 不是證據 | 高 |
...
| native IDs17–19 modifier | SDD56 UI-03 | 🟡 玩家／AI窄E1 | 已驗 `+0x22..+0x24` raw writer／duration；玩家正式grid現依`0x1D6C8`播放#80 selector0與八個DAC phases後才原子發布transaction，AI mode11直接消費transaction。`ApplyNativeRuntimeEquipmentRecalc`保存`0x1B750`精確重算；仍缺phase-expiry正式caller、status UI與同狀態逐音訊E2，不能把legacy Buff視為同一機制 | 高 |
| native IDs20–22、25–27 clear/application | SDD56 UI-03 | 🟡 玩家窄E1 | 六個ID均有raw clear/application executor及玩家`0x1D6C8` #80 selector0／八DAC phase正式入口，完整preflight後才交易；ID25已更正為只清raw `+5 bit7`，不混用`Acted`。status name、expiry UI、完整tick與同狀態逐音訊E2未閉合 | 高 |
| native ID23 relocation | SDD56 UI-03 | 🟡 玩家窄E1 | first-target、mode6 destination legality、`0x1D6C8` 八段色盤及兩次 `0x22253` 離場／入場均已接；完整預建成功後才發布演出，第二段結束才原子扣MP、寫座標與完成action。仍缺原版同狀態逐幀／逐音訊E2及精確鏡頭核對 | 中 |
| native IDs32–35 compound | SDD56 UI-03 | 🟡 | ID33／34已依class19可達selector接成玩家原子state transaction：33清raw `+0x25..+0x27`後以固定`0x320`回復，34依17→18→19順序發布；不扣MP只限已證實繞過debit sink的group4／5／6／7／20。ID32／35、`0x27FC9` indexed UI/SFX與E2仍未閉合；禁止以legacy combo外推 | 高 |
| 經驗值公式(攻擊/恢復/各系術) | doc02 §4.5 | 🟡 normalized approximation | legacy `growth.go`／`CastArea` 的獎勵不證明 native command 逐 ID 的 EXP route；ID34雖已有state transaction，EXP owner仍未知，IDs22／32／33／35亦未完整閉合 | 高 |
...
| 商店(一般商品) | `0x2e341→0x2f0b0/0x2f642/0x2f883/0x2f8ea`、doc56 UI-09 | 🟡 | purchase、sell、standalone equip與transfer四條production縱切已閉合。ch02主選單、購買清單／問句／不足金／收件者及賣出名冊／物品／問句已有多組全幀DOSBox E2；賣出又沿正常商店輸入閉合成功、`0x2D3FF`向上加款與返回原名冊九組AE=0。service2名冊／面板與service3五個穩定子面板也有route-patched partial E2，內容與幾何一致但動畫相位未同步。該party E2仍使用screenshot-only bootstrap。正常campaign另有JOIN→LOADCH首次typed roster與四種mutation→town→JSON save/load E1；這不是原版FD2.SAV或完整playthrough E2。transfer保存512/511/510/506訊息、source/item/destination/full loop、raw remove→append→source recalc與self-transfer。custom variant0保留generic fallback | 高；補recipient scroll、no-recipient/full、service2動畫相位與mutation／restore、service3 mutation／empty／full、其他章節同狀態E2，以及未修改一般玩家路徑 |
...
| 商店賣出(原價 75 折) | doc02 §4.6、`0x2F642..0x2F87C`、`0x2D3FF` | 🟡 | 正式indexed owner已接角色／物品／Yes-No、price×3/4、成功演出、向上金幣滾動、背包移除／重算與返回名冊。ch02 route-patched原版已有九組新增畫面AE=0，重製JSON亦保存交易；不是只剩normalized `SellSlot`。仍缺原版存檔、其他章節與未修改campaign E2 | 中；補剩餘玩家路徑，不重做已閉合callee與credit排程 |
