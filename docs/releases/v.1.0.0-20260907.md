# v.1.0.0-20260907 失敗建置標籤紀錄

此標籤的三平台工作流程誤把本機忽略的私人現代素材當成公開 clone 可見輸入，
因此 Linux、Windows 與 macOS 建置均失敗，未建立 GitHub Release，也不是可下載
版本。依不可移動標籤規則保留此紀錄；修正後的正式交付是
[`v.1.0.1-20260907`](v.1.0.1-20260907.md)。以下原規劃只作失敗原因追溯。
...
- 安裝現代素材包後，`F2` 在忠實原版與現代手繪主題間切換，不重設單位、回合、
  事件、鏡頭或存檔；公開引擎包未附私人逐幀素材，缺件時保持原主題並顯示原因。
