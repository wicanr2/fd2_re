...
class GenerateManifestTest(unittest.TestCase):
    def make_fixture(self, root: Path) -> tuple[Path, Path, Path]:
        ...
        (pack / "ui" / "fdother_010_town_label").mkdir(parents=True)
        (pack / "ui" / "fdother_010_town_label" / "frame.png").write_bytes(b"town-label")
        (pack / "ui" / "fdother_010_town_label" / "resource.json").write_text("{}", encoding="utf-8")
        ...
        (pack / "sfx" / "FDOTHER_082").mkdir(parents=True)
        (pack / "sfx" / "FDOTHER_082" / "resource.json").write_text("{}", encoding="utf-8")
        (pack / "sfx" / "FDOTHER_082" / "sample_001.ogg").write_bytes(b"OggS")
    ...
    def test_stable_assets_and_explicit_raw_inventory(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            town_label = next(item for item in manifest["assets"] if item["path"] == "ui/fdother_010_town_label/frame.png")
            self.assertEqual((town_label["source_file"], town_label["source_resource"]), ("FDOTHER.DAT", 10))
            ...
            sound_metadata = next(item for item in manifest["assets"] if item["path"] == "sfx/FDOTHER_082/resource.json")
            self.assertEqual((sound_metadata["kind"], sound_metadata["source_file"], sound_metadata["source_resource"]), ("metadata", "FDOTHER.DAT", 82))
            sound = next(item for item in manifest["assets"] if item["path"] == "sfx/FDOTHER_082/sample_001.ogg")
            self.assertEqual((sound["kind"], sound["source_file"], sound["source_resource"], sound["source_frame"]), ("sfx", "FDOTHER.DAT", 82, 1))
