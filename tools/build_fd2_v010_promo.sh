...
for path in "$live" "$original" "$comparison"; do
...
ffmpeg -hide_banner -loglevel warning -y -filter_threads 1 \
  ...
  -ss 8.5 -t 30.5 -i dist/promo/fd2-v0.1.0-live-gameplay.mp4 \
  ...
  -f lavfi -t 71.5 -i sine=frequency=110:sample_rate=48000 \
  -f lavfi -t 71.5 -i sine=frequency=165:sample_rate=48000 \
  -filter_complex "
    ...
    [3:v]scale=1280:720:force_original_aspect_ratio=decrease:flags=neighbor,pad=1280:720:(ow-iw)/2:(oh-ih)/2:0x081018,
         drawbox=x=0:y=654:w=1280:h=66:color=black@0.8:t=fill,
         drawtext=fontfile=${font}:text=第一關同狀態範圍比較　原版／重製／差異:fontcolor=white:fontsize=30:x=(w-text_w)/2:y=670[v3];
    [4:v]drawtext=fontfile=${font}:text=我們完成了什麼？:fontcolor=0xE7C35A:fontsize=45:x=(w-text_w)/2:y=75,
         ...
         drawtext=fontfile=${font}:text=Linux／Windows／macOS　四語基礎與外部社群語言包架構:fontcolor=white:fontsize=27:x=95:y=270:enable=between(t\,10\,15)[v4];
    [5:v]drawtext=fontfile=${font}:text=第一個公開版本現已釋出:fontcolor=white:fontsize=43:x=(w-text_w)/2:y=195,
         drawtext=fontfile=${font}:text=玩家需自備合法的《炎龍騎士團 2》原版資料:fontcolor=0xE7C35A:fontsize=30:x=(w-text_w)/2:y=300,
         drawtext=fontfile=${font}:text=非官方文化保存與潔淨室重製專案:fontcolor=white:fontsize=27:x=(w-text_w)/2:y=370[v5];
    [v0][v1][v2][v3][v4][v5]concat=n=6:v=1:a=0,format=yuv420p[v];
    [6:a][7:a]amix=inputs=2:normalize=0,volume=0.22,
         afade=t=in:st=0:d=2,afade=t=out:st=68.5:d=3[a]
