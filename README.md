...
| 反向工程 | 資產格式與多個底層原語已高度閉合；24 個標準戰後節點已全部接入正式執行期，戰鬥規則、敵方 AI、戰間服務、存檔與終局也都有具位址的窄切片 | 目前不能誠實換算成整支 EXE 百分比；已接入不等於已完成一般玩家 E2，完整指令／法術／物品交易、終局與長程驗收仍未閉合，詳見[反組譯覆蓋矩陣](docs/knowledge-base/58-fd2-exe-re-coverage.md) |
...
> **評估快照：2026-08-23。** 本節依目前正式執行期、介面證據矩陣與一般玩家
...
換句話說，現階段已經不是只有技術展示，但也還不能當成可從頭到尾穩定遊玩的
完整遊戲；剩餘工作的重心是把已完成的個別切片串成不中斷的正常玩家流程，補齊
戰場介面差異、長程存讀檔與結局驗收，最後才進入三平台打包與推廣影片製作。

這個估值不是依據已解出的位址數量，而是依據玩家交付門檻。截至本次快照，
最大的風險已不是「完全不知道原版在做什麼」，而是下列已知缺口尚未串成可驗收的完整遊戲：

- 截於局部節點的30戰戰役驗證，尚未完成一輪不靠直接進場或強制勝利的長程通關。
...
- 戰鬥指令、法術、物品與敵方人工智慧已有多條正式 E1；`0x20C6F` 已恢復的物品 type 5–24 數值交易均已接通。複合指令33／34也已由正式指令格接到受限class19玩家交易：33清除raw `+0x25..+0x27`後以固定`0x320`回復HP，34依17→18→19三段發布；兩者只在已證實BattleFig來源開放，MP目前只作原版可用性門檻。`0x1A866` 的 selector 1→0／2 狀態倒數、歸零重算與 FDTXT 481–486／DATO indexed 到期提示也已接到正式階段 owner；角色狀態面板亦會依 raw `+0x22..+0x24` 切換能力數字顏色，並為 `+0x25..+0x27` 顯示原版圖示。仍缺指令32／35、`0x27FC9` indexed演出、狀態高階名稱，以及精確演出／音訊驗證。
- 原版終局蒙太奇與20組尾段已可在重製端播放並停留，但第30戰正常玩家路徑、原始狀態連續性、精確音訊與輸入尚未完成 E2。
- 戰場排版與動畫仍有玩家可見差異；尚需同狀態原版／重製擷圖、一輪長程試玩，再進入 Linux、Windows 與 macOS 打包。
...
| 完成門檻 | 自我評估 | 關閉前仍必須做到 |
|---|---|---|
| 30戰戰役可達與戰間流程 | 部分完成 | 由正常玩家路徑逐段驗證戰後→城鎮／商店／整備→下一戰，不能只靠直接節點或測試投影 |
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | command 17–23、25–27玩家路徑與17–19敵方路徑已有 E1；command0 已由正式游標確認接到原版索引演出與原子數值交易，包含角色滑入、施術者效果、28幀／7元素錯開的目標效果、七段 HP 發布與尾段合成，達 `RUNTIME-E1`；command23包含目的地游標、色盤、離場／入場與原子交易；command24的原始學習鏈、FIGANI資源98、`0x29164`九段角色滑入、actor／target indexed base、`0x29C90`兩段20次背景滑動及原生狀態欄已達局部E1。複合指令33／34的受限class19玩家狀態交易也已接入正式確認路徑，但共用indexed演出尚未完成。物品 type 5–24 的已恢復數值交易均已接；selector 1→0／2 的狀態倒數、歸零重算與 indexed 到期提示已接到正式階段 owner。仍須補齊指令32／35、複合演出、狀態高階名稱、精確音訊及一般玩家連續路徑 |
| 戰場與戰間操作介面 | 部分完成 | 將目前集中在 ch02 的城鎮／商店證據擴到代表性早、中、晚章，補戰場、教會與整備主要路徑 |
| 隊伍持續與存讀檔 | 重製格式可用；原生相容部分完成 | 完成長程遊玩、章節邊界與有效原版存檔的正常玩家驗收 |
| 結局與音訊 | 重製端 E1 可達 | 關閉第30戰正常路徑、終局 records／globals 連續性、精確音訊、輸入與終局停留 |
| 一般玩家全程測試 | 尚未完成 | 至少完成無修改原版／重製的早中晚抽樣與一輪重製長程通關報告 |
...
- 敵方人工智慧已有多個原始模式的窄執行期消費，原版一般玩家敵方回合也可重現；
  command 17–23與25–27現已由玩家指令格或物品目的地游標接到正式交易，17–19 的敵方 raw selector 也已接入；
  ...
  只列為 E1，不列為一般玩家 E2。
- 玩家複合指令33／34已新增受限class19正式交易。兩者只接受原版已證實可達的
  BattleFig 4／5／6／7／20，並先在私有raw records完成全部目標後才發布；33清除
  `+0x25..+0x27`並以固定`0x320`回復HP，34依`0x22721→0x22866→0x22997`
  完成三段modifier。這只證明狀態交易與正式指令格接線，`0x27FC9` indexed畫面、
  score／EXP、指令32／35、敵方owner與一般玩家E2仍失敗即關閉。
...
仍保留。此圖僅作介面對照，不代表一般玩家 E2 或完整開場動畫已完成；
