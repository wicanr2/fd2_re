...
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | 補齊仍缺的指令／法術／物品交易及代表性敵方回合同狀態驗證 |
...
- 敵方人工智慧已有多個原始模式的窄執行期消費，原版一般玩家敵方回合也可重現；
  command 17–19 現已由 raw selector 接到原子 modifier 交易，正確保留 ID17 由
  record18 扣 MP 的特殊邊界。仍缺同一 raw 狀態的重製端配對、phase-expiry正式
  caller及完整命令／法術／物品效果；玩家grid與未證實演出沒有猜測開放。
