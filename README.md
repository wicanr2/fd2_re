...
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | command 17–23、25–27玩家路徑與17–19敵方路徑已有 E1；command23包含目的地游標、色盤、離場／入場與原子交易；command24的原始學習鏈、FIGANI資源98、`0x29164`九段角色滑入、actor／target indexed base、`0x29C90`兩段20次背景滑動及原生狀態欄已達局部E1，但精確音訊與一般玩家連續路徑未驗收；仍須補齊其他指令／法術／物品交易、效果到期與狀態介面，以及代表性敵方回合同狀態驗證 |
...
- 戰場操作新增一條未修改原版的一般玩家錨點：由標題 CONTINUE 正常選到悠妮，
  ...
  已由 Game 層回歸固定為數值 `RUNTIME-E1`；仍缺原版 indexed 演出、音訊與結果 E2。
