...
> **完整重製自評：75／100（約 75%）。尚餘約 25% 的產品整合、忠實度驗收與發行工作。**
...
| 戰役流程、隊伍持續與存讀檔 | 25 | 20 | 24 個標準戰後節點已接入；早／中戰間正式資料抽測，以及第29戰後經19人整備、存讀檔、最終戰前劇情至第30戰的連續 E1 已通過。最終整備存檔現可由全新 `Game` 冷讀，並維持完整 JOIN 時序一路進終局；仍缺代表性正常玩家 E2 與有效原版槽 |
| 戰鬥規則、玩家指令與敵方 AI | 20 | 17 | 正常產生端的主要交易與 indexed owner 已達 E1；玩家普通攻擊、敵方指令13、指令24及指令29多目標正式呈現回歸通過，仍缺原版同狀態逐音訊／逐幀 E2 |
| 操作介面與原版視覺忠實度 | 15 | 10 | ch02 城鎮／商店已有多個整幀相同結果；普通攻擊固定 fixture 的319×199內容區亦達 AE=0。外部末關候選另由普通鍵盤 CONTINUE 抵達第30戰並啟用正式 indexed compositor，已排除舊 PNG fallback 雜訊；但水／冰底圖仍有16,281像素差異，教會、整備及其他章節也缺一般玩家同狀態 E2 |
...
| 戰役與戰間正常玩家路徑 | 24 個標準戰後節點均已接；早／中戰間抽測及第29戰勝利→戰後→19人整備／冷讀檔→最終戰前→第30戰→終局已達跨全新 `Game` 的連續 E1 | 仍需未修改原版的一般玩家 E2；第30戰→結局的正式 E1 已接，但精確畫面／音訊與原版輸入仍待代表性比對 |
...
![重製端第30戰：一般鍵盤 CONTINUE 的原生索引戰場](docs/figures/native-battle-ch30-remake-indexed-candidate-e1.png)

*這張圖以外部末關候選 `FD2.SAV`，從正式標題選單送出普通 X11 鍵盤事件後抵達
`battle_ch30`；旁車固定回合12、camera `(16,16)` 與 cursor `(21,20)`。地形、單位、
前景與 HUD 均由玩家自備原版資產的 indexed compositor 產生，不再是舊 PNG fallback。
第三方存檔來源、固定時鐘，以及尚未閉合的水／冰底圖限制見
[擷取紀錄](docs/data/ui-traces/native-battle-ch30-original-candidate.json)，因此仍列 E1／候選 E2，
不宣稱逐像素一致。*
