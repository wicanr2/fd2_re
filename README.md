...
| 反向工程 | 資產格式與多個底層原語已高度閉合；24 個標準戰後節點已全部接入正式執行期，正常產生端可達的主要玩家指令、物品交易與敵方 AI 也已有來源約束的 E1 owner | 目前不能誠實換算成整支 EXE 百分比；剩餘重點是代表性同狀態 E2、精確音訊／時序與個別玩家可見 consumer，不再全面重解已閉合的指令或 AI，詳見[反組譯覆蓋矩陣](docs/knowledge-base/58-fd2-exe-re-coverage.md) |
| Go／Ebiten 重製 | 地圖、對話與戰鬥可操作；城鎮、商店、教會、整備、自有 JSON 存檔及場景 BGM 已有正式切片；最終戰前 `ch29_pre` 已接 `LOADCH`、21 句對話與七次原生 staging。第27戰缺天空之鑰分支與正式第30戰勝利路徑會播放來源約束的 `0x2BCE5` 前綴；最終整備冷讀檔亦已連續通過第30戰、角色蒙太奇、20 組尾段、`FDOTHER#59` 定格與隊伍回顧 | 尚缺代表性正常玩家戰役／戰間 E2、精確音訊時序／原版輸入與跨平台驗收；原版3%外層預算會讀未初始化區域值，已列為非阻擋考古項目 |
| 原版視覺比對 | ch02 城鎮 variant0 六項、variant1 正常五項、variant2 正常五項（後兩者為修改 LOAD 路徑），以及部分商店、讀檔選單已有整幀 RGB 相同證據；固定晚期槽的初始整備畫面也已完成第一組同狀態差分；正式 `church_ch02` 主選單與現行原始資源 oracle 為 `AE=0/64000` | 尚無可靠的全介面百分比；祕密選項、一般玩家城鎮路徑、整備角色格、教會 DOSBox caller 與其餘章節仍需同狀態比較，詳見[介面證據矩陣](docs/knowledge-base/57-ui-evidence-matrix.md) |

### 距離完整重製還有多遠

> **評估快照：2026-08-27；於晚期有效槽、第29／30戰回合、終局音訊、三平台封包及十九個原生對話切片驗證後重新核算。最新加入第28戰後五句原生對話、64筆正式戰況、`preparation_ch29`與冷讀存檔長鏈；第29戰依玩家可見99%相似門檻列為重製完成。**
> 本節依目前正式執行期、介面證據矩陣與一般玩家驗證狀態整理；後續成果應以
> 可重跑證據更新各門檻，不應只因新增反組譯筆記就下修剩餘比例。

以「玩家能從標題正常開始，走完30戰、戰間服務、存讀檔與結局，且三平台可交付」
作為完成定義，目前屬於**中後段整合期，還不是發行候選版**。依下表固定權重自評為：

> **完整重製自評：90／100（約 90%）。尚餘約 10% 的產品整合、代表性忠實度驗收與發行工作。第 29 戰已依 99% 玩家可見相似門檻列為產品完成，不再以完整原版存檔來源鏈或逐週期考證阻擋交付。**

這不是整支
`FD2.EXE` 的反組譯百分比，也不是畫面像素相似率；它只用下列玩家交付門檻估算。
同一領域只有新增可重跑的資料、正式執行期、一般玩家 E2 或平台產物時才調整分數；
新增孤立反組譯筆記不加分。現階段已經不是技術展示，但也還不是發行候選版。
全戰役長程遊玩改由使用者人工進行並回報問題，不列為代理程式持續執行項目。
近期完成的商店正常輸入回歸，以及未修改原版第29、30戰各自完成一輪玩家／敵方
控制權交接，分別補強既有 E1 與候選 E2。另一份固定雜湊晚期有效槽又由普通
`LOAD` 連續走過 save-NO、19人整備、最終戰前劇情並抵達可操作第30戰；這首次補上
有效原版槽與晚期整備的一般輸入錨點，因此戰役／存讀檔項增加1分。第三方來源仍使
它不能替代第29戰勝利當下寫槽的完整來源鏈。重製正式 selector 現也以同一固定雜湊
槽還原29人與60金幣並進最終戰整備，修正舊映射誤回第29戰的路由缺陷；這是既有
22分範圍內的 E1 補強。後續同一固定槽又在不清空敵軍、不直接呼叫回合捷徑的條件
下，由正式 END／YES 介面跑完整個第30戰敵軍回合並交還玩家控制；這同時找出並修正
persistent raw `+0x34/+0x35/+0x36` 未傳入戰場人工智慧的缺口，因此戰鬥／人工智慧
項增加1分。第三方存檔限制不變，這仍是重製端 `RUNTIME-E1`，不是完整 E2。
同樣的正式標題 `CONTINUE` 路徑現在也以固定雜湊 `fd2004` 進入第29戰，不清空敵軍、
不直接呼叫回合捷徑，完成 `END／YES→敵軍行動→PLAYER PHASE`。這次實際修正
scenario field-event 規則在原生存檔交接時遺失、mode 2 沒有路由到既有
`0x13FD4` owner、FIGANI 零延遲合法幀，以及恢復演出前 `0x12D7B` 鏡頭同步；
因此戰鬥／人工智慧再增加1分。第三方來源與停用音訊仍只支持 `RUNTIME-E1`。
終局音訊本輪再以實際本地 MT-32 資產驗證 `FDMUS_004→停止→FDMUS_018` 的解碼、
播放器建立與切換，短音效也會保留至自然結束並在退出時統一清理，因此結局與音訊
增加1分。無音效裝置的 Docker 驗證不等於人耳、實體裝置或三平台驗收，這些限制
仍保留在剩餘工作中。
固定晚期槽的初始整備畫面也已由正式 LOAD 擷取。新的 IDA 消費鏈與逐格
比對否定舊「灰階角色 selector／調色盤錯誤」斷言：28個角色格在圖像相位0
全部 `AE=0`，舊差異是截圖入口未凍結相位。凍結後只剩第二組 `19` 的39像素；
`0x31EA9..0x31EFB` 又證實兩組數字分別使用 FDOTHER #5 entries 31..40
與42..51。接通第二套字形後，這個固定槽、初始狀態與相位0已達
320×200整幀 `AE=0/64000`，操作介面項因此增加1分。這仍不代表其他章節、選取動畫、
音訊或第三方存檔來源鏈已完成 E2。
標題穩定主選單也已由目前原始碼重新建置擷取；三列校正到原生
`y=164／173／182` 後，320×200與未修改原版 oracle 達整幀 `AE=0/64000`。
AFM 3前的漢堂發行商畫面也已從玩家自備`FDOTHER #74 + #76`接入；正式runtime
第60幀與解碼oracle達640×400整幀`AE=0/256000`，新增後仍自然抵達同一AE0選單。
其後兩段原本誤記為wipe／`logozoom`的轉場，現已依`sub_286BD`真正語意改成索引
色盤內插：紅幕、真實ANI #1、近白到標題畫面，並保留原版半開區間0..254。
[重製淡入影格](docs/figures/title-transition-remake-e1.png)與相近原版影格的平均每色頻道
絕對誤差為0.077／255，已高於本專案99%玩家可見門檻；介面分數維持14／15，剩餘
限制是精確音訊、原版完整啟動E2及其他代表性戰間介面。
同一份最終整備存檔現又由全新 `Game` 冷讀，連續走過第30戰、勝利、兩段終局
文字、角色蒙太奇、20 組尾段、永久定格、隊伍最終狀態回顧及返回定格；全程核對
JOIN 順序與 persistent raw `+6/+7/+8/+0x20`。這補上先前誤列為缺口的重製端
records／globals 連續性，因此戰役／隊伍持續項增加1分。測試以已證實的略過契約
縮短中段，仍屬 `RUNTIME-E1`，不冒稱未修改原版的一般玩家 E2。

| 評分領域 | 權重 | 目前得分 | 判定依據 |
|---|---:|---:|---|
| 資產、資料格式與可編輯資料 | 20 | 18 | 主要容器、圖像、文字、字型、動畫、地圖與多份硬編碼劇情已可重生；仍有音訊執行期與少數 consumer 缺口 |
| 戰役流程、隊伍持續與存讀檔 | 25 | 23 | 24 個標準戰後節點已接入；第23戰戰果後現由正式具型別輸入完成記錄提示存檔、全新`Game`冷讀、15人選取、取消重選與肯定進第24戰前劇情。第29戰後經19人整備、存讀檔、最終戰前劇情至第30戰的連續 E1 亦已通過。最終整備存檔可由全新 `Game` 冷讀，並以正式 ch30 view／HUD 與 party→groups constructor 走完 END→YES、敵方回合、戰果、終局文字、角色蒙太奇、20 組尾段、永久定格、隊伍回顧及返回定格；外部有效原版槽亦已由普通 LOAD 走完19人整備至可操作第30戰。第29戰 writer 建槽來源只列證據限制，不阻擋99%相似門檻 |
...
| 三平台發行與推廣 | 10 | 7 | 最新程式已在一次性無網路Docker重建5,327,352-byte Linux AppImage（SHA-256 `32239daa…091f5`）與4,910,912-byte Windows交叉編譯ZIP（`c85e466b…ee11`），兩者均從空白工作目錄通過正式封包自我檢查。原生Windows [run 33038018401](https://github.com/wicanr2/fd2_re/actions/runs/33038018401)與macOS [run 33038019716](https://github.com/wicanr2/fd2_re/actions/runs/33038019716)又分別產出ZIP及universal DMG／tar.gz；下載後的三項封包均以artifact內相對路徑SHA-256 manifest在唯讀Docker複驗成功。這仍不等於Windows／macOS實體玩家桌面的視窗、輸入、存檔與音訊驗收；最終版號、簽章及公開推廣片素材策略尚待 |
| **總計** | **100** | **90** | **尚未達到發行候選版** |
...
- 原版終局蒙太奇與20組尾段已可在重製端播放並永久停留；Enter／Space 可選擇循環
  ...
  也已由資料接入正式終局；正式畫面不再疊加「來源約束 E1」等開發提示，只有玩家
  主動開啟除錯 HUD 時才顯示。重製端冷讀 persistent raw 已連續保存至定格與回顧，仍缺
  第30戰一般玩家 E2、原版呼叫當下完整動態狀態、精確音訊時序
  ...
  終局按鍵現由 `Game.Update` 與長鏈回歸共用同一正式消費端：蒙太奇期間的任意新鍵
  只保存 raw-change pending，永久定格的 Enter／Space 啟動回顧，回顧中的
  Enter／Space／Escape 回復同一定格；不再由測試直接改內部旗標冒充玩家輸入。
...
| 剩餘阻擋工作 | 目前狀態 | 完成 remake 前的關閉條件 |
|---|---|---|
| 戰鬥演出與人工智慧驗收 | 正常產生端的主要 E1 交易已接 | 抽驗代表性玩家／敵方指令的同一 raw 狀態、逐幀與音訊；修正仍可見的戰場排版／動畫差異。沒有正常產生端的敵方25及玩家28／30／31不猜接，也不列阻擋 |
| 戰役與戰間正常玩家路徑 | 24 個標準戰後節點均已接；第29戰勝利→戰後→19人整備／冷讀檔→最終戰前→第30戰的正式 view／HUD、END→YES、敵方回合、勝利、蒙太奇、20 組尾段、定格與回顧已形成跨全新 `Game` 的連續 E1 | 仍需未修改原版的一般玩家 E2；精確畫面／音訊與原版輸入仍待代表性比對 |
| 操作介面忠實度 | ch02 城鎮／商店有多組 E2；正式教會主選單有原始資源 oracle 的 AE=0 E1；晚期有效槽、整備與第30戰有候選E2長鏈，其餘多為 E1 | 補戰場 HUD／動畫、教會 DOSBox caller、整備同狀態像素比較及其他章節戰間介面；不要求無玩家影響的 DOS 硬體逐像素復刻 |
...
- 敵方人工智慧已有多個原始模式的窄執行期消費，原版一般玩家敵方回合也可重現；
  ...
  缺素材、色盤或原始記錄時維持失敗即關閉。六個 raw transient 的到期文字與
  indexed 狀態介面現已接入，不再列為缺口；目前仍缺同一 raw 狀態的原版／重製
  配對、精確音訊與代表性指令／法術／物品演出驗收，故只列為 E1，不列為一般玩家 E2。
...
- 正式來源約束 E1 已播放終局前綴、隊員最終狀態與20組原版資源尾段，最後停在
  ...
  原版 `0x2C2A6` 呼叫時完整動態狀態、精確音訊／輸入與一般玩家 E2；重製端
  persistent raw 從冷讀檔到定格／回顧的連續性已由有界回歸證實。原版
...
![正式 church_ch02 教會主選單](docs/figures/church-current-remake.png)

*這是正式 campaign 節點、原版索引資產與生產繪圖路徑的重製端 E1 擷取；固定
`selection=0,pulse=2,gold=1000` 後，最近鄰縮回 320×200 與現行原始資源 oracle
達 `AE=0/64000`。oracle 不是 DOSBox 原版截圖，因此不冒稱玩家 E2；狀態、雜湊與
舊基準勘誤見[擷取紀錄](docs/data/ui-traces/native-church-menu-ch02-remake-e1.json)。*
...
*這是 Docker／Xvfb 由正式 `story_ch30` 節點擷取的重製端 E1 畫面。其後連續回歸已
從第29戰勝利走過戰後、19人整備／存讀檔、完整 raw ch29 pre，再物化第30戰；圖片與
...
![未修改原版晚期有效槽：LOAD、19人整備、出戰確認與第30戰玩家控制](docs/figures/native-load-ch29-to-ch30-original-candidate.png)

同一固定槽的初始整備畫面已由重製正式 LOAD 入口配對。下圖依序為原版、重製
合法相位0與差異；凍結證據相位並分開兩套原版數字字形後，320×200全畫面
達 `AE=0/64000`。這是固定存檔與初始狀態的 E1 像素錨點，不外推其他整備狀態。

![固定晚期槽初始整備：原版、重製與差異](docs/figures/native-load-ch29-preparation-original-remake-phase0.png)

*固定雜湊 `fd2021` 外部存檔的 slot 0 保存 raw chapter `0x1c`。同一個未修改
`FD2.EXE` 程序由普通標題 `LOAD` 讀槽，選 NO 跳過再次記錄，連按19次確認完成
整備，再走完最終戰前劇情並抵達可操作第30戰。這關閉有效晚期槽與整備的候選 E2
錨點；但保存站註記所有隊員等級全滿，且沒有從第29戰勝利寫槽的完整來源鏈，不能
冒稱未修改全程通關。來源、輸入、雜湊與拒收路徑見
[擷取紀錄](docs/data/ui-traces/native-load-ch29-slot-to-ch30-original-candidate.json)。重製端
現也從標題 selection 0 依序消費 Down、確認閃爍、slot 0、連續19次選取與最終確認；
選人游標會和原版一樣在勾選後自動前進，並以固定隊長＋19人（含 authored scenario
原先漏列的 identity 3）抵達 `battle_ch30`。這是同一候選存檔的 `RUNTIME-E1`，
此固定槽後續也完成第30戰 END／YES／敵軍回合交接；目前自評以本頁上方最新的
90／100 快照為準，舊分數不再於圖片說明重複保存。*

![重製端第30戰：接通原版輔助底面的原生索引戰場](docs/figures/native-battle-ch30-remake-aux-candidate-e1.png)

*外部末關候選 `FD2.SAV` 已另由正式標題選單的普通 X11 鍵盤事件抵達
`battle_ch30`；本圖則以同一typed戰況固定raw phase 10，展示新接通的
`FDOTHER #55` 輔助底面、terrain、18筆鏡頭內單位、前景與HUD。與原版候選的
...
旁車證實抵達 `battle_ch30`、round 12及相同鏡頭／游標；再按一次 `Return` 後，
一次性開場閘門關閉並由原版 indexed 空游標操作面板接手。這關閉重製端測試捷徑及
「只載入但不能操作」疑慮，但第三方存檔來源仍只容許候選 E2。*
...
實際執行期擷取。穩定選單已達整幀 `AE=0`。開場現在也依 `sub_1F894` 的
535列捲動與 `450／330／210／110／25／10` 六個插播點，交錯播放 ANI.DAT 的
AFM 幕，不再把全部動畫播完後才快速捲動；屠龍白閃由舊約5秒延後到約10.9秒，
舊排程約29.4秒後自然抵達同一個 `AE=0` 選單。現在又依直接指令接上最前面的
`FDOTHER #74 + #76` 漢堂發行商幕，以8幀淡入、103幀停留、8幀淡出投影31個
BIOS tick；正式runtime第60幀與解碼oracle達640×400 `AE=0/256000`，新增後
frame1884仍自然抵達同一AE0選單。舊DOSBox `frame_000`其實已是後續AFM 3，
不是漢堂標誌；逐轉場、精確音訊與未修改原版runtime E2仍待補。

![重製端正式開場的漢堂發行商畫面](docs/figures/title-publisher-remake.png)

*此圖由正式正常啟動路徑第60幀擷取；美術來自玩家自備`FDOTHER.DAT`，倉庫只保留
驗證截圖，不含原始archive。資源雜湊、排程與限制見[擷取紀錄](docs/data/ui-traces/title-publisher-remake-e1.json)。*

| 原版 DOSBox | 重製端執行期 |
|---|---|
| ![原版標題選單](docs/figures/title-original-dosbox.png) | ![重製端標題選單執行期畫面](docs/figures/title-remake-runtime.png) |

![重製端依原版交錯時間軸播放至屠龍白閃](docs/figures/title-interleaved-dragon-remake.png)
...
頭像與 FDOTHER #4 字模，依原始字序逐字發布，最後再依 `sub_16B43` 保存畫面順序
收框並返回場景游標，達 caller-specific `RUNTIME-E1`。目前沒有把 test-only
...
macOS CI 目前產生未簽署的 universal `.app`、DMG 與 tar.gz。下載後若遭
Gatekeeper 隔離，測試者需先確認 artifact 來自本專案成功的 workflow，再執行
`xattr -dr com.apple.quarantine FD2.app`；正式公開版仍應補簽章／公證，不能把這個
測試者解鎖步驟當成最終發行方案。
