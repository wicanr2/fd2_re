...
- 戰鬥指令、法術、物品與敵方人工智慧已有多條正式 E1；敵方指令10–12現已依`0x15311→funcs_1541F`接回與玩家共用的60幀索引演出，玩家與敵方17–22也已接回各自wrapper內建的效果／遮罩／數字段；玩家保留專用八相位palette前段，敵方則直接進handler尾段。這些路徑都依Draw邊界原子發布MP／HP或raw欄位／RNG／`Acted`，不再同步跳過畫面。`0x20C6F` 已恢復的物品 type 5–24 數值交易均已接通。複合指令32–35已由正式指令格接上`0x27FC9`共用演出、各自專用尾段與原子交易。`0x1A866` 的 selector 1→0／2 狀態倒數、歸零重算與 FDTXT 481–486／DATO indexed 到期提示也已接到正式階段 owner；角色狀態面板亦會依 raw `+0x22..+0x24` 切換能力數字顏色，並為 `+0x25..+0x27` 顯示原版圖示。仍缺狀態高階名稱，以及其他有正常producer的敵方caller精確演出／音訊驗證。
...
| 完成門檻 | 自我評估 | 關閉前仍必須做到 |
|---|---|---|
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | command 17–23、25–27玩家路徑與17–19敵方路徑已有 E1；敵方17–19已依`0x1541F`跳表接回單段效果／遮罩／數字queue及原子交易，不誤套玩家專用palette前段。command0 已由正式游標確認接到原版索引演出與原子數值交易；command23包含目的地游標、色盤、離場／入場與原子交易；command24的原始學習鏈、FIGANI資源98、角色滑入、背景滑動及原生狀態欄已達局部E1。複合指令32–35現已接正式共用段、各自專用tail與原子交易，達受限class19玩家`RUNTIME-E1`。物品 type 5–24 的已恢復數值交易均已接；狀態倒數、歸零重算與 indexed 到期提示已接到正式階段 owner。仍須補齊狀態高階名稱、精確音訊及代表性玩家／敵方路徑 |
