...
| Go／Ebiten 重製 | 地圖、對話、部分戰鬥、城鎮、商店、教會、整備、自有存檔及場景 BGM 消費可操作；戰鬥曲與城鎮曲已有原版表格回歸；近似模式可從最終節點播放已證實的 `0x2BCE5` 前綴 | 尚缺完整 30 章玩家路徑、完整原生存檔相容、`0x2BCE5` 蒙太奇／原始輸入尾段、一般玩家終局 E2 與跨平台驗收 |
...
`native_ending_prefix` 合約，才會播放已證實的 `0x2BCE5` 前綴；到未還原的
`0x2C548` 閘門時只消費已核對的 `FDMUS_004`，等待確認後回到可編輯結語。
後續蒙太奇、`play_bgm(-1)`、`FDMUS_018`、原始按鍵尾段與一般玩家通關仍失敗即關閉。
