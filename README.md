...
| Go／Ebiten 重製 | 地圖、對話與戰鬥可操作；城鎮、商店、教會、整備、自有 JSON 存檔及場景 BGM 已有正式切片；最終戰前 `ch29_pre` 已接 `LOADCH`、21 句對話與七次原生 staging。第27戰缺天空之鑰分支與正式第30戰勝利路徑會播放來源約束的 `0x2BCE5` 前綴，最終戰再接角色最終狀態蒙太奇、20 組尾段與 `FDOTHER#59` 終局定格 | 尚缺代表性正常玩家戰役／戰間 E2、終局呼叫時 records／globals 連續性、精確音訊時序／原版輸入與跨平台驗收；原版3%外層預算會讀未初始化區域值，已列為非阻擋考古項目 |
...
> **評估快照：2026-08-26；於晚期有效槽、第29／30戰回合與終局音訊生命週期驗證後重新核算。**
...
> **完整重製自評：83／100（約 83%）。尚餘約 17% 的產品整合、忠實度驗收與發行工作。**
...
仍保留在剩餘工作中。
固定晚期槽的初始整備畫面也已由正式 LOAD 擷取，並依 `0x31EA9..0x31EFB` 修正
第一組數字誤畫成已選人數的缺陷；原版與重製現在都顯示 `19／19`。合法角色相位1
的320×200差分仍有3763像素，集中於灰階角色格，因此這次補強視覺 E1 證據但不把
總分調高；完整重製自評仍維持83／100，避免把尚未閉合的角色 selector／調色盤差異
提前算成完成。

| 評分領域 | 權重 | 目前得分 | 判定依據 |
|---|---:|---:|---|
| 資產、資料格式與可編輯資料 | 20 | 18 | 主要容器、圖像、文字、字型、動畫、地圖與多份硬編碼劇情已可重生；仍有音訊執行期與少數 consumer 缺口 |
| 戰役流程、隊伍持續與存讀檔 | 25 | 22 | 24 個標準戰後節點已接入；第29戰後經19人整備、存讀檔、最終戰前劇情至第30戰的連續 E1 已通過。最終整備存檔可由全新 `Game` 冷讀，並以正式 ch30 view／HUD 與 party→groups constructor 走完 END→YES、敵方回合、戰果、終局文字閘門及角色蒙太奇；外部有效原版槽亦已由普通 LOAD 走完19人整備至可操作第30戰。仍缺第29戰勝利→寫槽的完整原版來源鏈 |
...
| 操作介面與原版視覺忠實度 | 15 | 13 | ch02 城鎮／商店與普通攻擊固定fixture已有整幀或內容區AE=0。外部末關候選另由不略過開場、不固定標題計時的普通鍵盤CONTINUE抵達並操作第30戰；IDA閉合並接通`FDOTHER #55`輔助底面與`0x4DFCC` DAC cycle後，同一raw狀態與合法相位組合達320×200整幀AE=0。固定晚期槽的初始整備現也由正式LOAD擷取並修正`00／19→19／19`，但合法相位1仍有AE=3763的灰階角色格差異，因此本項暫不加分；教會與其他章節的代表性比對尚未完成 |
| 結局與音訊 | 10 | 9 | 正式第30戰可經一般戰果接縫進來源約束終局並停在定格；部署成員保留最終戰結果，未部署成員保留冷讀狀態，完整 JOIN 隊伍可進最終回顧。短音效播放器可疊播、保留至自然結束並於退出清理；實際 MT-32 資產亦已在無音效裝置的 Docker 中完成曲目4→停止→曲目18的解碼與切換。仍缺人耳、原版 records／globals 連續性、原版輸入與三平台精確音訊驗收 |
| 三平台發行與推廣 | 10 | 2 | 目前 HEAD 已通過 Linux Docker／Xvfb 建置、啟動擷圖與 Windows x86_64 PE 交叉編譯；尚缺 Windows 真機、macOS CI、最終封包與推廣影片 |
| **總計** | **100** | **83** | **尚未達到發行候選版** |
...
| 戰役與戰間正常玩家路徑 | 24 個標準戰後節點均已接；第29戰勝利→戰後→19人整備／冷讀檔→最終戰前→第30戰的正式 view／HUD、END→YES、敵方回合、勝利→終局已形成跨全新 `Game` 的連續 E1 | 仍需未修改原版的一般玩家 E2；精確畫面／音訊與原版輸入仍待代表性比對 |
| 操作介面忠實度 | ch02 城鎮／商店有多組 E2；晚期有效槽、整備與第30戰有候選E2長鏈，其餘多為 E1 | 補戰場 HUD／動畫、教會、整備同狀態像素比較及其他章節戰間介面；不要求無玩家影響的 DOS 硬體逐像素復刻 |
| 隊伍持續與原生存讀檔 | 重製格式可用，current-runtime SAVE／LOAD 與增援／JOIN raw 同步已有窄 E1；外部晚期有效槽已由原版普通LOAD抵達第30戰，重製也由標題正常事件還原29人／60金幣、完成19人整備並部署20人進最終戰 | 補第29戰勝利當下寫槽的完整來源鏈，並抽驗代表性章節的金錢、HP／MP、物品、裝備與隊伍順序；長程漂移由人工回報建立窄重現案例 |
| 結局與音訊 | 終局前綴、角色蒙太奇、20 組尾段、19×5 對話與永久定格均可達 E1 | 關閉第30戰 records／globals 連續性、原版輸入與精確音訊時序；不重現依賴未初始化堆疊的3%偶發外層，也不重做已完成的定格／回顧循環 |
...
合法相位1與差異；人數計數已修正一致，角色格仍有 AE=3763，故標為 E1 比較而非
逐像素完成。

![固定晚期槽初始整備：原版、重製與差異](docs/figures/native-load-ch29-preparation-original-remake-phase1.png)
