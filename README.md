...
| 原版視覺比對 | ch02 城鎮 variant0 六項、variant1 正常五項、variant2 正常五項（後兩者為修改 LOAD 路徑），以及部分商店、讀檔選單已有整幀 RGB 相同證據；固定晚期槽的初始整備畫面也已完成第一組同狀態差分 | 尚無可靠的全介面百分比；祕密選項、一般玩家城鎮路徑、整備角色格、教會與其餘章節仍需同狀態比較，詳見[介面證據矩陣](docs/knowledge-base/57-ui-evidence-matrix.md) |
...
> **完整重製自評：89／100（約 89%）。尚餘約 11% 的產品整合、忠實度驗收與發行工作。**
...
| 三平台發行與推廣 | 10 | 6 | 最新`a5bbaf3f`已重建Linux AppImage與Windows x86_64 ZIP。AppImage於Debian系及Ubuntu 24.04抵達`town_ch02`且畫面AE=0；Windows ZIP於Wine 9.0接收城鎮按鍵、進商店並以F5寫出可解析存檔。macOS真實`macos-14` runner除了完成arm64／amd64、lipo universal、`.app`與DMG／tar.gz，[run 32999934526](https://github.com/wicanr2/fd2_re/actions/runs/32999934526)更從空白cwd執行bundle binary，具型別驗證全部戰役轉場、36個唯一法術ID及所有劇情引用，並修正舊版只會查`Contents/MacOS/assets`的實際缺陷。這仍不等於Windows／macOS實體機視窗、輸入與音訊驗收；最終版號封包與推廣影片尚待 |
| **總計** | **100** | **89** | **尚未達到發行候選版** |
...
| Linux／Windows／macOS 發行 | AppImage已在Debian系與Ubuntu 24.04抵達`town_ch02`且畫面一致；Windows ZIP已由Wine 9.0驗證輸入、節點轉場與F5存檔；macOS run 32999934526已在真實`macos-14`執行bundle內universal binary並從空白cwd驗證完整可散布戰役資料 | 補Windows／macOS實體機視窗、輸入、存檔與音訊，處理簽章／Gatekeeper，最後建立版號封包與推廣影片 |
