...
- 戰鬥指令、法術、物品與敵方人工智慧已有多條正式 E1；`0x20C6F` 已恢復的物品 type 5–24 數值交易均已接通。複合指令32–35已由正式指令格接上`0x27FC9`共用演出、各自專用尾段與原子數值／raw marker／RNG／`Acted`交易。四者只在已證實BattleFig來源開放，MP目前只作原版可用性門檻。`0x1A866` 的 selector 1→0／2 狀態倒數、歸零重算與 FDTXT 481–486／DATO indexed 到期提示也已接到正式階段 owner；角色狀態面板亦會依 raw `+0x22..+0x24` 切換能力數字顏色，並為 `+0x25..+0x27` 顯示原版圖示。仍缺狀態高階名稱，以及代表性敵方caller的精確演出／音訊驗證。
