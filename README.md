...
- 戰鬥指令、法術、物品與敵方人工智慧已有多條正式 E1；`0x20C6F` 已恢復的物品 type 5–24 數值交易均已接通。複合指令32／33已由正式指令格接上`0x27FC9`共用演出、各自專用尾段與原子HP／raw marker／RNG／`Acted`交易；34／35也已接到受限class19玩家數值交易，但仍缺各自共用演出後的專用畫面尾段。四者只在已證實BattleFig來源開放，MP目前只作原版可用性門檻。`0x1A866` 的 selector 1→0／2 狀態倒數、歸零重算與 FDTXT 481–486／DATO indexed 到期提示也已接到正式階段 owner；角色狀態面板亦會依 raw `+0x22..+0x24` 切換能力數字顏色，並為 `+0x25..+0x27` 顯示原版圖示。仍缺複合指令34／35完整演出、狀態高階名稱，以及精確演出／音訊驗證。
...
| 完成門檻 | 自我評估 | 關閉前仍必須做到 |
|---|---|---|
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | command 17–23、25–27玩家路徑與17–19敵方路徑已有 E1；command0 已由正式游標確認接到原版索引演出與原子數值交易，包含角色滑入、施術者效果、28幀／7元素錯開的目標效果、七段 HP 發布與尾段合成，達 `RUNTIME-E1`；command23包含目的地游標、色盤、離場／入場與原子交易；command24的原始學習鏈、FIGANI資源98、`0x29164`九段角色滑入、actor／target indexed base、`0x29C90`兩段20次背景滑動及原生狀態欄已達局部E1。複合指令32／33現已接正式`0x27FC9`共用段、各自專用tail與原子交易，達受限class19玩家`RUNTIME-E1`；34／35仍只有正式狀態交易，尚缺完整演出。物品 type 5–24 的已恢復數值交易均已接；selector 1→0／2 的狀態倒數、歸零重算與 indexed 到期提示已接到正式階段 owner。仍須補齊複合指令34／35演出、狀態高階名稱、精確音訊及代表性玩家／敵方路徑 |
...
- 敵方人工智慧已有多個原始模式的窄執行期消費，原版一般玩家敵方回合也可重現；
  ...
  只列為 E1，不列為一般玩家 E2。
- 玩家複合指令32／33已新增受限class19正式indexed演出與交易；34／35已新增受限class19正式交易。四者只接受原版已證實可達的
  BattleFig 4／5／6／7／20，並先在私有raw records完成全部目標後才發布；33清除
  `+0x25..+0x27`並以固定`0x320`回復HP，34依`0x22721→0x22866→0x22997`
  完成三段modifier。ID32／33另已完成`0x27FC9`共用段、各自專用tail、音效與Draw邊界交易；
  score／EXP、34／35專用演出、敵方owner與一般玩家E2仍失敗即關閉。
