...
| 三平台發行與推廣 | 10 | 3 | Linux AppImage已由雜湊鎖定工具映像、無網路且指定UID/GID的容器產生，並以唯讀玩家資產從空工作目錄啟動至`town_ch02`；Windows x86_64 GUI PE與ZIP亦完成離線封包及雜湊驗證。macOS已具真正`.icns`的工作流程；仍缺Windows真機、macOS CI、其他Linux發行版抽測、最終版號封包與推廣影片 |
| **總計** | **100** | **86** | **尚未達到發行候選版** |
