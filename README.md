...
| 戰鬥規則、玩家指令與敵方人工智慧 | 部分完成 | command 17–23、25–27玩家路徑與17–19敵方路徑已有 E1；command0數值交易已有 E1，28幀／7元素錯開演出、原始效果資源與七段HP發布已達 `RE-CLOSED`／`DATA-READY`，但尚未接成正式完整戰鬥場景；command23包含目的地游標、色盤、離場／入場與原子交易；command24的原始學習鏈、FIGANI資源98、`0x29164`九段角色滑入、actor／target indexed base、`0x29C90`兩段20次背景滑動及原生狀態欄已達局部E1，但精確音訊與一般玩家連續路徑未驗收；仍須補齊其他指令／法術／物品交易、效果到期與狀態介面，以及代表性敵方回合同狀態驗證 |
...
- 戰場操作新增一條未修改原版的一般玩家錨點：由標題 CONTINUE 正常選到悠妮，
  ...
  已由 Game 層回歸固定為數值 `RUNTIME-E1`。原版 `0x26152` 的28幀、7元素錯開
  排程，FDOTHER #18／#20效果幀、#82子音效1、前後景疊圖順序與七段HP發布計畫
  也已整理為具型別且通過原始資源回歸的 `RE-CLOSED`／`DATA-READY` 元件；正式
  `Game` 尚未把它與通用角色／目標戰鬥場景完整接線，因此不宣稱 indexed 演出
  已達 E1，仍缺精確音訊與結果 E2。
