...
> **評估快照：2026-08-26；於商店正常輸入與末關回合交還驗證後重新核算。**
...
> **完整重製自評：79／100（約 79%）；重新核算後維持不變。尚餘約 21% 的產品整合、忠實度驗收與發行工作。**
...
近期完成的商店正常輸入回歸、未修改原版第30戰交還玩家操作權，以及另一份
固定雜湊候選由普通 CONTINUE 進入第29戰並開啟指令環，分別補強既有 E1 與候選
E2；這些原版晚期存檔仍受第三方來源限制，因此本次不把局部驗證直接換算成額外分數。

| 評分領域 | 權重 | 目前得分 | 判定依據 |
|---|---:|---:|---|
| 資產、資料格式與可編輯資料 | 20 | 18 | 主要容器、圖像、文字、字型、動畫、地圖與多份硬編碼劇情已可重生；仍有音訊執行期與少數 consumer 缺口 |
| 戰役流程、隊伍持續與存讀檔 | 25 | 21 | 24 個標準戰後節點已接入；第29戰後經19人整備、存讀檔、最終戰前劇情至第30戰的連續 E1 已通過。最終整備存檔可由全新 `Game` 冷讀，並以正式 ch30 view／HUD 與 party→groups constructor 走完 END→YES、敵方回合、戰果、終局文字閘門及角色蒙太奇；仍缺未修改原版同路徑 E2 與有效原版槽 |
...
| 操作介面忠實度 | ch02 城鎮／商店有多組 E2，其餘多為 E1 | 補戰場 HUD／動畫、教會、整備、有效讀檔槽及其他章節戰間介面的同狀態比較；不要求無玩家影響的 DOS 硬體逐像素復刻 |
| 隊伍持續與原生存讀檔 | 重製格式可用，current-runtime SAVE／LOAD 與增援／JOIN raw 同步已有窄 E1 | 以有效原版槽及代表性章節邊界驗證金錢、HP／MP、物品、裝備、隊伍順序與節點；長程漂移由人工回報建立窄重現案例 |
