...
> **完整重製自評：68／100（約 68%）。尚餘約 32% 的產品整合、忠實度驗收與發行工作。**
...
| 戰役流程、隊伍持續與存讀檔 | 25 | 18 | 24 個標準戰後節點已接入，戰後城鎮／整備邊界及多個 JOIN／存讀檔切片可運作；代表性正常玩家 E2 與有效原版槽仍不足 |
| 戰鬥規則、玩家指令與敵方 AI | 20 | 16 | 正常產生端的主要交易與 indexed owner 已達 E1；仍缺代表性逐幀／逐音訊 E2、部分可見狀態與精確演出 |
| 操作介面與原版視覺忠實度 | 15 | 8 | ch02 城鎮／商店已有多個整幀相同結果，但戰場、教會、整備及其他章節只部分驗證 |
| 結局與音訊 | 10 | 7 | 正式第30戰可進來源約束終局並停在定格；仍缺 records／globals 連續性、原版輸入與精確音訊驗收 |
| 三平台發行與推廣 | 10 | 1 | 目前 HEAD 已通過 Linux Docker／Xvfb 啟動擷圖與 Windows x86_64 PE 交叉編譯；尚缺 Windows 真機、macOS CI、最終封包與推廣影片 |
| **總計** | **100** | **68** | **尚未達到發行候選版** |
