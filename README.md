...
| 三平台發行與推廣 | 10 | 6 | 最新`a5bbaf3f`已重建Linux AppImage與Windows x86_64 ZIP。AppImage於Debian系及Ubuntu 24.04抵達`town_ch02`且畫面AE=0；Windows ZIP於Wine 9.0接收城鎮按鍵、進商店並以F5寫出可解析存檔。macOS真實`macos-14` runner除了完成arm64／amd64、lipo universal、`.app`與DMG／tar.gz，[run 32998426963](https://github.com/wicanr2/fd2_re/actions/runs/32998426963)更從空白cwd實際執行bundle binary並解析首章、終章、戰役及法術JSON，修正舊版只會查`Contents/MacOS/assets`的實際缺陷。這仍不等於Windows／macOS實體機視窗、輸入與音訊驗收；最終版號封包與推廣影片尚待 |
| **總計** | **100** | **89** | **尚未達到發行候選版** |
...
| Linux／Windows／macOS 發行 | AppImage已在Debian系與Ubuntu 24.04抵達`town_ch02`且畫面一致；Windows ZIP已由Wine 9.0驗證輸入、節點轉場與F5存檔；macOS run 32998426963已在真實`macos-14`執行bundle內universal binary並從空白cwd解析內建戰役資料 | 補Windows／macOS實體機視窗、輸入、存檔與音訊，處理簽章／Gatekeeper，最後建立版號封包與推廣影片 |
