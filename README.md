...
| Go／Ebiten 重製 | 地圖、對話、部分戰鬥、城鎮、商店、教會、整備、自有存檔及場景 BGM 消費可操作；戰鬥曲與城鎮曲已有原版表格回歸；近似模式可從最終節點播放已證實的 `0x2BCE5` 前綴與原資源角色蒙太奇 | 尚缺完整 30 章玩家路徑、完整原生存檔相容、`0x2C194` 終局尾段／精確 BIOS 按鍵對映、一般玩家終局 E2 與跨平台驗收 |
...
新輸入只在角色 portrait loop 承載已證實的「完成本輪後進 final loop」效果，不命名為
原版特定按鍵；cycle 完成或素材／raw provenance 不足後才確認回到可編輯結語。
`0x2C194` 尾段、`play_bgm(-1)`、`FDMUS_018`、精確 BIOS 按鍵與一般玩家通關仍失敗即關閉。
