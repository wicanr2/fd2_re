...
重製端另有受原始來源證據（raw provenance）保護的 mode 2／mode 3／mode 4／mode 5／
mode 7／mode 9／mode 10／mode 11 遊戲層 E1 回歸：mode 2 可完成物理移動／FIGANI
攻擊，mode 3／9 依 raw `+0x08` 查找完成 movement-only 路徑，mode 4／10 依 raw
目的地完成 movement-only 路徑，mode 5 可提交事件格的 raw state 尾端，mode 7 可完成
raw 目的地移動並提交 `+0x05`，mode 11 可依序消費兩段 command／
