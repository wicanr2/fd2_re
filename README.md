...
> **評估快照：2026-08-26；於晚期有效槽、第29／30戰回合與終局音訊生命週期驗證後重新核算。第29戰依玩家可見99%相似門檻列為重製完成。**
...
> **完整重製自評：88／100（約 88%）。尚餘約 12% 的產品整合、忠實度驗收與發行工作。**
...
| 三平台發行與推廣 | 10 | 5 | 最新`a5bbaf3f`已重建Linux AppImage與Windows x86_64 ZIP。AppImage於Debian系及Ubuntu 24.04抵達`town_ch02`且畫面AE=0；Windows ZIP於Wine 9.0接收城鎮按鍵、進商店並以F5寫出可解析存檔。macOS真實`macos-14` runner亦已完成arm64／amd64、lipo universal、`.app`、DMG／tar.gz與19,299,516-byte artifact上傳，[workflow run 32997009964](https://github.com/wicanr2/fd2_re/actions/runs/32997009964)成功。這仍不等於Windows／macOS實體機啟動與音訊驗收；最終版號封包與推廣影片尚待 |
| **總計** | **100** | **88** | **尚未達到發行候選版** |
...
| Linux／Windows／macOS 發行 | AppImage已在Debian系與Ubuntu 24.04抵達`town_ch02`且畫面一致；Windows ZIP已由Wine 9.0驗證輸入、節點轉場與F5存檔；macOS universal workflow run 32997009964已於真實`macos-14`成功建置並上傳artifact | 下載並解包macOS artifact；補Windows／macOS實體機啟動、輸入、存檔與音訊，處理簽章／Gatekeeper，最後建立版號封包與推廣影片 |
