...
- 涉及正式執行路徑、逐像素一致或一般玩家路徑的介面／戰役結論，必須依
  ...
  截圖橋接、修改路徑及重製端測試不得自行提升為 E2；唯一例外是 `111`
  定義的建構槽章收據（見下方「實作與驗證」）。
...
- 判斷 dosgolem 有沒有某項能力時，必須用**目前程式產生的收據**實測，不可拿
  舊的診斷輪次當現況。`work/` 底下保留的舊 run 有些是刻意留著的失敗樣本
  （例如檔案模式表受污染的 r11），拿它取樣會得出「能力缺失」的假結論。

## 實作與驗證

- 每個新垂直切片固定依「RE 知識庫 → 可審查規格 → 實作」順序進行。RE 階段先把
  雜湊、工具版本、位址空間、原始名稱、caller／consumer、raw bytes與證據分級
  寫入 canonical evidence並登記於 `58`；規格階段再定義 typed data、時序、
  失敗即關閉與驗收條件。兩者完成前不得先接 runtime，再用事後文件替猜測背書。
- 優先完成垂直切片：原版位元組／狀態 → 具型別解碼器 → 規則 →
  介面／存檔整合 → 決定性測試 → 原版／重製比較。
- 只在重製端通過的單元測試僅證明內部一致，不能證明忠於原版。
- 戰役可達性、隊伍持續性、商店／整備、存檔／讀檔與結局，最終都必須以未
  修改的一般玩家路徑驗收。
- 原版實機驗證若因敵軍強度或關卡長度無法通關，不得反覆刷關形成驗證迴圈。
  改用玩家合法存檔、相同狀態的短路徑或有界局部輸入實驗；使用修改、略過戰鬥、
  第三方存檔或直接進場時必須降低證據等級，不能冒稱完整一般玩家 `PLAYER-E2`。
  **例外（使用者 2026-09-15 定案）**：依
  [`111`](docs/goal/111-goal-original-parity-campaign-20260915.md)
  的規則——受版控工具依攻略校準建構的存檔槽、章內全程正常鍵盤輸入、
  抽樣完戰鬥節拍後以 `force-enemy-clear` 進戰後節點——所取得的章收據，
  視同該章完成 `PLAYER-E2`；收據仍須如實記錄槽來源與狀態注入。
  建構槽依受版控工具明示的政策值提高我方基底 AP／DP／DX（使用者 2026-09-17
  定案，見 [`114`](docs/goal/114-goal-boosted-slot-and-ch09-parity-20260917.md)）
  也屬於這條例外；政策值寫進 manifest 與收據，這種收據不得用來談傷害、存活或
  敵方選目標。章內週期性改寫狀態（例如鎖 HP）不在例外內。
